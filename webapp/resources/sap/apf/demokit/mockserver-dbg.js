...
sap.ui.define( [
...
], function(UI5MockServer, ApfUtils, Log, Storage, jQuery) {
    'use strict';
	...
	function creator(base) {
		var mockServers = [];
		//TODO: unhandled global jquery call found
		...
		var self = {
			...
			_loadAnalyticalConfiguration: function(requestObject) { // not required when initially all configurations are load into mockServer!
				...
				} else if (Array.isArray(requestObject.mParameters.oFilteredData.results)) {
			...
			mockHierarchyService: function () {
				// TODO: migration not possible. jQuery.sap.sjax is deprecated. Please use <code>fetch</code> API or native <code>XMLHttpRequest</code>
			...
			_aggregateData: function(results) {
				...
				if (results.length > 0) {
					...
					if (measures.length > 0) {
						...
						var resultValues = [];
						results.forEach(function(result) {
							if (resultValues && Array.prototype.indexOf.call(resultValues, result[dimensions[0]]) === -1) {
