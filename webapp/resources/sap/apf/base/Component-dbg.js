...
], function(UIComponent, api, uiHandler, jQuery) {
    'use strict';

	var component = sap.ui.core.UIComponent.extend("sap.apf.base.Component", {
		metadata : {
			"manifest" : "json",
			"library" : "sap.apf",
		...
		init : function() {
			...
			var ApiConstructor;
			if (!this.oApi) {
				baseManifest = component.prototype.getMetadata().getManifest();
			...
			if (this.getInjections() && this.getInjections().functions && this.getInjections().functions.isUsingCloudFoundryProxy
				...
				sap.apf.cloudFoundry.uiHandler.initRuntime(this);
			}
		},
		/**
		 * @public
