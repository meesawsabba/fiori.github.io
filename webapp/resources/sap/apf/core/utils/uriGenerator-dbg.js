...
sap.ui.define([
...
], function(MessageObject, EncodeURL){
	'use strict';
	/**
		...
		function retrieveParameters() {
			...
			function addParameter(index, value) {
				...
				if (parameters[index].dataType.type === 'Edm.String') {
					...
					result[parameters[index].name] = (EncodeURL(formatedValue));
					...
					result[parameters[index].name] = EncodeURL(sap.apf.utils.escapeOdata(value));
				} else {
	...
	uriGenerator.getSelectString = function(aSelectProperties){
		...
		aSelectProperties.forEach(function(selectProperty, index){
			result += EncodeURL(sap.apf.utils.escapeOdata(selectProperty));
