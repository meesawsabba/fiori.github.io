...
sap.ui.define([
...
], function(constants, utils, Ui5Filter, EncodeURL){
	'use strict';
	/**
	 * @private
	...
	FilterTerm.prototype.toUrlParam = function (conf) {
		...
		if (this.operator === constants.FilterOperators.StartsWith) {
			...
			param = '((' + EncodeURL(utils.escapeOdata(this.propertyName) + spaceCharacter + "ge" + spaceCharacter + value) + ')' + EncodeURL(spaceCharacter + 'and' + spaceCharacter) + '('
				+ EncodeURL(utils.escapeOdata(this.propertyName) + spaceCharacter + "le" + spaceCharacter + hvalue) + '))';
		} else {
			param = '(' + EncodeURL(utils.escapeOdata(this.propertyName) + spaceCharacter + this.operator.toLowerCase() + spaceCharacter + value) + ')';
		}
		return param;
	};
	/**
