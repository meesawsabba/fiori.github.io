sap.ui.define([
	...
	function _setVisibilityForAddAndRemoveIcons(oController) {
		...
		if(nIndex > 0){
			oPreviousPropertyView.oController.byId("idAddPropertyIcon").setVisible(false);
			}
	}
	function _setVisibiltyOfAddRemoveIconsAfterAdd(oController){
		var oPropertyTypeState = oController.getView().getViewData().oPropertyTypeState;
		var nIndex = oPropertyTypeState.indexOfPropertyTypeViewId(oController.getView().getId());
		if(nIndex > 0) {
			var noOfProperties = oPropertyTypeState.aProperties.length-1;
			var oPreviousPropertyView = oPropertyTypeState.getViewAt(nIndex - 1);
			var oCurrentPropertyView = oPropertyTypeState.getViewAt(nIndex);
			var isSaved = oController.oConfigurationEditor.isSaved();
		}
		if(nIndex > 0 && nIndex == noOfProperties && isSaved == false) {
			oPreviousPropertyView.oController.byId("idAddPropertyIcon").setVisible(false);
			oCurrentPropertyView.oController.byId("idAddPropertyIcon").setVisible(true);
			oCurrentPropertyView.oController.byId("idRemovePropertyIcon").setVisible(true);
			}
	}
	function _setVisibiltyOfAddRemoveIconsAfterRemove(oController, nIndex){
		var oPropertyTypeState = oController.getView().getViewData().oPropertyTypeState;
		var noOfProperties = oPropertyTypeState.aProperties.length;
		var oPreviousPropertyView = oPropertyTypeState.getViewAt(nIndex - 1);
		var isSaved = oController.oConfigurationEditor.isSaved();
		if(nIndex > 1 && nIndex == noOfProperties && isSaved == false) {
			oPreviousPropertyView.oController.byId("idAddPropertyIcon").setVisible(true);
			oPreviousPropertyView.oController.byId("idRemovePropertyIcon").setVisible(true);
			}
		if(nIndex === 1 && nIndex == noOfProperties && isSaved == false){
			oPreviousPropertyView.oController.byId("idAddPropertyIcon").setVisible(true);
		}
	...
	var constructor = sap.ui.core.mvc.Controller.extend("sap.apf.modeler.ui.controller.propertyType", {
		...
		handlePressOfAddPropertyIcon : function() {
			...
			if(this._shallAddPropertyBeHandled(itemsFromSelectControl, selectedKeyInSelectControl, noneValueFromTextReader, oPropertyTypeOrchestration)) {
				...
				oController.oConfigurationEditor.setIsUnsaved();
				_setVisibiltyOfAddRemoveIconsAfterAdd(oController);
			}
		},
		/**
		...
		handlePressOfRemovePropertyIcon : function() {
			...
			return oController.updateRepresentationAndView().then(function() {
				...
				oController.oConfigurationEditor.setIsUnsaved();
				if(nIndex !== 0){
					_setVisibiltyOfAddRemoveIconsAfterRemove(oController, nIndex);
				}
		 ...
		 * @param {any} itemsFromSelectControl - contains the properties that can be selected in select control (that are available in drop down)
