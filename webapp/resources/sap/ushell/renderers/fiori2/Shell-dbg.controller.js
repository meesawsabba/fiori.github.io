...
sap.ui.define([
    ...
    var closeAllDialogs = true,
        ...
        oShellModel,
        ShellModel = AppLifeCycleAI.getElementsModel(),
        oModel,
        oEpcmNavigationMode = {
            embedded: 0,
    ...
    return Controller.extend("sap.ushell.renderers.fiori2.Shell", {
        ...
        onInit: function () {
            ...
            AppLifeCycleAI.parseStatefulContainerConfiguration(oConfig.statefulApplicationContainer);
            ...
            if (oConfig && !oConfig.moveContactSupportActionToShellHeader) {
                aActions.push("ContactSupportBtn");
            }

            this.history = new History();
            this.oViewPortContainer = sap.ui.getCore().byId("viewPortContainer");
            AppLifeCycleAI.init(oConfig.appState, this.oViewPortContainer, oConfig.rootIntent, oConfig.disableHomeAppCache, {
            ...
            window.onbeforeunload = function () {
                if (sap.ushell.Container && sap.ushell.Container.getDirtyFlag()
                    // Work-around: skip data-loss-warning in IE when app is NWBC-based (as there is a seperate implementation)
            ...
            if (!oConfig.disableSignOut && (oConfig.sessionTimeoutTileStopRefreshIntervalInMinutes > 0 || oConfig.sessionTimeoutReminderInMinutes > 0)) {
                this._createSessionHandler(oConfig);
            }
         ...
         * For non-IE browsers the dirtyState=true gets handled asynchronously by undoing the
         * navigation and redoing the navigation. In case of an explace navigation there is
         * no dataloss popup.
         *
        ...
        handleDataLoss: function (targetIntent, currentIntent) {
            ...
            // This is needed in case of a hashChangeFailure
            ...
            // Built-in intents must never be opened ex-place.
            if (!Device.browser.msie && !bBuiltInIntent && (this.bExplaceNavigation || sNavMode === "explace" || sNavMode === "frameless")) {
                // Yes, just continue the navigation
                this.bExplaceNavigation = false;
                return this.oShellNavigation.NavigationFilterStatus.Continue;
            }

            // In case of dirty flag and non-external navigation, we need to ask the user
            if (bIsDirty && Device.browser.msie) {
            ...
            if (bIsDirty) {
                ...
                // _undoNavigation returns the NavigationFilterStatus Custom
                return this._restoreHashUsingAction(currentIntent, "undo");
            }
            // If dirty flag is false, we continue.
            // If it's true, we continue, but handle the additional logic in a promise.
            return this.oShellNavigation.NavigationFilterStatus.Continue;
        },
        ...
        _handleDirtyStateUserConfirm: function () {
            ...
            if (window.confirm(resources.i18n.getText("dataLossInternalMessage"))) {
        ...
        _doHashChange: function (oShellController, sShellHash, sAppPart, sOldShellHash, sOldAppPart, oParseError) {
            ...
            var oInMemoryApplicationInstance, iOriginalHistoryLength, sFixedShellHash, oDeferred;
        ...
        _resolveHashFragment: function (sShellHash) {
            ...
            var oResolvedHashFragment,
                ...
                oParsedShellHash = this.oURLParsing.parseShellHash(sShellHash),
                oDeferred = new jQuery.Deferred(),
        ...
        _openAppViaNWBC: function (oResolvedHashFragment) {
            ...
            return new Promise(function (fnResolve) {
                ...
                var oEpcm = utils.getPrivateEpcm(),
                    iEpcmNavigationMode = oEpcmNavigationMode[oResolvedHashFragment.navigationMode],
                    sUrlWithSapUserAndShellParam,
                    iMode,
                    oPromiseParams;
                ...
                oPromiseParams = that._appendUserIdToUrl("sap-user", oResolvedHashFragment.url).then(function (sUrlWithSapUser) {
                    ...
                    return new Promise(function (fnResolveParams) {
                        try {
                            ...
                            if (sAppType === "GUI" || sAppType === "TR" || sAppType === "NWBC" || sAppType === "WDA" || sFrameworkId === "GUI" || sFrameworkId === "WDA" || sFrameworkId === "NWBC") {
                ...
                oPromiseParams.then(
                    ...
                    function (oPostBodyParams) {
                        try {
                            ...
                            that._restorePreviousHashAfterOpenNewWindow(oResolvedHashFragment);
        ...
        getWrappedApplicationWithMoreStrictnessInIntention: function (sIntent, oMetadata, oShellHash, oResolvedNavigationTarget, sAppId, bFullWidth, sFixedShellHash) {
            ...
            window.setTimeout(function () {
                window.setTimeout(function () {
                    ...
                    if (Config.last("/core/shell/model/currentState/stateName") === "app") {
                        arg = sap.ui.getCore().byId("shellAppTitle").getFocusDomRef().getAttribute("id");
                    }
            ...
            if (sAppType === "GUI" || sAppType === "TR" || sAppType === "WDA" || sAppType === "NWBC" || sFrameworkId === "GUI" || sFrameworkId === "WDA" || sFrameworkId === "NWBC") {
        ...
        _openAppNewWindow: function (sUrl) {
            ...
            if (sUrl.trim().indexOf("sap-nwbc://") === 0 && (Device.browser.edge || Device.browser.msie)) {
                ...
                // - in IE with protected mode. Disable the blocker check for IE & EDGE (BCP:0020751295 0000019410 2020)
                // - in WorkZone's mobile client, when external nav happens. Just go on, when URL parameter is there in CURRENT window.
                if (!oNewWin && (
                    !(Device.browser.msie || Device.browser.edge) && !this._checkWindowLocationSearch("workzone-mobile-app=true")
                )) {
        ...
        onBeforeNavigate: function (oEvent) {
            var oToView = oEvent.getParameter("to"),
                ...
                oDashboardView = oOwnerComponent.byId("Shell-home-component-container"),
                oPagesView = oOwnerComponent.byId("pages-component-container"),
                bMenuVisible = oToView === oDashboardView || oToView === oPagesView;
        ...
        _reorderUserPrefEntries: function (aEntries) {
            ...
            for (var i = aEntries.length - 1; i >= 0; i--) {
                if (aOrderedIds.indexOf(aEntries[i].id) !== -1) {
                    ...
                    mSpeicalEntries[oSpecialEntry.id] = oSpecialEntry;
                }
            }

            aOrderedIds.forEach(function (sId) {
                var oEntry = mSpeicalEntries[sId];
                if (oEntry) {
