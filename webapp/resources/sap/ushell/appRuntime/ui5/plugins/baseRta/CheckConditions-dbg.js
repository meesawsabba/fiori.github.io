...
sap.ui.define([
	...
	var CheckConditions = {
		 ...
		 * @returns {Promise<boolean>} Resolves to <code>true</code> if flex is enabled.
		 * @private
		 */
		checkFlexEnabledOnStart: function () {
			return AppLifeCycleUtils.getCurrentRunningApplication()
				.then(function (oCurrentRunningApp) {
					var oRootComponent = oCurrentRunningApp.componentInstance;
					var mAppDescriptor = this._getAppDescriptor(oRootComponent);
					var vFlexEnabled = mAppDescriptor["sap.ui5"] && mAppDescriptor["sap.ui5"].flexEnabled;
