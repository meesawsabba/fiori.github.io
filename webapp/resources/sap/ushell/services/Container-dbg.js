...
sap.ui.define([
     ...
     */
    function createAdapter (sName, oSystem, sParameter, bAsync, bUseConfigAdapterOnly) {
        var oAdapterConfig = getServiceConfig(sName).adapter || {},
            sAdapterName;

        function getAdapterInstance () {
            return new (ObjectPath.get(sAdapterName))(oSystem, sParameter,
                { config: oAdapterConfig.config || {} }
            );
        }

        if (bUseConfigAdapterOnly === true) {
        ...
        if (!sap.ui.require(sModule)) {
            Log.error("sap.ushell.Container.createAdapter. The adapter " + sModule + " must be loaded before use!");
     ...
     * @typedef {object} sap.ushell.services.Container.DirtyState
     * @since 1.90.0
     * @public
    ...
    function Container (oAdapter) {
         ...
         * For CSP compliance, applications must always set bAsync:true.
        ...
        this.createRenderer = function (sRendererName, bAsync) {
            ...
            function getRendererInstance () {
                var oRenderer = new (ObjectPath.get(sComponentName))({ componentData: oComponentData }),
                ...
                if (bAsync && oRenderer instanceof sap.ui.core.UIComponent) {
                    return oRenderer.rootControlLoaded().then(function () {
            ...
            Log.error("sap.ushelContainer.createRenderer() should always be called with bAsync:true.");
            sap.ui.requireSync(sModule);
        ...
        /**
         * An enumeration for the application work protect mode state.
         * @type {sap.ushell.services.Container.DirtyState}
         * @since 1.21.1
         * @public
         * @member
         */
         ...
         *
         * @since 1.15.0
         * @deprecated As of 1.77 deprecated. Uses {@link#getServiceAsync} instead.
         * @public
         */
            ...
            function getServiceInstance (Service, bAsync) {
                ...
                if (mServicesByName.containsKey(sKey)) {
                    ...
                    oServiceAdapter = createAdapter(sServiceName, oAdapter.getSystem(),
                        sParameter, bAsync, Service.useConfigAdapterOnly);
                    if (bAsync) {
         ...
         *
         * @returns {object|Promise} a Promise that returns the FLP Platform's value
         * @since 1.96
         * @private
         */
        this.getFLPPlatform = function () {
                    ...
                    getPlugins.then(
                        function (oCDMSitePlugins) {
                            ...
                            try {
                                ...
                                oPluginManager.loadPlugins("EarlyLoading").catch(function (oError) {
                    ...
                    sap.ui.require(["sap/ushell/Config"], function (Config) {
                        if (Config.last("/core/darkMode/enabled")) {
                            sap.ushell.Container.getServiceAsync("DarkModeSupport")
                                .then(function (DarkModeSupportService) {
                                    DarkModeSupportService.setup();
                                });
                        }

                        if (Config.last("/core/shellHeader/newDesignSwitchVisible")) {
                            var sCurrentTheme = sap.ushell.Container.getUser().getTheme(sap.ushell.User.prototype.constants.themeFormat.ORIGINAL_THEME);
                            if (sCurrentTheme === sap.ushell.User.prototype.constants.NEW_DESIGN) {
                                Config.emit("/core/shellHeader/newDesignSwitchActive", true);
                            }
                        }
                    });
                });
