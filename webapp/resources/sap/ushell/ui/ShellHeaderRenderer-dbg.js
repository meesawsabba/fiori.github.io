...
sap.ui.define([
    ...
    ShellHeaderRenderer.render = function (rm, shellHeader) {
        ...
        // New Design Switch
        // Do not render it on phones when the endItemsOverflowBtn is shown
        var sCurrentRangeName = Device.media.getCurrentRange(Device.media.RANGESETS.SAP_STANDARD).name;
        var oNewDesignSwitch = shellHeader._oNewDesignSwitch;
        if (oNewDesignSwitch && sCurrentRangeName !== "Phone") {
            rm.renderControl(oNewDesignSwitch);
        }
    ...
    ShellHeaderRenderer.renderHeaderItems = function (rm, oHeader) {
        ...
        aItems.forEach(function (oItem) {
            if ((oItem.getId() === "homeBtn") && oHeader.getShowLogo()) {
                return; // do not render the home button when the logo is already shown (they have the same function)
            }
    ...
    ShellHeaderRenderer.renderHeaderEndItems = function (rm, oHeader) {
        oHeader.getHeadEndItems().forEach(function (oItem) {
            if (oItem && oItem.getId() !== "newDesignSwitchItem") { // newDesignSwitchItem is only to be rendered in the overflow
                rm.renderControl(oItem);
            }
        });
    };
