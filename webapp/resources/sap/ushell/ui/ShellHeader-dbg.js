...
sap.ui.define([
    ...
    "sap/ushell/EventHub",
    "sap/ushell/Config",
    "sap/ushell/ui/horizon/NewDesignSwitch",
    ...
    ShellHeader.prototype.init = function () {
        ...
        this.addDelegate({
            onBeforeFastNavigationFocus: function (oEvent) {
                ...
                this._accessKeyHandler.sendFocusBackToShell(oEvent);
            }.bind(this)
        });
        ...
        if (Config.last("/core/shellHeader/newDesignSwitchVisible")) {
            this._oNewDesignSwitch = new NewDesignSwitch("newDesignSwitch");
            this._oNewDesignSwitch.setParent(this);
        }
    };
    ...
    ShellHeader.prototype.exit = function () {
        ...
        if (this._oNewDesignSwitch) {
            this._oNewDesignSwitch.destroy();
            this._oNewDesignSwitch = null;
        }
    };
        ...
        if (this._accessKeyHandler) {
            if (this._accessKeyHandler.fromOutside || this._accessKeyHandler.bForwardNavigation) {
                ...
                if (window.document.activeElement === oAccessibilityHelper) {
                    ...
                    var oFocusable;
                    // Accessibility: To avoid invisible tab stops between address bar and shell header
                    // we focus on the last visible element in the DOM when we come from the shell header
                    for (var i = aSelectables.length - 1; i >= 0; i--) {
                        if (aSelectables[i].offsetWidth > 0 && aSelectables[i].offsetHeight > 0) {
                            oFocusable = aSelectables[i];
                            break;
                        }
                    }
                    if (oFocusable) {
    ...
    ShellHeader.prototype.getLogo = function () {
        return this.getProperty("logo") || (sap.ui.getCore().isThemeApplied() ? ThemingParameters._getThemeImage() || _sSapLogo : _sNoLogo);
    };
    ...
    ShellHeader.prototype.refreshLayout = function () {
        ...
        if (this.getSearchVisible()) {
            ...
            // If no place anymore, hide the "New Design" label
            var oNewDesignSwitch = document.getElementById("newDesignSwitch");
            var oNDLabel = document.getElementById("newDesignSwitch-lbl");
            if (oNewDesignSwitch && oNDLabel && oNewDesignSwitch.parentElement) {
                oNDLabel.style.display = "";
                // If label is cut off, hide it
                if (oNewDesignSwitch.getBoundingClientRect().left < oNewDesignSwitch.parentElement.getBoundingClientRect().left) {
                    oNDLabel.style.display = "none";
                }
            }
        }
    };
    ...
    ShellHeader.prototype._hideElementsForSearch = function () {
        var nReqWidth,
            ...
            oNewDesignSwitch = document.getElementById("newDesignSwitch");
        ...
        // New Design Switch
        if (oNewDesignSwitch) {
            aElements.push(oNewDesignSwitch);
        }
        ...
        if (oNewDesignSwitch) {
            // as the switch is inside of the container, add the switch width to the requested search width
            var sNDSWidth = oNewDesignSwitch.getBoundingClientRect().width || 0;
            nReqWidth = nReqWidth + sNDSWidth;
        }
