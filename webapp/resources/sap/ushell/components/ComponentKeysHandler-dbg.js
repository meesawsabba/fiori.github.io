    ...
    ComponentKeysHandler.prototype = {
        ...
        _goToTileOfGroup: function (vPosition, oGroup) {
            if (oGroup) {
                ...
                if (oTile && oTile.getDomRef()) {
                    ...
                    if (jqTile.length === 1) {
                        this.moveScrollDashboard(jqTile);
                        return true;
                    }
                }
            }
                ...
                if (sCurTileMode === sDestTileMode) {
                    ...
                    if (sDestTileMode === "LineMode") {
                        oEventInfo.callBack = function () {
                            setTimeout(function () {
                                aDestGroupContent = sDestTileMode === "LineMode" ? oDestGroup.getLinks() : oDestGroup.getTiles();
                                that.moveScrollDashboard(aDestGroupContent[nDestTileIndex].$());
                            }, 10);
                        };
                    } else {
                        setTimeout(function () {
        ...
        callbackSetFocus: function (oTile) {
            ...
            setTimeout(function () {
                if (oTile.oParent && oTile.oParent instanceof sap.ushell.ui.launchpad.Tile) {
                    that.moveScrollDashboard(jQuery(oTile.oParent.getDomRef()));
                } else {
                    that.moveScrollDashboard(oTile.$());
                }
            }, 0);
        },
        ...
        _getNextTile: function (sDirection, bMoveTile, oGivenInfo) {
            ...
            if (bMoveTile) {
                ...
                bLinksAllowed = this.oLaunchPageService.isLinkPersonalizationSupported(oTile);
            }
        ...
        setTileFocus: function (jqTile) {
            ...
            // removes tabindex from all tiles, cards and links
        ...
        _handleAnchorNavigationOverflowButtonArrowKeys: function (sDirection) {
            ...
            jQuery("#sapUshellAnchorBarOverflowButton").attr("tabindex", -1);

            if (["left", "up"].indexOf(sDirection) > -1) {
                aAnchorItems = jQuery(".sapUshellAnchorItem").filter(":visible");
                this._setAnchorItemFocus(aAnchorItems.eq(aAnchorItems.length - 1));
            }
        },
        ...
        _handleAnchorNavigationItemsArrowKeys: function (sDirection, jqFocused) {
            ...
            if (sDirection === "left" || sDirection === "up") {
                if (nIndexOfFocusedItem > 0) {
                    this._setAnchorItemFocus(aAnchorItems.eq(nIndexOfFocusedItem - 1));
                    return;
                }
            }
            ...
            if (sDirection === "right" || sDirection === "down") {
                if (nIndexOfFocusedItem < aAnchorItems.length - 1) {
                    ...
                    this._setAnchorOverflowButtonFocus();
                }
            }
        },

        _setAnchorOverflowButtonFocus: function () {
            var oButton = jQuery("#sapUshellAnchorBarOverflowButton");
            // remove tabindex from all anchor items
            jQuery(".sapUshellAnchorItem").get().forEach(function (oHTMLElement) {
                jQuery(oHTMLElement).attr("tabindex", -1);
            });
            oButton.attr("tabindex", 0);
            oButton.focus();
        },
                    ...
                    if (oInfo) {
                        ...
                        setTimeout(function () {
                            if (oInfo.oCurTile.getDomRef()) {
                                that.moveScrollDashboard(oInfo.oCurTile.$());
                            } else {
                                that.moveScrollDashboard(oInfo.oGroup.oPlusTile.$());
                            }
                        }, 100);
        ...
        _spaceButtonHandler: function (oEvent, jqFocused) {
            ...
            var jqClosestButton = jqFocused.find("[role=button]");
            if (jqClosestButton.length) {
                        ...
                        if (oEvent.keyCode === handler.keyCodes.ENTER) {
                            ...
                            if ($HeaderElement.length) {
                                setTimeout(function () {
                                    ...
                                    handler._setTabIndexOnTileContainerHeader($HeaderElement);
                                    $HeaderElement.focus();
                                }, 10);
                            }
                        }
