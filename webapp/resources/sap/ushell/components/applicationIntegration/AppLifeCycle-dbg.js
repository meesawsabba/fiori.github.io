...
sap.ui.define([
    ...
    function AppLifeCycle () {
        ...
        this.destroy = function (sAppId, oFrom, oDeferred, bForceDestroy) {
            ...
            if (bForceDestroy === undefined) {
                bForceDestroy = false;
            }
            this.removeControl(sAppId, bForceDestroy);

            if (oStorageEntry) {
                oInnerControl = BlueBoxHandler.getStateFul(oFrom.getUrl());

                if (oInnerControl && BlueBoxHandler.isStatefulContainerSupported(oInnerControl) && bForceDestroy === false) {
        ...
        this.handleExitApplication = function (sFromId, oFrom, sToId, oTo, handleExitApplication) {
            var that = this,
            ...
            if (sFromId && oFrom) { // FIXME oFrom.getApplicationType is not defined for Homepage and AppFinder in case disableHomeAppCache is true
                if (oFrom.getUrl() && BlueBoxHandler.isStatefulContainerSupported(BlueBoxHandler.getStateFul(oFrom.getUrl()))) {
                    ...
                    oPromiseAppClose = this.handleExitStateful(sFromId, oFrom);
                } else if (Storage.get(sFromId)) {
                    if (RelatedServices.isBackNavigation() === true && !handleExitApplication) {
        ...
        this.closeKeepAliveApps = function (fnFilterApps, arrClosePromises) {
            try {
                ...
                arrKeepAliveRestrictedApps.forEach(function (oRestrictedApp) {
                    ...
                    that.destroy(oRestrictedApp.appId, oRestrictedApp.container, oDeferredAppClose, true);
                });
            } catch (e) {
        ...
        this.onAfterNavigate = function (sFromId, oFrom, sToId, oTo) {
            ...
            var bIsShellApps = sToId.indexOf("Shell-appfinder-component") > 0 || sToId.indexOf("Shell-home-component") > 0 || sToId.indexOf("pages-component-container") > 0;
        ...
        this.isCachedEnabledAsAppParameter = function (oShellHash, oTarget, oKeepAliveMode) {
            ...
            //temporary fix for issue found in keep alive fiori apps when statefull container
        ...
        this.openApp = function (inAppId, oTarget, oShellHash, sFixedShellHash) {
            ...
            if (oContainer && BlueBoxHandler.isStatefulContainerSupported(oContainer)) {
                if (this.isAppOfTypeCached(appId, bIsAppOfTypeCachable, oKeepAliveMode) || this.isCachedEnabledAsAppParameter(oShellHash, oTarget, oKeepAliveMode)) {
                ...
                if (oContainer) {
                ...
                } else if (oTarget.applicationType === "URL" || oTarget.applicationType === "TR") {
                    ...
                    // more unique.
                    sIntent = oShellHash.semanticObject + "-" + oShellHash.action;
                    oTmpAppContainer = BlueBoxHandler.getById("application-" + sIntent);
                    if (oTmpAppContainer) {
                        ...
                        setup: {
                            executeServiceCallFn: function (oServiceParams) {
                                var oSetup = oServiceParams.oMessageData && oServiceParams.oMessageData.body,
                                    ...
                                    bIgnoreStatefull = false;
                                if (oSetup) {
                                    if (oServiceParams.oContainer.getIsKeepAlive() === true &&
                                        (sAppType === "GUI" || sAppType === "TR" || sAppType === "NWBC" || sAppType === "WDA" ||
                                            sFrameworkId === "GUI" || sFrameworkId === "WDA" || sFrameworkId === "NWBC")) {
                                        bIgnoreStatefull = true;
                                    }
                                    if (oSetup.isStateful === true && !bIgnoreStatefull) {
        ...
        this.handleOpenStateful = function (bIsInitial, bIsInCache, sToId, oInnerControl, oTarget, sFixedShellHash, oKeepAliveMode) {
            ...
            if (Storage.get(sToId) && bIsInitial === false) {
                this.reuseStateFulContainerAndRestore(oInnerControl, sToId, oHandler, sFixedShellHash, oTarget);
            } else {
                this.reuseStateFulContainer(oInnerControl, oTarget.url, sToId, oHandler, sFixedShellHash, oTarget);
        ...
        this.open = function (sIntent, sAppId, oShellHash, oTarget, fnWrapper, sFixedShellHash, oMetadata) {
            var that = this,
                oInnerControl,
                bReuseGuiV1StatefulContainer = (this.getWebGuiV1StatefulContainer(oTarget.url) !== undefined),
            ...
            bIsInCache = this.isAppOfTypeCached(sFullAppId, bIsAppOfTypeCachable, oKeepAliveMode) || this.isCachedEnabledAsAppParameter(oShellHash, oTarget, oKeepAliveMode);
            ...
            oDeferredDestroyApp.then(function () {
                oDeffedControlCreation.then(function () {
                    if (!bReuseGuiV1StatefulContainer && !bReuseStatefulContainer) {
                    ...
                    if (bReuseStatefulContainer) {
                        that.handleOpenStateful(bIsInitial, bIsInCache, "application" + sAppId, oInnerControl, oTarget, sFixedShellHash, oKeepAliveMode);
                    } else if (bReuseGuiV1StatefulContainer) {
                        that.reuseApplicationContainer(oInnerControl, "TR", oTarget.url);
                    }
        ...
        this.getWebGuiV1StatefulContainer = function (sUrl) {
            ...
            if (sUrl) {
                ...
                return oWebGuiV1StatefullCache[sKey];
            } else {
                return undefined;
            }
        };
        ...
        this.setWebGuiV1StatefulContainer = function (oApplicationContainer) {
            ...
            if (sUrl) {
                sKey = BlueBoxHandler._getBlueBoxCacheKey(sUrl);
                oWebGuiV1StatefullCache[sKey] = oApplicationContainer;
            }
        };
         ...
         *
         * @param {object} oWebGuiV1StatefullCache
        ...
        this.getWebGuiV1AllApplicationContainers = function () {
            return Object.keys(oWebGuiV1StatefullCache).map(function (sKey) {
                return oWebGuiV1StatefullCache[sKey];
