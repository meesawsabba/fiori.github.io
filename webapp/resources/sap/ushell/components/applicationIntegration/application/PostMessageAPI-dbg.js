...
sap.ui.define([
    ...
    PostMessageAPI.prototype._sendEmail = function (sTo, sSubject, sBody, sCc, sBcc, sIFrameURL, bSetAppStateToPublic) {
        ...
        function replaceIframeUrlToFLPUrl (sIFrameURL1, sFLPUrl1) {
            if (sIFrameURL1 && sIFrameURL1.length > 0
                && ((sSubject && sSubject.includes(sIFrameURL1)) || (sBody && sBody.includes(sIFrameURL1)))) {
                // Replace the URL
                if (sBody && sBody.includes(sIFrameURL1)) {
                    sBody = sBody.replace(sIFrameURL1, sFLPUrl1);
                }
                if (sSubject && sSubject.includes(sIFrameURL1)) {
                    sSubject = sSubject.replace(sIFrameURL1, sFLPUrl1);
                }
            }
        }
