...
sap.ui.define([
    ...
    */
    ...
    function handleMessageEvent (oContainer, oMessage) {
        if (oContainer && !oContainer.getActive()) {
            Log.debug(
    ...
    function render (oRenderManager, oContainer, sApplicationType, sUrl, sAdditionalInformation) {
        ...
        sUrl = oContainer._adjustURLForIsolationOpeningWithoutURLTemplate(sUrl);

        if (sApplicationType === "TR") {
            oContainer.setProperty("iframePostAllParams", true, true);
        }

        if (new UriParameters(window.location.href).get("sap-post") === "false") {
        ...
        if (bPostMechanismEnabled === true &&
            (sApplicationType === "NWBC" || sApplicationType === "TR" || sApplicationType === "WDA")) {
        ...
        oRenderManager
            ...
            .writeAttributeEscaped("title", resources.i18n.getText("AppilcationContainer.IframeTitle"))
            .addClass("sapUShellApplicationContainer")
            .writeClasses(oContainer)
            .addStyle("height", oContainer.getHeight())
            .addStyle("width", oContainer.getWidth())
        ...
        if (Config.last("/core/shell/enableFeaturePolicyInIframes") === true) {
            oRenderManager.writeAttribute("allow", sIFRAME_FEATURE_POLICY);
        }
        oRenderManager.write("></iframe>");
    }

    function adjustURLForIsolationOpeningWithoutURLTemplate (sUrl) {
        var urlParams = new UriParameters(document.URL);
        if (urlParams.get("sap-isolation-enabled") === "true") {
            var oParser = oUrlParsing,
                sHash = oParser.getHash(document.URL),
                sRoute = "",
                //this code is not productive for customers and there for still uses "getService".
                //it is in the process of being removed.
                sPlugins = "&sap-plugins=" + encodeURIComponent(sap.ushell.Container.getService("PluginManager")._getNamesOfPluginsWithAgents()),
                sDirection = "&sap-history-dir=" + (History.getInstance().getDirection() ? History.getInstance().getDirection() : "");

            if (sUrl.indexOf("&/") > 0) {
                sRoute = sUrl.substr(sUrl.indexOf("&/"));
            }

            if (sPlugins === "&sap-plugins=") {
                sPlugins = "";
            }
            ...
            if (sHash) {
                ...
                if (sParams && sParams.length >= 0) {
                    ...
                    oState2.save();
                    sUrl = sUrl.split("#")[0] + "&sap-intent-param=" + oState2.getKey() + sPlugins + sDirection + "#" + window.hasher.getHash();
                } else {
                    sUrl = sUrl.split("#")[0] + sPlugins + sDirection + "#" + window.hasher.getHash();
                }
            } else {
        ...
        //for debug - transformation of parameters from browser url to iframe url
            ...
            aParams.forEach(function (sParam) {
                if (sParam && sParam.length > 0) {
                    if (urlParams.has(sParam)) {
                        sUrl = sUrl.split("#")[0] + "&" + sParam + "=" + urlParams.get(sParam) + "#" + sUrl.split("#")[1];
                    }
                }
            });
    ...
    function buildHTMLElements (oRenderManager, oContainer, aAdditionalDataArray, aInfoArray, sUrl) {
        ...
        sStatements = "<input name=\"sap-flp-params\" value=\"" + encodeXML(valStr) + "\">";
        if (oContainer.getIframePostAllParams() === true) {
            var oIframeUriParams = new UriParameters(sUrl);
            sCompletePostStatements = buildHTMLForAllPostParams(oContainer, oIframeUriParams);
            if (sCompletePostStatements !== "") {
        ...
        oRenderManager
            ...
            .writeStyles()
            .write(">")
            .writeAttributeEscaped(sStatements + sCompletePostStatements)
            .write("</form>");
        ...
        oRenderManager
            ...
            .writeControlData(oContainer)
            .writeAccessibilityState(oContainer)
            .writeAttributeEscaped("sap-orig-src", sapOrigURL)
            .writeAttributeEscaped("title", resources.i18n.getText("AppilcationContainer.IframeTitle"))
            .addClass("sapUShellApplicationContainer")
            .writeClasses(oContainer)
            .addStyle("height", oContainer.getHeight())
        ...
        if (Config.last("/core/shell/enableFeaturePolicyInIframes") === true) {
            oRenderManager.writeAttribute("allow", sIFRAME_FEATURE_POLICY.replaceAll(";", " " + (new URI(sUrl).origin()) +";"));
        }
        oRenderManager.write("></iframe>");
    ...
    function buildHTMLForAllPostParams (oContainer, oIframeUriParams) {
        var sapIframeHintVal = "";
        var sStatement="", sStatements="";
        ...
        if (oContainer.getApplicationType() === "TR" || (sapIframeHintVal === "GUI")) {
            ...
            for (p in oParams) {
                ...
                valStr = oParams[p][0];
                sStatement = "<input name=\"" + p + "\" value=\"" + encodeXML(valStr) + "\">";
    ...
    ApplicationContainer.prototype._adjustURLForIsolationOpeningWithoutURLTemplate = adjustURLForIsolationOpeningWithoutURLTemplate;
