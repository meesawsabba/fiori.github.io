...
sap.ui.define([
    ...
    "sap/ushell/ui/horizon/NewDesignSwitchItem"
], function (
    ObjectPath,
    Device,
    ...
    var oHeaderDefaultProperties = {
        ...
        headItems: [],
        headerVisible: true,
        newDesignSwitchVisible: Config.last("/core/shellHeader/newDesignSwitchVisible"),
        newDesignSwitchActive: Config.last("/core/shellHeader/newDesignSwitchActive"),
        showLogo: false,
    ...
    function handleCurrentSpaceAndPage (oCurrentSpaceAndPage) {
        if (Config.last("/core/spaces/enabled") && (Config.last("/core/spaces/homeNavigationTarget") === "origin_page")) {
            if (oCurrentSpaceAndPage) {
                ((this && this.updateStates) || updateStates)({ // the "this" kludge is for testing (to use the stub instead of the closure)
                    propertyName: "homeUri",
                    value: "#" + oCurrentSpaceAndPage.hash
                });
            }
        }
    }
    ...
    function handleEndItemsOverflow (oParams) {
        ...
        if (sDeviceType === "Phone" && !bIsOverflowVisible && aEndItems.length > 2) {
            // Add the New Design switch list item, if required and was not added earlier.
            if (Config.last("/core/shellHeader/newDesignSwitchVisible") && aEndItems.indexOf("newDesignSwitchItem") === -1) {
                var oNewDesignSwitchItem = NewDesignSwitchItem.getSwitchItem();
                addHeaderEndItem(
                    [oNewDesignSwitchItem.getId()],
                    false,
                    ["home", "app"],
                    true
                );
    ...
    function switchState (sNewStateName) {
        ...
        // Update base state with the current value of the newDesignSwitchActive parameter
        oBaseState.newDesignSwitchActive = Config.last("/core/shellHeader/newDesignSwitchActive");
