...
sap.ui.define([
    ...
    Message,
    MessageType
    // DarkModesupport - preload the module. The instance is created by Container as sap.ushell.Container.getService("DarkModeSupport")
) {
    "use strict";

    // Note: use of Container.getService() is OK here because all services are preloaded already.
    ...
    return Controller.extend("sap.ushell.components.shell.Settings.appearance.Appearance", {
        ...
        onSaveThemes: function () {
            ...
            if (sNewThemeId && sNewThemeId !== sOriginalThemeId && oConfigModel.getProperty("/themeConfigurable")) {
                ...
                if (Config.last("/core/shellHeader/newDesignSwitchVisible")) {
                    var bIsNewDesign = sNewThemeId === sap.ushell.User.prototype.constants.NEW_DESIGN;
                    oPromise = Promise.all([
                        this._updateUserPreferences(oUser),
                        this._updateNewDesignSettings(sOriginalThemeId, bIsNewDesign)
                    ]);
                } else {
                    oPromise = this._updateUserPreferences(oUser);
                }
        ...
        // update user preferences with UserInfo service
        ...
        /**
         * Store the previous theme in personalization via the UserInfo service.
         * Store the state of the new design switch in personalization via the UserInfo service.
         *
         * @param {string} sOriginalThemeId The themeId to store
         * @param {boolean} bIsNewDesign Boolean indicating if the selected theme is the new design theme.
         * @return {Promise} A promise resolving when the save is complete.
         * @private
         * @since 1.96
         */
        _updateNewDesignSettings: function (sOriginalThemeId, bIsNewDesign) {
            return sap.ushell.Container.getServiceAsync("UserInfo")
                .then(function (oUserInfo) {
                    return Promise.all([
                        oUserInfo.updateNewDesignSetting(bIsNewDesign),
                        oUserInfo.updateThemeHistory(sOriginalThemeId)
                    ]);
                });
        },
