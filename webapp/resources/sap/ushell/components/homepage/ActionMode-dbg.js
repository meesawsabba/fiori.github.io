...
sap.ui.define([
    ...
    ActionMode.prototype._openActionsMenu = function (oEvent, oView) {
        ...
        //in a locked group we do not show any edit action but keep the actions defined in FLPD.
        ...
        // Edit actions are always added to a tile because the isLocked property is not transferred to the factory.
        // Here is the only place not to show them for locked groups.
        if (aActions.length && oTileModel.getProperty(sGroupBindingPath + "/isGroupLocked")) {
            ["tileSettingsBtn", "linkSettingsBtn", "ConvertToTile", "ConvertToLink", "moveTileDialog_action"].forEach(function (sAction) {
                var sText = resources.i18n.getText(sAction);
                for (var i = 0; i < aActions.length; i++) {
                    if (aActions[i].text === sText) {
                        aActions.splice(i, 1);
                        break;
                    }
                }
