...
sap.ui.define([
    ...
    var DashboardGroupsBox = baseObject.extend("sap.ushell.components.homepage.DashboardGroupsBox", {
            ...
            if (Device.system.desktop) {
                ComponentKeysHandler.getInstance().then(function (ComponentKeysHandlerInstance) {
                    ...
                    this.oGroupsContainer.addEventDelegate({
                        ...
                        onsapskipback: this._handleSkip.bind(this, false),
                        onsapskipforward: this._handleSkip.bind(this, true),
                                ...
                                if (jQuery(document.activeElement).closest(".sapUshellTileContainerHeader").length) {
                                    ...
                                    if (isCurrentElementTitle && !jqChildActions.length || document.activeElement.id === jqChildActions.last()[0].id) {
                                        if (jqCurrentTileContainer.find(".sapUshellTile, .sapUshellLink, .sapFCard").is(":visible")) {
                                            ...
                                            ComponentKeysHandlerInstance.goToLastVisitedTile(jqCurrentTileContainer, true);
                                            return;
                                        }
                                    }
        ...
        _handleSkip: function (bForward, oEvent) {
            var oDoneBtn = window.document.getElementById("sapUshellDashboardFooterDoneBtn");
            if (oDoneBtn) { // EditMode
                if (oDoneBtn === document.activeElement && !bForward) {
                    oEvent.preventDefault();
                    this.ComponentKeysHandlerInstance.goToTileContainer();
                } else if (oDoneBtn !== document.activeElement && bForward) {
                    oEvent.preventDefault();
                    oDoneBtn.focus();
                }
            }
