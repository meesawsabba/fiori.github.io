...
sap.ui.define([
    ...
    return UIComponent.extend("sap.ushell.components.visualizationOrganizer.Component", {
        ...
        init: function () {
            ...
            this.aPersonalizablePages = []; // the list of all pages which can be personolized
            ...
            return oLoadPopover.then(function () {
                ...
                return Promise.all([
                    ...
                    aSpaces.forEach(function (space) {
                        if (bOnlyHomePage && space.id !== sMyHomeSpaceId) {
        ...
        _applyOrganizationChangeToSection: function (oOpenBy, oVizInfo, oSectionContext) {
            var sVizId = decodeURIComponent(oVizInfo.id),
                ...
                oPageService = sap.ushell.Container.getService("Pages"), // Service is already loaded, getService() is justifie
                oVizChangeChain,
                sMessageToUser;
         ...
         */
        _applyOrganizationChange: function (oVizualizationChanges, bShowMessage) {
        ...
        /**
         * Get the lisy of all persinalizable pages
         *
         * @returns {array<pageInfo>} the list of persinalizable pages
         */
         ...
         * @see _organizeVisualizations
         */
        _applyBookmarkOrganizationChange: function (oVizualizationChanges, bShowMessage) {
            var iChangedVisualizations = (oVizualizationChanges.addToPageIds.length + oVizualizationChanges.deleteFromPageIds.length);
            if (!iChangedVisualizations) {
                return Promise.resolve();
            }
        ...
        _applyBookmarkTileChangeToSection: function (oVizInfo, oSectionContext) {
            var sUrl = oVizInfo.url,
                ...
                sPageId = oSectionContext.pageID,
                sSectionId = oSectionContext.sectionID,
                oPageService = sap.ushell.Container.getService("Pages"), // Service is already loaded, getService() is justified
                oVizChangeChain,
                sMessageToUser;
         ...
         * @property {set} title The tage title.
         */
        ...
        _applyChangeToPage: function (oOpenBy, oVizInfo, oPage) {
            ...
            var oChangePromise;
            if (oVizInfo.isBookmark) {
                oChangePromise = this._applyBookmarkOrganizationChange(oVizualizationChanges, false);
            } else {
                oChangePromise = this._applyOrganizationChange(oVizualizationChanges, false);
            }
