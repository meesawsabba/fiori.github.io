...
oFF.PlanningRsRequestDecorator.getResultsetRequestDecorator = function(application, systemDescription, requestStructure)
{
	...
	var planningRefreshStructure = refreshCommand.serializeToJson();
...
oFF.PlanningModelCommandHelper = {
	...
	{
			var commandIndex = oFF.PlanningModelCommandHelper.getCommandIndex(commands, commandName);
	...
	getResponsesReturnCodeStrict:function(responseStructure, messageManager, defaultReturnCode, useStrictErrorHandling)
	{
		...
		if (oFF.isNull(planningStructure))
		{
			...
			if (oFF.notNull(planningList))
			{
				...
				for (var i = 0; i < planningList.size(); i++)
				{
					...
					{
						var planningReturnCode = oFF.PlanningModelCommandHelper.isValidPlanningStructure(planningStructure, messageManager, defaultReturnCode, useStrictErrorHandling);
		...
		{
			hasPlanningStructure = true;
			returnCode = oFF.PlanningModelCommandHelper.isValidPlanningStructure(planningStructure, messageManager, defaultReturnCode, useStrictErrorHandling);
		}
	...
	isValidPlanningStructure:function(planningStructure, messageManager, defaultReturnCode, useStrictErrorHandling)
	{
			var returnCode = oFF.PrUtils.getIntegerValueProperty(planningStructure, "return_code", defaultReturnCode);
		if (oFF.isNull(planningStructure))
		{
	...
	processResponseEndActionSequence:function(commands, responses, version)
	{
			var commandResponse = oFF.PlanningModelCommandHelper.getCommandResponse(commands, responses, oFF.PlanningModelRequestType.END_ACTION_SEQUENCE.getName());
		if (oFF.notNull(commandResponse))
		{
			version.setActionSequenceId(null);
		}
	},
	processResponseKillActionSequence:function(commands, responses, version)
	{
			var commandResponse = oFF.PlanningModelCommandHelper.getCommandResponse(commands, responses, oFF.PlanningModelRequestType.KILL_ACTION_SEQUENCE.getName());
		if (oFF.notNull(commandResponse))
		{
			version.setActionSequenceId(null);
		}
	...
	{
			var commandResponse = oFF.PlanningModelCommandHelper.getCommandResponse(commands, responses, "get_version_state");
		...
		return oFF.PlanningModelCommandHelper.setVersionState(versionStateResponse, version);
	},
	setVersionState:function(versionStateResponse, version)
	{
		...
		oFF.PlanningModelCommandHelper.resetActionState(versionStateResponse, version);
		return true;
	},
	processResponseGetParameters:function(commands, responses, version)
	{
			var commandResponse = oFF.PlanningModelCommandHelper.getCommandResponse(commands, responses, "get_parameters");
		if (oFF.notNull(commandResponse))
		{
			oFF.PlanningModelCommandHelper.resetVersionParameters(commandResponse, version);
		}
	},
	processResponseStartActionSequence:function(commands, responses, response)
	{
			var request = response.getPlanningModelRequestVersion();
		var commandName = request.getRequestType().getName();
		var commandResponse = oFF.PlanningModelCommandHelper.getCommandResponse(commands, responses, commandName);
		if (oFF.isNull(commandResponse))
		{
			return false;
		}
	...
	processResponseStateDescriptions:function(commands, responses, response)
	{
			var request = response.getPlanningModelRequestVersion();
		var commandName = request.getRequestType().getName();
		var commandResponse = oFF.PlanningModelCommandHelper.getCommandResponse(commands, responses, commandName);
		...
		var descriptions = oFF.PlanningModelCommandHelper.getVersionStateDescriptionList(versionStateStructure);
	...
	getVersionStateDescriptionList:function(stateDescriptionsStructure)
	{
		...
		{
			var stateDescription = oFF.PlanningModelCommandHelper.getVersionStateDescription(statesList.getStructureAt(i));
	...
	{
			var commandResponse = oFF.PlanningModelCommandHelper.getCommandResponse(commands, responses, "get_versions");
		if (oFF.isNull(commandResponse))
		{
			return false;
		}
		...
		for (var i = 0; i < versionsList.size(); i++)
		{
			...
			if (!oFF.PlanningModelCommandHelper.processVersionStructure(versionStructure, model))
			{
				isOk = false;
			}
		}
	...
	processVersionStructure:function(versionStructure, model)
	{
		...
		oFF.PlanningModelCommandHelper.resetVersionParameters(versionStructure, planningVersion);
		...
		oFF.PlanningModelCommandHelper.resetActionState(versionStructure, planningVersion);
		return true;
	},
	...
	{
			var commandResponse = oFF.PlanningModelCommandHelper.getCommandResponse(commands, responses, "get_query_sources");
		if (oFF.isNull(commandResponse))
		{
			return false;
		}
	...
	{
			var commandResponse = oFF.PlanningModelCommandHelper.getCommandResponse(commands, responses, "get_action_parameters");
		if (oFF.isNull(commandResponse))
		{
			return false;
		}
	...
	{
			var commandResponse = oFF.PlanningModelCommandHelper.getCommandResponse(commands, responses, "get_actions");
		if (oFF.isNull(commandResponse))
		{
			return false;
		}
...
oFF.PlanningModel.updateStateFromResponse = function(application, systemName, request, response, messageCollector)
{
	...
	if (oFF.PlanningModelCommandHelper.getResponsesReturnCodeStrict(response, messageCollector, 9999, true) !== 0)
	{
		return;
	}
	var planningModel = planningService.getPlanningContext();
...
oFF.PlanningServiceConfig.prototype.getMatchingServiceForServiceName = function(serviceReferenceName)
{
	...
	else if (this.getSystemType() === oFF.SystemType.HANA)
	{
...
oFF.DataAreaCommand.prototype.serializeDataAreaCommandToStructure = function()
{
	...
	var planningContextDataArea = this.getDataArea();
...
oFF.DataAreaCommand.prototype.createCommandStructure = function()
{
	...
	this.getPlanningService().getInaCapabilities().exportActiveCapabilities(planningStructure);
...
oFF.DataAreaCommandResult.prototype.processResponseStructureCommand = function(responseStructure, messageManager, hasErrors)
{
	if (hasErrors)
	{
		return;
	}
	var commandResultsList = oFF.PrUtils.getListProperty(responseStructure, "CommandResults");
	...
	var executed = oFF.PrUtils.getBooleanProperty(commandResultStructure, "Executed").getBoolean();
...
oFF.PlanningModelCommand.prototype.createCommandStructure = function()
{
	...
	var request = oFF.PrFactory.createStructure();
	var commandList = oFF.PlanningModelCommandHelper.createCommandsList(request);
	this.fillCommandList(commandList, this.getPlanningContextCommandType());
	commandList.add(oFF.PlanningModelCommandHelper.createModelCommand(model, "get_versions"));
	return request;
};
oFF.PlanningModelCommand.prototype.fillCommandList = function(commandList, commandType)
{
	...
	if (commandType === oFF.PlanningContextCommandType.REFRESH || commandType === oFF.PlanningContextCommandType.HARD_DELETE)
	{
		...
		{
			commandList.add(oFF.PlanningModelCommandHelper.createModelCommand(this.getPlanningModel(), "delete_all_versions"));
		}
...
oFF.PlanningModelCommand.prototype.addCommandPerActiveVersionToList = function(commandList, commandName)
{
	var versions = this.getPlanningModel().getActiveVersions();
	for (var i = 0; i < versions.size(); i++)
	{
		var version = versions.get(i);
		var command = oFF.PlanningModelCommandHelper.createVersionCommand(version, commandName);
		this.addParametersToJson(command);
		commandList.add(command);
	}
};
oFF.PlanningModelCommand.prototype.addRefreshCommandsToList = function(commandList)
{
	var model = this.getPlanningModel();
	if (!model.isModelInitialized())
	{
		commandList.add(oFF.PlanningModelCommandHelper.createModelCommand(model, "get_query_sources"));
		commandList.add(oFF.PlanningModelCommandHelper.createModelCommand(model, "get_actions"));
		if (model.supportsVersionParameters())
		{
			commandList.add(oFF.PlanningModelCommandHelper.createModelCommand(model, "get_parameters"));
		}
		commandList.add(oFF.PlanningModelCommandHelper.createModelCommand(model, "get_action_parameters"));
	}
};
...
oFF.PlanningModelCommand.prototype.addParametersToJson = function(parametersStructure) {};
...
oFF.PlanningModelCommandResult.prototype.processResponseStructureCommand = function(responseStructure, messageManager, hasErrors)
{
	var returnCode = oFF.PlanningModelCommandHelper.getResponsesReturnCodeStrict(responseStructure, messageManager, 9999, true);
...
oFF.PlanningModelCommandResult.prototype.processResponseStructureInternal = function(responseStructure)
{
	var command = this.getPlanningModelCommand();
	var commands = oFF.PlanningModelCommandHelper.getCommandsList(command.serializeToJson());
	if (oFF.isNull(commands))
	{
		return false;
	}
	var responses = oFF.PlanningModelCommandHelper.getResponsesList(responseStructure);
	if (oFF.isNull(responses))
	{
		return false;
	}
	var commandType = this.getPlanningModelCommand().getPlanningContextCommandType();
...
oFF.PlanningModelCommandResult.prototype.refreshResponseStructrue = function(commands, responses, model)
{
	if (!oFF.PlanningModelCommandHelper.processResponseGetQuerySources(commands, responses, model))
	{
		return false;
	}
	if (!oFF.PlanningModelCommandHelper.processResponseGetActions(commands, responses, model))
	{
		return false;
	}
	if (!oFF.PlanningModelCommandHelper.processResponseGetParametersForModelTemplate(commands, responses, model))
	{
		return false;
	}
	return oFF.PlanningModelCommandHelper.processResponseGetActionParameters(commands, responses, model);
};
oFF.PlanningModelCommandResult.prototype.checkErrorState = function()
{
	if (this.getReturnCode() === 3042)
	{
		var planningModel = this.getPlanningModelCommand().getPlanningModel();
...
{
	var request = oFF.PrFactory.createStructure();
	var commands = oFF.PlanningModelCommandHelper.createCommandsList(request);
	...
	var actionSequenceId = this.getSequenceIdEffective();
	...
	{
		var actionDescription = this.getDescription();
	...
	else
	{
		...
		{
			var variables = this.getVariableProcessor().getVariables();
...
oFF.PlanningAction.prototype.addCommand = function(commandsList, commandName)
{
	var version = this.getVersion();
	var commandStructure = oFF.PlanningModelCommandHelper.createVersionCommand(version, commandName);
...
oFF.PlanningActionResult.prototype.processResponseStructureCommand = function(responseStructure, messageManager, hasErrors)
{
	var returnCode = oFF.PlanningModelCommandHelper.getResponsesReturnCodeStrict(responseStructure, messageManager, 9999, true);
	var responses = oFF.PlanningModelCommandHelper.getResponsesList(responseStructure);
	if (oFF.isNull(responses))
	...
	var action = this.getPlanningAction();
	var commands = oFF.PlanningModelCommandHelper.getCommandsList(action.serializeToJson());
	if (oFF.isNull(commands))
	{
		if (returnCode === 0)
		{
	...
	var version = action.getVersion();
	if (!oFF.PlanningModelCommandHelper.processResponseGetVersionState(commands, responses, version))
	{
		if (returnCode === 0)
		{
			returnCode = -1;
...
oFF.PlanningModelResponse.prototype.checkErrorState = function()
{
	if (this.getReturnCode() === 3042)
	{
		var planningModel = this.getPlanningModelRequest().getPlanningModel();
		planningModel.resetPlanningModel();
	}
};
...
oFF.DataAreaCommandResultClose.prototype._ff_c = "DataAreaCommandResultClose";

oFF.DataAreaCommandResultClose.prototype.processResponseStructureCommand = function(responseStructure, messageManager, hasErrors)
{
	if (hasErrors)
	{
		return;
	}
	var dataArea = this.getDataAreaCommand().getDataArea();
...
oFF.DataAreaRequestGetPlanningOperationMetadata.prototype.createCommandStructure = function()
{
	...
	var metadata = request.putNewStructure("Metadata");
	this.getPlanningService().getInaCapabilities().exportActiveCapabilities(metadata);
...
oFF.DataAreaResponseGetPlanningOperationMetadata.prototype.extractBaseDataSource = function(step)
{
...
oFF.PlanningModelCloseCommand.prototype.addParametersToJson = function(parametersStructure)
{
...
oFF.PlanningModelRequestCleanup.prototype.createCommandStructure = function()
{
	...
	commands.add(oFF.PlanningModelCommandHelper.createModelCommand(this.getPlanningModel(), "get_versions"));
	return request;
};
...
oFF.PlanningModelRequestRefreshActions.prototype.createCommandStructure = function()
{
	...
	var commands = oFF.PlanningModelCommandHelper.createCommandsList(request);
	var model = this.getPlanningModel();
	commands.add(oFF.PlanningModelCommandHelper.createModelCommand(model, "get_actions"));
	commands.add(oFF.PlanningModelCommandHelper.createModelCommand(model, "get_action_parameters"));
	return request;
};
...
oFF.PlanningModelRequestRefreshVersions.prototype.createCommandStructure = function()
{
	...
	var commands = oFF.PlanningModelCommandHelper.createCommandsList(request);
	var model = this.getPlanningModel();
	commands.add(oFF.PlanningModelCommandHelper.createModelCommand(model, "get_versions"));
	return request;
};
...
oFF.PlanningModelRequestUpdateVersionPrivileges.addShowVersionPrivilegesCommands = function(commandsList, planningModel)
{
	...
	for (var i = 0; i < queryDataSources.size(); i++)
	{
		...
		{
			var version = versions.get(j);
			oFF.PlanningModelRequestUpdateVersionPrivileges.addShowVersionPrivilegesCommand(commandsList, planningModel, queryDataSource, version);
		}
	}
};
oFF.PlanningModelRequestUpdateVersionPrivileges.addShowVersionPrivilegesCommand = function(commandsList, planningModel, dataSource, version)
{
...
oFF.PlanningModelRequestUpdateVersionPrivileges.addPrivilegeCommand = function(commandsList, versionPrivilege)
{
	var privilegeState = versionPrivilege.getPrivilegeState();
	if (privilegeState !== oFF.PlanningPrivilegeState.TO_BE_GRANTED && privilegeState !== oFF.PlanningPrivilegeState.TO_BE_REVOKED)
	{
...
oFF.PlanningModelRequestUpdateVersionPrivileges.prototype.createCommandStructure = function()
{
	...
	if (planningModel.isVersionPrivilegesInitialized())
	{
		...
		if (oFF.notNull(versionPrivileges))
		{
			for (var i = 0; i < versionPrivileges.size(); i++)
			{
				oFF.PlanningModelRequestUpdateVersionPrivileges.addPrivilegeCommand(commandsList, versionPrivileges.get(i));
			}
		}
	}
	oFF.PlanningModelRequestUpdateVersionPrivileges.addShowVersionPrivilegesCommands(commandsList, planningModel);
	return request;
};
...
oFF.PlanningModelRequestVersion.prototype.createCommandStructure = function()
{
	...
	{
		this.addCommand(commands, oFF.PlanningModelRequestType.CLOSE_VERSION.getName());
		this.addCommand(commands, oFF.PlanningModelRequestType.INIT_VERSION.getName());
	}
	else if (requestType !== oFF.PlanningModelRequestType.UPDATE_PARAMETERS)
	{
		...
		{
			var requestCloseVersion = this;
			if (requestCloseVersion.getCloseMode().isWithKillActionSequence())
			{
				this.addCommand(commands, oFF.PlanningModelRequestType.KILL_ACTION_SEQUENCE.getName());
			}
		}
		this.addParametersToJson(this.addCommand(commands, this.getRequestType().getName()));
	}
...
oFF.PlanningModelRequestVersion.prototype.addGetVersionStateCommand = function(commands, requestType)
{
...
oFF.PlanningModelRequestVersion.prototype.addGetParametersCommand = function(commands, requestType)
{
...
oFF.PlanningModelRequestVersion.prototype.addCommand = function(commandsList, commandName)
{
	var commandStructure = oFF.PlanningModelCommandHelper.createVersionCommand(this.getPlanningVersion(), commandName);
	commandsList.add(commandStructure);
	return commandStructure;
};
...
oFF.PlanningModelRequestVersion.prototype.addParametersToJson = function(parametersStructure) {};
...
oFF.PlanningModelResponseCleanup.prototype.processResponseStructureInternal = function(responseStructure, messageManager)
{
	var request = this.getPlanningModelRequest();
	var model = request.getPlanningModel();
	var commands = oFF.PlanningModelCommandHelper.getCommandsList(request.serializeToJson());
	var responses = oFF.PlanningModelCommandHelper.getResponsesList(responseStructure);
	return oFF.PlanningModelCommandHelper.processResponseGetVersions(commands, responses, model);
};
...
oFF.PlanningModelResponseRefreshActions.prototype.processResponseStructureInternal = function(responseStructure, messageManager)
{
	var request = this.getPlanningModelRequest();
	var commands = oFF.PlanningModelCommandHelper.getCommandsList(request.serializeToJson());
	var responses = oFF.PlanningModelCommandHelper.getResponsesList(responseStructure);
	return oFF.PlanningModelCommandHelper.processResponseGetActions(commands, responses, request.getPlanningModel());
};
...
oFF.PlanningModelResponseRefreshVersions.prototype.processResponseStructureInternal = function(responseStructure, messageManager)
{
	var request = this.getPlanningModelRequest();
	var commands = oFF.PlanningModelCommandHelper.getCommandsList(request.serializeToJson());
	var responses = oFF.PlanningModelCommandHelper.getResponsesList(responseStructure);
	return oFF.PlanningModelCommandHelper.processResponseGetVersions(commands, responses, request.getPlanningModel());
};
...
oFF.PlanningModelResponseUpdateVersionPrivileges.prototype.processResponseStructureInternal = function(responseStructure, messageManager)
{
	return this.processUpdate(responseStructure, messageManager);
};
oFF.PlanningModelResponseUpdateVersionPrivileges.prototype.processUpdate = function(responseStructure, messageManager)
{
	var request = this.getPlanningModelRequest();
	var model = request.getPlanningModel();
	model.setVersionPrivilegesInitialized();
	...
	oFF.PlanningModelResponseUpdateVersionPrivileges.resetVersionPrivilegesServerState(this.getPlanningModelRequest().getPlanningModel());
	for (var i = 0; i < commands.size(); i++)
	{
		var command = oFF.PrUtils.getStructureElement(commands, i);
	...
	oFF.PlanningModelResponseUpdateVersionPrivileges.resetVersionPrivilegesClientState(this.getPlanningModelRequest().getPlanningModel());
	return true;
};
...
oFF.PlanningModelResponseVersion.prototype.initResponseStructureCommand = function(responseStructure, messageManager)
{
...
oFF.PlanningModelResponseVersion.prototype.processResponseStructureInternal = function(responseStructure, messageManager)
{
	var responses = oFF.PlanningModelCommandHelper.getResponsesList(responseStructure);
	if (oFF.isNull(responses))
	{
	...
	var request = this.getPlanningModelRequestVersion();
	var commands = oFF.PlanningModelCommandHelper.getCommandsList(request.serializeToJson());
	if (oFF.isNull(commands))
	{
		return false;
	...
	var version = request.getPlanningVersion();
	oFF.PlanningModelCommandHelper.processResponseEndActionSequence(commands, responses, version);
	oFF.PlanningModelCommandHelper.processResponseKillActionSequence(commands, responses, version);
	oFF.PlanningModelCommandHelper.processResponseGetParameters(commands, responses, version);
	if (!oFF.PlanningModelCommandHelper.processResponseGetVersionState(commands, responses, version))
	{
...
{
	var request = oFF.DataAreaRequestGetPlanningOperationMetadata.prototype.createCommandStructure.call( this );
	var metadata = request.getStructureByKey("Metadata");
	var dataSource = metadata.getStructureByKey("DataSource");
	dataSource.putString("ObjectName", this.getPlanningFunctionIdentifier().getPlanningFunctionName());
...
oFF.DataAreaRequestGetPlanningSequenceMetadata.prototype.createCommandStructure = function()
{
	...
	var metadata = request.getStructureByKey("Metadata");
	var dataSource = metadata.getStructureByKey("DataSource");
	dataSource.putString("ObjectName", this.getPlanningSequenceIdentifier().getPlanningSequenceName());
...
oFF.DataAreaResponseGetPlanningFunctionMetadata.prototype.processResponseStructureInternal = function(responseStructure, messageManager)
{
	var command = oFF.PrUtils.getStructureProperty(responseStructure, "Command");
	...
	var request = this.getDataAreaRequestGetPlanningFunctionMetadata();
	var dataArea = request.getDataArea();
	if (!oFF.XString.isEqual(dataArea.getDataArea(), dataAreaName))
	{
		return false;
	}
	var instanceIdRequest = this.getDataAreaRequestGetPlanningFunctionMetadata().getInstanceId();
	if (oFF.isNull(instanceIdRequest))
	{
		return false;
	}
	var objectName = oFF.PrUtils.getStringValueProperty(dataSource, "ObjectName", null);
	var planningFunctionIdentifier = this.getDataAreaRequestGetPlanningFunctionMetadata().getPlanningFunctionIdentifier();
	...
	var baseDatasource = this.extractBaseDataSource(command);
...
oFF.DataAreaResponseGetPlanningSequenceMetadata.prototype.processResponseStructureInternal = function(responseStructure, messageManager)
{
	var command = oFF.PrUtils.getStructureProperty(responseStructure, "Command");
	if (oFF.isNull(command))
	{
	...
	var dataSource = oFF.PrUtils.getStructureProperty(command, "DataSource");
	var dataAreaName = oFF.PrUtils.getStringValueProperty(dataSource, "DataArea", "DEFAULT");
	var request = this.getDataAreaRequestGetPlanningSequenceMetadata();
	var dataArea = request.getDataArea();
	if (!oFF.XString.isEqual(dataArea.getDataArea(), dataAreaName))
	{
		return false;
	}
	var instanceIdRequest = this.getDataAreaRequestGetPlanningSequenceMetadata().getInstanceId();
	if (oFF.isNull(instanceIdRequest))
	{
		return false;
	}
	...
	var objectName = oFF.PrUtils.getStringValueProperty(dataSource, "ObjectName", null);
	var planningSequenceIdentifier = this.getDataAreaRequestGetPlanningSequenceMetadata().getPlanningSequenceIdentifier();
	...
	var dimensions = oFF.PrUtils.getListProperty(command, "Dimensions");
	var variables = oFF.PrUtils.getListProperty(command, "Variables");
	var stepMetadataList = this.extractStepMetadataList(command);
...
oFF.DataAreaResponseGetPlanningSequenceMetadata.prototype.extractStepMetadataList = function(command)
{
	...
	for (var i = 0; i < steps.size(); i++)
	{
		...
		var baseDataSource = this.extractBaseDataSource(step);
...
oFF.PlanningModelRequestVersionClose.prototype.addParametersToJson = function(parametersStructure)
{
	var closeMode = this.getCloseMode();
...
oFF.PlanningModelRequestVersionEndActionSequence.prototype.addParametersToJson = function(parametersStructure)
{
...
oFF.PlanningModelRequestVersionSetParameters.prototype.addParametersToJson = function(parametersStructure)
{
...
oFF.PlanningModelRequestVersionSetTimeout.prototype.addParametersToJson = function(parametersStructure)
{
...
oFF.PlanningModelRequestVersionStartActionSequence.prototype.addParametersToJson = function(parametersStructure)
{
	var sequenceId = this.getSequenceId();
	if (oFF.XStringUtils.isNotNullAndNotEmpty(sequenceId))
	{
...
oFF.PlanningModelRequestVersionStateDescriptions.prototype.addParametersToJson = function(parametersStructure)
{
...
oFF.PlanningModelRequestVersionUndoRedo.prototype.addParametersToJson = function(parametersStructure)
{
...
oFF.PlanningModelResponseVersionStartActionSequence.prototype.processResponseStructureInternal = function(responseStructure, messageManager)
{
	var result = oFF.PlanningModelResponseVersion.prototype.processResponseStructureInternal.call( this , responseStructure, messageManager);
	if (!result)
	{
		return false;
	}
	...
	var request = this.getPlanningModelRequestVersion();
	var commands = oFF.PlanningModelCommandHelper.getCommandsList(request.serializeToJson());
	return oFF.PlanningModelCommandHelper.processResponseStartActionSequence(commands, responses, this);
};
...
oFF.PlanningModelResponseVersionStateDescriptions.prototype.processResponseStructureInternal = function(responseStructure, messageManager)
{
	var result = oFF.PlanningModelResponseVersion.prototype.processResponseStructureInternal.call( this , responseStructure, messageManager);
	if (!result)
	{
	...
	var request = this.getPlanningModelRequestVersion();
	var commands = oFF.PlanningModelCommandHelper.getCommandsList(request.serializeToJson());
	return oFF.PlanningModelCommandHelper.processResponseStateDescriptions(commands, responses, this);
};
...
oFF.PlanningModelRequestVersionInit.prototype.addParametersToJson = function(parametersStructure)
{
	oFF.PlanningModelRequestVersionSetParameters.prototype.addParametersToJson.call( this , parametersStructure);
	var description = this.getPlanningVersion().getVersionDescription();
	if (oFF.XStringUtils.isNotNullAndNotEmpty(description))
	{
		parametersStructure.putString("description", description);
	}
	var restoreBackupType = this.getRestoreBackupType();
