...
sap.ui.define(
  ...
  function( jQuery, BaseHandler ){
      ...
      this.create = function(oChainedControl, oControlProperties) {
        ...
        if (oControlProperties.statistics) {
          ...
          oDownloadButton.attachPress(function() {
            var newCommand = new sap.zen.dsh.sapbi_Command("DOWNLOAD_STATISTICS");
            newCommand.addParameter(new sap.zen.dsh.sapbi_Parameter("TARGET_ITEM_REF", "ZEN_PROFILING"));
            newCommand.addParameter(new sap.zen.dsh.sapbi_Parameter("MILLI_SECONDS", oIgnoreLowTimes.getSelectedKey()));
            newCommand.addParameter(new sap.zen.dsh.sapbi_Parameter("DOWNLOAD_TYPE", oDownloadType.getSelectedKey()));
            sap.zen.dsh.sapbi_page.sendCommand(newCommand);
          });
        ...
        var oCloseButton = this.createButton(
          {
            text: "Close",
            press: function(){
              oControl.close();
    ...
    var instance = new StatisticsPanelHandler();
