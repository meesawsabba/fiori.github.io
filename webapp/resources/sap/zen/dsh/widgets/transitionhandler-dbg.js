  ...
  function(jQuery) {
    ...
    var Base = function() {
      ...
      this.preOperations = function(oContext) {
        var oAbsLayout = oContext.oAbsLayout;
        var oComponentProperties = oContext.oComponentProperties;
        var position = this.positioner.calculatePosition(oComponentProperties);
      ...
      this.reDocking = function(ref) {
        var dockingPosition = this.positioner.calculateRedocking(ref);
        ref.css({
    ...
    var SlideHorizontal = function() {
      var that = this, slideFrom = "left", position, toHandle = null;
      Base.apply(this, arguments);
      this.preOperations = function(oContext) {
        var oAbsLayout = oContext.oAbsLayout;
        var oComponentProperties = oContext.oComponentProperties;
        var offset = this.positioner.getHorizontalOffset(oComponentProperties);
      ...
      this.close = function($Ref, iDuration, fnCallback) {
        ...
        var cssObj = (slideFrom === "left" ? {"left"  : (-$Ref.width() - 30)+"px"}  : {"right" : (-$Ref.width() - 30)+"px"} );
        var viewportWidth = jqBody.width();
        var deltaWidth = $Ref.outerWidth()-$Ref.width();
        var width;
        var dockingPosition;
    ...
    var SlideVertical = function() {
      ...
      var that = this, slideFrom = "top", position, toHandle = null;
      Base.apply(this, arguments);
      this.preOperations = function(oContext) {
        var oComponentProperties = oContext.oComponentProperties;
        var offset = this.positioner.getVerticalOffset(oComponentProperties);
        position = this.positioner.calculatePosition(oComponentProperties),
      ...
      this.close = function($Ref, iDuration, fnCallback) {
        var cssObj = (slideFrom === "top" ? {"top"  : (-$Ref.height() - 30)+"px"} : {"bottom" : (-$Ref.height() - 30)+"px"});
        var viewportHeight = jQuery("body").height();
        var deltaHeight = $Ref.outerHeight()-$Ref.height(), height, dockingPosition;
        if(toHandle) {
          clearTimeout(toHandle);
        }
        else {
