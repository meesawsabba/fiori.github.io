...
sap.ui.define(
  ...
  function(jQuery, _, BaseHandler, List, StandardListItem) {
    "use strict";
    StandardListItem.extend(
      "sap.zen.StandardListItemWithKey", {
        metadata : {
          properties : {
            key : "string"
          }
      ...
      "sap.zen.MListWithHeight",
      {
        ...
        onAfterRendering : function(evt) { // is called when the Control's area is
          ...
          if (List.prototype.onAfterRendering) {
            List.prototype.onAfterRendering.apply(this, [ evt ]);
          }
      ...
      this.create = function (oChainedControl, oControlProperties) {
        ...
        oListBox.attachSelect(
          function (oControlEvent) {
            ...
            var command = that.prepareCommand(
              oControlProperties.command,"__KEYS__", keys
            );
            eval(command);
          }
        );
      ...
      function itemsAreChanged(oControl, oControlProperties) {
        ...
        if (aGetSelectedItems) {
          ...
          for (i = 0; i < aGetSelectedItems.length; i++) {
            if(jQuery.inArray(aGetSelectedItems[i].getKey(),aSelectedItemKey) !== -1){
              counter++;
            }
          }
    ...
    var instance = new ListBoxHandler();
