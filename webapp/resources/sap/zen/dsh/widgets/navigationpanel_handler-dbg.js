...
sap.ui.define(
  [
    ...
    "sap/zen/dsh/utils/jq"
  ],
  function(jQuery, Log, JSONModel, HAlign, BaseHandler) {
    "use strict";
    var NavigationPanelHandler = function() {
      ...
      this.create = function(oChainedControl, oControlProperties) {
        ...
        BaseHandler.dispatcher.registerUnhandledDropHandler(oControlProperties.id, this.onUnhandledDrop);
        BaseHandler.dispatcher.registerDragDropCancelHandler(oControlProperties.id, this.onEscKeyPressed);
        //prepare model
        ...
        oControl.onAfterRendering = function() {
          var jqControl = this.$();
          var aTrs = jQuery(jqControl.children("tbody")).children("tr");
          var aTrsLength = aTrs.length - 1;
          for (var i = 0; i < aTrsLength; i++) {
            var aTr = aTrs[i];
            jQuery(aTr).css("height", "0px");
          }
        };
        return oControl;
      };
      ...
      this.getRowFactory = function(oControl, sKeyPath, sTextPath, fTextFormatter, bMarker, fCallback) {
        return function(sRowId, oContext) {
          ...
          if (bMarker) {
            ...
            markerMatrixCell = new sap.zen.commons.layout.MatrixLayoutCell({hAlign: HAlign.Center});
          ...
          matrixCell.addContent(
            that.createLabel(
              ...
              "zenDimensionId"
            ).addStyleClass("zenHideFilterPanel")
          );
          ...
          if (bMeasure) {
            oRowTemplate.addEventDelegate({
              onAfterRendering:function() {
                var that = oRowTemplate;
                jQuery(that.getDomRef()).attr("tabIndex","0");
          ...
          var oCLM = new sap.m.CustomListItem(sListItemId, {
            content: [oRowTemplate],

          });
          oCLM.onAfterRendering= function(){
            var that = this;
            jQuery(that.getDomRef()).attr("tabIndex","0");
          };
          return oCLM;
      ...
      this.getDimensionsOnAfterRendering = function(oControl, sRowRepeaterId, sRowRepeaterRowsId, sRowRepeaterColumnsId) {
        return function() {
          ...
          var jqRowRepeater = jQuery(document.getElementById(sRowRepeaterId));
          ...
          var oDraggable = {
            helper: function() {
              // fill context
              var dimId = that.getDimensionNameAndIdForJQElement(jQuery(this)).dimensionName;
              var oPayload = BaseHandler.dispatcher.createDragDropPayload(oControl.getId());
              oPayload.oDragDropInfo.sDimensionName = dimId;
              BaseHandler.dispatcher.setDragDropPayload(oPayload);
              oJqCurrentDraggable = jQuery(this);
              return jQuery("<div class='zenDnDHelper'></div>").append(jQuery(jQuery(this).find("span").not(".sapUiIcon")[0]).clone());
            },
            appendTo: ( function(){
              var oRootContorl = BaseHandler.dispatcher.getRootControlForComponentId(
                sRootComponentId
              );
              if(!oRootContorl){
                Log.error("Cannot find root control: " + sRootComponentId );
                return null;
              }
              return document.getElementById(
                BaseHandler.dispatcher.getRootControlForComponentId(
                  sRootComponentId
                ).getId()
              );
            }()),
            revert: function(ui) {
              if (BaseHandler.dispatcher.isDragDropCanceled()) {
                this.context.ZENcancel = function() {
                  var elements;
                  if (jQuery(document.getElementById(sRowRepeaterRowsId)).find(ui).length > 0 || jQuery(document.getElementById(sRowRepeaterColumnsId)).find(ui).length > 0) {
                    elements = ui.sortable("cancel").find("li");
                    jQuery(elements[elements.length - 1]).remove();
                  }
                };
                return true;
              }
              var oPayload = BaseHandler.dispatcher.getDragDropPayload();
              if (oPayload && ui && oPayload.oInterComponentAcceptStatus[jQuery(ui).attr("id")] === true) {
                return false;
              }
              //logic to determine, if the element is already in rows/columns
              var dimensionElement = that.getDimensionNameAndIdForJQElement(jQuery(this));
              if (ui) {
                ...
                for (var i = 0; i < aDimensions.length; i++) {
                  var qvDimension = that.getDimensionNameAndIdForJQElement(jQuery(aDimensions[i]));
            ...
            start: function (e) {
              ...
              var jqElement = jQuery(e.toElement);
          ...
          jqRepeaterElements.mousedown(
            function(e) {
              BaseHandler.dispatcher.closeContextMenu();
              //prevent only, if not the expand icon for measures was clicked
              if (e && (!e.target || !jQuery(e.target).hasClass("sapzenExpandMeasuresIcon"))) {
                if (e.preventDefault) {
                  e.preventDefault();
                }
      ...
      this.getAxisOnAfterRendering = function(sAxis, sConnectWith, oControl, fApply) {
        return function() {
          ...
          var jqPage = jQuery(document.getElementById(sRowRepeaterId)).children("ul");
          jQuery(document.getElementById(sRowRepeaterId+"-body")).height("100%");
          jqPage.height("100%");
          jqPage.find(".sapUiRrNoData").remove();
          jqPage.find(".sapMListNoData").remove();
          var jqRow = jQuery(document.getElementById(sRowRepeaterId)).find("li");
          ...
          if (jqPage.sortable) {
            ...
            jqPage.sortable(
              {
                containment: jQuery(document.getElementById(oControl.getId())),
                ...
                helper: function(e,elem) {
                  ...
                  BaseHandler.dispatcher.setDragDropPayload(oPayload);
                  oJqCurrentSortable = jQuery(this);
                  return jQuery(
                    "<div class='zenDnDHelper'></div>"
                  ).append(jQuery(elem).find("span").clone());
                },
                ...
                update: function(e, ui) {
                  ...
                  var aElements = jQuery(this).find("li");
                  ...
                  for (i = 0; i < aElements.length; i++) {
                    var sDimensionId = that.getDimensionNameAndIdForJQElement(jQuery(aElements[i])).dimensionName;
                    if (sDimensionId) {
                      var bPush = true;
                  ...
                  for (i = 0; i < aElementsToRemove.length; i++) {
                    jQuery(aElements[aElementsToRemove[i]]).remove();
                  }
      ...
      this.getContextMenuAction = function(sContextMenuComponentId, oClickedUI5Component, oDomClickedElement) {
        ...
        if (sCommand) {
          ...
          else if (sClickedTag === "td" || sClickedTag === "li" ) {
