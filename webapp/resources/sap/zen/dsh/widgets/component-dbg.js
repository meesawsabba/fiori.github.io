  ...
  function(jQuery, Log, Request){
    ...
    var Component = function(phxOwner) {
        ...
        if (!bIsInPropertyDispatch) {
          if (typeof (sap.zen.dsh.DSH_deployment) == "undefined") {
            ...
            sap.zen.dsh.putInQueue(function(){
              sap.zen.dsh[buddhaId].exec(sCommand);
            });
          }
        }
      };
      ...
      function createCommand(aNames) {
        ...
        command.addParameter(new sap.zen.dsh.sapbi_Parameter(this.getTargetParam(), this.oComponentProperties.id));
      ...
      this.firePropertiesChanged = function(aNames) {
        if (!bIsInPropertyDispatch) {
          if (typeof (sap.zen.dsh.DSH_deployment) == "undefined") {
            ...
            sap.zen.dsh.putInQueue(function() {
              sap.zen.dsh[buddhaId].exec(idToUse + ".doSDKPVT(" + JSON.stringify(tempReturnJSON) + ");");
            });
          }
        }
      };
