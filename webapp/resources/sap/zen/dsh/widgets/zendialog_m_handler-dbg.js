...
sap.ui.define(
  ...
  function(jQuery, _, BaseHandler, Dialog, MatrixLayout, HAlign) {
    "use strict";
    sap.zen.ZenDialogHandler = function () {
      ...
      this.createAndAdd = function (oChainedControl, oControlProperties, oComponentProperties, fAppendToParentFunclet, oArgForFunclet) {
        ...
        if (this.bIsRuntimeMode === true) {
          ...
          this.oTopLevelControl.destroy = function() {
            if (BaseHandler.dispatcher.oCurrentVarDlg) {
              BaseHandler.dispatcher.oCurrentVarDlg.destroy();
              delete BaseHandler.dispatcher.oCurrentVarDlg;
            }
            loOrigFuncDestroy.apply(this, arguments);
          };
          this.oTopLevelControl.aZenChildren = [];
          this.oTopLevelControl.aZenChildren.push(
            BaseHandler.dispatcher.oCurrentVarDlg
          );
          if (typeof(sap.zen.dsh.DSH_deployment) !== "undefined" && sap.zen.dsh.DSH_deployment === true) {
        ...
        if (oControlProperties) {
          if (oControlProperties.closeDialogCommand && oControlProperties.closeDialogCommand.length > 0) {
            ...
            if (this.validateDialogContent(oControl, oControlProperties, oComponentProperties)) {
              ...
              //Log.error("Invalid Dialog Content");
            }
          }
        }
      };
      ...
      this.initDialog = function(oControl) {
        ...
        if (!oControl.oDialog) {
            ...
            oControl.getId() + "_dlg",
            {
              ...
              "afterOpen" : function() {
                ...
                if (jQuery.browser.mozilla) {
                  loJqPayloadDiv = jQuery(document.getElementById(oControl.oDialogProperties.oContentControl.getId()));
          ...
          oControl.oDialog.attachBrowserEvent(
            "keydown",
            function(e) {
              if (e.which === 27) {
          ...
          BaseHandler.dispatcher.oCurrentVarDlg = oControl.oDialog;
          if (this.bIsRuntimeMode === true) {
            BaseHandler.dispatcher.oCurrentVarDlg.oDispatcherHook = this.oTopLevelControl;
          }
        }
        // update Dialog
        if (oControl.oDialog) {
      ...
      this.initDialogButtons = function(oControl) {
        ...
        var loFuncPress = function () {
          if (this.zenOkBtn === true) {
            BaseHandler.dispatcher.enableReady(true);
          }
      ...
      this.initDialogFooter = function(oControl) {
        ...
        for (i = 0; i < ltFooterContent.length; i++) {
          loContent = ltFooterContent[i];
          if (loContent) {
      ...
      this.openDialog = function() {
        var loDialog = BaseHandler.dispatcher.oCurrentVarDlg;
        if (loDialog) {
          loDialog.open();
        }
      };
      ...
      function addLoadingIndicatorCss() {
        ...
        jQuery("head").prepend("<style>.customLoadingIndicatorZenClass{" + lContent + "}</style>");
        that.loadingIndicatorCssHasBeenSet = true;
      }
      /**
       * Create Matrix Layout
       */
      this.createMatrixLayout = function(oControlProperties) {
        var loMatrixLayout = new MatrixLayout(
          oControlProperties["id"],
          {
            "layoutFixed": false,
            "height": "100%",
      ...
      this.createMatrixLayoutButtonArea = function () {
        ...
        loButtonRow.addCell(loMsgViewCell);
        return loButtonRow;
      };
      /**
      ...
      this.initMatrixLayoutRootArea = function () {
        var loJqMainDiv = jQuery(document.getElementById(sap.zen.ZenDialogHandler.anchorDivId));
        if(!loJqMainDiv.length) {
          loJqMainDiv = jQuery("<div id=\"" + sap.zen.ZenDialogHandler.anchorDivId + "\"></div>");
          jQuery("body").append(loJqMainDiv);
        }
        return loJqMainDiv;
      };
      /**
      ...
      this.calculateAndSetMatrixLayoutDialogDivSizes = function() {
        this.oDomContentArea = jQuery(".zenDialogContentArea");
        ...
        if (!this.bIsRuntimeMode) {
          ...
          jQuery(document.getElementById(this.oTopLevelControl.oMatrixLayout.getId())).parent().css("padding", "0px").css("height", "100%");
      ...
      this.setMatrixLayoutInitialSizes = function () {
        if (this.bBrowserResize) {
          this.initialWindowHeight = parseInt(jQuery(window).innerHeight(), 10);
          this.initialWindowWidth = parseInt(jQuery(window).innerWidth(), 10);
            ...
            if (ltContent && ltContent.length > 0) {
              ...
              for (var i = 1; i < ltContent.length; i++) {
                if (ltContent[i] !== this.oTopLevelControl.oDialogProperties.oMsgControl) {
                  ltNewContent.push(ltContent[i]);
                }
              }
              loDialogToolbar.removeAllContent();
              for (i = 0; i < ltNewContent.length; i++) {
                loDialogToolbar.addContent(ltNewContent[i]);
              }
            }
          }
        }
      ...
      this.browserResizeListener = function () {
        ...
        var newHeight = parseInt(jQuery(window).innerHeight(), 10);
        var deltaY = newHeight - this.initialWindowHeight;
        var newWidth = parseInt(jQuery(window).innerWidth(), 10);
    ...
    sap.zen.ZenDialogHandler.closeDialog = function() {
      ...
      if (loDialog) {
        ...
        BaseHandler.dispatcher.dispatchRemove(loDispatcherHook);
      }
    };
    var instance = new sap.zen.ZenDialogHandler();
