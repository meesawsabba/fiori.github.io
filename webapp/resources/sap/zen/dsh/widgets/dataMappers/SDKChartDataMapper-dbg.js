...
sap.ui.define(
  ...
  function(
    jQuery, Log, BaseHandler,SDKUtilsHelper, SDKChartTypeHelper, SDKDataMapperHelper, ChartException, CrossTableDataSetTemp
  ){
    "use strict";
    Log.info("Loaded SDKChartDataMapper");
    function SDKChartDataMapper () {
      ...
      this.setNumberOfDimensions = function (data) {
        ...
        jQuery.each(dimensions, function (index, dim) {
      ...
      this.setAnalysisAxisValue = function () {
      };
      ...
      var createMeasureValuesGroup = function (data, analysisAxis) {
        ...
        var tuples = {};
        jQuery.each(data.dimensions, function (index, dimension) {
        ...
        if (data.cvomType && data.cvomType === "viz/waterfall" || data.cvomType === "viz/horizontal_waterfall" || data.cvomType === "viz/stacked_waterfall" || data.cvomType.indexOf("pie") !== -1) {
          measureAxis = _this.getMeasureAxis();
          measureIndex = _this.getMeasureIndex();
          tuples = {};
          jQuery.each(data.dimensions, function (index, dimension) {
            if (dimension.containsMeasures) {
              tuples = dimension.members;
      ...
      this.createAnalysisAxis = function () {
        return null;
      };
      ...
      this.mergeValues = function (values, newValues, axis) {
        if (axis == "ROWS") {
          ...
          jQuery.each(newValues, function (index, value) {
      ...
      this.getDimensionLabel = function (dimension) {
