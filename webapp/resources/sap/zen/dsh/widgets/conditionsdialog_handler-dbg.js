...
sap.ui.define(
  ...
  function(jQuery, Log,_, BaseHandler) {
    "use strict";
    var ConditionsDialogHandler = function () {
      var handler = this;
      BaseHandler.apply(this, arguments);
      this.sLocalizationPrefix = "com.sap.zen.firefly.impl.";
      this.oRessources = jQuery.sap.resources({url: sap.ui.resource("sap.zen.dsh","i18n/localization/localization.properties")});
      ...
      this.create = function(oChainedControl, oControlProperties) {
        ...
        var oControl = oChainedControl || new sap.m.Button(oControlProperties.id, {
          visible: false
        });
        var oDialog =  getDialog(oModel);
      ...
      function getDialog(oModel) {
        ...
        var oLayout = new sap.m.VBox({fitContainer: true});
        var oRessourcesComp = sap.ui.getCore().getLibraryResourceBundle("sap.ui.comp");
        var oFilterPanelLayout = getFilterPanel(oModel);
        var oFilterPanel = oFilterPanelLayout.getContent()[0];
        var oAdditionalSettingsLayout = getAdditionalSettings();
        ...
        oDialog.attachBeforeOpen(function () {
          oFilterPanel.setConditions(
            oModel.getProperty("/condition/thresholds")
          );
        });
      ...
      function getFilterPanel(oModel) {
        ...
        var oConditionPanel = oFilterPanel._oIncludeFilterPanel;
        var fOrgGetFormatedConditionText = oConditionPanel._getFormatedConditionText;
    ...
    var instance = new ConditionsDialogHandler();
