...
sap.ui.define(
  ...
  function(jQuery, _, BaseHandler, MDialog) {
    "use strict";
    var DialogHandler = function() {
      ...
      this.createAndAdd = function(chainedControl, oControlProperties, oComponentProperties, fAppendToParentFunclet, oArgForFunclet) {
        if(sap.zen.designmode) {
          ...
          BaseHandler.dispatcher.updateComponentProperties(loAbsLayout, oComponentProperties);
            ...
            setTimeout(function(){
              if(that.footerArea){
                ...
                for(var i = ltPositions.length-1; i>=0; i--){
                  ...
                  loPosition.updatePosition({"right":lRight+"px","top":"0px"});
      ...
      this.initDialog = function(oControl) {
        ...
        var lShowDialog = Object.prototype.hasOwnProperty.call(loProperties, "show") && loProperties.show;

        // create Dialog
        if (!oControl.oDialog) {
          if (lShowDialog) {
            oControl.oDialog = new MDialog(
              oControl.getId() + "_dlg",
              {
      ...
      this.initDialogContent = function(oControl) {
        ...
        loAbsLayout = sap.ui.getCore().byId(oControl.getId() + "_abslayout");
        if (!loAbsLayout) {
        ...
        this.updateChildren(
          oControl.oDialogProperties.tDialogContent, loAbsLayout,
          function(oNewControl, index) {
            dispatcher.insertIntoAbsoluteLayoutContainer(loAbsLayout, oNewControl, index);
          },
          function(oControlToRemove) {
    ...
    var instance = new DialogHandler();
