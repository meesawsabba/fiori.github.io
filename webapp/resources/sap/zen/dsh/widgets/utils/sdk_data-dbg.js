...
sap.ui.define(
  ...
  function(jQuery,_,InfoChartException) {
    ...
    function SdkData(rawSdkResultSet) {
      ...
      applyMemberDisplayModeToDimensionMembers(this._dimensions);
      ...
      this._tupleTree = _.reduce(mergedData.tuples, function(tree, tuple, tupleIndex) {
        ...
        var measureValue = rawSdkResultSet.data[tupleIndex];
        var measureMemberKey = mergedData.measureMembers[tuple[mergedData.measureIndex]].key;
        var tupleWithoutMeasures = removeIndex(tuple, mergedData.measureIndex);
        var dimensionMembers = getDimensionMembers(mergedData.dimensions, tupleWithoutMeasures);
        var leaf = {
          "isLeaf": true,
          "measures": {}
        };
    ...
    SdkData.prototype.keepDimensions = function(dimensionKeys) {
      ...
      function removeDimensionIfNecessary(dimensionIndex, tupleTree) {
        ...
        var shouldRemoveDimension = !tupleTree.isLeaf && !_.includes(dimensionKeys, levelDimension.key);
        var memberToKeep, newTree;
        if (tupleTree.isLeaf) {
          newTree = tupleTree;
        } else if (shouldRemoveDimension) {
          memberToKeep = findFirstPropertyInObject(tupleTree, _.map(levelDimension.members, "key"));
      ...
      this._dimensions = _.filter(this._dimensions, function(dimension) {
        return _.includes(dimensionKeys, dimension.key);
      });
    };

    function applyMemberDisplayModeToDimensionMembers(tDimensions) {
      tDimensions.forEach(function (oDimension) {
        if (oDimension.displayMode && oDimension.displayMode !== "TEXT") {
          oDimension.members.forEach(function (oMember) {
            if (oDimension.displayMode === "KEY") {
              oMember.text = oMember.key + " (" + oMember.text + ")";
            } else if (oDimension.displayMode === "KEY_TEXT") {
              oMember.text = oMember.key + " | " + oMember.text;
            } else if (oDimension.displayMode === "TEXT_KEY") {
              oMember.text = oMember.text + " | " + oMember.key;
            }
          });
        }
      });
    ...
    SdkData.prototype.getConditionalFormatValues = function() {
      var that = this, conditionalFormatMap = {};
      _.forEach(getTupleArray(this._tupleTree, this._dimensions), function(leaf) {
        _.forEach(leaf.measures, function(measure, measureName) {
          if (measure.conditionalFormat) {
            var dimensionKeys = _.map(that._dimensions, "key");
            var memberKeys = _.map(leaf.tuple, "key");
            var conditionalFormatValue = _.object(dimensionKeys, memberKeys);
    ...
    function toFlatTupleData(sdkData, removeTotals) {
      var flatTupleData = _.map(
        ...
        function(leaf) {
          var tupleDimensionData = _.map(
            leaf.tuple,
            function(dimensionMember) {
              return {
          ...
          var tupleMeasureData = _.map(
            sdkData._measureMembers,
    ...
    */
    function getTupleArray(
      tupleTree, dimensions, removeTotals
    ) {
      var tupleArray = [];
      var currentTuplePath = [];
      var addTuples = function(
        tupleTree, tupleArray, currentTuplePath
      ) {
        if (tupleTree.isLeaf) {
          tupleTree.tuple = _.map(currentTuplePath, function(memberKey, dimensionIndex) {
            var member = _.find(dimensions[dimensionIndex].members, { "key": memberKey });
          ...
          _.forEach(
            tupleTree,
            function(childTree, dimensionMemberKey) {
      ...
      return _.sortBy(tupleArray, function(tuple) {
        var indices = _.map(tuple.measures, "index");
    ...
    function mergeExternalDimensionsAndStructs(rawSdkResultSet) {
      ...
      var tuples = cloneArray(rawSdkResultSet.tuples) || [];
      var externalMeasuresDimension;
      ...
      if(rawSdkResultSet.externalDimensions) {
        ...
        externalMeasuresDimension = _.find(rawSdkResultSet.externalDimensions, _.property("containsMeasures"));
