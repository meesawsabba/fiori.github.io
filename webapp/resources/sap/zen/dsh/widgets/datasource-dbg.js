...
sap.ui.define(
  ...
  function(jQuery, _, BaseHandler, Component){
    "use strict";
    var DataSource =  function() {
      ...
      this.componentDeleted = function() {
        BaseHandler.dispatcher.sdkDsDelete(this.oComponentProperties.id);
      };
      /**
      ...
      this.updateDataInto = function(oSelection, oOptions, oTarget) {
        ...
        if (data) { // TODO: fetchData currently returns undefined for bad selections. Not sure how to proceed here.
          jQuery.each(data, function(name, entry) {
            oTarget[name] = entry;
          });
        } else {
      ...
      this.fireUpdate = function(bViaBackend) {
        ...
        var delta = [];
        var dsEntries = BaseHandler.dispatcher.sdkDsFindInstances(id);
        jQuery.each(dsEntries, function(compName, compEntry) {
          var deltaEntry = {
            "component": {
              "id": compName,
              "content": {
          ...
          jQuery.each(compEntry, function(propName, entry) {
            that.updateDataInto(entry.selection, entry.options, entry.data);
            deltaEntry.component.content.control[propName] = entry.data;
          });
          if (BaseHandler.dispatcher.getRootControlForComponentId(compName)) {
            delta.push(deltaEntry);
          }
          else if (!BaseHandler.dispatcher.isDispatching())  {
            // Unsubscribe
        ...
        if (!BaseHandler.dispatcher.isDispatching()) {
          BaseHandler.dispatcher.dispatchDelta(delta);
        }
        if (bViaBackend) {
