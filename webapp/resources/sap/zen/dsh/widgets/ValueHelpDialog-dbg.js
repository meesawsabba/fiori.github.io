...
sap.ui.define(
  ...
  function(
     ...
     */
    var ValueHelpDialog = Dialog.extend(
    ...
    ValueHelpDialog.prototype.update = function(tMessages) {
        ...
        if (sap.ui.table.Table && this._oTable instanceof sap.ui.table.Table) {
          ...
          if (this.oRows.aKeys) {
            ...
            for (i = 0; i < ltSelectedItems.length; i++) {
              ...
              for (j = 0; j < this.oRows.aIndices.length; j++) {
          ...
          for (i = 0; i < ltSelectedItems.length; i++) {
            ...
            for ( j = 0; j < loTable.getItems().length; j++) {
              ...
              if (loRowData[this.getKey()] === lKey) {
                loTable.setSelectedItem(loColListItem, true);
                //break;ZEN
              }
            }
          }
      ...
      this._updateMessageView(tMessages);
    };
    ...
    ValueHelpDialog.prototype._createCollectiveSearch = function() {
      ...
      this.oSelectionButton = new Button({
    ...
    ValueHelpDialog.prototype._createTokenizer = function() {
        ...
        tokenUpdate: function(oControlEvent) {
          ...
          if (oControlEvent.getParameter("type") === Tokenizer.TokenChangeType.Removed) {
            ...
            if (this._hasListeners("tokenRemove")) {
              this._bIgnoreSelectionChange = true;
              this.fireTokenRemove(loEventArgs);
              this._bIgnoreSelectionChange = false;
            } else {
              loEventArgs.useDefault = true;
            }
      ...
      this._oRemoveAllSelectedItemsBtn = new Button(
        {
          type: sap.m.ButtonType.Transparent,
      ...
      this._oExcludedTokens = new Tokenizer(
        {
          tokenChange: function(oControlEvent) {
            ...
            if (oControlEvent.getParameter("type") === Tokenizer.TokenChangeType.Removed || oControlEvent.getParameter("type") === sap.m.Tokenizer.TokenChangeType.RemovedAll) {
              var ltTokenKeys = [];
              var ltRemovedTokens = oControlEvent.getParameter("removedTokens");
      ...
      // this "remove all" button is a workaround and should be part of the Tokenizer itself
      this._oRemoveAllExcludeItemsBtn = new Button(
        {
          type: sap.m.ButtonType.Transparent,
      ...
      var loHorizontalLayout2 = new HorizontalLayout(
        {
      ...
      this._oTokenizerPanel = new Panel(
        {
    ...
    ValueHelpDialog.prototype._createNavigationControl = function() {
      if (!this._isPhone()) {
        this._oTabBar = new IconTabBar(
          {
            ...
            items: [
              ...
              new IconTabFilter(
                {
                  visible: true,
                  text: this._oRb.getText("VALUEHELPDLG_RANGESTITLE", [""]),
        ...
        this._oSelectItemLI = new StandardListItem(
          {
            type: sap.m.ListType.Navigation,
            title: this._oRb.getText("VALUEHELPDLG_ITEMSTABLE")
          }
        ).data(
          "key", sap.zen._ValueHelpViewMode.PHONE_LIST_VIEW
        );

        this._oDefineConditionsLI = new StandardListItem(
          {
            type: sap.m.ListType.Navigation,
            title: this._oRb.getText("VALUEHELPDLG_RANGESTITLE", [""])
          }
        ).data(
          "key", sap.zen._ValueHelpViewMode.PHONE_CONDITIONS_VIEW
        );
        this._oMainListMenu = new List(
          {
        ...
        this._oMainListMenu.setLayoutData(
          new FlexItemData(
            {
              shrinkFactor: 0
            }
        ...
        this._oAdvancedSearchLink.setLayoutData(
          new FlexItemData(
            {
              shrinkFactor: 0
            }
    ...
    ValueHelpDialog.prototype._createFooterControls = function() {
      ...
      this._oButtonCancel = new Button({
    ...
    ValueHelpDialog.prototype._updateView = function(newViewMode) {
      ...
      if (this._oMainListMenu && this._oVBox) {
        ...
        if (this._oFilterBar && this._oFilterBar._handleVisibilityOfToolbar) { //ZEN
    ...
    /**
     * _updateFooterControls
     * Updates the footer buttons
     *
     * @private
     */
      ...
      if (this._oMainLayout && oCtrl) {
        ...
        if (this._oFilterBar) {
          ...
          if (this._isPhone()) {
            ...
            this._oColSearchBox.setLayoutData(
              new FlexItemData(
                {
                  shrinkFactor: 0
                }
            ...
            if (this._oFilterBar.setLayoutData) { //ZEN
              ...
              this._oFilterBar.setLayoutData(
                new FlexItemData(
                  {
                    shrinkFactor: 0
                  }
    ...
    ValueHelpDialog.prototype._initializeTable = function() {
      ...
      this._oTable.attachRowSelectionChange(_handleTableRowSelectionChange.bind(this));

      if (!(this._oTable.getParent() instanceof sap.ui.comp.smarttable.SmartTable)) {  //this._bTableCreatedInternal) {
        this._oTable.bindAggregation("columns", "columns>/cols", _handleTableAggregation.bind(this));
      }
    };

    function _handleTableRowSelectionChange(oControlEvent) {
      ...
      if (loEventArgs.useDefault) {
        if (sap.ui.table.Table && this._oTable instanceof sap.ui.table.Table) {
          ...
          for (i = 0; i < ltRowIndices.length; i++) {
            lRowIndex = ltRowIndices[i];

            loContext = loTable.getContextByIndex(lRowIndex);
            loRow = loContext ? loContext.getObject() : null;
            if (!loRow) {
          ...
          // Single Selection: destroy all selections first
          ...
          for (i = 0; i < ltRowIndices.length; i++) {
            lRowIndex = ltRowIndices[i];
            var lIsRowSelected = loTable.isIndexSelected(lRowIndex);
            ...
            if (loRow) {
              ...
              this._addRemoveTokenByKey(lKey, loRow, lIsRowSelected);
              this._changeTableRowSelectionForKey(lKey, lIsRowSelected);
            }
          }
        } else {
          for (i = 0; i < loEventParams.listItems.length; i++) {
            var loColListItem = loEventParams.listItems[i];

            loContext = loColListItem.getBindingContext();
            loRow = loContext ? loContext.getObject() : null;
            if (loRow) {
              lKey = loRow[this.getKey()];
    ...
    function _handleTableAggregation(sId, oContext) {
      if (sap.ui.table.Table && this._oTable instanceof sap.ui.table.Table) {
    ...
    ValueHelpDialog.prototype._changeTableRowSelectionForKey = function(key, select) {
      ...
      if (sap.ui.table.Table && loTable instanceof sap.ui.table.Table) {
        ...
        // Handle selection update of the m.table
        for (i = 0; i < loTable.getItems().length; i++) {
    ...
    ValueHelpDialog.prototype.setSelection = function(selected, key, oRow) {
      ...
      if (key) {
        if (selected) {
          ...
          lTokenExists = this._removeTokenFromTokenizer(key, this._oSelectedTokens);
        }
      }
    ...
    ValueHelpDialog.prototype.setTokens = function(tTokens) {
      ...
      for (var i = 0; i < tTokens.length; i++) {
        ...
        if (loToken.data("range")) {
          ...
          lIsRange = loRange.isRange;
        }

        // retrieve the correct Object based on value and type
        if (loRange) {
          loCurrentKeyField = this._getKeyFieldByKey(loRange.keyField);
    ...
    ValueHelpDialog.prototype._createRanges = function() {
      ...
      if (this._aExcludeRangeOperations) {
        for (lType in this._aExcludeRangeOperations) {
     ...
     *
     * @private
     * @param {string} sOperation the operation type sap.zen.dsh.ValueHelpRangeOperation
    ...
    var instance = new ValueHelpDialog();
    BaseHandler.dispatcher.addHandlers(instance.getType(), instance, "DataSourceControlDecorator");
