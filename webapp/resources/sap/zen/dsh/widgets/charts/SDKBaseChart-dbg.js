...
sap.ui.define(
  ...
  function(
    ...
    TextArea,
    BaseHandler,
    SDKGenericDataMapper,
    SDKUtilsHelper,
    SDKChartDataFeedingHelper
  ) {
    "use strict";
    Control.extend(
      "sap.zen.SDKBaseChart",
      {
        // the control API:
        metadata : {
          properties : {
        ...
        createErrorLayout : function(){
          if(sap.ui.getCore().byId(this.getId() + "_error")){
            sap.ui.getCore().byId(this.getId() + "_error").destroy();
          }
          var oMessageContainer =  new sap.zen.commons.layout.AbsoluteLayout(this.getId() + "_error");
          var isM = BaseHandler.dispatcher.isMainMode();
          var oMessage = (isM) ? new TextArea(this.getId() + "_errormessage") : new sap.ui.commons.TextView(this.getId() + "_errormessage");
