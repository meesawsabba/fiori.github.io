...
sap.ui.define(
  ...
  function(
    jQuery, TouchHandler, CrossRequestManager, RenderingConstants,
    Utils
  ){
    "use strict";
    ...
    sap.zen.crosstab.SelectionHandler = function (oCrosstab) {
      ...
      this.provideSelectionForAllClickedCells = function () {
        jQuery.each(oClickedCells, function (sCellId, oClickedCell) {
          ...
          if (!oClickedCellFromModel) {
            if (oClickedCell.isHeaderCell()) {
              if (oClickedCell.getArea().isRowHeaderArea()) {
                oClickedCellFromModel = oClickedCell.getArea().getCellWithRowSpan(
                  oClickedCell.getRow(),
                  oClickedCell.getCol()
                );
              } else if (oClickedCell.getArea().isColHeaderArea()) {
                oClickedCellFromModel = oClickedCell.getArea().getCellWithColSpan(
                  oClickedCell.getRow(),
                  oClickedCell.getCol()
                );
              }
      ...
      this.addClickedCellsForSpannedHeaderCells = function () {
        ...
        jQuery.each(oClickedCells, function (sCellId, oClickedCell) {
          ...
          if (oClickedCell.isHeaderCell()) {
            ...
            if (oArea.isRowHeaderArea()) {
              ...
              jQuery.each(oCells, function (sId, oCell) {
        ...
        jQuery.extend(oClickedCells, oAddedHeaderCells);
      };
      this.extractClickedCellsFromSelection = function() {
        ...
        if (oSelection) {
          ...
          if (oSelection.bFromBackend === true) {
            ...
            if (aCoordArray) {
              for (i = 0; i < aCoordArray.length; i++) {
      ...
      this.checkSingleCellClicked = function (oModelCell) {
        ...
        if (oModelCell.isHeaderCell() === true) {
          jQuery.each(oClickedCells, function (sClickedCellId, oClickedCell) {
          ...
          if (!document.fSapCrosstabOnKeyUpHandler) {
            document.fSapCrosstabOnKeyUpHandler = function (e) {
              if (e.which === 17) {
                ...
                jQuery.each(document.oSapCrosstabOnSelectHandlerReg, function (sId, oHandlerInfo) {
            ...
            jQuery(document).on("keyup", document.fSapCrosstabOnKeyUpHandler);
          }
        }
      };
      ...
      this.getCellsForSelectionState = function() {
        ...
        jQuery.each(oConsolidatedClickedCells, function (sId, oClickedCell) {
      ...
      this.consolidateClickedCells = function () {
        ...
        jQuery.each(oClickedCells, function (sId, oCell) {
          sAxis = oCell.getArea().getAxisName();
          if (sAxis === RenderingConstants.ROW_AXIS) {
            sKey = oCell.getText() + " " + oCell.getMergeKey() + " " + oCell.getCol();
            if (!oRowCells[sKey]) {
              oRowCells[sKey] = oCell;
            }
          } else if (sAxis === RenderingConstants.COL_AXIS) {
        ...
        return jQuery.extend(oRowCells, oColCells, oOtherCells);
      };

      this.selectCells = function (oModelCell, bHoverMode) {
        var oSelectedCells = this.getSelectedCells(oModelCell);

        jQuery.each(oSelectedCells, function (sCellId, oCell) {
      ...
      this.getSelectedCells = function (oModelCell, bRemoveSelection) {
        ...
        if (oArea.isRowHeaderArea() || oArea.isColHeaderArea()) {
          var oSelectedHeaderCells = oArea.getSelectedCellsBySelectionCoordinates(
            oModelCell.getRow(), oModelCell.getCol()
          );
          var oSelectedDataCells = oCrosstab.getDataArea().getSelectedCellsByHeaderSelection(
            oModelCell,
            bRemoveSelection
          );
          oResult = jQuery.extend({}, oSelectedHeaderCells, oSelectedDataCells);
        } else if (oArea.isDataArea()) {
          oSelectedDataCells = {};
          ...
          oResult = jQuery.extend({}, oSelectedDataCells, oSelectedRowHeaderCells, oSelectedColHeaderCells);
        }
      ...
      this.removeSelection = function (oModelCell, bHoverMode) {
        ...
        jQuery.each(oCellsToBeRemoved, function (sRemoveCellId, oRemoveCell) {
          if (!bHoverMode) {
      ...
      this.removeAllPreviousSelectionEffects = function () {
        jQuery.each(oClickedCells, function (key, value) {
      ...
      this.selectRowHeaderCell = function (oModelCell, bHoverMode) {
        ...
        var oDomCell = (jQuery(document.getElementById(oModelCell.getId())));
      ...
      this.hasSelection = function() {
        return Utils.getSizeOf(oClickedCells) > 0;
      };
        ...
        if (oCurrentlyHoveredCell) {
          ...
          if (oCell) {
            if (oCell === oCurrentlyHoveredCell) {
              oFoundCell = jQuery(oDomMoveSrc).find(jQuery(oDomMoveTarget));
            ...
            oDomCell = jQuery(document.getElementById(oCurrentlyHoveredCell.getId()));
            oFoundCell = oDomCell.find(jQuery(oDomMoveSrc));
            if (oFoundCell && oFoundCell.length > 0) {
              // check target
              oFoundCell = oDomCell.find(jQuery(oDomMoveTarget));
              if (!(oFoundCell && oFoundCell.length > 0)) {
                bRemoveHoverEffect = true;
              }
            }
