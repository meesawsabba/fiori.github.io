      ...
      this.getContextMenuAction = function (sContextMenuComponentId, oDomClickedElement) {
        ...
        if (oCrosstab.isSelectable() === true) {
          ...
          if (oSelectionHandler && oSelectionHandler.hasSelection() === true) {
            ...
            bRemoveSelection = !(jQuery(oElement).hasClass(sCssClass));
          }
        }
        var sAreaType = oCell.getArea().getAreaType();
        var sAxis = "";
        if (sAreaType === RenderingConstants.TYPE_COLUMN_HEADER_AREA) {
          sAxis = "COLS";
        } else if (sAreaType === RenderingConstants.TYPE_ROW_HEADER_AREA) {
          sAxis = "ROWS";
        } else if (sAreaType === RenderingConstants.TYPE_DIMENSION_HEADER_AREA) {
          var oDimInfo;
        ...
        } else if (sCellType === RenderingConstants.TYPE_DATA_CELL) {
          sAxis = "DATA";
        } else {
          return null;
        }
