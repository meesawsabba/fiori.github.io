...
sap.ui.define(
  ...
  [
    "jquery.sap.global",
    "sap/zen/crosstab/paging/PagingConstants",
    "sap/zen/crosstab/rendering/RenderingConstants",
    "sap/zen/crosstab/DataCell",
    "sap/zen/crosstab/HeaderCell"
  ],
  function(jQuery, PagingConstants, RenderingConstants, DataCell, HeaderCell){
    "use strict";
    ...
    sap.zen.crosstab.paging.Page = function (oPagePos, sPageKey, oPageManager) {
      var iStatus = PagingConstants.PAGE_STATUS_UNKNOWN;
      ...
      this.removeData = function () {
        iStatus = PagingConstants.PAGE_STATUS_UNKNOWN;
      ...
      function checkHeaderAreaForRemoval (oArea, iTileIndex) {
        ...
        if (oAreaTileInfo.iRefCnt === 0) {
          oAreaTileInfo.iStatus = PagingConstants.PAGE_STATUS_UNKNOWN;
      ...
      function checkAreas () {
        oCrosstabAreasToBeFilled = {};
        addArea(oDataArea, 0);
        if (oPageManager.getHeaderTileInfo(oRowHeaderArea, oPagePos.iRow).iStatus !== PagingConstants.PAGE_STATUS_LOADED) {
          addArea(oRowHeaderArea, oPagePos.iRow);
        }
        if (oPageManager.getHeaderTileInfo(oColHeaderArea, oPagePos.iCol).iStatus !== PagingConstants.PAGE_STATUS_LOADED) {
          addArea(oColHeaderArea, oPagePos.iCol);
        }
        if (oPageManager.getHeaderTileInfo(oDimensionHeaderArea, 0).iStatus !== PagingConstants.PAGE_STATUS_LOADED) {
      ...
      function setLoadedStatusAfterDataHandling () {
        ...
        if (oAreaInfo) {
          ...
          setHeaderTileStatus(
            oAreaInfo.oArea, oPagePos.iRow,
            PagingConstants.PAGE_STATUS_LOADED
          );
        }
        oAreaInfo = oCrosstabAreasToBeFilled[oDimensionHeaderArea.getAreaType()];
        if (oAreaInfo) {
          setHeaderTileStatus(oAreaInfo.oArea, 0, PagingConstants.PAGE_STATUS_LOADED);
        }
      }
      function determineAreasForLoadingCells (oArea) {
        ...
        addArea(oDataArea, 0);
        if (oArea.isDataArea()) {
          if (oPageManager.getHeaderTileInfo(oRowHeaderArea, oPagePos.iRow).iStatus === PagingConstants.PAGE_STATUS_UNKNOWN) {
            addArea(oRowHeaderArea, oPagePos.iRow);
            setHeaderTileStatus(
              oRowHeaderArea, oPagePos.iRow,
              PagingConstants.PAGE_STATUS_LOADING
            );
          }
          if (oPageManager.getHeaderTileInfo(oColHeaderArea, oPagePos.iCol).iStatus === PagingConstants.PAGE_STATUS_UNKNOWN) {
            addArea(oColHeaderArea, oPagePos.iCol);
            setHeaderTileStatus(
              oColHeaderArea, oPagePos.iCol,
              PagingConstants.PAGE_STATUS_LOADING
            );
          }
          if (oPageManager.getHeaderTileInfo(oDimensionHeaderArea, 0).iStatus === PagingConstants.PAGE_STATUS_UNKNOWN) {
            addArea(oDimensionHeaderArea, 0);
            setHeaderTileStatus(
              oDimensionHeaderArea, 0,
              PagingConstants.PAGE_STATUS_LOADING
            );
          }
        } else {
          if (oArea.isColHeaderArea()) {
            ...
            setHeaderTileStatus(oArea, oPagePos.iCol, PagingConstants.PAGE_STATUS_LOADING);
          } else if (oArea.isRowHeaderArea()) {
            addArea(oArea, oPagePos.iRow);
            setHeaderTileStatus(oArea, oPagePos.iRow, PagingConstants.PAGE_STATUS_LOADING);
          } else if (oArea.isDimensionHeaderArea()) {
            addArea(oArea, 0);
            setHeaderTileStatus(oArea, 0, PagingConstants.PAGE_STATUS_LOADING);
          }
        }
      }
      ...
      function createLoadingCell (oArea, iRow, iCol) {
        ...
        if (oArea.isDataArea()) {
          oCell = new DataCell();
          oCell.addStyle(RenderingConstants.STYLE_DATA_CELL);
        } else {
          oCell = new HeaderCell();
          oCell.addStyle(RenderingConstants.STYLE_HEADER_CELL);
          oCell.setRowSpan(1);
          oCell.setColSpan(1);
        }
        oCell.setArea(oArea);
        ...
        oCell.addStyle(RenderingConstants.STYLE_LOADING);
        return oCell;
      }
    };
