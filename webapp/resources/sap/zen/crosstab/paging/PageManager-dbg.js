...
sap.ui.define(
  ...
  [
    ...
    "sap/zen/crosstab/datahandler/JsonDataHandler"
  ],
  function(jQuery, Page, PagingConstants, RequestHandler, CellMerger, JsonDataHandler){
    "use strict";
    jQuery.sap.declare("sap.zen.crosstab.paging.PageManager");
    sap.zen.crosstab.paging.PageManager = function (oCrosstab) {
      var oJsonDataHandler = new JsonDataHandler(oCrosstab);
      var oCellMerger = new CellMerger(this);
      ...
      this.removeHeaderTileRequest = function (oArea, iTileIndex, sPageKey) {
        ...
        if (oTiles) {
          ...
          if (oTileInfo) {
            var iRequestIndex = jQuery.inArray(sPageKey, oTileInfo.aRequestingPages);
            if (iRequestIndex !== -1) {
              oTileInfo.aRequestingPages.splice(iRequestIndex, 1);
              if (oTileInfo.aRequestingPages.length === 0) {
                oTileInfo.iStatus = PagingConstants.PAGE_STATUS_UNKNOWN;
              }
            }
          }
        }
      ...
      this.ensureCellAvailable = function (oArea, iRow, iCol) {
        var oPage = null;
        var iTileStatus = PagingConstants.PAGE_STATUS_UNKNOWN;
        ...
        if (!oAlreadyLoadedCells[sCellKey]) {
          ...
          if (oPagePos) {
            ...
            oPage = this.getPage(oPagePos);
            if (bIsHeaderCell) {
              if (iTileStatus === PagingConstants.PAGE_STATUS_UNKNOWN) {
                bRequest = true;
              }
            } else {
              bRequest = !oPage;
            ...
            if (bRequest) {
              ...
              if ((oPage && oPage.getStatus() === PagingConstants.PAGE_STATUS_LOADED)
                  || iTileStatus
                  && iTileStatus === PagingConstants.PAGE_STATUS_LOADED) {
      ...
      this.createPage = function (oPagePos) {
        ...
        if (sPageKey) {
          oPage = new Page(oPagePos, sPageKey, this);
