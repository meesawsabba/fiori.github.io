...
sap.ui.define(
  ...
  function(jQuery,RenderingConstants, IHeaderCell, TextConstants, Utils) {
    "use strict";
    ...
    sap.zen.crosstab.HeaderCellRenderer.render = function (oRenderManager, oControl) {
      ...
      if (iColspan > 1) {
        rm.writeAttributeEscaped("colspan", iColspan);
      }
      if (iRowspan > 1) {
        rm.writeAttributeEscaped("rowspan", iRowspan);
      }
      rm.write(">"); // td element
      // cell layout div
      rm.write("<div");
      rm.writeAttributeEscaped("id", oControl.getId() + "_cellLayoutDiv");
      rm.writeAttributeEscaped("tabindex", RenderingConstants.TABINDEX);
      ...
      // tooltip
      if (sRenderText) {
        rm.writeAttributeEscaped("title", sRenderText);
      }
      ...
      if (oControl.isEntryEnabled()) {
        ...
        rm.write("<div");
        rm.writeAttributeEscaped("id", "vhlp_" + oControl.getId());
        ...
        rm.writeAttributeEscaped("class", sClassString);
        rm.write(">");
        rm.write("</div>");
        rm.write("</td>");
        rm.write("</tr>");
      ...
      function createCellContent() {
        rm.write("<div");
        rm.writeAttributeEscaped("id", oControl.getId() + "_textContentDiv");
        rm.writeAttributeEscaped("tabindex", RenderingConstants.TABINDEX);
        ...
        if (sDrillState) {
          ...
          if (iLevel > 0) {
            if (isRowHeaderArea) {
              ...
              rm.writeAttributeEscaped("xtabspacer-cellid", oControl.getId());
              rm.writeStyles();
              rm.write("></div>");
              rm.write("</td>");
            }
          ...
          if ((sDrillState === "L" && iLevel > 0) || sDrillState !== "L") {
            ...
            rm.write("<div");
            if (oControl.getHierarchyAction()) {
              rm.writeAttributeEscaped("id", "hier_" + oControl.getId());
            }
            rm.writeAttributeEscaped("class", sClass);
            // Tooltip
            var sHierarchyTooltip = oControl.getHierarchyTooltip();
            if (sHierarchyTooltip) {
              rm.writeAttributeEscaped("title", sHierarchyTooltip);
            }
            rm.write("></div>");
            rm.write("</td>");
          }
      ...
      function createSortingImage() {
        if (sSort) {
          ...
          if (sSortTooltip) {
            rm.writeAttributeEscaped("title", sSortTooltip);
          }
          rm.write("></div>");
        }
      }
        ...
        if (oCrosstab.getPropertyBag().isEnableColResize() === true) {
          if (oArea.isColHeaderArea() || oArea.isDimHeaderArea()) {
            if (oCrosstab.getPropertyBag().isTestMobileMode() || oCrosstab.getPropertyBag().isMobileMode()) {
              ...
              // resize handle div
              rm.write("<div");
              rm.writeAttributeEscaped("id", "resi_" + oControl.getId());
              if(sSort){
              ...
              rm.writeAttributeEscaped("title", oCrosstab.getPropertyBag().getText(
