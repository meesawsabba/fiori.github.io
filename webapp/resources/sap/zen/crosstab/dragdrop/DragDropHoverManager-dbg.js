  ...
  function(jQuery) {
    ...
    sap.zen.crosstab.dragdrop.DragDropHoverManager = function (oCrosstab) {
      ...
      this.getCellFromJqCell = function(oJqCell) {
        var sId = jQuery(oJqCell).attr("id");
      ...
      this.getDropCell = function(e) {
        ...
        jQuery.each(aDroppables, function(iIndex, oDomCell) {
      ...
      this.cleanupDropCells = function() {
        ...
        jQuery.each(aMarkedDropCells, function(index, oDomCell) {
          oJqCell = jQuery(oDomCell);
      ...
      this.onDropCellOver = function(e) {
      ...
      this.cleanupDropAreas = function() {
        if (oJqCurrentDropArea) {
          jQuery.each(oDroppablesDisabledByArea, function(sId, oDroppable) {
      ...
      this.onDropAreaOver = function(e) {
        var sCellId;
        var oCell;
        var sDragAreaType;

        that.cleanupDropAreas();
        oJqCurrentDropArea = jQuery(document.getElementById(e.target.id));
      ...
      this.provideDraggableCellCursor = function(oJqCell) {
        ...
        oJqCell.on("mouseenter",function() {
          jQuery(this).css("cursor", "move");
          jQuery(this).find("div").not(".sapzencrosstab-HeaderCellDivSortable, .sapzencrosstab-columnResizeHandle, .sapzencrosstab-columnResizeHandleWithSort, .sapzencrosstab-HeaderCellDivHierarchy").css("cursor", "move");
        });

        oJqCell.on("mouseleave",function() {
          jQuery(this).css("cursor", "default");
          jQuery(this).find("div").not(".sapzencrosstab-HeaderCellDivSortable, .sapzencrosstab-columnResizeHandle, .sapzencrosstab-columnResizeHandleWithSort, .sapzencrosstab-HeaderCellDivHierarchy").css("cursor", "default");
        });
        ...
        oJqCell.draggable("option", "drag", function(e, ui) {
          ...
          oDragDropUtils.setCurrentDragHelper(jQuery(ui.helper));
          ...
          if (!bIsWithinCrosstab) {
            ...
            jQuery.each(aAlternatingElements, function(iIndex, oDomElement) {
              if (oDomElement) {
                jQuery(oDomElement).removeClass("sapzencrosstab-HeaderCellAlternating");
                jQuery(oDomElement).addClass("sapzencrosstab-AlternatingDummy");
              }
            });
            // ui.helper.addClass("ui-icon ui-icon-trash");
            jQuery(ui.helper).find(".sapzencrosstab-Trashcan").show();
            ...
            jQuery(ui.helper).find(".sapzencrosstab-Trashcan").hide();
            aAlternatingElements = ui.helper.find(".sapzencrosstab-AlternatingDummy");
            jQuery.each(aAlternatingElements, function(iIndex, oDomElement) {
              if (oDomElement) {
                jQuery(oDomElement).addClass("sapzencrosstab-HeaderCellAlternating");
                jQuery(oDomElement).removeClass("sapzencrosstab-AlternatingDummy");
              }
            });
            if (bMovedOutOfCrosstab) {
              if (oCurrentDropCell) {
                oCell = oDragDropUtils.findCell(jQuery(e.toElement));
