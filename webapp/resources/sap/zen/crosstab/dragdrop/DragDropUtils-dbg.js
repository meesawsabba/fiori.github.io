...
sap.ui.define(
  ...
  function(jQuery, Log) {
    "use strict";
    Log.info("loaded drag drop utils");
    ...
    sap.zen.crosstab.dragdrop.DragDropUtils = function (oCrosstab) {
      ...
      this.getAreaInfo = function(oJqArea, sAxisName) {
        ...
        oAreaInfo.oJqCell = jQuery(document.getElementById(sCellId));
      ...
      this.getCellInfoFromDropArea = function(e, sDropAreaSuffix) {
        ...
        oJqDropArea = jQuery(document.getElementById(e.target.id));
      ...
      this.checkDragRevert = function(oDroppable) {
        ...
        var oRevertPosInfo;
        var oPos;
        oRevertPosInfo = jQuery(this).data("oRevertPosInfo");
        if (oRevertPosInfo) {
          oPos = that.getRevertPosition(oRevertPosInfo);
          jQuery(this).data("uiDraggable").originalPosition = oPos;
        }

        if (!sap.zen.Dispatcher.instance.isDragDropCanceled()) {
          sId = jQuery(oDroppable).data("xtabcellid");
          if (sId && sId.length > 0) {
            bRevertDrop = jQuery(oDroppable).data("xtabrevertdrop");
            return bRevertDrop;
          } else {
            sId = jQuery(oDroppable).attr("id");
      ...
      this.makeCellDraggable = function(oJqCell, fHelper) {
        ...
        jQuery(oJqCell.draggable()).mousedown(function(e) {
      ...
      this.getRevertPosition = function(oRevertPosInfo) {
        ...
        oJqRevertCell = jQuery(document.getElementById(oRevertCell.getId()));
      ...
      this.setCursorAt = function(oCell, oJqHelper) {
        var iWidth = 0;
        var iLeft = 0;
        var oJqCrosstabDiv = jQuery(document.getElementById(oCrosstab.getId()));
