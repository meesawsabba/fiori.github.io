  ...
  function(jQuery) {
    ...
    sap.zen.crosstab.dragdrop.DragDropAreaRenderer = function (oCrosstab) {
      ...
      this.createFlexDiv = function(oCell, oJqParent, sIdSuffix, sCssClassName, oCssStyles, oAfterBurnerStyles, fAfterBurnerFunction, bPrepend) {
        ...
        oJqArea = jQuery(document.getElementById(sId));
        if (oJqArea.length === 0) {
      ...
      this.repositionDropAreasForHeaderScrolling = function() {
        ...
        if (oCrosstab.isHeaderHScrolling()) {
          ...
          if (aDropAreas) {
            jQuery.each(aDropAreas, function(iIndex, oDomDropArea) {
              oJqDropArea = jQuery(oDomDropArea);
              oJqCell = oCrosstab.getRenderSizeDiv().find("#" + jQuery.sap.encodeCSS(oJqDropArea.data("xtabcellid")));
              if (oJqCell.length > 0) {
