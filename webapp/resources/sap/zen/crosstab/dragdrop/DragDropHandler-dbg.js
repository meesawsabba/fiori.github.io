...
sap.ui.define(
  [
    "jquery.sap.global",
    "sap/base/Log",
    "sap/zen/crosstab/dragdrop/DragDropUtils",
    "sap/zen/crosstab/dragdrop/DragDropAreaRenderer",
    "sap/zen/crosstab/dragdrop/DragDropHoverManager",
    "sap/zen/crosstab/dragdrop/MemberDragDropHandler",
    "sap/zen/crosstab/TextConstants",
    "sap/zen/crosstab/utils/Utils"
  ],
  function(jQuery, Log) {
    "use strict";
    Log.info("Load Drag Drop Handler");
    ...
    sap.zen.crosstab.dragdrop.DragDropHandler = function (oCrosstab, oDragDropCommands) {
      ...
      this.addExternalDimensionDropAreasIfOnlyMeasures = function() {
        ...
        if (sap.zen.crosstab.utils.Utils.isDispatcherAvailable()) {
          if (sap.zen.Dispatcher.instance.isInterComponentDragDropEnabled() && oCrosstab.getHeaderInfo().hasOnlyMeasureStructure()) {
            ...
            if (oCell && document.getElementById(oCell.getId())) {
      ...
      function checkDimHeaderDropAccept() {
        var oPayload;
        var oDragDropInfo;
        var oDimInfo;
        var oCell;
        ...
        if (!oDragDropUtils.checkDroppableInArea(jQuery(this), oDragDropUtils.determineValidHeaderRect())) {
        ...
        if (oPayload.oDragDropInfo) {
          ...
          if (oDimInfo && oDimInfo.sDimensionName === oDragDropInfo.sDimensionName) {
            return that.returnFromDimHeaderDropAccept(oCell, false, true);
          }
        } else {
          return that.returnFromDimHeaderDropAccept(oCell, false, true);
      ...
      function handleDimHeaderDrop(e) {
        ...
        if (oDragDropUtils.checkDropAllowedOnCrosstabElement(e)) {
          ...
          if (oPayload && oPayload.oDragDropInfo) {
            ...
            sDestinationDimensionName = oDimInfo ? oDimInfo.sDimensionName : null;
      ...
      this.getDragGhostCellHtml = function(oCell, oDimInfo, sMeasureStructureText) {
        ...
        if (!oDimInfo.bIsStructure) {
          oJqCellLayoutDiv = jQuery(document.getElementById(oCell.getId() + "_cellLayoutDiv"));
      ...
      this.createDimHeaderCellDragObject = function(aDragCells, oDimInfo) {
        ...
        if (oDimInfo.sAxisName === "ROWS") {
          oCell = aDragCells[0];
          iHeight = jQuery(document.getElementById(oCell.getId())).outerHeight();
      ...
      function dimHeaderCellDraggableHelper() {
        var oCell;
        var oJqHelper;
        var oDimInfo;
        var aDragCells = [];
        ...
        oCell = oDragDropHoverManager.getCellFromJqCell(this);
        if (oCell) {
          oDragDropUtils.setCurrentJqDragCell(jQuery(this));
          oDimInfo = oCrosstab.getHeaderInfo().getDimensionInfoByRowCol(oCell);
      ...
      this.makeAboveAreaDroppable = function(oJqAboveArea, sCssClass) {
        ...
        var fHandleAboveAreaDrop = function(e) {
          var oCellInfo;
      ...
      this.makeBelowAreaDroppable = function(oJqBelowArea, sCssClass) {
        ...
        var fHandleBelowAreaDrop = function(e) {
          var oCellInfo;
      ...
      this.makeBeforeAreaDroppable = function(oJqBeforeArea, sCssClass) {
        ...
        var fHandleBeforeAreaDrop = function(e) {
          var oCellInfo;
      ...
      this.makeAfterAreaDroppable = function(oJqAfterArea, sCssClass) {
        ...
        var fHandleAfterAreaDrop = function(e) {
          var oCellInfo;
      ...
      this.enableDimHeaderDropAreas = function() {
        var oJqCrosstab = jQuery(document.getElementById(oCrosstab.getId()));
        var sCssClass = null;
        ...
        jQuery.each(aBeforeAreas, function(index, oDomBeforeArea) {
          that.makeBeforeAreaDroppable(jQuery(oDomBeforeArea), sCssClass);
        });

        sCssClass = "sapzencrosstab-columnAfterCellDropArea";
        aAfterAreas = oJqCrosstab.find("." + sCssClass);
        jQuery.each(aAfterAreas, function(index, oDomAfterArea) {
          that.makeAfterAreaDroppable(jQuery(oDomAfterArea), sCssClass);
        });

        sCssClass = "sapzencrosstab-columnAfterCellDropAreaWithSort";
        aAfterAreasWithSort = oJqCrosstab.find("." + sCssClass);
        jQuery.each(aAfterAreasWithSort, function(index, oDomAfterAreaWithSort) {
          that.makeAfterAreaDroppable(jQuery(oDomAfterAreaWithSort), sCssClass);
        });

        sCssClass = "sapzencrosstab-rowAboveCellDropArea";
        aAboveAreas = oJqCrosstab.find("." + sCssClass);
        jQuery.each(aAboveAreas, function(index, oDomAboveArea) {
          that.makeAboveAreaDroppable(jQuery(oDomAboveArea), sCssClass);
        });

        sCssClass = "sapzencrosstab-rowBelowCellDropArea";
        aBelowAreas = oJqCrosstab.find("." + sCssClass);
        jQuery.each(aBelowAreas, function(index, oDomBelowArea) {
          that.makeBelowAreaDroppable(jQuery(oDomBelowArea), sCssClass);
        });
      };
      ...
      this.enableDimHeaderPivotCell = function() {
        var oJqCell;
        if (oPivotCell) {
          oJqCell = jQuery(document.getElementById(oPivotCell.getId()));
      ...
      this.onUnhandledDrop = function (e, ui, oPayload) {
        ...
        if (oDragDropUtils.checkMouseInRenderSizeDiv(e) === true) {
          ...
          sap.zen.Dispatcher.instance.setDragDropCanceled(true);
        } else if (oPayload && oPayload.oDragDropInfo) {
          oDragDropInfo = oPayload.oDragDropInfo;
      ...
      this.registerCrosstabAsDropHandler = function() {
        var oJqCrosstabDiv;
        oJqCrosstabDiv = jQuery(document.getElementById(oCrosstab.getId()));
