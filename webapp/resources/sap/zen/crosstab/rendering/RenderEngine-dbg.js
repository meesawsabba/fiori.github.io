...
sap.ui.define(
  [
    ...
    "sap/zen/crosstab/rendering/CrossRequestManager",
    "sap/zen/crosstab/utils/Utils"
  ],
  function(
    jQuery, ScrollbarRenderer, DomElementProvider, Measuring,
    CellStyleHandler, RenderingConstants, CrossRequestManager, Utils
  ) {
    "use strict";
    ...
    sap.zen.crosstab.rendering.RenderEngine = function (oCrosstab) {
      ...
      var oDomElementProvider = new DomElementProvider();
      var oMeasuring = new Measuring(oCrosstab, oDomElementProvider);

      var oCrossRequestManager = new CrossRequestManager(oCrosstab, this);
      ...
      this.renderResizeOutline = function () {
        if ((oCrosstab.getWidth() === "auto" && oCrosstab.hResize())
          ...
          if (oTableDiv && oTableDiv[0]) {
            ...
            var oHScrollTab = oTableDiv.find("#" + jQuery.sap.encodeCSS(oCrosstab.getId() + "_hScrollTab"));
            ...
            var oVScrollTab = oTableDiv.find("#" + jQuery.sap.encodeCSS(oCrosstab.getId() + "_vScrollTab"));
            ...
            if (oCrosstab.getPropertyBag().hasToolbar()) {
              var oToolbar = oTableDiv.find("#" + jQuery.sap.encodeCSS(oCrosstab.getId() + "_toolbar"));
      ...
      this.addAfterFinishRenderingHandler = function (fHandler) {
        if (jQuery.inArray(fHandler, aAfterFinishRenderingHandlers) === -1) {
          aAfterFinishRenderingHandlers.push(fHandler);
        }
      };

      this.removeAfterFinishRenderingHandler = function (fHandler) {
        var iIndex = jQuery.inArray(fHandler, aAfterFinishRenderingHandlers);
        if (iIndex !== -1) {
      ...
      this.renderArea = function (oArea, iStartRow, iRowCnt, iStartCol, iColCnt, sRowStyle) {
        ...
        for (iRow = iStartRow; iRow < iEndRow; iRow++) {
          if (sRowStyle) {
            ...
            oRenderManager.writeAttributeEscaped("class", sRowStyle);
      ...
      function determineIsIE8 () {
        ...
        if (jQuery.browser.msie) {
          ...
          if (jQuery.browser.fVersion && jQuery.browser.fVersion === 8) {
      ...
      this.renderDimensionHeaderArea = function () {
        ...
        this.renderArea(
          oDimensionHeaderArea, iRenderStartRow, iRenderRowCnt, iRenderStartCol, iRenderColCnt,
      ...
      this.renderDataArea = function () {
         ...
         */
        oDomDataArea.html("<tbody></tbody>");
        oDomDataAreaBody = jQuery(document.getElementById(dataAreaId).firstChild);
        oDomDataAreaBody.html("");
        ...
        var iVerticalOverlap = getVerticalScrollingOverlap(
          iRenderStartRow, iRenderRowCnt, iRenderStartCol,
          iRenderColCnt
        );
        var iHorizontalOverlap = getHorizontalScrollingOverlap(
          iRenderStartRow, iRenderRowCnt, iRenderStartCol,
          iRenderColCnt
        );
      ...
      function cloneArea(oJqArea) {
        ...
        if (bIsMsIE === true) {
          if (oCrosstab.getPropertyBag().isDragDropEnabled() === true) {
            ...
            jQuery.each(aCells, function(iIndex, oDomCell) {
              var oJqCell = jQuery(oDomCell);
      ...
      function removeDataAreaRows (iOverlap) {
        ...
        var oDomDataAreaTBody = jQuery(oDomDataAreaClone[0].firstChild);
        var oTrs = oDomDataAreaTBody.find("> tr");
        if (iOverlap > 0) {
          // remove rows from the top
          for (var i = 0; i < iOverlap; i++) {
            jQuery(oTrs[i]).remove();
          }
        } else {
          // remove rows from the bottom
          var iTrs = oTrs.length;
          for ( i = iTrs + iOverlap; i < iTrs; i++) {
            jQuery(oTrs[i]).remove();
          }
        }
        oDomDataArea.replaceWith(oDomDataAreaClone);
        oDomElementProvider.addElement(oDataArea.getId(), jQuery(document.getElementById(oDataArea.getId())));
      }
      function removeDataAreaCols (iOverlap) {
        ...
        if (iOverlap > 0) {
          ...
          for (var i = 0; i < iNumberOfRows; i++) {
            var oTds = jQuery(oTrs[i]).find("> td");
            for (var j = 0; j < iOverlap; j++) {
              jQuery(oTds[j]).remove();
            }
          }
        } else {
          // remove cols from right
          for (i = 0; i < iNumberOfRows; i++) {
            oTds = jQuery(oTrs[i]).find("> td");
            var iEndCol = oTds.length - Math.abs(iOverlap) - 1;
            for ( j = oTds.length - 1; j > iEndCol; j--) {
              jQuery(oTds[j]).remove();
            }
          }
        }
        oDomDataArea.replaceWith(oDomDataAreaClone);
        oDomElementProvider.addElement(
          oDataArea.getId(), jQuery(document.getElementById(oDataArea.getId()))
        );
      }

      function renderRemainingDataAreaRows (
        iOverlap, iRenderStartRow, iRenderRowCnt, iRenderStartCol, iRenderColCnt,
        that
      ) {
        var oDomDataAreaTBody = jQuery(document.getElementById(oDataArea.getId()).firstChild);
      ...
      function renderRemainingDataAreaCols(
        iOverlap, iRenderStartRow, iRenderRowCnt, iRenderStartCol, iRenderColCnt
      ) {
        ...
        if (iOverlap > 0) {
          ...
          for (i = 0; i < oTrs.length; i++) {
            ...
            for (iCol = iNewStartCol; iCol < iNewStartCol - iOverlap; iCol++) {
              oRenderManager.renderControl(oDataArea.getCell(iRenderStartRow + i, iCol));
            }
            oRenderManager.write(sInnerHtml);
            oRenderManager.write("</tr>");
      ...
      this.renderColHeaderArea = function (iRenderStartCol, bUseFullContentWidth) {
         ...
         */
        oDomColHeaderArea.html("<tbody></tbody>");
        oDomColHeaderAreaBody = jQuery(document.getElementById(colHeaderAreaId).firstChild);
        oDomColHeaderAreaBody.html("");
        ...
        while (checkFunclet(iColOffset, iMaxColOffset, iEffectiveWidth, iAvailableWidth, bHScrolledToEnd)) {
          ...
          for (iRow = 0; iRow < iRowCnt; iRow++) {
            ...
            oDomRow = oDomColHeaderArea.find("#" + jQuery.sap.encodeCSS(sRowId))[0];
      ...
      this.renderRowHeaderArea = function (iStartRow) {
         ...
         */
        oDomRowHeaderArea.html("<tbody></tbody>");
        oDomRowHeaderAreaBody = jQuery(document.getElementById(rowHeaderAreaId).firstChild);
        oDomRowHeaderAreaBody.html("");
      ...
      this.appendRowsAfterResize = function () {
        ...
        if (oRowHeaderArea.getRenderStartRow() > 0) {
          ...
          while (
            iEffectiveHeight <= iAvailableHeight
              && oRowHeaderArea.getRenderStartRow() > 0
              && oRowHeaderArea.getRenderStartRow() + oRowHeaderArea.getRenderRowCnt() === oRowHeaderArea.getRowCnt()
          ) {
      ...
      this.setUserColWidths = function (oArea) {
        ...
        oDomArea = jQuery(document.getElementById(oArea.getId()));
        forceColWidths(oArea, oDomArea, true);
        oDomElementProvider.addElement(oArea.getId(), jQuery(document.getElementById(oArea.getId())));
      ...
      this.adjustColWidths = function (oUpperArea, oLowerArea) {
        ...
        oDomUpperArea = jQuery(document.getElementById(oUpperArea.getId()));
        oDomLowerArea = jQuery(document.getElementById(oLowerArea.getId()));
        ...
        oDomElementProvider.addElement(oUpperArea.getId(), jQuery(document.getElementById(oUpperArea.getId())));
        oDomElementProvider.addElement(oLowerArea.getId(), jQuery(document.getElementById(oLowerArea.getId())));
      };
      ...
      function postProcessInputRow (iRow, iColCnt) {
        ...
        for (iCol = 0; iCol < iColCnt; iCol++) {
          ...
          if (oCell && oCell.isEntryEnabled()) {
            oJqCell = jQuery(document.getElementById(oCell.getId()));
            oJqLayoutDiv = jQuery(document.getElementById(oCell.getId() + "_cellLayoutDiv"));
            Utils.getWidthFromStyle(oJqLayoutDiv);
            if (aColGranularity && aColGranularity.length > 0) {
              ...
              // fix the content div
              iIconTdWidth = jQuery(oJqLayoutDiv.find("td")[1]).outerWidth();
              oJqTd = jQuery(oJqLayoutDiv.find("td")[0]);
              iPaddingLeft = parseInt(oJqTd.css("padding-left")) || 0;
              iPaddingRight = parseInt(oJqTd.css("padding-right")) || 0;

              oJqContentDiv = oJqTd.find("#" + jQuery.sap.encodeCSS(oCell.getId() + "_contentDiv"));
      ...
      this.adjustRowHeights = function (oLeftArea, oRightArea) {
        ...
        oDomLeftArea = jQuery(document.getElementById(oLeftArea.getId()));
        oDomRightArea = jQuery(document.getElementById(oRightArea.getId()));
        ...
        oDomElementProvider.addElement(oLeftArea.getId(), jQuery(document.getElementById(oLeftArea.getId())));
        oDomElementProvider.addElement(oRightArea.getId(), jQuery(document.getElementById(oRightArea.getId())));
      };
          ...
          for (iRow = iRenderStartRow; iRow <= iRenderEndRow; iRow++) {
            for (iCol = iRenderStartCol; iCol <= iRenderEndCol; iCol++) {
              ...
              if (oCell) {
                ...
                if (oCell) {
                  oContentDiv = jQuery(document.getElementById(oCell.getId() + "_contentDiv"));
      ...
      function setColWidths (oArea, oDomArea, oDomAreaClone, bCheckFixedSetting) {
        ...
        for (i = 0; i < aCellInfo.length; i++) {
          ...
          if (oCellInfo) {
            ...
            oDomCell = oDomArea.find("#" + jQuery.sap.encodeCSS(oCellInfo.sId));
            oDomCellClone = oDomAreaClone.find("#" + jQuery.sap.encodeCSS(oCellInfo.sId));
            iContentWidth = calculateContentWidth(
              oDomCell, oCellInfo.iWidth, oArea.getCell(
                oCellInfo.iRow, oCellInfo.iCol)
            );
            var oCell = sap.ui.getCore().getControl(oCellInfo.sId);
            if (oCell.isEntryEnabled() && oArea.isRowHeaderArea()) {
              oDomContentDiv = jQuery(oDomCellClone.find("#" + jQuery.sap.encodeCSS(oCellInfo.sId + "_cellLayoutDiv")));
            } else {
              oDomContentDiv = jQuery(oDomCellClone.find("#" + jQuery.sap.encodeCSS(oCellInfo.sId + "_contentDiv")));
            }
      ...
      function forceRowHeight (oArea, oDomArea, oDomAreaClone, oCellInfo, oHandledRowSpanCells) {
        ...
        for (iCol = oArea.getRenderStartCol(); iCol < iEndCol; iCol++) {
          ...
          if (oCell) {
            oDomCell = oDomArea.find("#" + jQuery.sap.encodeCSS(oCell.getId()));
            oDomCellClone = oDomAreaClone.find("#" + jQuery.sap.encodeCSS(oCell.getId()));
            if (oDomCell.length > 0) {
              ...
              oDomContentDiv = jQuery(oDomCellClone.find("#" + jQuery.sap.encodeCSS(oCell.getId() + "_contentDiv")));
        ...
        for (iRow = oArea.getRenderStartRow(); iRow < iEndRow; iRow++) {
          ...
          if (oCell) {
            ...
            if (oDomCell.length > 0) {
              ...
              if (oCell.isEntryEnabled() && oArea.isRowHeaderArea()) {
                oDomContentDiv = jQuery(oDomCellClone.find("#" + jQuery.sap.encodeCSS(oCell.sId + "_cellLayoutDiv")));
              } else {
                oDomContentDiv = jQuery(oDomCellClone.find("#" + jQuery.sap.encodeCSS(oCell.getId() + "_contentDiv")));
              }
              oDomContentDiv.width(iContentWidth);
      ...
      function calculateContentWidth (oDomCell, iCommonWidth, oCell) {
        ...
        if (oCell !== null && oCell.isEntryEnabled()) {
          if (oCell.isEntryEnabled() && oCell.getArea().isRowHeaderArea()) {
            var oDomContentDiv = jQuery(oDomCell.find("#" + jQuery.sap.encodeCSS(oCell.getId() + "_cellLayoutDiv")));
          } else {
            oDomContentDiv = oDomCell.find("#" + jQuery.sap.encodeCSS(oCell.getId() + "_contentDiv"));
          }
      ...
      function calculateRowHeight (oDomCell, iCellHeight) {
        var oParent = jQuery(oDomCell).parent();
      ...
      this.determineHeaderScrollbarVisible = function () {
        ...
        if (oScrollbarVisibility) {
          ...
          if (oDimensionHeaderArea.getColCnt() > 0) {
            ...
            oJqContainerDiv = jQuery(document.getElementById(oCrosstab.getId() + "_dimHeaderArea_container"));
            oJqScrollDiv = jQuery(document.getElementById(oCrosstab.getId() + "_upperLeft_scrollDiv"));
          }
          if (iHeaderWidth === 0) {
            if (oRowHeaderArea.getColCnt() > 0) {
              iHeaderWidth = oMeasuring.getAreaSize(oRowHeaderArea).iWidth;
              oJqContainerDiv = jQuery(document.getElementById(oCrosstab.getId() + "_rowHeaderArea_container"));
              oJqScrollDiv = jQuery(document.getElementById(oCrosstab.getId() + "_lowerLeft_scrollDiv"));
            }
          }
      ...
      this.adjustRenderSizeDivSize = function () {
        if (bAdjustFrameDivs || !bDivSizesValid) {
          ...
          if (oCrosstab.getRenderMode() === RenderingConstants.RENDERMODE_COMPACT) {
            var iScrollbarHeight = (
              oScrollbarVisibility.bHasHScrollbar ||
                oScrollbarVisibility.bHasHHeaderScrollbar) ? oMeasuring.getBrowserScrollbarWidth() : 0;
            var iToolbarHeight = oCrosstab.getPropertyBag().getToolbarHeight();
      ...
      this.renderScrollbars = function () {
        ...
        if (oScrollbarVisibility.bHasHScrollbar || oScrollbarVisibility.bHasVScrollbar
          ...
          oScrollbarRenderer = new ScrollbarRenderer(
            oCrosstab, oRenderManager,
            oMeasuring, oDomElementProvider
          );
          oScrollbarRenderer.renderScrollbars(oScrollbarVisibility);
          oScrollbarRenderer.attachHandlers(
            oScrollManager.hScrollHandler, oScrollManager.vScrollHandler,
      ...
      this.appendLeftCols = function (oArea, iNumberOfCols) {
        ...
        var oJqTable = jQuery(document.getElementById(oArea.getId()));
        ...
        for (iRow = iStartRow; iRow <= iEndRow; iRow++) {
            ...
            if (oCell) {
              ...
              if (iColSpan > 1) {
                ...
                if (iRemainingColSpan > 0) {
                  var oCellToBeRemoved = jQuery(oRow).find("> :first-child");
          ...
          var sRowHtml = jQuery(jQuery(oRows[iRowCounter])).html();
          oRenderManager.write(sRowHtml);
      ...
      this.appendTopRows = function (oArea, iNumberOfRows) {
        ...
        if (aColIndexes.length > 0) {
          var oRows = jQuery(document.getElementById(oArea.getId())).children("tbody").children("tr");
          var oFirstRow = jQuery(oRows[0]);
          var i = 0;
          for (i = 0; i < aColIndexes.length; i++) {
            oCell = oFirstRow.find("> :nth-child(" + (aColIndexes[i] + 1) + ")");
            if (oCell.length > 0) {
          ...
          for (var j = 0; j < aCellIds.length; j++) {
            var oCellToBeRemoved = oFirstRow.find("#" + jQuery.sap.encodeCSS(aCellIds[j]));
            oCellToBeRemoved.remove();
          }
          ...
          for (i = 1; i < oRows.length; i++) {
            ...
            oRenderManager.writeAttribute("class", "sapzencrosstab-HeaderRow");
            oRenderManager.write(">");
            oRenderManager.write(jQuery(oRows[i]).html() + "</tr>");
          }
      ...
      function initDomElementProvider () {
        ...
        oDomElementProvider.addElement(sColHeaderAreaId, oTableDiv.find("#" + jQuery.sap.encodeCSS(sColHeaderAreaId)));
        oDomElementProvider.addElement(sRowHeaderAreaId, oTableDiv.find("#" + jQuery.sap.encodeCSS(sRowHeaderAreaId)));
        oDomElementProvider.addElement(sDimHeaderAreaId, oTableDiv.find("#" + jQuery.sap.encodeCSS(sDimHeaderAreaId)));
        oDomElementProvider.addElement(sDataAreaId, oTableDiv.find("#" + jQuery.sap.encodeCSS(sDataAreaId)));
      }
      ...
      function displayLoadingIndicator () {
        ...
        var oAnimDiv = oTableDiv.find("#" + jQuery.sap.encodeCSS(oCrosstab.getId() + "_loadingAnimationDiv"));
      ...
      this.hideLoadingIndicator = function () {
        ...
        var oAnimationDiv = oTableDiv.find("#" + jQuery.sap.encodeCSS(oCrosstab.getId() + "_loadingAnimationDiv"));
      ...
      this.registerNewScrollbarsNotification = function (fHandler) {
        if (jQuery.inArray(fHandler, aNewScrollbarEventHandlers) === -1) {
      ...
      this.checkScrollbarSize = function () {
        if (oScrollbarVisibility.bHasVScrollbar) {
          var oDomUpperRightPad = jQuery(document.getElementById(oCrosstab.getId() + "_upperRightPad"));
          var oDomUpperScrollDiv = null;

          if (oCrosstab.hasDimensionHeaderArea()) {
            oDomUpperScrollDiv = jQuery(document.getElementById(oCrosstab.getId() + "_upperLeft_scrollDiv"));
          } else if (oCrosstab.hasColHeaderArea()) {
            oDomUpperScrollDiv = jQuery(document.getElementById(oCrosstab.getId() + "_upperRight_scrollDiv"));
          }
        ...
        if (oScrollbarVisibility.bHasHScrollbar) {
          var oDomLowerLeftPad = jQuery(document.getElementById(oCrosstab.getId() + "_lowerLeftPad"));
          var oDomLeftScrollDiv;

          if (oCrosstab.hasRowHeaderArea()) {
            oDomLeftScrollDiv = jQuery(document.getElementById(oCrosstab.getId() + "_lowerLeft_scrollDiv"));
          } else {
            oDomLeftScrollDiv = jQuery(document.getElementById(oCrosstab.getId() + "_upperLeft_scrollDiv"));
          }
      ...
      this.updateRenderingOfInputCellAfterCheck = function (oCell) {
        var oJqCell = jQuery(document.getElementById(oCell.getId()));
        if (oJqCell.length > 0) {
          var sClassName = CellStyleHandler.getCompleteStyleName(
            RenderingConstants.STYLE_INVALID_VALUE, oCell.getCellType());
          if (oCell.hasStyle(RenderingConstants.STYLE_INVALID_VALUE) === true) {
            oJqCell.addClass(sClassName);
          } else {
            oJqCell.removeClass(sClassName);
          }
          var oJqContentDiv = oJqCell.find("#" + jQuery.sap.encodeCSS(oCell.getId() + "_contentDiv"));
      ...
      function manipulateHeaderAreaClass (sPostFix, bAdd) {
        var oArea = jQuery(document.getElementById(oCrosstab.getId() + sPostFix));
        if (oArea.length > 0) {
          if (bAdd === true) {
      ...
      this.getLeftScrollDivs = function () {
        ...
        if (oCrosstab.hasRowHeaderArea()) {
          oJqRowHeaderDiv = jQuery(document.getElementById(oCrosstab.getId() + "_lowerLeft_scrollDiv"));
        }
        if (oCrosstab.hasDimensionHeaderArea()) {
          oJqDimHeaderDiv = jQuery(document.getElementById(oCrosstab.getId() + "_upperLeft_scrollDiv"));
        }
      ...
      this.determineUnrestrictedRightAreaWidth = function() {
        ...
        if (oArea && oArea.getRenderColCnt() > 0) {
          ...
          if (oCell) {
            iShim = parseInt(jQuery(document.getElementById(oCell.getId())).css("border-right-width"), 10);
          }
        }
        ...
        if (oDataArea) {
          jQuery(document.getElementById(oDataArea.getId())).find("tbody").empty();
          oDataArea.restoreData();
        }
        if (oColHeaderArea) {
          jQuery(document.getElementById(oColHeaderArea.getId())).find("tbody").empty();
      ...
      this.enableHeaderResizeHandle = function(bEnable) {
        ...
        oJqHeaderResizeHandle = jQuery(document.getElementById(oCrosstab.getId() + "_headerResizeHandle"));
      ...
      this.getLeftAreaContainerWidth = function () {
        ...
        if (oCrosstab.hasDimensionHeaderArea() === true) {
          oJqLeftAreaContainer = jQuery(document.getElementById(oCrosstab.getId() + "_dimHeaderArea_container"));
          iLeftAreaContainerWidth = oJqLeftAreaContainer.outerWidth();
        } else if (oCrosstab.hasRowHeaderArea() === true) {
          oJqLeftAreaContainer = jQuery(document.getElementById(oCrosstab.getId() + "_rowHeaderArea_container"));
          iLeftAreaContainerWidth = oJqLeftAreaContainer.outerWidth();
        }
      ...
      this.getRightAreaContainerWidth = function () {
        ...
        if (oCrosstab.hasColHeaderArea() === true) {
          oJqRightAreaContainer = jQuery(document.getElementById(oCrosstab.getId() + "_colHeaderArea_container"));
          iRightAreaContainerWidth = oJqRightAreaContainer.outerWidth();
        } else if (oCrosstab.hasDataArea() === true) {
          oJqRightAreaContainer = jQuery(document.getElementById(oCrosstab.getId() + "_dataArea_container"));
          iRightAreaContainerWidth = oJqRightAreaContainer.outerWidth();
        }
      ...
      this.getSectionWidth = function () {
        ...
        if (oCrosstab.hasDimensionHeaderArea()) {
          oJqLeftScrollDiv = jQuery(document.getElementById(oCrosstab.getId() + "_upperLeft_scrollDiv"));
        } else if (oCrosstab.hasRowHeaderArea()) {
          oJqLeftScrollDiv = jQuery(document.getElementById(oCrosstab.getId() + "_lowerLeft_scrollDiv"));
        }

        iLeftSectionCssContribution = Utils.getWidthOfMarginBorderPadding(oJqLeftScrollDiv);

        if (oCrosstab.hasColHeaderArea()) {
          oJqRightScrollDiv = jQuery(document.getElementById(oCrosstab.getId() + "_upperRight_scrollDiv"));
        } else if (oCrosstab.hasDataArea()) {
          oJqRightScrollDiv = jQuery(document.getElementById(oCrosstab.getId() + "_lowerRight_scrollDiv"));
        }

        iRightSectionCssContribution = Utils.getWidthOfMarginBorderPadding(oJqRightScrollDiv);
      ...
      this.updateHeaderResizeDiv = function () {
        var oJqHeaderResizeDiv = jQuery(document.getElementById(oCrosstab.getId() + "_headerResizeHandle"));
        var iCssLeft = 0;
        ...
        if (oJqHeaderResizeDiv && oJqHeaderResizeDiv.length > 0) {
          ...
          if (isNaN(iCssLeft) || !iCssLeft) {
            ...
            var iRenderSizeDivWidth = jQuery(document.getElementById(oCrosstab.getId() + "_renderSizeDiv")).outerWidth();
