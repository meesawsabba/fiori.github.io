...
sap.ui.define(
  ...
  function(jQuery, Measuring, Popup, OpenState, TextConstants,Utils) {
    "use strict";
    ...
    sap.zen.crosstab.rendering.ScrollManager = function (oCrosstab, oRenderEngine) {
      ...
      function determineRenderedRowsInArea(oArea) {
        var iRowCnt = 0;
        if (oArea) {
          iRowCnt = jQuery(document.getElementById(oArea.getId())).children("tbody").children("tr").length;
        }
        return iRowCnt;
      }
      ...
      function handleHScrolling (poEvent) {
        ...
        if (oParameters) {
          ...
          if (iStartCol !== oColHeaderArea.getRenderStartCol()) {
            ...
            var bLastColRendered = oColHeaderArea.getRenderColCnt() + oColHeaderArea.getRenderStartCol() === oColHeaderArea.getColCnt();
      ...
      this.positionHScrollDiv = function (bScrollToEnd) {
        var oURS = jQuery(document.getElementById(oCrosstab.getId() + "_upperRight_scrollDiv"));
        var oLRS = jQuery(document.getElementById(oCrosstab.getId() + "_lowerRight_scrollDiv"));
      ...
      function displayHPopup (iScrollPos) {
        ...
        var sLabelText = oCrosstab.getPropertyBag().getText(
          TextConstants.COL_TEXT_KEY
        ) + " "
            + Utils.padWithZeroes(iScrollPos + 1, iColCntLength) + "/" + oColHeaderArea.getColCnt();
        if (!iHPopupWidth) {
          if (Utils.isMainMode()) {
          ...
          if (Utils.isMainMode()) {
            iHPopupWidth = jQuery(document.getElementById(oPopup.getContent().getId())).outerWidth();
          } else {
            iHPopupWidth = jQuery(document.getElementById(oPopup.getContent().getId())).innerWidth();
          }
        }
        if (Utils.isMainMode()) {
      ...
      function renderHPopup (sLabelText) {
        ...
        var iPopupWidth = jQuery(document.getElementById(oPopup.getContent().getId())).outerWidth();
      ...
      function displayVPopup (iScrollPos) {
        ...
        var sLabelText = oCrosstab.getPropertyBag().getText(TextConstants.ROW_TEXT_KEY) + " "
            + Utils.padWithZeroes(iScrollPos + 1, iRowCntLength) + "/" + oRowHeaderArea.getRowCnt();
        ...
        if (!iVPopupWidth) {
          ...
          if (Utils.isMainMode()) {
            iVPopupWidth = jQuery(document.getElementById(oPopup.getContent().getId())).outerWidth();
          } else {
            iVPopupWidth = jQuery(document.getElementById(oPopup.getContent().getId())).innerWidth();
          }
        }
        sLabelText = sLabelText + "\n" + aHeaderTexts.join("\n");
        if (Utils.isMainMode()) {
      ...
      function renderVPopup (sLabelText) {
        ...
        var iPopupHeight = jQuery(document.getElementById(oPopup.getContent().getId())).outerHeight();
      ...
      function determineRenderedColsInArea(oArea) {
        ...
        var iColSpan = 1;
        var iColCnt = 0;
        jQuery(document.getElementById(oArea.getId())).children("tbody").children("tr:last").children("td").each(function(index, domTd) {
          sColSpan = jQuery(domTd).attr("colspan");
          if (!sColSpan) {
            iColSpan = 1;
          } else {
      ...
      function handleVScrolling (poEvent) {
        ...
        if (oParameters) {
          ...
          if (iStartRow !== oRowHeaderArea.getRenderStartRow()) {
            ...
            var bLastRowRendered = oRowHeaderArea.getRenderRowCnt() + oRowHeaderArea.getRenderStartRow() === oRowHeaderArea.getRowCnt();
      ...
      this.positionVScrollDiv = function(bScrollToEnd) {
        var oLRS = jQuery(document.getElementById(oCrosstab.getId() + "_lowerRight_scrollDiv"));
        var oLLS = jQuery(document.getElementById(oCrosstab.getId() + "_lowerLeft_scrollDiv"));
