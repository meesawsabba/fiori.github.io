...
sap.ui.define(
  [
    ...
    "sap/zen/crosstab/HeaderResizer",
    "sap/zen/crosstab/ColResizer"
  ],
  function(
    jQuery, TouchHandler, SelectionHandler, CrossRequestManager,
    RenderingConstants,  Utils, CrosstabKeyboardNavHandler, HeaderResizer,
    ColResizer
  ){
    "use strict";
    ...
    sap.zen.crosstab.EventHandler = function (oCrosstab) {
      ...
      var oKeyboardHandler = new CrosstabKeyboardNavHandler(oCrosstab, this);
      ...
      this.findTargetId = function (oDomTarget) {
        ...
        sCellId = jQuery(oDomTarget).attr("xtabspacer-cellid");
        if (sCellId && sCellId.length > 0) {
          sTargetId = sCellId;
        } else {
          oJqClosestDiv = jQuery(oDomTarget).closest("div");
      ...
      this.postPlanningValue = function () {
        if (oCrosstab.isPlanningMode() === true && oInputField && oInputField.length > 0) {
          var oJqInputField = jQuery(document.activeElement);
        ...
        if (oInputField.length === 0) {
          ...
          var onLoseFocus = function (e) {
            if (oInputField.val() !== sRenderText) {
              ...
              // the new value
              var sEncodedRenderText = jQuery("<div></div>").text(sRenderText).html();
          ...
          var checkTargetIsInCrosstab = function(e) {
            ...
            if (e && e.relatedTarget && e.relatedTarget.id && oCrosstab.getValueHelpStatus() !== sap.zen.crosstab.VHLP_STATUS_OPENING) {
              ...
              oJqTarget = jQuery(document.getElementById(sTargetId));
          ...
          var onFocusLost = function (e) {
            var oJqDiv = jQuery(document.getElementById(sTargetId + "_contentDiv"));
            oJqDiv.html(html);
          ...
          if (bCellIsDataCell) {
            ...
            sSetContentDivWidth = Utils.getWidthFromStyle(oContentDiv);
          }
          ...
          oInputField = jQuery(document.getElementById(sTargetId + "_input"));
          if (Utils.isMainMode()) {
      ...
      this.getContextElement = function(iClientX, iClientY) {
        ...
        var i = 0;
        var oJqElement = jQuery(document.elementFromPoint(iClientX, iClientY));
        ...
        while (sId && sId.indexOf("droparea") > -1) {
          ...
          oJqElement = jQuery(document.elementFromPoint(iClientX, iClientY));
          sId = oJqElement.attr("id");
        }
      ...
      this.attachEvents = function () {
        ...
        if (oCrosstab.getPropertyBag().isMobileMode() || oCrosstab.getPropertyBag().isTestMobileMode()) {
          ...
          oTouchHandler = new TouchHandler(this, oCrosstab);
          ...
          if (oCrosstab.getPropertyBag().isEnableColResize() === true) {
            var oTouchColResizer = new ColResizer(oCrosstab);
          ...
          /*MU undefine oJqTitleDiv oTouchHandler.registerTouchEvents(oJqTitleDiv);*/
        } else {
          ...
          if (oCrosstab.getUserHeaderWidthCommand() && oCrosstab.getUserHeaderWidthCommand().length > 0) {
            oHeaderResizer = new HeaderResizer(oCrosstab);
          ...
          // Column resize
          if (oCrosstab.getPropertyBag().isEnableColResize() === true) {
            oColResizer = new ColResizer(oCrosstab);
              ...
              if (oCell) {
                if (oCrosstab.isPlanningMode()) {
                  ...
                  if (oPositions.iSelectionStartPos >= 0 || oPositions.iSelectionEndPos >= 0) {
                    ...
                    that.handleInputEnabledCell(
                      e.target.id, oPositions.iSelectionStartPos,
