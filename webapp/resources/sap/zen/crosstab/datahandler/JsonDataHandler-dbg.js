  ...
  function(
    ...
    sap.zen.crosstab.datahandler.JsonDataHandler = function (oCrosstab) {
      ...
      function createDefaultCrosstabTextCache () {
        ...
        oPropertyBag.addText(TextConstants.ROW_TEXT_KEY, "Row");
        oPropertyBag.addText(TextConstants.COL_TEXT_KEY, "Column");
        ...
        oPropertyBag.addText(TextConstants.MEASURE_STRUCTURE_TEXT_KEY, "Measure Structure");
      ...
      function readTranslatedTexts (oTexts) {
        if (sap.zen.CrosstabTextCache.filled === false) {
          ...
          if (oTexts) {
            ...
            oPropertyBag.addText(TextConstants.MEASURE_STRUCTURE_TEXT_KEY, oTexts.measurestructtext || "Measure Structure");
      ...
      this.determineBasicAreaData = function (oCrosstabData, bIsFirstPage) {
        ...
        if (!oJsonTableControl.rows) {
          ...
          if (bIsFirstPage || oJsonTableControl.changed) {
            ...
            var oCellApi = new CrosstabCellApi(
              oCrosstab, iFixedRowHeaders, iFixedColHeaders,
          ...
          if (oJsonTableControl.zebra) {
            ...
            oCrosstab.getPropertyBag().setZebraMode(RenderingConstants.ZEBRA_FULL);
          }
        }
        ...
        if (oCrossRequestManager) {
          ...
          if (oJsonTableControl.clienthpos !== undefined && oJsonTableControl.clientvpos !== undefined
            ...
            if (!oCrossRequestManager.hasSavedVScrollInfo() && !oCrossRequestManager.hasSavedHScrollInfo()) {
              oCrossRequestManager.setScrollData(
                parseInt(oJsonTableControl.clienthpos, 10),
      ...
      function createDataCell (iRow, iCol) {
        ...
        oDataCell.addStyle(
          oCrosstab.getPropertyBag().isCozyMode() ? RenderingConstants.STYLE_DATA_CELL_COZY : RenderingConstants.STYLE_DATA_CELL
        );
        return oDataCell;
      }
      var addDataCell = function (oJsonCellControl, iModelRow, iModelCol) {
        ...
        if (oCrosstab.getPropertyBag().getZebraMode() !== RenderingConstants.ZEBRA_OFF) {
          if (iRowIndex % 2 === 1) {
            oDataCell.addStyle(RenderingConstants.STYLE_ALTERNATING);
          }
        }
      ...
      var addRowHeaderCell = function (oJsonCellControl, iModelRow, iModelCol) {
        ...
        addHeaderCell(
          oJsonCellControl, oRowHeaderArea, iModelRow - iFixedColHeaders, iModelCol, iActualModelRow,
          iModelCol
        );
      };
      ...
      var addColumnHeaderCell = function (oJsonCellControl, iModelRow, iModelCol) {
        ...
        addHeaderCell(
          oJsonCellControl, oColHeaderArea, iModelRow, iModelCol - iFixedRowHeaders, iModelRow,
          iActualModelCol
        );
      };
      ...
      function createHeaderCell (oArea, iRow, iCol) {
        ...
        oHeaderCell.addStyle(oCrosstab.getPropertyBag().isCozyMode() ? RenderingConstants.STYLE_HEADER_CELL_COZY : RenderingConstants.STYLE_HEADER_CELL);
        return oHeaderCell;
      }
      ...
      var parseCommonCellData = function (oJsonCellControl, oCommonCell) {
        ...
        if (exceptionVisualizations) {
          for ( var index in exceptionVisualizations) {
            if (
              ...
              if (viz) {
                CellStyleHandler.setExceptionStylesOnCell(
                  oCommonCell, viz.formattype,
                  viz.alertlevel
                );
              }
            }
        ...
        if (oJsonCellControl.isemphasized) {
          oCommonCell.addStyle(RenderingConstants.STYLE_EMPHASIZED);
        }

        // planning
        if (!(oCrosstab.getPropertyBag().isMobileMode() || oCrosstab.getPropertyBag().isTestMobileMode())) {
          if (oJsonCellControl.isdataentryenabled) {
            oCommonCell.addStyle(RenderingConstants.STYLE_DATA_ENTRY_ENABLED);
          ...
          if (oJsonCellControl.hasinvalidvalue) {
            oCommonCell.addStyle(RenderingConstants.STYLE_INVALID_VALUE);
          }
          if (oJsonCellControl.hasnewvalue) {
            oCommonCell.addStyle(RenderingConstants.STYLE_NEW_VALUE);
          }
          if (oJsonCellControl.islocked) {
            oCommonCell.addStyle(RenderingConstants.STYLE_LOCKED);
          }
        }
        if (oJsonCellControl.isresult) {
          ...
          oCommonCell.addStyle(RenderingConstants.STYLE_TOTAL);
        }
      ...
      var parseHeaderCellData = function (oJsonCellControl, oHeaderCell, oHeaderArea) {
        ...
        if (oCrosstab.getPropertyBag().getZebraMode() === RenderingConstants.ZEBRA_FULL) {
          if (oHeaderArea.isRowHeaderArea() && oHeaderCell.getRow() % 2 === 1 && oHeaderCell.getRowSpan() === 1) {
            ...
            if (oHeaderInfo) {
              if ((oHeaderCell.getCol() + oHeaderCell.getColSpan() - 1) >= oHeaderInfo.getStartColForInnermostDimension()) {
                oHeaderCell.addStyle(RenderingConstants.STYLE_ALTERNATING);
              }
            }
          }
        }
      ...
      var saveHierarchyLevelInfo = function (oArea, oCell, iLevel) {
        if (oArea.getAreaType() === RenderingConstants.TYPE_COLUMN_HEADER_AREA) {
        ...
        } else if (oArea.getAreaType() === RenderingConstants.TYPE_ROW_HEADER_AREA) {
