sap.ui.define(["sap/ui/core/UIComponent", "sap/ovp/cards/rta/SettingsDialogConstants",
    "sap/ui/core/routing/Router", "sap/base/util/ObjectPath",
        "sap/ui/model/json/JSONModel", "sap/ovp/ui/DashboardLayoutUtil", "sap/ui/core/mvc/ViewType",
    "sap/ui/model/resource/ResourceModel", "sap/ovp/app/resources", "sap/ui/core/CustomData", "sap/ushell/Config", "sap/base/util/merge", "sap/base/util/isEmptyObject", "sap/ovp/cards/ovpLogger", "sap/ui/Device", "sap/ovp/cards/CommonUtils", "sap/ui/generic/app/navigation/service/NavigationHandler", "sap/ovp/placeholder/placeholderHelper", "sap/m/App", "sap/ui/rta/RuntimeAuthoring" /*Don't Remove this. Important for Personalization to work*/],

    function (UIComponent, SettingsDialogConstants, Router, ObjectPath,
              JSONModel, DashboardLayoutUtil, ViewType, ResourceModel, OvpResources, CustomData, Config, merge, isEmptyObject, ovpLogger, Device, CommonUtils, NavigationHandler, placeholderHelper, App) {
        "use strict";
        ...
        return UIComponent.extend("sap.ovp.app.Component", {
            ...
            isDate: function(oProperty) {
                if (((oProperty["type"] === "Edm.DateTime" && oProperty["sap:display-format"] === "Date") || (oProperty["type"] === "Edm.String" && oProperty["com.sap.vocabularies.Common.v1.IsCalendarDate"] && oProperty["com.sap.vocabularies.Common.v1.IsCalendarDate"].Bool === "true")) && oProperty["sap:filter-restriction"] === "interval") {
                    return true;
                }
                return false;
            },
            _getDatePropertiesFromEntitySet: function(oFilterBarEntityType) {
                var aProperties = oFilterBarEntityType.property, aDateProperties = [];
                for (var key in  aProperties) {
                    var oDatePropertyPath = {};
                    if (this.isDate(aProperties[key])) {
            ...
            createXMLView: function (ovpConfig) {
                ...
                if (ovpConfig.globalFilterEntityType) {
                    ...
                    if (CommonUtils.isODataV4(oFilterModel)) {
                        var aSelectionFields = oFilterMetaModel.getObject("/" + ovpConfig.globalFilterEntityType + "@com.sap.vocabularies.UI.v1.SelectionFields");
                        aAllControlConfiguration = aSelectionFields;
                    ...
                    if (ovpConfig.filterSettings && ovpConfig.filterSettings.dateSettings) {
                        ...
                        if (uiModel.getProperty("/useDateRangeType") && !isEmptyObject(oDatePropertiesSettings)) {
                 ...
                 * temp
                 */
                var oView = new sap.ui.core.mvc.XMLView(sMainComponentId, {
                    height: "100%",
                    preprocessors: {
                        xml: {
                            bindingContexts: {
            ...
            _checkForAuthorizationForCards: function (ovpConfig) {
                return new Promise(function (resolve, reject) {
                    this.ovpConfig = ovpConfig;
                    var oNavigationHandler = new NavigationHandler(this);
                    CommonUtils.enable(this, oNavigationHandler);
                    var oNavigationHandler = CommonUtils.getNavigationHandler();
                    var cardsIntent = [];
                    for (var cardId in this.ovpConfig.cards) {
                        var oCard = this.ovpConfig.cards[cardId];
                        if (oCard && oCard.settings && oCard.settings.requireAppAuthorization) {
                            cardsIntent.push(oCard.settings.requireAppAuthorization);
                        }
                    }
                    if (cardsIntent.length > 0 && oNavigationHandler && oNavigationHandler.oCrossAppNavService) {
                        return oNavigationHandler.oCrossAppNavService.isIntentSupported(cardsIntent).done(function (oResponse) {
                            var unsupportedCards = [];
                            for (var cardId in this.ovpConfig.cards) {
                                if (this.ovpConfig.cards[cardId].settings.requireAppAuthorization && oResponse[this.ovpConfig.cards[cardId].settings.requireAppAuthorization].supported === false) {
                                    unsupportedCards.push(cardId);
                                }
                            }
                            for (var i = 0; i < unsupportedCards.length; i++) {
                                for (var cardId in this.ovpConfig.cards) {
                                    if (unsupportedCards[i] === cardId) {
                                        delete this.ovpConfig.cards[cardId];
                                        break;
                                    }
                                }
                            }
                            resolve(this.ovpConfig);
                        }.bind(this))
            ...
            createContent: function () {
                ...
                if (oFilterModel && !this.getAggregation("rootControl")) {
                    Promise.all(aPromises).then(function(aResponse){
                        return this._checkForAuthorizationForCards(aResponse[1]);
