sap.ui.define([
    ...
    function(UIActions, Device, jQuery, merge, ovpLogger, jUtils) {
	"use strict";
    ...
    Rearrange.prototype._resizeEndHandler = function (evt, cardElement) {
        if (cardElement) {
            ...
            if (Device.system.desktop) {
                document.querySelector("body").classList.remove("sapOVPDisableUserSelect", "sapOVPDisableImageDrag");
            }
    ...
    Rearrange.prototype._resizeMoveHandler = function (actionObject) {
        ...
        if (actionObject.element) {
            ...
            if (!this.uiActions.isResizeY) {//If the resize is done only in X-direction
                ...
                if ((oCard.template === "sap.ovp.cards.list" || oCard.template === "sap.ovp.cards.v4.list") && ghostWidth > this.layoutUtil.getColWidthPx() * 2 ||
                    ((oCard.template === "sap.ovp.cards.linklist" || oCard.template === "sap.ovp.cards.v4.linklist") && oCard.settings.listFlavor === 'carousel' &&
            ...
            if (ghostHeight <= leastHeight) {
                ...
                if (!this.uiActions.isResizeX && (oCard.template === 'sap.ovp.cards.list' || oCard.template === 'sap.ovp.cards.v4.list' || oCard.template === 'sap.ovp.cards.table' || oCard.template === 'sap.ovp.cards.v4.table')) {
    ...
    Rearrange.prototype._dragEndHandler = function (evt, cardElement, cloneObj) {
        if (cardElement) {
            ...
            jQuery(cloneObj).one("webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend", function (event) {
                ...
                jUtils.addStyleToAllElements(document.querySelectorAll(".sapUshellEasyScanLayoutInner"), "height", iContainerHeight + "px");
                jUtils.addStyleToAllElements(document.querySelectorAll(".sapUshellEasyScanLayoutInner"), "zIndex", "1");
                jUtils.addStyleToAllElements(document.querySelectorAll(".sapUshellEasyScanLayout"), "height", iContainerHeight + "px");
                this.layoutUtil.setAriaPos();
