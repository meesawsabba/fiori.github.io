sap.ui.define(["sap/ui/core/Control",
    ...
    function (Control, jQuery, Device, ResizeHandler, OvpResources, OVPUtils, KeyCodes, F6Navigation, jUtils) {
        "use strict";
        ...
        var DashboardLayout = Control.extend("sap.ovp.ui.DashboardLayout", {
            ...
            renderer: {
                ...
                render: function (oRm, oControl) {
                    ...
                    var oLayoutData = oControl.dashboardLayoutUtil.updateLayoutData(ctrlWidth ? ctrlWidth : window.innerWidth);
                    ...
                    oRm.writeClasses();
                    bRTL ? oRm.style("margin-right", oLayoutData.marginPx + "px") : oRm.style("margin-left", oLayoutData.marginPx + "px");
                    oRm.writeStyles();
                    oRm.write(">");
                    ...
                    if (aCards.length > 0) {
                        ...
                        for (counter = 0, iLength = filteredItems.length; counter < iLength; counter++) {
                            ...
                            oRm.addClass(aStyleClasses.join(' '));
                            oRm.writeClasses();
                            oRm.style("transform", "translate3d(" + card.dashboardLayout.left + " ," + card.dashboardLayout.top + " ,0px)");
                            oRm.style("-ms-transform", "translate3d(" + card.dashboardLayout.left + " ," + card.dashboardLayout.top + " ,0px)");
                            oRm.style("-moz-transform", "translate3d(" + card.dashboardLayout.left + " ," + card.dashboardLayout.top + " ,0px)");
                            oRm.style("-webkit-transform", "translate3d(" + card.dashboardLayout.left + " ," + card.dashboardLayout.top + " ,0px)");
                            oRm.style("height", card.dashboardLayout.height);
                            oRm.style("width", card.dashboardLayout.width);
                            oRm.writeStyles();
                            oRm.write( "tabindex='0' aria-setsize=" + iLength + " aria-posinset=" + (counter + 1));
        ...
        KeyboardNavigation.prototype.init = function (ovpLayout) {
            ...
            var dynamicPageContent = document.querySelectorAll(".sapFDynamicPageContent");
        ...
        KeyboardNavigation.prototype.layoutItemFocusHandler = function () {
            ...
            if (jqFocused) {
                ...
                if (jqFocused.hasClass('easyScanLayoutItemWrapper')) {
                    ...
                    var sCardText = OvpResources.getText("Card");
                    if (cardCountDiv.length === 0) {
                        sCountDivId = "countDiv_" + new Date().getTime();
                        var sDummyDivForCardCount = '<div id="' + sCountDivId + '" class="cardCount" aria-label="' + sCardText + '" hidden></div>';
                        jqFocused.append(sDummyDivForCardCount);
                    } else {
                        sCountDivId = cardCountDiv[0].id;
                        cardCountDiv.attr('aria-label', sCardText);
                    }
            ...
            switch (e.keyCode) {
                ...
                case this.keyCodes.ARROW_DOWN:
                    ...
                    if (cardId && e.ctrlKey) {
                        ...
                        if (((oCard.template === "sap.ovp.cards.linklist" || oCard.template === "sap.ovp.cards.v4.linklist") && 
                                oCard.settings.listFlavor === 'carousel' && oCard.dashboardLayout.rowSpan >= 45)) {
                            return;
                        }
                        oSpan = {
                            rowSpan:  oCard.dashboardLayout.rowSpan + 3,
                ...
                case this.keyCodes.ARROW_LEFT:
                    ...
                    if (cardId && e.ctrlKey) {
                        ...
                        if (bIsRTL) {
                            if ((oCard.template === "sap.ovp.cards.list" && oCard.dashboardLayout.colSpan === 2) || 
                                ((oCard.template === "sap.ovp.cards.linklist" || oCard.template === "sap.ovp.cards.v4.linklist") && oCard.settings.listFlavor === 'carousel' && oCard.dashboardLayout.colSpan === 3)) {
                                return;
                            }
                            oSpan = {
                                rowSpan:  oCard.dashboardLayout.rowSpan,
        ...
        KeyboardNavigation.prototype.setFocusInCaseOfSpace = function (oCard, sType) {
            var aCards = Object.assign([], this.layoutModel.aCards),
                ...
                aNextCard = aCards.filter(function(oCard) {
                    return sType === "Right" ? oAriaPosMap[oCard.id] === currentPos + 1 : oAriaPosMap[oCard.id] === currentPos - 1;
                }, oAriaPosMap);
