sap.ui.define(["sap/ovp/cards/generic/Card.controller", "sap/ovp/cards/charts/VizAnnotationManager",
    "sap/viz/ui5/data/FlattenedDataset", "sap/ovp/cards/OVPCardAsAPIUtils", "sap/ovp/app/resources", "sap/base/util/each", "sap/ovp/cards/ovpLogger", "sap/ovp/app/FilterHelper", "sap/ui/core/Fragment"],
    function (CardController, VizAnnotationManager, FlattenedDataset, OVPCardAsAPIUtils, OvpResources, each, ovpLogger, FilterHelper, Fragment) {
        "use strict";
...
//				this.bFlag = true;
                ...
                this.eventhandler = function (sChannelId, sEventName, aFilters) {
                    ...
                    var oGlobalFilter = that.oMainComponent.getMacroFilterBar();
                    var oFilterModel = oGlobalFilter.getModel();
                    var oCardProperties = that.getView().getModel('ovpCardProperties');
                    var oCardModel = oCardProperties.getData().entityType.name;
                    if (oEntityType && oFilterModel) {
                        var aRelevantFilters = FilterHelper.getEntityRelevantFilters(oEntityType, aFilters, oCardModel, oFilterModel);
                        var aAllFilters = FilterHelper.mergeFilters(aRelevantFilters, that.selectionVaraintFilter);
                        try {
                ...
                if (this.oMainComponent && this.oMainComponent.isMacroFilterBar && this.oCardComponentData && this.oCardComponentData.mainComponent) {
...
//				sap.ovp.cards.charts.VizAnnotationManager.hideDateTimeAxis(vizFrame);
                    ...
                    if (binding && binding.getPath()) {
                        ...
                        Fragment.load("sap.ovp.cards.charts.generic.noData").then(function(oControl){
