sap.ui.define([
...
], function (Dialog, Button, PayLoadUtils, OVPCardAsAPIUtils, SettingsConstants, CommonUtils,
    Device, MessagePopover, MessagePopoverItem, Link, MessageBox, AnnotationHelper, ViewType,
    JSONModel, OvpResources, OVPUtils, merge, ovpLogger) {
    "use strict";
    ...
    function getLabelWithPropertyName(sKey, oEntityType, sPropertyName, sLabel) {
        if (oEntityType[sKey] && oEntityType[sKey][sPropertyName]) {
            var sPropertyValue = typeof oEntityType[sKey][sPropertyName] === "string" ?
                oEntityType[sKey][sPropertyName] : oEntityType[sKey][sPropertyName].String; // OData V4 property value is string
    ...
    function getAnnotationLabelV4(oEntityType, sKey) {
        ...
        } else if (sKey.indexOf(".PresentationVariant") !== -1
    ...
    function getVisibilityOfElement(oCardProperties, sElement, isViewSwitchEnabled, iIndex, sCardType) {
        ...
        switch (sElement) {
            ...
            case "staticLink":
            case "links":
                var bFlag = ((oCardProperties.template === "sap.ovp.cards.linklist" || oCardProperties.template === "sap.ovp.cards.v4.linklist") && !!oCardProperties.staticContent);
    ...
    function setVisibilityForFormElements(oCardProperties) {
        ...
        if ((oCardProperties.template === "sap.ovp.cards.linklist" || oCardProperties.template === "sap.ovp.cards.v4.linklist") && !!oCardProperties.staticContent) {
    ...
    function addManifestSettings(oData) {
        ...
        if (oData.lineItem) {
            oData.lineItem.forEach(function (item) {
                if ((bIsOdataV4 && item.value === "@" + oData.annotationPath) || item.value === oData.annotationPath) {
    ...
    function addAllEntitySetV4(oData) {
        ...
        aEntitySets.forEach(function (entitySet) {
            var sLabel = OvpResources.getText("OVP_KEYUSER_LABEL_DEFAULT_LABEL_WITH_QUALIFIER", [entitySet.name]);
            var sValue = oMetaDataSnapshot && oMetaDataSnapshot[entitySet.entityType] && oMetaDataSnapshot[entitySet.entityType]["sap:label"];
                ...
                if (oEntityType.hasOwnProperty(key) && key.indexOf(oVariantName.sVariant) !== -1) {
                    if (oVariantName.sVariant === ".LineItem") {
                        var variant = {
                            ...
                            fields: bIsOdataV4 ? getAnnotationLabelV4(oEntityType, key) : getAnnotationLabel(oEntityType, key)
                        };
                        aVariants.push(variant);
                        iOptionCounter++;
                    } else {
                        aVariants.push({
                            name: bIsOdataV4 ? getAnnotationLabelV4(oEntityType, key) : getAnnotationLabel(oEntityType, key),
                            value: bIsOdataV4 ? key.substring(1) : key
                        });
                    }
                }
        ...
        getDialogBox: function (oComponentContainer) {
            return new Promise(function (resolve, reject) {
                ...
                if (layer === OVPUtils.Layers.vendor) {
                    ...
                    var oi18nModel = sap.ui.getCore().byId(sAppComponentId + "---mainView").getModel("i18n");
                    var aCustomBundles = oi18nModel.oData.bundle && oi18nModel.oData.bundle.oi18nModel;
                    if (Array.isArray(aCustomBundles) && aCustomBundles[0] && aCustomBundles[0].aPropertyFiles[0] && aCustomBundles[0].aPropertyFiles[0].mProperties) {
                ...
                if (oCardProperties.template === "sap.ovp.cards.linklist" || oCardProperties.template === "sap.ovp.cards.v4.linklist") {
                ...
                if (oCardProperties.layoutDetail === "resizable") {
                    ...
                    if (!oCardProperties.defaultSpan) {
                        ...
                        oCardPropertiesModel.setProperty("/defaultSpan/rows",
                            supportedCards.indexOf(oCardProperties.template) ? oCardPropertiesModel.getProperty("/cardLayout/noOfItems") : oCardPropertiesModel.getProperty("/cardLayout/rowSpan"));
                    } else {
                    ...
                    if (checkIfCardTemplateHasProperty(oCardProperties.template, 'chart') || oCardProperties.template === 'sap.ovp.cards.linklist' || oCardProperties.template === 'sap.ovp.cards.v4.linklist') {
                ...
                // settings view
                var oSettingsView = new sap.ui.core.mvc.XMLView("settingsView", {
