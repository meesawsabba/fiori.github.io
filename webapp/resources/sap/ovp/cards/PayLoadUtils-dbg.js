...
sap.ui.define([
        ...
        "sap/base/util/merge"
    ], function(jQuery, CommonUtils, SettingsConstants, OVPUtils, merge) {
        "use strict";
        ...
        function copyFormattedArrayWithoutReference(newArray, sType) {
            return jQuery.map(merge({}, newArray), function (value) {
        ...
        function createManifestForKPICard(settingsUtils) {
            ...
            if (newSettings["addCustomParameters"]) {
                oCardManifest.settings.staticParameters = {};
                var aAllStaticParameters = newSettings['aAllStaticParameters'];
                if (Array.isArray(aAllStaticParameters) && aAllStaticParameters.length > 0) {
                    for (var i = 0; i < aAllStaticParameters.length; i++) {
                        var oStaticParameters = aAllStaticParameters[i];
                        oCardManifest.settings.staticParameters[oStaticParameters.key] = oStaticParameters.value;
                    }
                }
            }
	...
	*/
	function createTextTranslationObjectForCloneCard(sApplicationId, sCardId, oCardManifest, oSettingsUtils ) {
        ...
        function getFinalPayload(settingsUtils, cardType) {
            ...
            oFlexCardManifest = merge({}, oCardManifest);
            oFlexCardManifest.id = cardId;
            if (this._oCardManifestSettings.selectedKPI) {
                oFlexCardManifest.settings.selectedKPI = merge({}, this._oCardManifestSettings.selectedKPI);
            }
            ...
            if (sLayer === OVPUtils.Layers.vendor) {
                writeToi18nFile(sApplicationId, merge({},oFinalData.oText));
        ...
        var oPayLoadUtils = {
            ...
            getPayLoadForEditCard: function(settingsUtils) {
                var sCardId = settingsUtils.oAppDescriptor.id, oParameters = {
                    "cardId": sCardId
                }, oText, aEntityPropertyChange, oCardManifest = merge({}, settingsUtils.oAppDescriptor),
                    oOriginalCardManifest = merge({}, settingsUtils.oAppDescriptor),
                ...
                oText = merge({}, oPayLoadUtils.oText);
                if (sLayer === OVPUtils.Layers.vendor) {
            ...
            getPayLoadForCloneCard: function(oComponentContainer) {
                return new Promise(function (resolve, reject) {
                    var oParameters = {
                        ...
                        oCardManifest = merge({}, oMainComponent._getCardFromManifest(oComponentData.cardId)),
                        sLayer = CommonUtils._getLayer();
                    ...
                    if (sLayer === OVPUtils.Layers.vendor) {
                        writeToi18nFile(sApplicationId, merge({}, oText));
                        oText = createTranslationTextObjectForDTA();
                    }
                    oParameters.card[cardId] = oCardManifest;
