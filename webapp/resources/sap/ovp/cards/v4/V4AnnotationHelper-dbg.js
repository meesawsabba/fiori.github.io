...
sap.ui.define(["sap/ui/core/format/NumberFormat", "sap/ui/core/format/DateFormat",
    "sap/ovp/cards/CommonUtils", "sap/ui/model/odata/v4/AnnotationHelper", "sap/ui/model/FilterOperator", "sap/ui/Device", "sap/ui/model/analytics/odata4analytics", "sap/ui/generic/app/navigation/service/SelectionVariant", "sap/ovp/app/resources", "sap/base/strings/formatMessage", "sap/base/util/isPlainObject", "sap/base/util/each", "sap/ovp/cards/MetadataAnalyser", "sap/base/security/encodeURL", "sap/base/util/merge", "sap/ovp/cards/ovpLogger", "sap/base/util/isEmptyObject", "sap/ovp/app/OVPUtils"],
    function (NumberFormat, DateFormat, CommonUtils, OdataAnnotationHelper,
        FilterOperator, Device, odata4analytics, SelectionVariant, OvpResources, formatMessage, isPlainObject, each, MetadataAnalyser, encodeURL, merge, ovpLogger, isEmptyObject, OVPUtils) {
        "use strict";
        ...
        function getSortedDataFields(iContext, aCollection) {
            ...
            if (!aSortedFields) {
                ...
                aSortedFields = aCollection.filter(function (item) {
                    if (item.$Type === "com.sap.vocabularies.UI.v1.DataField" && aDataPointsValues.indexOf(item.Value.$Path) === -1) {
                        return true;
                    }
                    return false;
                });
        ...
        function isDataFieldForAnnotation(oItem) {
            if (oItem.$Type === "com.sap.vocabularies.UI.v1.DataFieldForAnnotation" &&
        ...
        function isDataFieldForAnnotationWithDataPointOrContact(oItem) {
            if ((oItem.$Type === "com.sap.vocabularies.UI.v1.DataFieldForAnnotation" && oItem.Target.$AnnotationPath.match(/@com.sap.vocabularies.UI.v1.DataPoint.*/)) ||
                (oItem.$Type === "com.sap.vocabularies.UI.v1.DataFieldForAnnotation" && oItem.Target.$AnnotationPath.match(/@com.sap.vocabularies.Communication.v1.Contact.*/))) {
                return true;
            }
            return false;
        }
        ...
        function _formatDataPoint(iContext, oItem, oEntityType, oMetaModel, dontIncludeUOM) {
            ...
            if (bIgnoreSapText == true) { //as part of supporting V4 annotation
                if (oEntityTypeAnnotation && (oEntityTypeAnnotation["@com.sap.vocabularies.Analytics.v1.Measure"] || oEntityTypeAnnotation["sap:aggregation-role"] == "measure")) {
                    bMeasuresType = true;
                }
            }
            ...
            if (!bMeasuresType && oEntityTypeAnnotation) {
                var txtValue;
                if (oEntityTypeAnnotation["com.sap.vocabularies.Common.v1.Text"]) {  //as part of supporting V4 annotation
                    txtValue = oEntityTypeAnnotation["com.sap.vocabularies.Common.v1.Text"] ? oEntityTypeAnnotation["com.sap.vocabularies.Common.v1.Text"] : oEntityTypeAnnotation["com.sap.vocabularies.Common.v1.Text"].$Path;
        ...
        function getTextArrangementBinding(iContext, oEntityType, oProperty, sIdBinding) {
            ...
            oTextArrangement = oProperty["@" + AnnotationType.TEXT + "@" + AnnotationType.TEXT_ARRANGEMENT];
        ...
        function _formatField(iContext, oItem, oEntityType, oMetaModel, bDontIncludeUOM, bIncludeOnlyUOM, bUseSimplePath) {
            ...
            var oEntityTypeProperty = getEntityType(oMetaModel, oEntityType, oItem.Value.$Path),
                result = "",
                functionName,
                oStaticValues;
            ...
            if (!bIncludeOnlyUOM) {
                ...
                if (oEntityTypeProperty && (oEntityTypeProperty.$Type === 'Edm.DateTime' || oEntityTypeProperty.$Type === 'Edm.DateTimeOffset')) {
                    ...
                    result = generatePathForField([oItem.Value.$Path ? oItem.Value.$Path : sProperty.split("/")[sProperty.split("/").length - 1]], functionName, oStaticValues);
                    ...
                    var iScale = (oItem.ValueFormat && oItem.ValueFormat.NumberOfFractionalDigits) ? oItem.ValueFormat.NumberOfFractionalDigits : 0, sUnitPath;
                    ...
                    if (oEntityTypeProperty["@Org.OData.Measures.V1.ISOCurrency"]) { //as part of supporting V4 annotation
                        sUnitPath = oEntityTypeProperty["@Org.OData.Measures.V1.ISOCurrency"].$Path ? oEntityTypeProperty["@Org.OData.Measures.V1.ISOCurrency"].$Path : oEntityTypeProperty["@Org.OData.Measures.V1.ISOCurrency"];
                    } else if (oEntityTypeProperty["@Org.OData.Measures.V1.Unit"]) {
                        sUnitPath = oEntityTypeProperty["@Org.OData.Measures.V1.Unit"].$Path ? oEntityTypeProperty["@Org.OData.Measures.V1.Unit"].$Path : oEntityTypeProperty["@Org.OData.Measures.V1.Unit"];
                    } else if (oEntityTypeProperty["@sap:unit"]) {
                        sUnitPath = oEntityTypeProperty["@sap:unit"];
                    }
                    var oUnitProperty = sUnitPath ? oMetaModel.getProperty("/" + oEntityType.$Type)[sUnitPath] : null,
                        aParts = [oItem.Value.$Path ? oItem.Value.$Path : sProperty.split("/")[sProperty.split("/").length - 1]];
                    ...
                    if (bUseSimplePath) {
                        ...
                        var sFormattedResult = OdataAnnotationHelper.format(oItem.Value, iContext.getModel() ? {context: iContext} : {context: iContext.getInterface(0)});
        ...
        function getEntityType(oMetaModel, oEntityType, sProperty) {
            ...
            var oEntitySetProperties = oMetaModel.getObject("/" + oEntityType.$Type.split(".")[1] + "/" + sProperty);
         ...
         * @returns {string}
         */
        function generatePathForField(aParts, sFormatterName, oStaticValues, isDecimal) {
            var sPath = "", iLength = aParts.length;
            if (iLength > 1 || oStaticValues) {
                sPath = "{parts:[";
                for (var i = 0; i < iLength; i++) {
                    if (isDecimal) {
                        sPath += ("{path: '" + aParts[i] + "',type:'sap.ui.model.odata.type.Decimal'}" + (((i === iLength - 1) && !oStaticValues) ? "]" : ", "));
                    } else {
        ...
        function formatNumberCalculation() {
            var oStaticValue = arguments[arguments.length - 1], value = arguments[0];
         ...
         */
        function formatDate() {
            var oStaticValues = arguments[arguments.length - 1], value = arguments[0];
        ...
        function formatCurrency() {
            var oStaticValues = arguments[arguments.length - 1], value = arguments[0], currency = arguments[1];
            var sFormattedValue = "";
            ...
            if (currency) {
                ...
                sFormattedValue = oStaticValues.currencyString ? oCurrencyFormatter.format(value, oStaticValues.currencyString) : oCurrencyFormatter.format(value);
            }
            ...
            if (oCriticalityProperty) {
                ...
                if (oCriticalityProperty.$Path) {
                    var sCriticalitySimplePath = '$'
                        + OdataAnnotationHelper.format(
                            oCriticalityProperty, {context: oInterface});
        ...
        function formatDataPointToValue(iContext, oDataPoint, oCriticalityConfigValues, isKpiHeader) {
            ...
            if (oDataPoint.Criticality) {
                ...
                sState = formThePathForCriticalityStateCalculation(iContext, oDataPoint, oCriticalityConfigValues, isKpiHeader);
            }

            return sState;
        }

        function formThePathForCriticalityStateCalculation(iContext, oDataPoint, oCriticalityConfigValues, isKpiHeader) {
            ...
            var deviationLow = getPathOrPrimitiveValue(oDataPoint.CriticalityCalculation.DeviationRangeLowValue, isKpiHeader);
            var deviationHigh = getPathOrPrimitiveValue(oDataPoint.CriticalityCalculation.DeviationRangeHighValue, isKpiHeader);
            var toleranceLow = getPathOrPrimitiveValue(oDataPoint.CriticalityCalculation.ToleranceRangeLowValue, isKpiHeader);
            var toleranceHigh = getPathOrPrimitiveValue(oDataPoint.CriticalityCalculation.ToleranceRangeHighValue, isKpiHeader);
        ...
        function formatColor () {
            ...
            return calculateCriticalityState(
                ...
                oStaticValues.bIsDeviationLowBinding ? parseFloat(arguments[index++].split(",").join("")) : oStaticValues.deviationLow,
                oStaticValues.bIsDeviationHighBinding ? parseFloat(arguments[index++].split(",").join("")) : oStaticValues.deviationHigh,
                oStaticValues.bIsToleranceLowBinding  ? parseFloat(arguments[index++].split(",").join("")) : oStaticValues.toleranceLow,
                oStaticValues.bIsToleranceHighBinding ? parseFloat(arguments[index++].split(",").join("")) : oStaticValues.toleranceHigh,
        ...
        function getSelectFields(iContext, oMetaModel, oEntityType, aAnnotationsPath) {
            ...
            for (var i = 0; i < aAnnotationsPath.length; i++) {
                ...
                sAnnotationPath = oEntityType.$Type.split(".")[1] + "/@" + aAnnotationsPath[i];
                oBindingContext = iContext.getModel(0);
                var oContext = {};
                ...
                aColl = oBindingContext.getObject("/" + sAnnotationPath);
                ...
                for (var j = 0; j < aColl.length; j++) {
                    ...
                    if (sRecordType === "com.sap.vocabularies.UI.v1.DataField") {
                        ...
                        sFormattedField = _formatField(oContext.getInterface(0), oItem, oEntityType, oMetaModel, undefined, undefined, true);
                    ...
                    if (sFormattedField) {
                        ...
                        var result = OdataAnnotationHelper.format(oItem.Value, {context: oContext.getInterface(0)} );
                    ...
                    if (aItemValue && aItemValue.length > 0) {
                        ...
                        for (var k = 0; k < aItemValue.length; k++) {
                            ...
                            if (!aSelectFields[sItemValue]) {
                                ...
                                oEntityTypeProperty[sItemValue] = getEntityType(oMetaModel, oEntityType, sItemValue);
                                var sUnitPropName = getUnitColumnV4(sItemValue, oEntityTypeProperty);
        ...
        function getPropertiesFromBindingString(sBinding) {
            ...
            if (sBinding.substring(0, 2) === "{=") {
                ...
                while (matchWithNoPath) {
                    ...
                    if (matchWithNoPath[0].indexOf("path") === -1 && matchWithNoPath[0].indexOf("@i18n") !== 0 && matchWithNoPath[0].indexOf("model: 'ovpCardProperties'") <= -1) {
        ...
        function getPathOrPrimitiveValue(oItem, isKpiHeader) {
            if (oItem) {
                if (oItem.$Path) {
                    return isKpiHeader ? "{path:'" + oItem.$Path + "Agg" + "', type:'sap.ui.model.odata.type.Decimal'}" : "{path:'" + oItem.$Path + "'}";
                } else {
        ...
        function formatUnit(iContext, aCollection, index) {
            ...
            var oEntityTypeProperty = oMetaModel && oMetaModel.getData()["$Annotations"][oEntityType.$Type + "/" +  oItem.Value.$Path];
        ...
        function formatKPIHeaderState(iContext, oDataPoint) {
            if (oDataPoint && oDataPoint.Criticality && oDataPoint.Criticality.$Path) {
                return "{parts: [{path:'" + oDataPoint.Criticality.$Path + "Agg', type:'sap.ui.model.odata.type.Decimal'}], formatter: 'sap.ovp.cards.v4.V4AnnotationHelper.kpiValueCriticality'}";
            } else {
        ...
        function semanticObjectOfDataField(iContext, oEntitySet, aCollection, index) {
            ...
            if (oEntityTypeProps) {
                ...
                if (semanticObjectOfDF) {
                    return OdataAnnotationHelper.format(semanticObjectOfDF, iContext.getModel() ? {context: iContext} : {context: iContext.getInterface(0)});
                } else {
                    return "";
                }
            }
        ...
        function isFirstDataPointPercentageUnit(iContext, aCollection) {
            ...
            if (oDataPoint && oDataPoint.Value && oDataPoint.Value.$Path) {
                ...
                if (oProperty && oProperty[oEntityName][oDataPoint.Value.$Path]["Org.OData.Measures.V1.Unit"]) {
        ...
        function _resolveTransactionalParameterizedEntitySet(oParameterInfo, oDataModel, oEntitySet, oSelectionVariant) {
            var sPath = "";
        ...
        function resolveAnalyticalParameterizedEntitySet(oDataModel, oEntitySet, oSelectionVariant, aParameters, oGlobalFilter) {
            var path = "";
        ...
        function resolveParameterizedEntitySet(oDataModel, oEntitySet, oSelectionVariant, aParameters, oGlobalFilter) {
            ...
            if (MetadataAnalyser.checkAnalyticalParameterisedEntitySet(oDataModel, oEntitySet.name)) {
                path = resolveAnalyticalParameterizedEntitySet(oDataModel, oEntitySet, oSelectionVariant, aParameters, oGlobalFilter);
            } else {
                ...
                if (oParameterInfo) {
                    ...
                    path = _resolveTransactionalParameterizedEntitySet(oParameterInfo, oDataModel, oEntitySet, oSelectionVariant);
                }else {
                    path = "/" + oEntitySet.name;
                }
            }
        ...
        function getContactAnnotation(aCollection, index) {
            var aContactFields = aCollection.filter(function (oItem) {
                if (oItem.$Type === "com.sap.vocabularies.UI.v1.DataFieldForAnnotation" && oItem.Target.$AnnotationPath.indexOf("@com.sap.vocabularies.Communication.v1.Contact") !== -1) {
                    return true;
                }
                return false;
            });
        ...
        function getAggregateNumber(iContext, oEntitySet, oDataPoint, oSelectionVariant) {
            ...
            if (bParams) {
                ...
                ret += "{path: '/" + oEntitySet.$Type.split(".")[1] + "'";
            }
            ...
            var oModel = iContext.getModel(0);
            var properties = oModel.getObject("/" + oEntitySet.$Type.split(".")[1] + "/");
            ...
            var aggregateParameter = getAggregateOfParameter(measure, annotations, oEntitySet.$Type);
            paramsArr.push("'" + measure + "Agg' : { 'name' : '" + measure + "', 'with' : '" + aggregateParameter + "'}");
            if (oDataPoint && oDataPoint.Criticality && oDataPoint.Criticality.$Path) {
                aggregateParameter = getAggregateOfParameter(oDataPoint.Criticality.$Path, annotations, oEntitySet.$Type);
                paramsArr.push("'" + oDataPoint.Criticality.$Path + "Agg' : { 'name' : '" + oDataPoint.Criticality.$Path + "', 'with' : '" + aggregateParameter + "'}");
            }
            ...
            if (oDataPoint && oDataPoint.CriticalityCalculation && oDataPoint.CriticalityCalculation.DeviationRangeLowValue && oDataPoint.CriticalityCalculation.DeviationRangeLowValue.$Path) {
                aggregateParameter = getAggregateOfParameter(oDataPoint.CriticalityCalculation.DeviationRangeLowValue.$Path, annotations, oEntitySet.$Type);
                paramsArr.push("'" + oDataPoint.CriticalityCalculation.DeviationRangeLowValue.$Path + "Agg' : { 'name' : '" + oDataPoint.CriticalityCalculation.DeviationRangeLowValue.$Path + "', 'with' : '" + aggregateParameter + "'}");
            }
            if (oDataPoint && oDataPoint.CriticalityCalculation && oDataPoint.CriticalityCalculation.ToleranceRangeLowValue && oDataPoint.CriticalityCalculation.ToleranceRangeLowValue.$Path) {
                var aggregateParameter = getAggregateOfParameter(oDataPoint.CriticalityCalculation.ToleranceRangeLowValue.$Path, annotations, oEntitySet.$Type);
                paramsArr.push("'" + oDataPoint.CriticalityCalculation.ToleranceRangeLowValue.$Path + "Agg' : { 'name' : '" + oDataPoint.CriticalityCalculation.ToleranceRangeLowValue.$Path + "', 'with' : '" + aggregateParameter + "'}");
            }
            ...
            if (oDataPoint && oDataPoint.CriticalityCalculation && oDataPoint.CriticalityCalculation.DeviationRangeHighValue && oDataPoint.CriticalityCalculation.DeviationRangeHighValue.$Path) {
                var aggregateParameter = getAggregateOfParameter(oDataPoint.CriticalityCalculation.DeviationRangeHighValue.$Path, annotations, oEntitySet.$Type);
                paramsArr.push("'" + oDataPoint.CriticalityCalculation.DeviationRangeHighValue.$Path + "Agg' : { 'name' : '" + oDataPoint.CriticalityCalculation.DeviationRangeHighValue.$Path + "', 'with' : '" + aggregateParameter + "'}");
            }
            if (oDataPoint && oDataPoint.CriticalityCalculation && oDataPoint.CriticalityCalculation.ToleranceRangeHighValue && oDataPoint.CriticalityCalculation.ToleranceRangeHighValue.$Path) {
                var aggregateParameter = getAggregateOfParameter(oDataPoint.CriticalityCalculation.ToleranceRangeHighValue.$Path, annotations, oEntitySet.$Type);
                paramsArr.push("'" + oDataPoint.CriticalityCalculation.ToleranceRangeHighValue.$Path + "Agg' : { 'name' : '" + oDataPoint.CriticalityCalculation.ToleranceRangeHighValue.$Path + "', 'with' : '" + aggregateParameter + "'}");
            }
            ...
            if (oDataPoint && oDataPoint.TrendCalculation && oDataPoint.TrendCalculation.ReferenceValue && oDataPoint.TrendCalculation.ReferenceValue.$Path) {
                var aggregateParameter = getAggregateOfParameter(oDataPoint.TrendCalculation.ReferenceValue.$Path, annotations, oEntitySet.$Type);
                paramsArr.push("'" + oDataPoint.TrendCalculation.ReferenceValue.$Path + "Agg' : { 'name' : '" + oDataPoint.TrendCalculation.ReferenceValue.$Path + "', 'with' : '" + aggregateParameter + "'}");
            }
            ...
            var parameters = "parameters:{'$$aggregation' : {'aggregate' : {" + paramsArr.join(",") + "}}" + sCustomParameter + "}";
        ...
        function getUnitColumnV4(measure, properties, forSubtitleUOM) {
            var tempUnit;
            var sPath, sString, obj;
            for (obj in properties) {
                if (obj !== measure) {
                    continue;
                }
                if (properties[obj].annotations && properties[obj].annotations.hasOwnProperty("@Org.OData.Measures.V1.ISOCurrency")) { //as part of supporting V4 annotation
                    sPath = properties[obj].annotations["@Org.OData.Measures.V1.ISOCurrency"].$Path;
                    sString = properties[obj].annotations["@Org.OData.Measures.V1.ISOCurrency"].$String;
                    if (sPath) {
                        return sPath;
                    } else {
                ...
                if (properties[obj].annotations && properties[obj].annotations.hasOwnProperty("@Org.OData.Measures.V1.Unit")) {
                    sPath = properties[obj].annotations.hasOwnProperty("@Org.OData.Measures.V1.Unit").$Path;
                    sString = properties[obj].annotations["@Org.OData.Measures.V1.Unit"].$String;
                    if (sPath) {
                        tempUnit = sPath;
                    } else {
                ...
                if (properties[obj].annotations && properties[obj].annotations.hasOwnProperty("sap:unit")) {
                    return properties[obj].annotations["sap:unit"];
                }
        ...
        function formThePathForAggregateNumber(dataPoint) {
            ...
            if (dataPoint && dataPoint.ValueFormat && dataPoint.ValueFormat.NumberOfFractionalDigits) {
                this.getModel("ovpCardProperties").oData.NumberOfFractionalDigits = dataPoint.ValueFormat.NumberOfFractionalDigits;
            }
            var oStaticValues = {
                NumberOfFractionalDigits: this.getModel("ovpCardProperties").oData.NumberOfFractionalDigits ? this.getModel("ovpCardProperties").oData.NumberOfFractionalDigits : 0,
                percentageAvailable: this.getModel("ovpCardProperties").oData.percentageAvailable ? this.getModel("ovpCardProperties").oData.percentageAvailable : false
            };
        ...
        function KpiValueFormatter () {
            var oStaticValue = arguments[arguments.length - 1], kpiValue = arguments[0];
            //argument[0] may be recieved as null or a string("2,345") so convert to decimal
            kpiValue = kpiValue ? parseInt(kpiValue.split(",").join(""), 10) : null;
        ...
        function formPathForPercentageChange(dataPoint) {
            ...
            if (dataPoint.TrendCalculation.ReferenceValue.$Path) {
                return "{parts: [{path:'" + dataPoint.Value.$Path + "Agg', type:'sap.ui.model.odata.type.Decimal'}, {path:'" + dataPoint.TrendCalculation.ReferenceValue.$Path +
                    "'}], formatter: 'sap.ovp.cards.v4.V4AnnotationHelper.returnPercentageChange'}";
            } else {
                return "{parts: [{path:'" + dataPoint.Value.$Path + "Agg', type:'sap.ui.model.odata.type.Decimal'}], formatter: 'sap.ovp.cards.v4.V4AnnotationHelper.returnPercentageChange'}";
            }
        }
        ...
        function formPathForTargetValue(oDataPointAnnotation) {
            ...
            var oEntityType = oModel.getProperty("/entityType");
            var oMetaModel = oModel.getProperty("/metaModel");
            if (oDataPointAnnotation && oDataPointAnnotation.ValueFormat && oDataPointAnnotation.ValueFormat.NumberOfFractionalDigits) {
                oModel.setProperty("/iNumberOfFractionalDigits", oDataPointAnnotation.ValueFormat.NumberOfFractionalDigits);
            }
            if (oDataPointAnnotation.TrendCalculation.ReferenceValue) {
                if (oDataPointAnnotation.TrendCalculation.ReferenceValue.$Path) {
                    var oEntityTypeProperty = getEntityType(oMetaModel, oEntityType, oDataPointAnnotation.TrendCalculation.ReferenceValue.$Path);
                    if (oEntityTypeProperty["Org.OData.Measures.V1.Unit"]) {
                        var oUnit = oEntityTypeProperty["Org.OData.Measures.V1.Unit"];
                        if (oUnit.String == "%") {
                    ...
                    return "{parts: [{path:'" + oDataPointAnnotation.Value.$Path + "Agg', type:'sap.ui.model.odata.type.Decimal'},{path:'" + oDataPointAnnotation.TrendCalculation.ReferenceValue.$Path + "Agg', type:'sap.ui.model.odata.type.Decimal'}], formatter: 'sap.ovp.cards.v4.V4AnnotationHelper.TargetValueFormatter'}";
                }
                if (oDataPointAnnotation.TrendCalculation.ReferenceValue) {
                    oModel.setProperty("/sManifestTargetValue", oDataPointAnnotation.TrendCalculation.ReferenceValue);
                    return "{parts: [{path:'" + oDataPointAnnotation.Value.$Path + "Agg', type:'sap.ui.model.odata.type.Decimal'}], formatter: 'sap.ovp.cards.v4.V4AnnotationHelper.TargetValueFormatter'}";
                }
            }
        }
        ...
        function isPresentationVarientPresent(oPresentationVariant) {
            if (oPresentationVariant && oPresentationVariant.GroupBy && !(isEmptyObject(oPresentationVariant.GroupBy)) && oPresentationVariant.GroupBy[0].hasOwnProperty('$PropertyPath')) {
                return true;
            } else {
                return false;
            }
        ...
        function getAttributeValueForEntityProperty(oMetaModel, oEntityType, sPropertyName, sAttributeName) {
            ...
            if (!oProp) {
                oLogger.error("No Property Found for with Name '" + sPropertyName + " For Entity-Type '" + oEntityType.name + "'");
                return;
            }
