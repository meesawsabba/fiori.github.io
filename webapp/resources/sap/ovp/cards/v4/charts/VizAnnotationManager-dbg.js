 ...
 */
sap.ui.define(["sap/ui/thirdparty/jquery", "sap/ovp/cards/v4/charts/Utils", "sap/ui/model/odata/CountMode",
        "sap/ovp/cards/v4/V4AnnotationHelper", "sap/ui/core/format/NumberFormat", "sap/ui/core/format/DateFormat",
    ...
    "sap/ovp/app/OVPUtils", "sap/base/util/each", "sap/base/util/merge", "sap/ovp/cards/ovpLogger", "sap/base/util/isEmptyObject"],
    function (jQuery, Utils, CountMode, CardAnnotationHelper, NumberFormat, DateFormat,
              ...
              MeasureDefinition, FeedItem, CoreElement, OvpResources, OVPUtils, each, merge, ovpLogger, isEmptyObject) {
        "use strict";
        ...
        var aSupportedChartTypesForStableColors = [
            constants.COLUMNSTACKED_CHARTTYPE
        ];
        ...
        function isV4TimeSeriesSemantics(oDataModel, sEntitySet, dimensionName) {
            ...
            return semantics["@com.sap.vocabularies.Common.v1.IsCalendarYear"] || semantics["@com.sap.vocabularies.Common.v1.IsCalendarYearMonth"] ||
            semantics["@com.sap.vocabularies.Common.v1.IsCalendarYearQuarter"] || semantics["@com.sap.vocabularies.Common.v1.IsCalendarYearWeek"];
        }
        /*
        ...
        function formatItems(iContext, oEntitySet, oSelectionVariant, oPresentationVariant, oDimensions, oMeasures, chartType) {
            ...
            each(oDimensions, function (i, d) {
                ...
                if (oMetadata && oMetadata[tmp]) {
                    if (oMetadata[tmp]["annotations"] && oMetadata[tmp]["annotations"]["@" + textKeyV4]) { //as part of supporting V4 annotation
                        if (typeof oMetadata[tmp]["annotations"]["@" + textKeyV4] === 'string' && oMetadata[tmp]["annotations"]["@" + textKeyV4] && tmp != oMetadata[tmp]["annotations"]["@" + textKeyV4]) {
                            dimensionsList.push(oMetadata[tmp]["annotations"]["@" + textKeyV4]);
                        } else if (oMetadata[tmp]["annotations"]["@" + textKeyV4].$Path && tmp != oMetadata[tmp]["annotations"]["@" + textKeyV4].$Path) {
            ...
            ret += ", parameters: {$apply:'groupby((" + dimensionsList.join(",") + "),";
            ret += "aggregate(";
...
//										showScale: false,
        ...
        function checkExists(term, annotation, type, bMandatory, logViewId, contentFragment) {
            ...
            if (contentFragment &&
                ...
                (!annoTerm.ChartType || !annoTerm.ChartType.$EnumMember)) {
                 oLogger.error(logViewId + errorMessages.CARD_ERROR + errorMessages.MISSING_CHARTTYPE + errorMessages.CHART_ANNO);
                return ret;
            }
            ret = true;
            return ret;
        ...
        function validateCardConfiguration(oController) {
            ...
            if (!(oCardsModel = oController.getCardPropertiesModel())) {
                 oLogger.error(logViewId + errorMessages.CARD_ERROR + "in " + errorMessages.CARD_CONFIG + errorMessages.NO_CARD_MODEL);
                return ret;
            }
        ...
        function checkRolesForProperty(queue, config, type) {
            ...
            allowedRoles = jQuery.grep(allowedRoles, function (role, i) {
             ...
             oLogger.error(errorMessages.CARD_ERROR + errorMessages.INVALID_REDUNDANT +
        ...
        function buildVizAttributes(vizFrame, handler, chartTitle, oController) {
            var oCardsModel, entityTypeObject, chartAnno, chartContext, bShowDataLabel;
            var chartType, allConfig, config, aDimensions, aMeasures, bShowDataLabelCardsAsApi = false;
            var oVizProperties;
            ...
            if (!config) {
                 oLogger.error(errorMessages.CARD_ERROR + "in " + errorMessages.CARD_CONFIG +
            ...
            if (!(aMeasures = chartContext.MeasureAttributes) || !aMeasures.length) {
                ...
                var dpNumberOfFractionalDigits = datapointAnnotationPath && datapointAnnotationPath.ValueFormat && datapointAnnotationPath.ValueFormat.NumberOfFractionalDigits && datapointAnnotationPath.ValueFormat.NumberOfFractionalDigits;
...
//		}
            ...
            if (!bEnableStableColors && aSupportedChartTypesForSemanticColors.indexOf(chartContext.ChartType.$EnumMember) >= 0 &&
                ...
                if (colorPaletteDimension) {
                    if (oMetadata && oMetadata[colorPaletteDimension]) {
                        if (oMetadata[colorPaletteDimension][constants.LABEL_KEY_V4]) { //as part of supporting V4 annotation
                            feedName = getLabelFromAnnotationPath(oMetadata[colorPaletteDimension][constants.LABEL_KEY_V4] ? oMetadata[colorPaletteDimension][constants.LABEL_KEY_V4] : oMetadata[colorPaletteDimension][constants.LABEL_KEY_V4].Path, vizFrame, oMetadata);
                ...
                if (bIsMinMax) {
                    ...
                    if (chartContext["MeasureAttributes"][0] &&
                        ...
                        if (entityTypeObject && entityTypeObject[sDataPointPath]) {
                            ...
                            if (oMinMaxParams && oMinMaxParams.MaximumValue && oMinMaxParams.MaximumValue &&
                                oMinMaxParams.MinimumValue && oMinMaxParams.MinimumValue) {
                                aChartScales.push({
                                    feed: "valueAxis",
...
//		var minFractionDigits = Number(dataContext.NumberFormat.NumberOfFractionalDigits.Int);
            ...
            */
            if ((chartType === "column" || chartType === "bar" || chartType === "vertical_bullet") && config.feeds && (config.feeds.length > 0)) {
                ...
                if (feed.type) {
                    ...
                    if (!feed.role) {
                        ...
                        if (aFeedProperties.length < min) {
                             oLogger.error(errorMessages.CARD_ERROR + errorMessages.MIN_FEEDS + chartType +
                        ...
                        each(aFeedProperties, function (i, val) {
                            ...
                            if (oMetadata && oMetadata[property]) {
                                if (oMetadata[property]["annotations"]) {
                                    if (oMetadata[property]["annotations"]["@" + constants.LABEL_KEY_V4]) { //as part of supporting V4 annotation
                                        if (oMetadata[property]["annotations"]["@" + constants.LABEL_KEY_V4] instanceof String && oMetadata[property]["annotations"]["@" + constants.LABEL_KEY_V4]) {
                                            feedName = getLabelFromAnnotationPath(oMetadata[property]["annotations"]["@" + constants.LABEL_KEY_V4], vizFrame, oMetadata);
                                        } else if (oMetadata[property]["annotations"]["@" + constants.LABEL_KEY_V4].$Path) {
                                            feedName = oMetadata[property]["annotations"]["@" + constants.LABEL_KEY_V4].$Path;
                                        } else {
                                    ...
                                    if (oMetadata[property]["annotations"]["@" + constants.TEXT_KEY_V4]) { //as part of supporting V4 annotation
                                        textColumn = oMetadata[property]["annotations"]["@" + constants.TEXT_KEY_V4] ? oMetadata[property]["annotations"]["@" + constants.TEXT_KEY_V4] : oMetadata[property]["annotations"]["@" + constants.TEXT_KEY_V4].$Path;
                            ...
                            if ((edmType === "Edm.DateTime" || edmType === "Edm.DateTimeOffset") && textColumn === property) {
                                displayBindingPath = "{path:'" + property + "', formatter: 'sap.ovp.cards.v4.charts.VizAnnotationManager.returnDateFormat'}";
                            } else {
                                //Check if there's a text arrangement annotation
                                if (oMetadata[property] && oMetadata[property]["annotations"] && oMetadata[property]["annotations"]["@" + constants.TEXT_KEY_V4] && oMetadata[property]["annotations"]["@" + constants.TEXT_KEY_V4 + "@" + constants.TEXT_ARRANGEMENT_ANNO]) {
                                    ...
                                    oTextArrangement = oMetadata[property]["annotations"]["@" + constants.TEXT_KEY_V4 + "@" + constants.TEXT_ARRANGEMENT_ANNO];
                                    ...
                                    switch (sTextArrangementType) {
                                        ...
                                        case "com.sap.vocabularies.UI.v1.TextArrangementType/TextFirst" :
                                            displayBindingPath = "{" + textColumn.$Path + "}" + " " + "(" + "{" + property + "}" + ")";
                                            break;
                                        case "com.sap.vocabularies.UI.v1.TextArrangementType/TextLast" :
                                            displayBindingPath = "{" + property + "}" + " " + "(" + "{" + textColumn.$Path + "}" + ")";
                                            break;
                                        case "com.sap.vocabularies.UI.v1.TextArrangementType/TextOnly" :
                                            displayBindingPath = "{" + textColumn.$Path + "}";
                                            break;
                            ...
                            if (feedtype === "Dimension") {
                                ...
                                var isV4CalendarYear = isV4TimeSeriesSemantics(vizFrame.getModel(), entitySet, property);//as part of supporting V4 annotation
            ...
            if (oColorPalette && oColorPalette.dimensionSettings && Object.keys(oColorPalette.dimensionSettings).length > 0) {
            ...
            if (bEnableStableColors && aSupportedChartTypesForStableColors.indexOf(chartContext.ChartType.$EnumMember) >= 0 && !Boolean(bColorRestriction)) {
                ...
                for (var i = 0; i < aChartDimensions.length; i++) {
                    ...
                    if (aDimensionPaths.indexOf(sDimensionPath) > -1) {
                        ...
                        bStableColoringByDimensionValue = bStableColoringByDimensionValue && aColorRules.every(_fnCheckColorNotDefined);
                    }
                }
            ...
            if (chartType === "donut" || bShowDataLabel) {
                if (chartType === "donut") {
                    oVizProperties.legendGroup = {layout : {height : 0.1}}; //to remove the whitespace between legends and donut
                }
            ...
            if (bSupportsTimeSemantics){
                ...
                var semantics = vizFrame.getModel().getMetaModel().getObject("/" + entitySet + "/" + timeDimension + "@");
        ...
        function setChartUoMTitle(vizFrame, vizData, chartTitle, entityType) {
            ...
            if (!(aMeasures = chartContext.MeasureAttributes) || !aMeasures.length) {
                 oLogger.error(errorMessages.CHART_ANNO_ERROR + "in " + errorMessages.CHART_ANNO + " " +
        ...
        function getSelectedDataPoint(vizFrame, controller) {
            ...
            vizFrame.attachSelectData(function (oEvent) {
                ...
                var allData = vizFrame.getDataset().getBinding("data").getCurrentContexts().map(function (x) {
                    return x.getObject();
                });
                                ...
                                if (dimensionArrayNames[j] === dimensions[k]) {
                                    for (var key in oMetadata) {
                                        if (annotations[oEntityType.$Type + "/" + key]) {
                                            var propertyName = (typeof annotations[oEntityType.$Type + "/" + key]["@" + constants.LABEL_KEY_V4] === 'string' && annotations[oEntityType.$Type + "/" + key]["@" + constants.LABEL_KEY_V4]) || annotations[oEntityType.$Type + "/" + key][constants.LABEL_KEY] || annotations[oEntityType.$Type + "/" + key][constants.NAME_KEY] || annotations[oEntityType.$Type + "/" + key][constants.NAME_CAP_KEY] || key;
