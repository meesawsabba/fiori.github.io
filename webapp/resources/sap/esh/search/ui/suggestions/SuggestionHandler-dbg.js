...
sap.ui.define([
    ...
    "sap/esh/search/ui/suggestions/SinaSuggestionProvider",
    "sap/esh/search/ui/suggestions/AppSuggestionProvider",
    "sap/esh/search/ui/suggestions/TimeMerger",
 ...
 * @param {*} AppSuggestionProvider
 * @param {*} TimeMerger
 * @param {*} SuggestionType
 */
function (i18n, SearchHelper, SinaSuggestionProvider, AppSuggestionProvider, TimeMerger, SuggestionType) {
    "use strict";
    var suggestions = {};
    // =======================================================================
    // suggestions handler
    ...
    suggestions.SuggestionHandler.prototype = {
        ...
        getSuggestionProviders: function () {
            // check cache
            var that = this;
            if (that.suggestionProvidersPromise) {
                return that.suggestionProvidersPromise;
            }
            that.suggestionProvidersPromise = that.model.initBusinessObjSearch().then(function () {
                // link to sina
                ...
                suggestionProviders = [];
                if (that.model.config.isUshell) {
                    suggestionProviders.push(that.appSuggestionProvider);
                }
        ...
        doSuggestionInternal: function (filter) {
            ...
            that.getSuggestionProviders().then(function (suggestionProviders) {
            ...
            switch (insertSuggestion.uiSuggestionType) {
                ...
                case SuggestionType.Object:
                    ...
                    if (this.model.config.FF_bOptimizedQuickSelectDataSourceLabels) {
                        if (this.model.getDataSource().subType ===
                            this.sinaNext.DataSourceSubType.Filtered) {
                            ...
                            if (typeof this.model.config.getFirstSpaceCondition === "function") {
                                ...
                                var firstSpaceCondition = this.model.config.getFirstSpaceCondition(this.model.getProperty("/uiFilter"));
        ...
        insertIntoSuggestionList: function (insertSuggestions, suggestions) {
            ...
            for (var i = 0; i < insertSuggestions.length; ++i) {
                ...
                if (this.isHeaderGenerationEnabled() &&
                    !this.suggestionHeaders[insertSuggestion.position]) {
