...
], function (i18n, SearchModel, Toolbar, ToolbarDesign, Button, Link, ToolbarLayoutData, ToolbarSpacer, ActionSheet, PlacementType, InvisibleText, IconPool, ItemNavigation) {
    "use strict";
    var module = sap.ui.core.Control.extend("sap.esh.search.ui.controls.SearchRelatedObjectsToolbar", {
        metadata: {
            properties: {
                itemId: "string",
                navigationObjects: {
        ...
        init: function () {
            ...
            sap.esh.search.ui.controls.SearchRelatedObjectsToolbar._allOfMyCurrentInstances.push(that);
            that.setAggregation("_relatedObjectActionsToolbar", new Toolbar({
            ...
            that.setAggregation("_ariaDescriptionForLinks", new InvisibleText({
        ...
        exit: function () {
            ...
            var allOfMyCurrentInstances = sap.esh.search.ui.controls.SearchRelatedObjectsToolbar._allOfMyCurrentInstances;
            ...
            if (that.searchLayoutChangedIsSubscribed) {
                that.searchLayoutChangedIsSubscribed = false;
                that.getModel().unsubscribe("searchLayoutChanged", that._layoutToolbarElements, that);
            }
        },
        ...
        _renderToolbar: function (oRm) {
            ...
            if (navigationObjects.length > 0) {
                ...
                that._overFlowButton = new Button({
                ...
                that._overFlowSheet = new ActionSheet({
        ...
        onAfterRendering: function () {
            var that = this;
            if (that.getAggregation("_relatedObjectActionsToolbar")) {
                that._layoutToolbarElements();
                that._addAriaInformation();
            }
            if (!that.searchLayoutChangedIsSubscribed) {
                that.getModel().subscribe("searchLayoutChanged", that._layoutToolbarElements, that);
                that.searchLayoutChangedIsSubscribed = true;
            }
        },
        ...
        _setupItemNavigation: function () {
            ...
            if (!that.theItemNavigation) {
                that.theItemNavigation = new ItemNavigation();
