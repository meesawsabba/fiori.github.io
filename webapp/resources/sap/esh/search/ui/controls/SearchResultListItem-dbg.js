...
sap.ui.define([
    ...
    "sap/esh/search/ui/controls/twitter/SearchTweet",
    "sap/esh/search/ui/SearchHelper",
    "sap/esh/search/ui/controls/SearchRelatedObjectsToolbar",
    "sap/m/Button",
    "sap/m/ButtonType",
    "sap/m/Label",
    "sap/m/Text",
...
], function (i18n, SearchText, SearchLink, SearchTweet, SearchHelper, SearchRelatedObjectsToolbar, Button, ButtonType, Label, Text, CheckBox, ListType, Icon, IconPool, InvisibleText) {
    "use strict";
    var noValue = "\u2013"; // dash
    return sap.ui.core.Control.extend("sap.esh.search.ui.controls.SearchResultListItem", {
        // the control API:
        metadata: {
            properties: {
                dataSource: "object",
            ...
            aggregations: {
                ...
                _multiLineDescriptionTweet: {
                    type: "sap.esh.search.ui.controls.twitter.SearchTweet",
                    multiple: false,
                    visibility: "hidden",
                },
                _selectionCheckBox: {
        ...
        init: function () {
            ...
            that.setAggregation("_typeText", new SearchText()
                ...
                .addStyleClass("sapUshellSearchResultListItem-MightOverflow"));
            that.setAggregation("_typeLinkAriaDescription", new InvisibleText({
                text: i18n.getText("result_list_item_type_link_description"),
            }));
            that.setAggregation("_typeLink", new SearchLink({
            ...
            // maxLines: 5 // property not supported by SearchTweet control
                ...
                .addStyleClass("sapUshellSearchResultListItem-MightOverflow")
                .data("islongtext", "true", true));
            that.setAggregation("_selectionCheckBox", new CheckBox({
            ...
            that.setAggregation("_expandButton", new Button({
                type: ButtonType.Transparent,
                press: function () {
                    that.toggleDetails();
                },
            }));
            that.setAggregation("_relatedObjectActionsToolbar", new SearchRelatedObjectsToolbar().addStyleClass("sapUshellSearchResultListItem-RelatedObjectsToolbar"));
            that.setAggregation("_titleLabeledByText", new InvisibleText());
            that.setAggregation("_attributesLabeledByText", new InvisibleText());
            that.setAggregation("_expandStateLabeledByText", new InvisibleText());
        },
        ...
        _renderTitleContainer: function (oRm) {
            ...
            if (that.getTitleIconUrl && that.getTitleIconUrl()) {
                var oIcon = new Icon({
                    src: that.getTitleIconUrl(),
                });
                titleLink.setAggregation("icon", oIcon);
            }
            oRm.renderControl(titleLink);
            if (that.getTitleInfoIconUrl()) {
                var oInfoIcon = new Icon({
                    src: that.getTitleInfoIconUrl(),
                }).addStyleClass("sapUshellSearchResultListItem-TitleInfoIcon");
            ...
            /// Object Type
            if (!that.getModel().config._eshClickableObjectType ||
                that.getModel().getDataSource() === that.getDataSource()) {
                ...
                var href = that
                    .getModel()
                    .searchUrlParser.renderFromParameters(this.getModel().boTopDefault, uiFilterClone, true);
                var typeLink = that.getAggregation("_typeLink");
                typeLink.setText(that.getType());
                typeLink.setHref(href);
                typeLink.setTooltip(i18n.getText("searchInDataSourceTooltip", [that.getDataSource().labelPlural]));
        ...
        _renderAllAttributes: function (oRm, itemAttributes) {
            ...
            var i = 0, k, numberOfRenderedAttributes = 0;
            ...
            for (; !(additionalWhyFoundAttributesDesktop <= 0 && additionalWhyFoundAttributesTablet <= 0) &&
                i < itemAttributes.length; i++) {
                ...
                if (itemAttribute.longtext === undefined ||
                ...
                var _rowCountTablet = -1, _rowCountDesktop = -1, _attributeWeight = {
                    desktop: 1,
                    tablet: 1,
                };
                ...
                if (isLongText) {
                    longTextColumnNumber =
                        attributeLayout.longTextColumnNumber ||
                            that._howManyColumnsToUseForLongTextAttribute(valueWithoutWhyfound);
                ...
                if (isLongText && includeImageAttribute && distributionOfAttributesDesktop[0] === 1) {
                    ...
                    for (k = 0; k < distributionOfAttributesDesktop.length; k++) {
                        if (distributionOfAttributesDesktop[k] + _attributeWeight.desktop <=
                ...
                if (isLongText && includeImageAttribute && distributionOfAttributesTablet[0] === 1) {
                    ...
                    for (k = 0; k < distributionOfAttributesTablet.length; k++) {
                        if (distributionOfAttributesTablet[k] + _attributeWeight.tablet <=
                ...
                oRm.write('"');
                if (isLongText) {
                    oRm.write(' data-sap-searchresultitem-attributeweight-desktop="' +
                        _attributeWeight.desktop +
                        '"');
                    oRm.write(' data-sap-searchresultitem-attributeweight-tablet="' +
                        _attributeWeight.tablet +
                        '"');
                }
                oRm.write(">");
                label = new Label({
            ...
            if (includeImageAttribute) {
                ...
                if (availableSpaceOnFirstLineDesktop > 0 || availableSpaceOnFirstLineTablet > 0) {
                    ...
                    oRm.write(' data-sap-searchresultitem-attributeweight-desktop="' +
                        ...
                        '"');
                    oRm.write(' data-sap-searchresultitem-attributeweight-tablet="' +
                        availableSpaceOnFirstLineTablet +
                        '"');
                    oRm.write("></li>");
                }
            }
        ...
        _renderImageAttribute: function (oRm, imageIsOnlyAttribute) {
            var that = this;
            if (!that.getImageUrl() ||
                (that.getImageFormat() && that.getImageFormat().toLowerCase() === "documentthumbnail")) {
                return;
            }
        ...
        _renderAccessibilityInformation: function (oRm) {
            ...
            if (parentListItem) {
                that._renderAriaDescriptionElementForTitle(oRm, 
                /* withPrefix = */ true, 
                /* doRender= */ true);
                that._renderAriaDescriptionElementForAttributes(oRm, 
                /* withPrefix = */ /* doRender= */ true);
                that._renderAriaDescriptionElementForCollapsedOrExpandedState(oRm, 
                /* withPrefix = */ /* doRender= */ true);
                parentListItem.addEventDelegate({
                    onAfterRendering: function () {
                        var $parentListItem = $(parentListItem.getDomRef());
                        that._addAriaDescriptionToParentListElement(parentListItem, 
                        /* includeTotalCountElement = */ true);
                        $parentListItem.on("focusin", function (event) {
                            var $relatedTarget = $(event.relatedTarget);
                            if ($relatedTarget.hasClass("sapUshellSearchResultListItem") ||
                                ($relatedTarget.closest(".sapUshellSearchResultListItemApps").length >
                                    0 &&
                                    !$relatedTarget.hasClass("sapUshellResultListMoreFooter"))) {
                                that._renderAriaDescriptionElementForTitle(oRm, 
                                /* withPrefix = */ false, 
                                /* doRender= */ false);
                                that._renderAriaDescriptionElementForAttributes(oRm, 
        ...
        getAccessibilityInfo: function () {
            ...
            if (sap.ui.core.Control.prototype.getAccessibilityInfo) {
                accInfo =
                    sap.ui.core.Control.prototype.getAccessibilityInfo.apply(this, arguments) || accInfo;
            }
        ...
        _renderAriaDescriptionElementForTitle: function (oRm, withPrefix, doRender) {
            var that = this;
            that._searchResultListPrefix =
                that._searchResultListPrefix || i18n.getText("result_list_announcement_screenreaders");
        ...
        _renderAriaDescriptionElementForAttributes: function (oRm, doRender) {
            ...
            var $attributes = $(that.getDomRef())
                .find(".sapUshellSearchResultListItem-Attributes")
                .find(".sapUshellSearchResultListItem-MainAttribute");
            var labelledByText;
            if ($attributes.length === 0) {
                ...
                $attributes.each(function () {
                    ...
                    if ($this.is(":visible") && $this.attr("aria-hidden") !== "true") {
                        var attributeKey = $this
                            .find(".sapUshellSearchResultListItem-AttributeKey")
                            .text();
        ...
        _getExpandAreaObjectInfo: function () {
            ...
            var expandedHeight = resultListItem
                .find(".sapUshellSearchResultListItem-AttributesAndActions")
                .height();
        ...
        showDetails: function () {
            ...
            var animation01 = expandAreaObjectInfo.attributesExpandContainer
                .animate({
                ...
                progress: function (animation, progress, remainingMs) {
                    if (!secondAnimationStarted && progress > 0.5) {
                        animation02 = expandAreaObjectInfo.relatedObjectsToolbar
                            .animate({
                            opacity: 1,
                        }, remainingMs)
                            .promise();
        ...
        hideDetails: function () {
            ...
            var attributeHeight = resultListItem.find(".sapUshellSearchResultListItem-MainAttribute").outerHeight(true) +
                resultListItem
                    .find(".sapUshellSearchResultListItem-ExpandSpacerAttribute")
                    .outerHeight(true);
            var secondAnimationStarted = false;
            var deferredAnimation01 = expandAreaObjectInfo.attributesExpandContainer
                .animate({
                ...
                progress: function (animation, progress, remainingMs) {
                    if (!secondAnimationStarted &&
                        remainingMs <= expandAreaObjectInfo.fadeInOrOutAnimationDuration) {
                        secondAnimationStarted = true;
                        var deferredAnimation02 = $(expandAreaObjectInfo.elementsToFadeInOrOut)
                            .animate({
                            opacity: 0,
                        }, expandAreaObjectInfo.fadeInOrOutAnimationDuration)
                            .promise();
        ...
        onAfterRendering: function () {
            ...
            that._renderAriaDescriptionElementForTitle(undefined, 
            /* withPrefix = */ /* doRender= */ false);
            that._renderAriaDescriptionElementForAttributes(undefined, 
            /* withPrefix = */ /* doRender= */ false);
            that._renderAriaDescriptionElementForCollapsedOrExpandedState(undefined, 
        ...
        setAriaExpandedState: function () {
            ...
            var $parentListItem = that.getParentListItem()
                ? $(that.getParentListItem().getDomRef())
                : $that.closest("li");
