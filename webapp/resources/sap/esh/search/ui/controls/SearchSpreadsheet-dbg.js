...
sap.ui.define([
    ...
    "sap/m/MessageBox",
], function (i18n, Controller, Spreadsheet, SearchResultListFormatter, MessageBox) {
    "use strict";
    return Controller.extend("sap.ui.export.sample.table.Spreadsheet", {
        onExport: function () {
            ...
            if (that.table === undefined) {
                ...
                that.table = sap.ui.getCore().byId(tableId);
                that.model = that.table.getModel();
            }
            var dataExportButtonId = document.querySelectorAll('[id$="-dataExportButton"]')[0].id;
            sap.ui.getCore().byId(dataExportButtonId).setEnabled(false); // deactivate download button
            // search query
            ...
            var successHandler = function (searchResultSet) {
                ...
                var formatter = new SearchResultListFormatter(that.model);
            ...
            var errorHandler = function (error) {
                ...
                var dataExportButtonId = document.querySelectorAll('[id$="-dataExportButton"]')[0].id;
                sap.ui.getCore().byId(dataExportButtonId).setEnabled(true); // activate download button, when search fails
                ...
                MessageBox.information(i18n.getText("exportDataInfo"), {
                    ...
                    onClose: function (oAction) {
                        ...
                        if (oAction == MessageBox.Action.CANCEL) {
                            var dataExportButtonId = document.querySelectorAll('[id$="-dataExportButton"]')[0].id;
                            sap.ui.getCore().byId(dataExportButtonId).setEnabled(true); // activate download button, when download is canceled
                            that.model.setProperty("/isBusy", false);
                        }
                    },
                });
        ...
        _parseColumns: function (firstSearchResult) {
            ...
            titleDescriptionLabel = titleDescriptionLabel + "(" + i18n.getText("titleDescription") + ")";
            ...
            for (i = 0; i < firstSearchResult.detailAttributes.length; i++) {
                // exclude image url
                if (firstSearchResult.detailAttributes[i].metadata.type !==
                    that.model.sinaNext.AttributeType.ImageUrl) {
        ...
        _getRelatedValues: function (row, attribute) {
            var that = this;
            // single attribute
            if (attribute.value !== undefined && attribute.valueFormatted !== undefined) {
                // value of current attrinute
                if (that._isNumberType(attribute)) {
                    // use value for number attribute
                    row[attribute.id] = attribute.value;
                }
                else {
                    // use valueFormatted for other type attribute
                    row[attribute.id] = attribute.valueFormatted;
                }
            ...
            if (attribute.attributes) {
                for (var i = 0; i < attribute.attributes.length; i++) {
                    ...
                    if (attributeTemp &&
                        attributeTemp.id &&
                        attributeTemp.value !== undefined &&
                        attributeTemp.valueFormatted !== undefined) {
                        if (that._isNumberType(attributeTemp)) {
                            // use value for number child-attribute
                            row[attributeTemp.id] = attributeTemp.value;
                        }
                        else {
                            // use valueFormatted for other type child-attribute
        ...
        _isNumberType: function (attribute) {
            var isNumber = false;
            if (attribute.metadata && attribute.metadata.type) {
                if (attribute.metadata.type === this.model.sinaNext.AttributeType.Double ||
                    attribute.metadata.type === this.model.sinaNext.AttributeType.Integer) {
                    isNumber = true;
                }
            }
            return isNumber;
        ...
        _doExport: function () {
            ...
            new Spreadsheet(oSettings).build().then(function () {
                var dataExportButtonId = document.querySelectorAll('[id$="-dataExportButton"]')[0].id;
                sap.ui.getCore().byId(dataExportButtonId).setEnabled(true); // activate download button
                that.model.setProperty("/isBusy", false);
            }, function () {
                var dataExportButtonId = document.querySelectorAll('[id$="-dataExportButton"]')[0].id;
                sap.ui.getCore().byId(dataExportButtonId).setEnabled(true); // activate download button
