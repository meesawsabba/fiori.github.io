...
sap.ui.define(["sap/esh/search/ui/SearchHelper", "sap/m/List"], function (SearchHelper, List) {
    "use strict";
    return List.extend("sap.esh.search.ui.controls.SearchResultList", {
        init: function () {
            if (List.prototype.init) {
                // check whether superclass implements the method
                List.prototype.init.apply(this, arguments); // call the method with the original arguments
            }
        ...
        onAfterRendering: function () {
            ...
            List.prototype.onAfterRendering.apply(that, arguments);
        ...
        _doCollectListItemsForNavigation: function () {
            ...
            for (i = 0; i < aRows.length; i++) {
                var oRow = aRows[i];
                if ($(oRow).hasClass("sapUshellSearchResultListItemApps")) {
        ...
        _prepareResizeHandler: function () {
            ...
            var resizeHandler = function () {
                ...
                if (currentWindowWidthIndex != lastWindowWidthIndex) {
                    ...
                    for (var i = 0; i < aMyListItems.length; i++) {
                        var oMyItem = aMyListItems[i];
                        if (oMyItem.getContent() &&
                            oMyItem.getContent().length > 0 &&
        ...
        disableSelectionMode: function (animated) {
            ...
            if (searchResultList.hasClass("sapUshellSearchResultList-ShowMultiSelection")) {
                var newPadding = currentAttributesPadding - checkBoxWidth;
                var animation01 = checkBoxExpandContainers
                    .animate({
                    width: 0,
                    opacity: 0,
                }, animationDuration)
                    .promise();
                var animation02 = attributesContainers
                    .animate({
                    "padding-left": newPadding,
                }, animationDuration)
                    .promise();
