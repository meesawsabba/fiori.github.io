 ...
 */
sap.ui.define(["./i18n", "sap/ui/model/Sorter"], 
/**
 *
 * @param {*} i18n
 * @param {*} Sorter
 */
function (i18n, Sorter) {
    "use strict";
    var module = {};
    var SearchAdvancedCondition;
    module.setSearchAdvancedCondition = function (_SearchAdvancedCondition) {
        SearchAdvancedCondition = _SearchAdvancedCondition;
    };
    jQuery.extend(module, {
        init: function (dialog) {
            ...
            that.oFloatNumberFormat =
                sap.ui.core.format.NumberFormat.getFloatInstance(oNumberFormatOptions);
        ...
        updateDetailPage: function (oListItem, sFilterTerm, bInitialFilters) {
            ...
            if (sDataType === "string" || sDataType === "text") {
                oDetailPageAttributeListContainer = oDetailPage
                    .getContent()[that.POS_ICONTABBAR].getItems()[that.POS_TABBAR_LIST].getContent()[0]
                    .getContent()[that.POS_ATTRIBUTE_LIST_CONTAINER];
                oDetailPageAttributeList = oDetailPage
                    .getContent()[that.POS_ICONTABBAR].getItems()[that.POS_TABBAR_LIST].getContent()[0]
                    .getContent()[that.POS_ATTRIBUTE_LIST_CONTAINER].getContent()[0];
                oAdvancedContainer = oDetailPage
                    .getContent()[that.POS_ICONTABBAR].getItems()[that.POS_TABBAR_CONDITION].getContent()[0];
                oSettings = oDetailPage
                    .getContent()[that.POS_ICONTABBAR].getItems()[that.POS_TABBAR_LIST].getContent()[0]
                    .getContent()[that.POS_SETTING_CONTAINER];
            }
            else {
                oDetailPageAttributeListContainer =
                    oDetailPage.getContent()[that.POS_ATTRIBUTE_LIST_CONTAINER];
                oDetailPageAttributeList = oDetailPage
                    .getContent()[that.POS_ATTRIBUTE_LIST_CONTAINER].getContent()[0];
            ...
            if (sFilterTerm) {
                ...
                if (sFilterTerm === undefined && (sDataType === "string" || sDataType === "text")) {
                    ...
                    oDetailPage
                        .getContent()[that.POS_ICONTABBAR].getItems()[that.POS_TABBAR_LIST].getContent()[0]
                        .getSubHeader()
                        .getContent()[that.POS_TOOLBAR_SEARCHFIELD].setValue("");
                }
            }
            ...
            if (oDetailPage.getContent()[that.POS_ATTRIBUTE_LIST_CONTAINER]) {
                ...
                switch (sDataType) {
                    ...
                    case "date": // eslint-disable-line no-fallthrough
                        that.oDateTimeFormat =
                            sDataType === "timestamp" ? that.oTimestampFormat : that.oDateFormat;
                        for (k = 1; k < oAdvancedConditionList.length; k++) {
                            oAdvancedCondition = oAdvancedConditionList[k];
                            oAdvancedCheckBox =
                                oAdvancedCondition.getContent()[that.POS_ADVANCED_CHECKBOX];
                    ...
                    case "number": // eslint-disable-line no-fallthrough
                        that.oNumberFormat =
                            sDataType === "integer" ? that.oIntegernumberFormat : that.oFloatNumberFormat;
                        for (k = 1; k < oAdvancedConditionList.length; k++) {
                            oAdvancedCondition = oAdvancedConditionList[k];
                            oAdvancedCheckBox =
                                oAdvancedCondition.getContent()[that.POS_ADVANCED_CHECKBOX];
                            var oAdvancedInputLeft = oAdvancedCondition
                                .getContent()[that.POS_ADVANCED_INPUT_LAYOUT].getContent()[0];
                            var oAdvancedInputRight = oAdvancedCondition
                                .getContent()[that.POS_ADVANCED_INPUT_LAYOUT].getContent()[2];
                            var oAdvancedLebel = oAdvancedCondition
                                .getContent()[that.POS_ADVANCED_INPUT_LAYOUT].getContent()[1];
                                ...
                                if (!isNaN(oAdvancedInputLeftValue) &&
                                    ...
                                    oConditionGroup = model.sinaNext.createComplexCondition({
                                        valueLabel: that.oNumberFormat.format(oAdvancedInputLeftValue) +
                                            ...
                                            that.oNumberFormat.format(oAdvancedInputRightValue),
                                        operator: model.sinaNext.LogicalOperator.And,
                                        conditions: [fromCondition, toCondition],
                                        userDefined: true,
                                    });
                ...
                var oAdvancedContainer = oDetailPage
                    .getContent()[that.POS_ICONTABBAR].getItems()[that.POS_TABBAR_CONDITION].getContent()[0];
                            ...
                            if (item.filterCondition.conditions) {
                                for (var j = 0; j < item.filterCondition.conditions.length; j++) {
                                    ...
                                    if (condition.operator === "Ge") {
                                        oInputBoxLeft.setValue(condition.valueLabel ||
                                            that.oNumberFormat.format(condition.value));
                                    }
                                    if (condition.operator === "Le") {
                                        oInputBoxRight.setValue(condition.valueLabel ||
                                            that.oNumberFormat.format(condition.value));
                                    }
                                }
                            }
        ...
        deleteAdvancedCondition: function (oAdvancedCondition) {
            var that = this;
            var oAdvancedContainer = oAdvancedCondition.getParent();
            var oDetailPage = oAdvancedCondition
                .getParent()
                .getParent()
                .getParent()
                .getParent()
