...
sap.ui.define(
	...
	function(
			...
			renderer: {
				render: function(oRm, oControl) {
					...
					if (oMicroChart) {
						...
						if (!oControl.getShowOnlyChart() && oControl.getUomPath()) {
		...
		MicroChartContainer.prototype.onAfterRendering = function() {
			...
			if (!that._checkIfChartRequiresRuntimeLabels()) {
				return;
			}
			if (oBinding) {
		...
		MicroChartContainer.prototype.setShowOnlyChart = function(sValue) {
			var that = this;
		...
		MicroChartContainer.prototype._setRuntimeChartLabelsAndUnitOfMeasure = function() {
			...
			if (oUnitOfMeasureLabel && sUnitOfMeasurePath && aContexts.length && !this.getShowOnlyChart()) {
		 ...
		 *
		 * @param {string} sImprovementDirection The ImprovementDirection to be used for getting the criticality
		 * @param {string} sValue The value from Datapoint to be measured
		 * @param {string} sDeviationLow The value for Lower Deviation level
		 * @param {string} sToleranceLow The value for Lower Tolerance level
		 * @param {string} sAcceptanceLow The value for Lower Acceptance level
		 * @param {string} sAcceptanceHigh The value for Higher Acceptance level
		 * @param {string} sToleranceHigh The value for Higher Tolerance level
		 * @param {string} sDeviationHigh The value for Higher Deviation level
		 * @returns {string} Criticality to be plotted.
		 * @private
		 */
