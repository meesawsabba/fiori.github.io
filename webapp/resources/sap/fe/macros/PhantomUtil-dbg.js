...
sap.ui.define(
	...
	function(JSONModel, Log, ManagedObject, ResourceModel, XMLPreprocessor, SyncPromise, TraceInfo, ObjectPath) {
		 ...
		 * @param {Element} oNode The dom node to consider
		 * @param {string} sAttributeName The name of the current attribute
		 * @param {object} oVisitor The templating visitor
		 * @param {boolean} bDoNotResolve Whether we should ignore the value set in the attribute
		...
		function _getMetadataContext(oSettings, oNode, sAttributeName, oVisitor, bDoNotResolve) {
			...
			if (!bDoNotResolve && oNode.hasAttribute(sAttributeName)) {
				...
				if (!oMetadataContext) {
					if (sAttributeName === "metaPath" && oSettings.currentContextPath) {
					...
					} else if (sAttributeName === "contextPath") {
						if (sAttributeValue.startsWith("/")) {
							// absolute path - we just use this one
							sMetaPath = sAttributeValue;
						} else {
		...
		function resolve(oMacroDefinition, oNode, oVisitor, isPublic) {
			...
			return oTargetPromise
				.then(function(aProps) {
					if (aProps != null) {
						...
						if (oCreateFn) {
							...
							oProps = oCreateFn.call(oMacroDefinition, oProps, oControlConfig, oSettings, oAggregations);
				...
				.then(function() {
					...
					var iChildIndex;
					for (iChildIndex = 0; iChildIndex < oParent.children.length; ++iChildIndex) {
						if (oParent.children[iChildIndex] === oNode) {
							break;
						}
					}
					var oPromise;
					if (oMacroDefinition.getTemplate) {
						var oTemplate = oMacroDefinition.getTemplate(oAttributesModel.oProps);
								...
								if (oMacroElement !== null) {
									...
									if (sAggregationName !== "customData" && sAggregationName !== "dependents") {
										var sSlotName =
											...
											sAggregationName;
										var oTargetElement = oMacroElement.querySelector("slot[name='" + sSlotName + "']");
										if (oTargetElement) {
											oTargetElement.replaceWith.apply(oTargetElement, oNewChild.children);
										}
									} else {
