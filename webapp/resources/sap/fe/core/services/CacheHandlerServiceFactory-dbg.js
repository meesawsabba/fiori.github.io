...
sap.ui.define(
	...
	function(Service, ServiceFactory, ResourceModel, CacheManager, hash) {
		"use strict";
			...
			getETags: function() {
				...
				var sManifestHash = hash(
					JSON.stringify({
						...
						viewData: this.oComponent.getViewData()
					})
				);
		...
		return ServiceFactory.extend("sap.fe.core.services.CacheHandlerServiceFactory", {
			...
			createInstance: function(oServiceContext) {
				...
				if (!this._oInstanceRegistry[sMetaModelId]) {
					this._oInstanceRegistry[sMetaModelId] = new CacheHandlerService(
						Object.assign(
							{
								factory: this,
								scopeObject: null,
				...
				var that = this;
				return this._oInstanceRegistry[sMetaModelId].initPromise
					.then(function() {
						return that._oInstanceRegistry[sMetaModelId];
					})
					.catch(function(e) {
						// In case of error delete the global instance;
						that._oInstanceRegistry[sMetaModelId] = null;
						throw e;
					});
			},
