	...
	function(
		...
		var TemplatedViewService = Service.extend("sap.fe.core.services.TemplatedViewService", {
					...
					.then(function(aDependenciesResult) {
						...
						return new Promise(function(resolve) {
							sap.ui.require(
								...
								function(TemplateConverter, MetaModelConverter) {
									resolve(
										that.createView(
											oResourceModel,
											sStableId,
			...
			refreshView: function(oComponent) {
				...
				return that
					.createView(that.resourceModel, that.stableId, "", "", undefined, that.TemplateConverter, that.MetaModelConverter)
					.then(function() {
			...
			createView: function(
				...
				that.MetaModelConverter = MetaModelConverter;
				var oContext = this.getContext(),
		...
		var oServiceFactory = ServiceFactory.extend("sap.fe.core.services.TemplatedViewServiceFactory", {
			_oInstanceRegistry: {},
			iCreatingViews: 0,
			createInstance: function(oServiceContext) {
				oServiceFactory.prototype.iCreatingViews++;
		...
		oServiceFactory.getNumberOfViewsInCreationState = function() {
			return oServiceFactory.prototype.iCreatingViews;
		};
