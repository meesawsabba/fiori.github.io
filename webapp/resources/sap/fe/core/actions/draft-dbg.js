...
sap.ui.define(
	...
	function(Dialog, Button, Text, MessageBox, messageHandling, CommonUtils, Log) {
		"use strict";
		...
		function executeDraftActivationAction(oContext, oAppComponent, sGroupId) {
						...
						function() {
							if (bHasPrepareAction) {
								...
								if (aTargetPaths && aTargetPaths.length > 0) {
									oSideEffectsService.requestSideEffects(aTargetPaths, oContext).catch(function(oError) {
										Log.error("Error while requesting side effects", oError);
									});
								} else {
									requestMessages(oContext, oSideEffectsService).catch(function(oError) {
		 ...
		 * @ui5-restricted
		 */
		function executeDraftPreparationAction(oContext, oAppComponent, mParameters) {
			...
			if (!oContext.getProperty("IsActiveEntity")) {
				...
				var sGroupId = mParams.groupId || oOperation.getGroupId();
					...
					.then(function() {
						if (mParams.bPrepareForEdit) {
							...
							var sActionName = getActionName(oContext, draftOperations.PREPARE),
								...
								aTargetPaths = oBindingParameters && oBindingParameters.pathExpressions;
							if (aTargetPaths && aTargetPaths.length > 0) {
								oSideEffectsService.requestSideEffects(aTargetPaths, oContext, sGroupId).catch(function(oError) {
									Log.error("Error while requesting side effects", oError);
								});
							} else {
		...
		function requestMessages(oContext, oSideEffectsService) {
			...
			if (sMessagesPath) {
				return oSideEffectsService.requestSideEffects([{ $PropertyPath: sMessagesPath }], oContext);
			}
		}
		 ...
		 * @ui5-restricted
		 */
		function createDraftFromActiveDocument(oContext, oAppComponent, mParameters) {
		 ...
		 * @ui5-restricted
		 */
		function activateDocument(oContext, oAppComponent, mParameters) {
			var mParam = mParameters || {};
			...
			return Promise.resolve(mParam.fnBeforeActivateDocument ? mParam.fnBeforeActivateDocument(oContext) : true)
				.then(function(bExecute) {
					...
					var oPreparePromise = executeDraftPreparationAction(oContext, oAppComponent, {
						groupId: sBatchGroup,
						bPrepareForEdit: false
					});
					oContext.getModel().submitBatch(sBatchGroup);

					var oActivatePromise = executeDraftActivationAction(oContext, oAppComponent, sBatchGroup);
