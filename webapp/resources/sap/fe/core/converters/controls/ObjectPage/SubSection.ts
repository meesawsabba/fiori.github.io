...
function createCustomHeaderFacetSubSection(customHeaderFacet: CustomObjectPageHeaderFacet): ObjectPageSubSection {
	const subSectionID = CustomSubSectionID(customHeaderFacet.key);
	...
	switch (facetDefinition.$Type) {
		case UIAnnotationTypes.CollectionFacet:
			...
			if (hasTable(facets)) {
				...
				const actions = getFacetActions(facetDefinition, converterContext),
					formCollectionSubSection: FormSubSection = {
						...subSection,
						type: SubSectionType.Form,
						formDefinition: createFormDefinition(facetDefinition, converterContext, actions),
						level: level,
						actions: actions.filter(action => action.facetName === undefined)
					};
...
function createFormActionReducer(
		...
		switch (referenceTarget.term) {
			case UIAnnotationTerms.FieldGroup:
				...
				manifestActions = getActionsFromManifest(
					...
					undefined,
					undefined,
					facetDefinition.fullyQualifiedName as string
				);
				break;
			case UIAnnotationTerms.Identification:
			case UIAnnotationTerms.StatusInfo:
	...
	actions = dataFieldCollection.reduce((actions, dataField: DataFieldAbstractTypes) => {
		...
		switch (dataField.$Type) {
			...
			case UIAnnotationTypes.DataFieldForAction:
				...
				actions.push({
					...
					facetName: dataField.Inline ? (facetDefinition.fullyQualifiedName as string) : undefined
				});
				break;
		}
		return actions;
