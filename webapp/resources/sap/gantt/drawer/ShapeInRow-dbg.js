...
sap.ui.define([
	...
	ShapeInRow.prototype._drawLine = function (aShape, oShape) {
		...
		aShape
			...
			.attr("aria-label", function (d) {
				oStyle = oShape.getStyle(d, fFindObjectInfo(this, oShape));
		...
		aShape
			...
			.text(function (d) {
				...
				if (nTruncateWidth > -1) { // do truncating
					that._textTruncate(d, oSelf, nTruncateWidth, oShape.getEllipsisWidth(d, ObjectInfo), oShape.getId());
	...
	ShapeInRow.prototype._textTruncate = function (oData, oSelf, nTruncateWidth, nEllipsisWidth, id) {
		...
		if (nTextLength > nTruncateWidth) { // truncate needed
			...
			if (bEllipsisAppear) {
				if (sap.ui.Device.browser.name === "cr"){
					...
					if (bRTL && (sap.ui.Device.browser.edge || sap.ui.Device.browser.safari)) {
						oSelf.text("..." + sText)
							.classed(id, true);
					} else {
						oSelf.append("tspan")
