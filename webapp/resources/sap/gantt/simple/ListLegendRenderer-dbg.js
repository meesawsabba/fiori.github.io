...
function (Utility, RenderUtils, LegendShapeGroupOrientation, IconPool) {
	...
	ListLegendRenderer.renderShape = function (oRm, oItem, bHasInteractiveItem) {
		...
		if (oShape.isA("sap.gantt.simple.BaseConditionalShape")) {
			var baseConditionalShape = oShape.getAggregation('shapes')[oShape.getActiveShape()];
	...
	ListLegendRenderer.normalizeShapeGroup = function (allShapes, subShape, orientation, iWidth, legendItemHeight, index, yBias) {
		//Handling BaseConditionalShape in ListLegendItem
		if (subShape.isA("sap.gantt.simple.BaseConditionalShape")) {
			var baseConditionalShape = subShape.getAggregation('shapes')[subShape.getActiveShape()];
	...
	ListLegendRenderer.renderShapeGroupRecursively = function(oRm, oShape) {
		...
		} else if (oShape.isA("sap.gantt.simple.BaseConditionalShape")) {
			var baseConditionalShape = oShape.getAggregation('shapes')[oShape.getActiveShape()];
			if (baseConditionalShape.isA('sap.gantt.simple.LegendShapeGroup')) {
