...
sap.ui.define([
	...
	var GanttUtils = {
		...
		getValueX: function(oShape) {
			...
			if (vTime) {
				var oShapeData = this.isShapeLargerThanTotalHorizon(oShape);
				if (!bRTL) {
					vTime = oShapeData.startTime ? oShapeData.startTime : vTime;
				} else {
					vTime = oShapeData.endTime ? oShapeData.endTime : vTime;
				}
		...
		calculateLevelForShapes: function (allShapes, startTimeProperty, endTimeProperty) {
			...
			/**
			 * Returns shape's updated start and end time if shape is greater than the total horizon
			 * @param {sap.gantt.simple.BaseShape} oShape Shape instance.
			 * @returns {object} returns shape start and end time.
			 */
			isShapeLargerThanTotalHorizon : function (oShape) {
				var oGantt = oShape.getGanttChartBase();
				var startTime = Format.abapTimestampToDate(oShape.getTime()),
					endTime = Format.abapTimestampToDate(oShape.getEndTime());
				if (oGantt) {
					var oTotalHorizon = oGantt.getAxisTimeStrategy().getTotalHorizon();
					var oGanttStartTime = Format.abapTimestampToDate(oTotalHorizon.getStartTime());
					if (startTime < oGanttStartTime) {
						startTime = oGanttStartTime;
					}
					var oGanttEndTime = Format.abapTimestampToDate(oTotalHorizon.getEndTime());
					if (endTime > oGanttEndTime) {
						endTime = oGanttEndTime;
					}
				}
				return {
					startTime: startTime,
