...
sap.ui.define([
	 ...
	 */
	 GanttChartWithTable.prototype._getShapeIdPath = function() {
		...
		if (this.getSelectedShapeUid() && this.getSelectedShapeUid().length > 0) {
			...
			var oBindingInfo = this.getTable().getBindingInfo("rows"),
				sModelName = oBindingInfo && oBindingInfo.model;
			var bindingContext = aAllRows[0].getBindingContext(sModelName),
				sObject = bindingContext.getObject(),
				sPath = bindingContext.getPath();
			var parts = sPath.split("'");
			var shapeId = parts[1];
			Object.keys(sObject).forEach(function(key) {
				if (sObject[key] === shapeId) {
					path = key;
				}
			});
		}
	...
	GanttChartWithTable.prototype.findAndSelect = function (propertyValue, propertyName, overrideExisting, rowSelection) {
		...
		var path;
		if (rowSelection) {
			path = this.getTable().getRowSettingsTemplate().mBindingInfos.rowId.parts[0].path;
			if (overrideExisting) {
				this.searchTxt = [];
				this.searchProperty = [];
			}
			...
			path = this._getShapeIdPath();
		}
		var selectedShapeIDs = [];
		data.forEach(function(item) {
			if (item[path]) {
				if (rowSelection) {
					selectedShapeIDs.push({rowId: item[path], Selected: true});
				} else {
					selectedShapeIDs.push({ShapeId: item[path], Selected: true});
				}
				item.selected = true;
			}
		});
	...
	GanttChartWithTable.prototype.findAndDeselect = function (propertyValue, propertyName, rowSelection) {
		...
		data.forEach(function(item) {
			if (item[path]) {
				if (rowSelection) {
					deSelectedShapeIDs.push({rowId: item[path], Selected: false});
				} else {
					deSelectedShapeIDs.push({ShapeId: item[path], Selected: false});
				}
				item.selected = false;
			}
		});
	 ...
	 * @param {string} propertyName property to search in (optional parameter)
	 * @returns list of search results
	 * @public
	 * @since 1.89
	 */
	GanttChartWithTable.prototype.findAll = function (propertyValue, propertyName) {
		var selectedData = [];
		var data = this.getTable().getBinding().getModel().oData;
		Object.keys(data).forEach(function(key) {
			var obj = data[key];
			if (propertyName) {
				if (obj[propertyName] !== undefined &&
					obj[propertyName] !== null &&
					obj[propertyName].toString().toLowerCase().indexOf(propertyValue.toString().toLowerCase()) !== -1) {
				...
				Object.keys(obj).forEach(function(propName) {
					...
					if (obj[propName] !== undefined &&
						obj[propName] !== null &&
						obj[propName].toString().toLowerCase().indexOf(propertyValue.toString().toLowerCase()) !== -1) {
						selectedData.push(obj);
	...
	GanttChartWithTable.prototype.handleShapePress = function(mParam) {
		...
		if (oShape instanceof sap.gantt.simple.BaseConditionalShape) {
			...
			if (oShape._getActiveShapeElement() instanceof sap.gantt.simple.BaseGroup) {
				oShape._getActiveShapeElement().getShapes().forEach(function (oChildShape) {
				...
				sStartDate = oShape._getActiveShapeElement().getTime();
				sEndDate = oShape._getActiveShapeElement().getEndTime();
				oShape.setTime(sStartDate);
				oShape.setEndTime(sEndDate);
			}
	...
	GanttChartWithTable.prototype.onBeforeRendering = function(oEvent) {
		...
		//Attach the DataReceived event on the Table when GanttRowAction / Control Template is present.
		if (oRowActionTemplate && oRowActionTemplate.isA("sap.gantt.simple.GanttRowAction") &&
			oRowActionTemplate.getControlTemplate()) {
					...
					aAggregations.forEach(function(oShape){
						if (oShape instanceof sap.gantt.simple.BaseConditionalShape) {
							...
							if (bFlag && oShape.getShapeUid()) {
								...
								if (!(bRowExpanded && !this.getShowParentRowOnExpand())) {
									aShapesInRow = aShapesInRow.concat(oShape);
								}
							}
