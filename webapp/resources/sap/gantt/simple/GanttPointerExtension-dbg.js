...
sap.ui.define([
	...
	GanttPointerExtension.prototype.customBar = function (oConatiner,val,timeValue) {
		...
		var oDateFormat = DateFormat.getInstance({ pattern: oConatiner.getStatusBarDatePattern() }); //Returns a DateFormat instance for date
	    var oTimeFormat = DateFormat.getTimeInstance({ pattern: oConatiner.getStatusBarTimePattern() }); //Returns a DateFormat instance for time
		oConatiner.msgText.setWidth(val ? "70%" : "100%");
		oConatiner.dateTimeText.setWidth(val ? "30%" : "0%");
		oConatiner.dateTimeText.setText(val ? oDateFormat.format(timeValue) + " " +  oTimeFormat.format(timeValue) : _oEmptyText.getText());
    };
