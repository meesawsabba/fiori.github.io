...
// --------------------------------------------------------------------------------
sap.ui.define([
	"sap/ui/core/format/NumberFormat", "sap/ui/core/format/DateFormat", "sap/m/P13nConditionPanel", "sap/ui/comp/odata/ODataType", "sap/ui/model/odata/type/Currency", "sap/ui/model/odata/type/Unit"
], function(NumberFormat, DateFormat, P13nConditionPanel, ODataType, Currency, Unit) {
	"use strict";
	...
	var FormatUtil = {
		 ...
		 * @param {integer} iFractionDigits the UnitSpecificScale provided for the particular UoM or Currency
		 * @param {integer} iMaxFractionDigits Maximum fraction digit support for alignment of decimal point
		 * @returns {integer} difference between the configure decimals and the actual decimal found in the formatted value
		 * @private
		 */
		 ...
		 * @returns {integer} Max currency fraction digit
		 * @private
		 */
		 ...
		 * @param {object} mCodeList Measures code list
		 * @returns {integer} Max measure unit fraction digit
		 * @private
		 */
		 ...
		 */
		getInlineGroupFormatterFunction: function(oFieldMetadata, bDisableDescription, oDateFormatSettings, bPreserveDecimals) {
			...
			if (oFieldMetadata.unit) {
				if (oFieldMetadata.isCurrencyField) {
					return FormatUtil.getInlineAmountFormatter(bPreserveDecimals);
				} else {
					fnInternalFormatter = FormatUtil.getInlineMeasureUnitFormatter(bPreserveDecimals);
