...
sap.ui.define([
	...
	SmartVariantManagement.prototype._isUIAdaptationMode = function() {
		return (sap.ui.dt && sap.ui.dt.OverlayRegistry && sap.ui.dt.OverlayRegistry.getOverlay(this.getId()));
	};

	SmartVariantManagement.prototype._getContent = function(oVariant) {
		if (this._mShadowContent[oVariant.getId()] && this._isUIAdaptationMode()) {
			return this._mShadowContent[oVariant.getId()];
		}

		return oVariant.getContent();
	};

	SmartVariantManagement.prototype.assignShadowContent = function(sKey, oShadowContent) {
		this._mShadowContent[sKey] = oShadowContent;
	};

	SmartVariantManagement.prototype._removeShadowContent = function(sKey) {
		if (this._mShadowContent[sKey]) {
			delete this._mShadowContent[sKey];
		}
	};
	...
	SmartVariantManagement.prototype._createVariantItem = function(oVariant) {
		this._mVariants[oVariant.getId()] = oVariant;
		...
		oVariantItem.setText(oVariant.getName());
	...
	SmartVariantManagement.prototype._createVariantEntries = function(mVariants) {
		var sVariantKey, sAppStandardVariantKey = null;
		...
		if (mVariants) {
			...
			if (mVariants.standardVariant) {
				...
				var oContent = this._getVariantContent(mVariants.standardVariant.getId());
				if (Object.keys(oContent).length > 0) {
					sAppStandardVariantKey = mVariants.standardVariant.getId();
				}
			}
		...
		if (this._oPersoControl) {
			...
			if (this._getIndustrySolutionMode() && sAppStandardVariantKey) {
				this._sAppStandardVariantKey = sAppStandardVariantKey;

				if (sAppStandardVariantKey !== this.STANDARDVARIANTKEY) {
					this.setStandardVariantKey(sAppStandardVariantKey);
				}
			}
	...
	SmartVariantManagement.prototype._flUpdateVariant = function(oVariant, mProperties) {
		...
		if (mProperties.hasOwnProperty("content") || mProperties.hasOwnProperty("name")) {
			...
			if (oUpdatedVariant) {
				this._mVariants[oUpdatedVariant.getId()] = oUpdatedVariant;
			}
		}
	...
	SmartVariantManagement.prototype._flRemoveVariant = function(oVariant) {
		...
		if (oDeletedVariant) {
			delete this._mVariants[oDeletedVariant.getId()];   // update
		}
	};
	...
	SmartVariantManagement.prototype._applyCurrentVariantId = function(sVariantId, bDoNotApplyVariant) {
		var oVariant, oContent;

		if (this._oPersoControl) {
			oVariant = this._getVariantById(sVariantId);

			if (oVariant) {
				oContent = this._getVariantContent(sVariantId);

				if (oContent) {
					this._setSelectionByKey(sVariantId); // set the current selected variant
					if (!bDoNotApplyVariant) {
						if (this.isPageVariant()) {
							this._applyVariants(oContent, "SET_VM_ID", oVariant.getExecuteOnSelection());
						} else {
							this._applyVariant(this._oPersoControl, oContent, "SET_VM_ID", false, oVariant.getExecuteOnSelection());
						}
					}
				}
			}
	...
	SmartVariantManagement.prototype._initialize = function(parameter, oCurrentControlWrapper) {
		...
		SyncPromise.resolve(oResult).then(function(oResult) {
			...
			if (this._getCurrentVariantId() === this.getStandardVariantKey()) {
				...
				if (this.getExecuteOnSelectForStandardVariant() && this._oAppStdContent) {
					var oItem = this.getItemByKey(this._getCurrentVariantId());
					if (oItem) {
...
// TODO: clarify with CV: handling with vendor standard not clear
								if (this._isIndustrySolutionModeAndVendorLayer() && (oVariantInfo.key === this.getStandardVariantKey())) {
								...
								if ((oVariantInfo.lifecycleTransportId !== null) && (oVariantInfo.lifecycleTransportId !== undefined)) {
									mParameters.transportId = oVariantInfo.lifecycleTransportId;
								}

								if ((oVariantInfo.lifecyclePackage !== null) && (oVariantInfo.lifecyclePackage !== undefined)) {
									mParameters.packageName = oVariantInfo.lifecyclePackage;
								}
	...
	SmartVariantManagement.prototype._newVariant = function(oVariantInfo) {
		...
		var sId, bIsStandardVariant = false;
		...
		if (oVariantInfo) {
			...
			return SyncPromise.resolve(this._fetchContentAsync()).then(function(oContent) {
				...
				if (oContent) {
//TODO: clarify with CV: handling of vendor standard
				...
				var mParams = {
					...
					executeOnSelection: oVariantInfo.exe,
					id: sId
				};
	...
	SmartVariantManagement.prototype._deleteVariants = function(aVariantInfo) {
		...
		if (aVariantInfo && aVariantInfo.length) {
			...
			for (var i = 0; i < aVariantInfo.length; i++) {
				...
				var oVariant = this._getVariantById(aVariantInfo[i]);
				if (oVariant) {
					sTransportId = this._appendLifecycleInformation(oVariant, aVariantInfo[i]);
					oVariant.setRequest(sTransportId);
	...
	SmartVariantManagement.prototype._getExecuteOnSelection = function(oVariant) {
		return oVariant.getExecuteOnSelection();
	};
