...
sap.ui.define([
	...
	'sap/base/Log',
	"sap/base/util/merge"
], function(jQuery, library, Element, Factory, JSONModel, MetadataAnalyser, BindingMode, Log, merge) {
	"use strict";
	...
	SemanticObjectController.prototype._onNavigate = function(oEvent) {
		...
		if (this.getBeforeNavigationCallback() &&
			...
			this.getBeforeNavigationCallback()(merge({}, oNavigationInfo)).then(function(bNavigate) {
				if (bNavigate === true) {
					this.fireNavigate(oNavigationInfo);
					window.location.href = oParameters.href;
				}
			}.bind(this));
		} else {
			this.fireNavigate(oNavigationInfo);
