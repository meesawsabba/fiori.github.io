...
sap.ui.define([
...
], function(jQuery, library, EventProvider, ODataType, MetadataAnalyser, FormatUtil, DateTimeUtil, Log, Token, Filter, FilterOperator) { //
	"use strict";
	...
	// declare the FilterProvider class usage. Can't be set in define clause, cycle dependencies...
	var FilterProvider;
	...
	BaseValueListProvider.prototype.getValueListAnnotation = function(){
		return this._oMetadataAnalyser.getValueListAnnotationLazy(this._sFullyQualifiedFieldName);
	};
			...
			if (this.sValueListEntitySetName && this.sKey) {
				...
				for (i = 0; i < iLen; i++) {
					...
					if (oField.visible) {
						...
						if (aHighImportanceFields && aHighImportanceFields.indexOf(oField) !== -1) {
							this._aHighImportanceCols.push(this._getColumnConfigFromField(oField));
						}
	...
	BaseValueListProvider.prototype.readData = function (aInput) {
		...
		if (!FilterProvider) {
			FilterProvider = sap.ui.require("sap/ui/comp/smartfilterbar/FilterProvider"); // because of cycle in define (via FilterProvider)
		}
		var aFilters = [], sInput;
