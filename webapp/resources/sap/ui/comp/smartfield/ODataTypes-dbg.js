...
sap.ui.define([
		...
		if (oProperty && oProperty.property && oProperty.property.type) {
			...
			switch (oProperty.property.type) {
				...
				case "Edm.String":
					...
					if (mConstraints.isDigitSequence && !mSettings.composite) {
				...
				case "Edm.Guid":
					...
					if (MetadataAnalyser.hasTextArrangementAnnotation(oProperty.property) &&
						...
						oValidationSettings = {validateException: {resourceBundle: "sap.ui.comp", text: "ENTER_GUID_OR_DESCRIPTION"}};
					}
	...
	ODataTypes.prototype.getCurrencyDisplayFormatter = function(mSettings) {
		var oFormat = new CurrencyType({ showMeasure: false, emptyString: NaN } );
	...
	ODataTypes.prototype.getUOMDisplayFormatter = function(oProperty, mSettings) {
		var oFormat = new UnitType( { showMeasure: false, emptyString: NaN }, {scale: oProperty && oProperty.scale ? oProperty.scale : 0} );
