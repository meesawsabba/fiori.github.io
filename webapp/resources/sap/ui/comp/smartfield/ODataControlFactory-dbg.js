...
sap.ui.define([
	...
	ODataControlFactory.prototype._createEdmDisplay = function() {
		var oInnerControl,
			...
			oControlSelectorConfig = this._oSelector.checkComboBox(bIgnoreComboBox),
		...
		mAttributes.emptyIndicatorMode = EmptyIndicatorMode.On;
		...
		if (bDatePicker) {
			...
			if (
				...
				// A composite type is needed in this mode
				this.oType = this._getTextArrangementType();
			}
		...
		if (bObjectIdentifier) {
			...
			if (typeof mAttributes.text.formatter === "undefined") {
			...
			bWasDisplayCombobox = this._oSelector._checkComboBox(bIgnoreComboBox).combobox && (this._oParent.getFetchValueListReadOnly() || (vTextAnnotationPath === undefined));
	...
	ODataControlFactory.prototype._addObjectStatusAttributes = function(mAttributes) {
		...
		var fIcon = function() {
			var	oIcons = {};
			oIcons[ValueState.Error] = "sap-icon://message-error";
			oIcons[ValueState.Warning] = "sap-icon://message-warning";
			oIcons[ValueState.Success] = "sap-icon://message-success";
	...
	ODataControlFactory.prototype._createEdmString = function() {
		...
		if (
			...
			this._oParent.getMode() === "edit"
		) {
			bDefaultTextInEditModeSource = true;
		}

		this.oTextArrangementDelegate.bValidMetadata = bEdmPropertyTypeSupported && this.oTextArrangementDelegate.checkRequiredMetadata(sTextInEditModeSource, bDefaultTextInEditModeSource);

		if (this.oTextArrangementDelegate.bValidMetadata) {
			mAttributes = this.createAttributes("", this._oMetaData.property, mNames);
			oSettings.valueListNoValidation = this._oParent._getComputedTextInEditModeSource() === "ValueListNoValidation";
			mAttributes[sValuePropertyMap] = this.oTextArrangementDelegate.getBindingInfo(oSettings);
		} else {
	...
	ODataControlFactory.prototype._createComboBox = function(mSettings) {
		...
		if (bIsMComboBoxRequired){
			sValuePropertyMap = ODataControlFactory.getBoundPropertiesMapInfoForControl(ComboBox.getMetadata().getName(), {
				propertyName: "value"
			})[0];
			mAttributes = {
			...
			sValuePropertyMap = ODataControlFactory.getBoundPropertiesMapInfoForControl(CompComboBox.getMetadata().getName(), {
				propertyName: "value"
			})[0];
			mAttributes = {
				change: function (oEvent) {

					if (this._oParent.getFirstInnerControl().setEnteredValue) {
						this._oParent.getFirstInnerControl().setEnteredValue();
					}
					var sValue = oEvent.getSource().getSelectedKey();
					if (!sValue) {
			...
			"configdata": {
				...
				onText: function(oInnerControl) {
					...
					if (oInnerControl) {
						...
						if (typeof oInnerControl.setEmptyIndicatorMode === "function") {
							oInnerControl.setEmptyIndicatorMode(EmptyIndicatorMode.Off);
						}
	...
	ODataControlFactory.prototype.triggerCreationOfControls = function(oMetaData) {
		try {
			...
			var bHasFieldControl = this._hasBoundFieldControl();
			var bRebindVLOnly = this._bReBindOnVL && bHasFieldControl;
	 ...
	 * @private
	 */
	ODataControlFactory.prototype._checkTextInDisplayModeValueList = function () {
		...
		return !!(
			...
			(oEdmProperty.type === "Edm.String" || oEdmProperty.type === "Edm.Guid") &&
	...
	/**
	 * Method will re-initiate and re-bind if needed mainly when transitioning from "edit" to "display" mode to allow
	 * text arrangement in display mode with value list to work.
	 */
	ODataControlFactory.prototype.reBindIfNecessary = function () {
		if (
			this._bInitialized && // Control has been initialized
			this.bPending === false && // We are not in a pending state
			this._checkTextInDisplayModeValueList() // We need the re-bind for this mode
		) {
			if (!this._oModel) {
				return;
			}
			...
			this._oModel.getMetaModel().loaded().then(function () {
				...
				vValueListAnnotationPath = this._oMetaData.annotations.valuelist;
				...
				return false;
			}.bind(this)).then(function (oValueListAnnotations) {
				if (
					oValueListAnnotations && // If there are value list annotation available
					this._oParent && // Parent is not destroyed in the process
					this._oParent.getMode() === "display" // Parent is still in display mode
				) {
					this.reBind();
				}
	...
	/**
	 * Reinitiate and rebind the control
	 */
	ODataControlFactory.prototype.reBind = function () {
		this._bReBindOnVL = true;
		this.bind().finally(function () {
			this._bReBindOnVL = false;
		}.bind(this));
	};
	...
	ODataControlFactory.prototype.bind = function() {
		...
		if (!(!this._bInitialized && !this.bPending) && !this._bReBindOnVL) {
		...
		var oValueListAnnotationLoadedPromise = oMetaModelLoadedPromise
		.then(function onMetaModelLoaded() {
			...
			this._bTextInDisplayModeValueList = this._checkTextInDisplayModeValueList();

			if (
				(
					this._oParent.getMode() !== "display" && // In display mode we ignore the textInEditModeSource
		...
		.then(function onValueListAnnotationLoaded(oValueListAnnotations) {
			...
			if (oValueListAnnotations && this.oTextArrangementDelegate) {
				...
				// Upon changing the model context of the SmartField control we might need to update
				// the rendered value of the control when we are in a text arrangement scenario.
				// Attach to the model context change event of the SmartField
				if (!this.bHasListenerModelContextChange) {
					this.bHasListenerModelContextChange = true;
					this._oParent.attachModelContextChange(this._handleModelContextChangeEvent.bind(this));
				}
			}
		}.bind(this))
		.catch(function(oError) {
	...
	ODataControlFactory.prototype._handleModelContextChangeEvent = function () {
		// Only when the SmartField has a binding context and a data model
		if (this._oParent && this._oParent.getModel() && this._oParent.getBindingContext()) {
			this.oTextArrangementDelegate.fetchIDAndDescriptionCollectionIfRequired();
		}
	};
	 ...
	 * @param {oControlEvent} {@link sap.ui.base.Event}
