...
sap.ui.define([
	...
	Core, MLibrary, List, Popover, StandardListItem, MultiInput, MultiComboBox, Token, Tokenizer, SmartField,
	MetadataAnalyser, ParseException, ValidateException, BindingMode, ODataType, ValueHelpProvider, FormatUtil,
	DateFormat, FilterProvider, Log, deepEqual, library, coreLibrary, ResizeHandler, Link, Text, FlexBox, HBox, isEmptyObject
) {
	"use strict";
	...
	function onOK(oControlEvent) {
		...
		if (this.oControl instanceof sap.m.MultiInput) {
			...
			if (oSmartMultiInput.getBindingContext() && oSmartMultiInput.getBinding("value").getBindingMode() === "TwoWay") {
	...
	SmartMultiInput.prototype._onDisplayResize = function() {
		var iMultiInputWidth = this.getDomRef().clientWidth,
			...
			bIsSizeReduced = this._iHBoxWidth + iNMoreWidth > iMultiInputWidth,
			bIsSizeIncreased = this._iHiddenLabelsCount && (this._iHBoxWidth + iNMoreWidth + this._iFirstHiddenTokenLength < iMultiInputWidth);
