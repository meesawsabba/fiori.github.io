...
sap.ui.define([
	...
	var StandardDynamicDateRangeKeys = mLibrary.StandardDynamicDateRangeKeys.concat(["LAST2WEEKS", "LAST3WEEKS", "LAST4WEEKS", "LAST5WEEKS", "NEXT2WEEKS", "NEXT3WEEKS", "NEXT4WEEKS", "NEXT5WEEKS"]);
	...
	DateRangeType.prototype._initializeFilterItemNew = function() {
		...
		this._bindValueState(this._oInput);
		this._oInput._reApplyFocusToElement = this._reApplyFocusToElement.bind(this._oInput, this);
		this._oInput.setBindingContext(this.getContext(), "$smartEntityFilter");
		this._oInput.setModel(this.getModel(), "$smartEntityFilter");
		this.bFireFilterChange = false;
		this.getModel().checkUpdate(true);
	...
	DateRangeType.prototype._reApplyFocusToElement = function (oDateRangeType, oToPage, oValue) {
		var oDateRangeListItem,
			oOptionsListPage = this._oNavContainer.getPages()[0],
			oDefaultOperation = oDateRangeType.getDefaultOperation() || DateRangeType.Operations["DATERANGE"];

		if (oToPage !== oOptionsListPage || oValue) {
			// if we have a value or we are not on option page just do not reapply focus
			return;
		}

		var oElementToFocus = this._oOptionsList.getItems().find(function(oListItem) {
			if (!oListItem.isA("sap.m.DynamicDateRangeListItem")) {
				return false;
			}

			var sOptionKey = oListItem.getOptionKey();

			if (sOptionKey === "DATERANGE") {
				oDateRangeListItem = oListItem;
			}

			return sOptionKey === oDefaultOperation.key;
		});

		if (!oElementToFocus) {
			oElementToFocus = oDateRangeListItem || jQuery(oToPage.getDomRef().querySelector("section")).firstFocusableDomRef();
		}
	...
	DateRangeType.prototype._createCustomFixedDDOption = function (oCustomOperation) {
		...
		var oCustomDynamicDateOption = new CustomDynamicDateOption({
			...
			enhanceFormattedValue: function () {
				...
				if (oCustomOperation.category.toUpperCase().startsWith('FIXED.') && oCustomOperation.type === "int"){
					return false;
				}
				return true;
			},
	...
	DateRangeType.prototype._createCustomDynamicDDOption = function (oCustomOperation) {
		var that = this;
		var oCustomDynamicDateOption = new sap.m.CustomDynamicDateOption({
			key: this._getCustomDDOptionName(oCustomOperation.key),
			getText: function () {
	...
	DateRangeType.prototype._isNewDynamicDateRangeEnabled = function () {
		return UriParameters.fromQuery(window.location.search).get("sap-ui-xx-smartFilterBarDynamicDateRange");
	};
		...
		if (this._isNewDynamicDateRangeEnabled() && this._oInput && oCondition.key && oCondition.operation) {
			...
			var oOption = this._getDDROption(oCondition.operation),
				...
				bValidCondition = function(oOperation, aValues) {
					...
					if (oOperation.hasOwnProperty("value1") && !aValues[0]){
						return false;
					}

					if (oOperation.hasOwnProperty("value2") && !aValues[1]){
						return false;
					}
