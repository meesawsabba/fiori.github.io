...
sap.ui.define([
	...
	SmartChart.prototype._adjustOverlayHeight = function() {
		if (sap.ui.Device.browser.safari || sap.ui.Device.browser.chrome) {
	...
	SmartChart.prototype._renderOverlay = function(bShow) {
		...
		if (this._oChart) {
			...
			if (bShow && $overlay.length === 0) {
				...
				this.sOverlayResizeListenerId = sap.ui.core.ResizeHandler.register($this, this._adjustOverlayHeight.bind(this));
			} else if (!bShow) {
				$overlay.remove();
				sap.ui.core.ResizeHandler.deregister(this.sOverlayResizeListenerId);
			}
		}
	};
	/**
	...
	SmartChart.prototype._createChart = function() {
		...
		if (sap.ui.Device.browser.safari || sap.ui.Device.browser.chrome || sap.ui.Device.browser.firefox){
	...
	SmartChart.prototype.getChartAsync = function() {
		...
		if (!this._oChart && this._bMetadataIsInitialised) {
			...
			Log.error("Accesing the inner chart before the metadata is initialized will not work. Instead, wait for the initialized event!");
			return Promise.reject(new Error('Accesing the inner chart before the metadata is initialized will not work. Instead, wait for the initialized event!'));
		} else {
