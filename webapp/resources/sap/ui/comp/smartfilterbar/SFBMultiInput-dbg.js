...
function (
	...
	SFBMultiInput.prototype.onBeforeRendering = function () {
		...
		// BCP: 2180341533 we do this only on first rendering as input controls now invalidate on keystroke
		if (!this._bAfterFirstRendering && this.getValue()) {
			this._pendingAutoTokenGeneration = true;
			this._validateCurrentText(true);
			this._pendingAutoTokenGeneration = false;
		}

		this._bAfterFirstRendering = true;
	};
