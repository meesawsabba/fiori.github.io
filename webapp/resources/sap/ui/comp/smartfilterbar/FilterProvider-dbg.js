...
sap.ui.define([
	...
	FilterProvider.ASSOCIATE_VALUE_LISTS = "AssociateValueLists";
	...
	FilterProvider.prototype._createInitialModel = function(bUseDefaultValues) {
		var oJSONData, iGroupLength, iFieldLength, iHiddenFieldLen, oGroup, j, i;
		oJSONData = {};
	...
	FilterProvider.prototype._associateValueHelpDialog = function (oControl, oFieldViewMetadata, bSupportRanges, bSupportMultiselect) {
		if (this._getSuppressValueListsAssociation()) {
			this._attachEventOnce(FilterProvider.ASSOCIATE_VALUE_LISTS, function(){
	...
	FilterProvider.prototype._associateValueList = function(oControl, sAggregation, oFieldViewMetadata, bHasTypeAhead, bSuppressValueListsAssociation) {
		var bAnnotaionIsToBeLoaded = oFieldViewMetadata.visibleInAdvancedArea || (oFieldViewMetadata.groupId === FilterProvider.BASIC_FILTER_AREA_ID),
			oSettings = {
				control: oControl,
	...
	FilterProvider.prototype._createFieldMetadata = function(oFilterFieldODataMetadata, sParamPrefix) {
		...
		if (!vConditionType && this._bUseDateRangeType && (oFieldViewMetadata.fControlConstructor === DateRangeSelection)) {
		...
		if (vConditionType) {
			...
			try {
				...
				if (fConditionTypeRequire) {
					this._enhanceConditionTypeInfo(fConditionTypeRequire, sConditionType, vConditionType, oFieldViewMetadata);
