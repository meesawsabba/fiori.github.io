...
sap.ui.define([
			...
			if (!bAvailableVariantManagement) {
				if (this.getEnableBasicSearch()) {
					...
					oBasicSearchControl.addDelegate(this._onAfterRenderingBasicSearchDelegate, this);
						...
						oFirstFilterFieldControl = aVisibleFiltersFieldArray[0].getControl();
			...
			for (i = 0; i < iLen; i++) {
				...
				for (j = 0; j < iFieldLen; j++) {
					...
					if (Array.isArray(aHiddenFields) && aHiddenFields.indexOf(oField.name) !== -1) {
						continue;
					} else if (oField.name === FilterProvider.BASIC_SEARCH_FIELD_ID) {
	...
	SmartFilterBar.prototype._handleEnter = function(oControl) {
		...
		oControl.attachBrowserEvent("keyup", function(e) {
			if (e.which === 13 && !oControl.__bSuggestInProgress && (oControl.isA("sap.m.InputBase") || oControl.isA("sap.m.Select"))) {
				this.search();
			}
		}.bind(this));
	};
	...
	SmartFilterBar.prototype._createFieldInAdvancedArea = function(oFilterGroup, oField) {
		oField.factory = function() {
			...
			if (oField.isCustomFilterField) {
				...
				this._attachCustomControlCustomDataChange(oFilterGroupItem.getControl().getCustomData());
			}
	 ...
	 * @param {string} sLabel the new QuickInfo text.
	 */
	...
	SmartFilterBar.prototype._getFiltersWithAssignedValues = function(bOnlyVisible) {
		...
		if (bOnlyVisible) {
			...
			oData = this._oFilterProvider.getFilledFilterData(this._getAllFilterAndParameterNames());
		}
	 ...
	 * @private
	 */
	SmartFilterBar.prototype._checkIfFilterHasValue = function (sFilterName) {
	...
	SmartFilterBar.prototype._getVisibleControlsLoadingPromises = function () {
		...
		this.determineMandatoryFilterItems().forEach(function (oFilterItem) {
			...
			if (
				...
				oControl.isBound("items") && // Items aggregation is bound to a model
...
//
	...
	SmartFilterBar.prototype.associateValueLists = function() {
		if (this.getSuppressValueListsAssociation() && this._oFilterProvider) {
			this._oFilterProvider._fireEvent(FilterProvider.ASSOCIATE_VALUE_LISTS);
			...
			this._fireValueListAnnotationLoaded();
	...
	SmartFilterBar.prototype._fireValueListAnnotationLoaded = function(oParameters) {
		if (this._oFilterProvider && Array.isArray(this._oFilterProvider.aFilterAnnotations)){
			Promise.all(this._oFilterProvider.aFilterAnnotations).then(function(){
	...
	SmartFilterBar.prototype.setCustomFilterData = function(oJson) {
		if (this._oFilterProvider && this._oFilterProvider.getModel()) {
			this._oFilterProvider.getModel().setProperty("/" + FilterProvider.CUSTOM_FIELDS_MODEL_PROPERTY, oJson);
		}
	};
	...
	SmartFilterBar.prototype.getCustomFilterData = function() {
		if (this._oFilterProvider && this._oFilterProvider.getModel()) {
			return this._oFilterProvider.getModel().getProperty("/" + FilterProvider.CUSTOM_FIELDS_MODEL_PROPERTY);
		}
	};
