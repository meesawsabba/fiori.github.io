...
sap.ui.define([
	...
	SmartTable.prototype._refreshHeaderText = function() {
		...
		if (this._bUpdateTableAnnouncement) {
	...
	SmartTable.prototype._toggleShowHideDetails = function(bValue) {
		...
		this._renderShowHideDetailsButton();
	};
	 ...
	 * @private
	 */
	SmartTable.prototype._renderShowHideDetailsButton = function() {
		var oResourceBundle = Core.getLibraryResourceBundle("sap.ui.comp"), sText;

		sText = this.bHideDetails ? oResourceBundle.getText("TABLE_SHOWDETAILS_TEXT") : oResourceBundle.getText("TABLE_HIDEDETAILS_TEXT");
		this._oShowHideDetailsButton.setTooltip(sText);
		this._oShowHideDetailsButton.setText(sText);
	};
	...
	SmartTable.prototype._createShowHideDetailsButton = function() {
		this._oShowHideDetailsButton = new Button(this.getId() + "-btnShowHideDetails", {
			visible: false,
			press: [
				function() {
					this._toggleShowHideDetails(!this.bHideDetails);
				}, this
			]
		});
	...
	SmartTable.prototype._addExportToExcelToToolbar = function() {
		...
		if (!this._oUseExportToExcel) {
			...
			if (this._oUseExportToExcel) {
				this._oUseExportToExcel.addStyleClass("sapUiCompSmartTableToolbarContent");
			}
		}
	...
	SmartTable.prototype._createExportColumnConfiguration = function(mCustomConfig) {
		var aColumns, i, iLen, oColumn, oColumnData, sLabel, sPath, sTemplate, sAdditionalPath, oAdditionalColumn, sSplitLabel, oAdditionalProperty, sHAlign, nWidth, sType, inputFormat, oType, oColumnDataType, aSheetColumns = [], bUtcDefault, bUtc, vUseUTCDateTime;
			...
			if (oColumn.getVisible()) {
				...
				if (sPath) {
					...
					if (oColumnData.unit) {
						...
						/* Ignore description in case of split cells and displayBehavior "idOnly" */
						if (!bSplitCells) {
							sTemplate = FormatUtil.getFormattedExpressionFromDisplayBehaviour(oColumnData.displayBehaviour, "{0}", "{1}");
						} else if (oColumnData.displayBehaviour === "descriptionOnly") {
							sPath = oColumnData.description;
						} else if (oColumnData.displayBehaviour !== "idOnly") {
					...
					aSheetColumns.push({
						...
						property: oColumnData.description && !bSplitCells ?
	...
	SmartTable.prototype.openDialogForKeyUser = function(sStyleClass, fCallBack) {
		...
		this._oPersController.openDialog({showReset: false, useAvailablePanels: true, styleClass: sStyleClass});
	};
	...
	SmartTable.prototype._initialiseMetadata = function(oEvt) {
		...
		if (!oControl.bIsInitialised) {
			ODataModelUtil.handleModelInit(oControl, oControl._onMetadataInitialised, true);
		}
	};
	...
	SmartTable.prototype._onMetadataInitialised = function() {
		...
		var oP13nDialogSettings = this._getP13nDialogSettings();

		// apply changes to p13nDialogSettings if changed and the personalizationController is not yet intialized
		if (!deepEqual(this._oP13nDialogSettings, oP13nDialogSettings)) {
			this._oP13nDialogSettings = oP13nDialogSettings;
			this._bIsFilterPanelEnabled = (this._oP13nDialogSettings && this._oP13nDialogSettings.filter) ? this._oP13nDialogSettings.filter.visible !== false : true;
			this._bIsSortPanelEnabled = (this._oP13nDialogSettings && this._oP13nDialogSettings.sort) ? this._oP13nDialogSettings.sort.visible !== false : true;
	...
	SmartTable.prototype._aStaticProperties = [
		"entitySet", "ignoredFields", "initiallyVisibleFields", "ignoreFromPersonalisation", "tableType", "useTablePersonalisation", "enableAutoBinding", "persistencyKey", "smartFilterId", "showDetailsButton", "enableAutoColumnWidth"
	];

	SmartTable.prototype._aToolbarRelatedProperties = [
		"useExportToExcel", "exportType", "showFullScreenButton", "showPasteButton", "editTogglable", "placeToolbarInTable"
	];
	...
	SmartTable.prototype._createTableProvider = function() {
		var oModel, sEntitySetName, sIgnoredFields, bPreserveDecimals;
	...
	SmartTable.prototype._onColumnPress = function(oEvent) {
		...
		this._aSortButton[0].setVisible(bSortButtonVisible);
		this._aSortButton[1].setVisible(bSortButtonVisible);
	 ...
	 * @private
	 */
	SmartTable.prototype._onCustomSort = function(oEvent, bAscend) {
		var oColumnData = this._oColumnClicked.data("p13nData");
		...
		if (oColumnData) {
			if (oColumnData.sorted && oColumnData.sorted.ascending === bAscend) {
				...
				this._oColumnClicked.setSortIndicator(sSortOrder);
			} else {
				oColumnData.sorted = {
					"ascending": bAscend
				};
	 ...
	 * @param {string[]} [mConfig.additionalProperty] The additional visible fields of the column on top of the additional fields of the metadata
	...
	SmartTable.prototype._calcColumnWidth = function(oField, bAdditionalProperty, mConfig) {
		...
		if (mConfig.additionalProperty) {
			sAdditionalProperty = mConfig.additionalProperty.concat(sAdditionalProperty.split(",")).filter(function(sProperty, iIndex, aProperties) {
			    return sProperty && aProperties.indexOf(sProperty) == iIndex;
			}).join(",");
		}
		...
		if (mConfig.label) {
			...
			oField.labelWidth = fLabelWidth = TableUtil.calcHeaderWidth(sLabel, fWidth, iMax, iMin);
		}
	...
	SmartTable.prototype._setWidthForCustomColumn = function(oColumn) {
		...
		var sVisibleProperty = vAutoColumnWidth.visibleProperty || [oColumnData.leadingProperty, oColumnData.additionalProperty].join(",");
		...
		oField.label = this._getColumnLabel(oColumn);
		var fWidth = this._calcColumnWidth(oField, false, {
			additionalProperty: aVisibleProperty,
	...
	SmartTable.prototype._updateVisibleColumnsWidthForEdit = function() {
		if (this._oTable && this._oTableProvider && this.getEnableAutoColumnWidth()) {
			this._oTable.getColumns().forEach(function(oColumn) {
				if (oColumn.getVisible()) {
					var oColumnData = oColumn.data("p13nData");
					var sLeadingProperty = oColumnData && oColumnData.leadingProperty;
					if (sLeadingProperty) {
	...
	SmartTable.prototype._createColumn = function(oField, sId) {
		var oColumn = new Column(sId, {
			...
			showSortMenuEntry: oField.sortable && this._bIsSortPanelEnabled,
		...
		if (!oField.width && this.getEnableAutoColumnWidth()) {
			oColumn.setWidth(this._calcColumnWidth(oField) + "rem");
		}
	...
	SmartTable.prototype._createAnalyticalColumn = function(oField, sId) {
		...
		var oColumn = new AnalyticalColumn(sId, {
			...
			template: oField.template,
			groupHeaderFormatter: FormatUtil.getInlineGroupFormatterFunction(oField, true, this._getDateFormatSettings(), this._bPreserveDecimals)
		});
	...
	SmartTable.prototype._createInfoToolbar = function() {
		...
		if (this._isMobileTable) {
			...
			if (iFiltersLength) {
				...
				var sFilterText = oResourceBundle.getText("SMARTTABLE_INFOTOOLBAR_FILTERED_BY", [oListFormat.format(aFiltersColumnLabel)]);
		...
		if (this._isMobileTable && this._oCurrentVariant.group && this._oCurrentVariant.group.groupItems) {
			...
			if (sPath) {
				...
				if (oFieldMetadata) {
					...
					fnGroupFunction = FormatUtil.getInlineGroupFormatterFunction(oFieldMetadata, false, this._getDateFormatSettings(), this._bPreserveDecimals);
				}
						...
						if (oColumnData.edmType === "Edm.String" || oColumnData.type === "string") {
							if (!oColumnData.nullable) {
								...
								aFilterArrReference.push(new Filter({
									and: !!oModelItem.exclude,
									filters: [
										new Filter(sPath, sFilterOperator, ""),
										new Filter(sPath, sFilterOperator, null)
									]
								}));
							}
						} else if (oColumnData.nullable) {
