...
sap.ui.define([
	...
	FilterController.prototype.getPanel = function(oPayload) {
		...
		var oSmartFilter = this._getSmartFilterBar();
		this._aDropdownFields = oSmartFilter && oSmartFilter._oFilterProvider && oSmartFilter._oFilterProvider._aFilterBarDropdownFieldMetadata;
			...
			sap.ui.require([
				...
				var fnSuggestCallback = function(oControl, sFieldName) {
					...
					if (oControl.isA("sap.m.ComboBox") || oControl.isA("sap.m.MultiComboBox")) {
						...
						sDisplayBehaviour = "idOnly"; // set default display behavior for dropdowns
						if (oSmartFilter && oSmartFilter._oFilterProvider) {
							sDisplayBehaviour = oSmartFilter._oFilterProvider._sTextArrangementDisplayBehaviour;
							this._aDropdownFields.forEach(function(oField) {
								if (oField.name === sFieldName) {
									var oResult = oField["com.sap.vocabularies.Common.v1.Text"];
									if (oResult) {
										sDisplayBehaviour = oSmartFilter._oFilterProvider._oMetadataAnalyser.getTextArrangementValue(oResult);
									} else if (oField["com.sap.vocabularies.UI.v1.TextArrangement"]) {
										sDisplayBehaviour = oSmartFilter._oFilterProvider._oMetadataAnalyser.getTextArrangementValue(oField);
									}
								}
							});
						}
