...
// Provides Controller
sap.ui.define([
	'./UIManager', 'sap/base/Log', 'sap/ui/thirdparty/jquery', 'sap/ui/base/ManagedObject', 'sap/m/library', 'sap/ui/comp/library', './ColumnsController', './FilterController', './GroupController', './SortController', './DimeasureController', './SelectionController', './Util', './ChartWrapper', './SelectionWrapper', './ColumnHelper', 'sap/m/P13nDialog', './Validator', 'sap/ui/model/json/JSONModel', 'sap/ui/Device', 'sap/ui/model/BindingMode'
], function(COMPUIManager, Log, jQuery, ManagedObject, MLibrary, CompLibrary, ColumnsController, FilterController, GroupController, SortController, DimeasureController, SelectionController, Util, ChartWrapper, SelectionWrapper, ColumnHelper, P13nDialog, Validator, JSONModel, Device, BindingMode) {
	"use strict";
	...
	Controller.prototype.reset = function(o, aKeys) {
		return new Promise(function(resolve){
			...
			for (var sKey in this._oSettingCurrent) {
				...
				if (oSubController && oSubController.getAdaptationUI() && oSubController.getAdaptationUI().setP13nModel) {
					oSubController.getAdaptationUI().setP13nModel(oSubController.getAdaptationModel());
				}
			}
			resolve();
		}.bind(this));
	...
	Controller.prototype.validateP13n = function(vControl, sKey, oAdaptationUI) {
		...
		return Validator.checkGroupAndColumns(sTableType, this._oSettingCurrent, oColumnKey2ColumnMap, oControlDataReduceTotal, []).then(function(aResultTotal) {
			...
			return Validator.checkSaveChanges(sTableType, this._oSettingCurrent, oPersistentDeltaData, aResultTotal);
		}.bind(this)).then(function(aResultTotal) {
			...
			if (aResultTotal.length > 0) {
				aResultTotal.forEach(function(oValidation){
					for (var sKey in this._oSettingCurrent) {
						...
						if (oValidation.panelTypes.indexOf(sKey) > -1 && oSubController){
							oSubController.getAdaptationUI().setMessageStrip(new sap.m.MessageStrip({
				...
				for (var sKey in this._oSettingCurrent) {
					var oSubController = this._oSettingCurrent[sKey].controller;
					if (oSubController && oSubController.getAdaptationUI() && oSubController.getAdaptationUI().isA("sap.ui.mdc.p13n.panels.BasePanel")) {
	...
	Controller.prototype.handleP13n = function() {
		return new Promise(function(resolve){
			...
			if (oWrapper.isA("sap.ui.mdc.p13n.panels.Wrapper")) {
