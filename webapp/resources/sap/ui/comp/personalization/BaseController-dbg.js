...
sap.ui.define([
	...
	BaseController.prototype.getAdaptationModel = function() {
		var oTransientData = this.getInternalModel().getProperty("/transientData");
		...
		if (!this._oAdaptationModel) {
			var oAdaptationModel = new JSONModel({
				items: aItems,
				presenceAttribute: this._getPresenceAttribute()
			});
			this._oAdaptationModel = oAdaptationModel;
			this._oAdaptationModel.setSizeLimit(10000);
		} else {
			this._oAdaptationModel.setProperty("/items", aItems);
		}
