...
sap.ui.define([
	 ...
	 *        case, the FilterBar consists of two logical areas, one containing the search field with Go button and one - the advanced area.
	 *        The filters are aligned in a grid with labels above each. The number of items per row depends on the width of the value help dialog.
     *        By default, the filter bar will show up to 8 filters. If there are more than 8 filters, a new button 'Show All Filters' appears
	 *        in the place of the 8th filter. Clicking on this button displays all filters.
					...
					if (oElement.items) {
						for (i = 0; i < oElement.items.length; i++) {
							...
							if (oItem && oItem.filterItem && oItem.filterItem.getVisible()) {
								if (bConsiderOnlyVisibleFields) {
									if (oItem.filterItem.getVisibleInFilterBar() || this._checkIfFilterHasValue(oItem.filterItem.getName())/* || oItem.filterItem.getPartOfCurrentVariant()*/) {
...
// };
	 ...
	 * @param {array} of filter items marked as non partOfCurrentVariant
		...
		if (this._oAdaptFiltersDialog) {
			...
			for (var i = 0; i < aItems.length; i++) {
				if (aItems[i].hasOwnProperty("filterItem")) {
					...
					if (aItems[i].isFiltered !== bHasValue) {
						aItems[i].isFiltered = bHasValue;
//
//						bChange = true;
					}
				}
				...
				if (!this._oAdaptFiltersDialog) {
					...
					var fChange = function(oEvent) {
						var i,
							...
							bVisible = sReason === "Add" || sReason === "SelectAll";
					...
					oFilterPanel.setItemFactory(function(oContext) {
						...
						if (!oItem.contentReplaced && oItem.container && oItem.container.getContent()[1] === oControl) {
							...
							oItem.container.removeContent(oItem.control);
							oItem.container.insertContent(oItem.control.clone(), 1);
						}
...
// }
	...
	FilterBar.prototype.fetchVariant = function() {
		...
		var aFiltersInfo, oVariant = {}, sBasicSearch, mFilterValues;
