...
sap.ui.define([
	...
	ContentConnector.prototype._observeChanges = function(change) {
		if (change.name === "contentResources") {
			// if a contentResource is being removed do not schedule an
			// update (which means waiting for another resource to be added before the current one is cleaned)
			// rather clean everything up immediately
			if (change.mutation === "remove") {
				this._setContent(null, null);
			} else {
	...
	ContentConnector.removeContentManagerResolver = function(resolver) {
		var isFunction = typeof resolver === "function",
			...
			isObject = typeof resolver === "object",
		...
		for (var i = 0, count = resolvers.length; i < count; ++i) {
			if (isFunction || isString) {
				...
				if (typeof resolvers[i] === "object" && resolver.pattern && resolvers[i].pattern === resolver.pattern) {
					resolvers.splice(i, 1);
					return true;
				}
			}
