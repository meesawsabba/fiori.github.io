...
sap.ui.define([
	...
	Viewport.prototype._handleResize = function(event) {
		...
		this._camera.update(this._width, this._height, false, true);
	 ...
	 */
	Viewport.prototype.hitTest = function(x, y, withTolerance) {
	...
	Viewport.prototype.zoomTo = function(what, nodeRef, crossFadeSeconds, margin) {
		...
		var activateCamera = function(boundingBox) {
	...
	Viewport.prototype.hover = function(x, y) {
		...
		if (this._hotspotElement && this._hotspotElement !== elem) {
			...
			delete this._hotspotElement._isHovered;
			delete this._hotspotElement._hoverColor;
			if (!this.getShowAllHotspots()) {
				if (!this._hotspotElement.isSelected(vsm._mask)) {
					this._hotspotElement.resetOpacity();
				}
			}
		...
		if (elem && elem._vkGetNodeContentType() === NodeContentType.Hotspot) {
			...
			// Setting a property on the Element that determines how it is rendered
			// should be moved to ViewStateManager
			this._hotspotElement._isHovered = true;
			this._hotspotElement._hoverColor = this.getHotspotColorABGR();
			if (!this.getShowAllHotspots()) {
				if (!this._hotspotElement.isSelected(vsm._mask)) {
					var alpha = abgrToColor(this.getHotspotColorABGR()).alpha;
					elem.setOpacity(alpha);
				}
			}
	...
	Viewport.prototype.setShowAllHotspots = function(value) {
		...
		if (vsm) {
			...
			var showAllTintColour = this.getShowAllHotspotsTintColor();
			nodes.forEach(function(node) {
				if (node.opacity !== undefined && node.domRef) {
					if (value) {
						// Setting a property on the Element that determines how it is rendered
						// should be moved to ViewStateManager
						node._alwaysVisible = true;
						node.setTintColor(node.sMask, showAllTintColour);
						node.setOpacity(1);
					} else if (node !== hotspotElement) {
						delete node._alwaysVisible;
						node.setTintColor(node.sMask, null);
						node.resetOpacity();
					}
				}
			});
	...
	Viewport.prototype.showHotspots = function(nodeRefs, show, color) {
		...
		// if the highlight color is not passed as argument, we use the default highlight color
		var highlightColor = color || this.getHotspotColorABGR();

		nodeRefs.forEach(function(node) {
			if (show) {
				node._alwaysVisible = true;
				node.setTintColor(0, highlightColor);
				node.setOpacity(1);
			} else {
				delete node._alwaysVisible;
				node.setTintColor(0, null);
				node.resetOpacity();
			}
		}, this);
