...
sap.ui.define([
	...
	var SceneBuilder = function(rootNode, contentResource, resolve, reject) {
		...
		this._parametrics = new Map();			// parametricId -> parametric content
		...
		this._detailsByImageId = new Map();		// Image details by image id
	...
	SceneBuilder.prototype.createNode = function(nodeInfo, sceneId) {
		...
		if (nodeInfo.parametricId) {
			...
			var parametricContent = this._parametrics.get(nodeInfo.parametricId);
			if (parametricContent) {
				this.setParametricContent(nodeInfo.sid, parametricContent, sceneId);
			}
	...
	SceneBuilder.prototype.setParametricContent = function(nodeId, parametricContent, sceneId) {
		...
		this._parametrics.set(parametricContent.id, parametricContent);
	...
	SceneBuilder.prototype.createImageNote = function(annotation, sceneId) {
		...
		// When called from SceneContext nodeId can be present on annotation. This can be a limitation
