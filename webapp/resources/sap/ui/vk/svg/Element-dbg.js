...
sap.ui.define([
	...
	Element.prototype._updateColor = function(mask) {
		...
		if (domRef !== null) {
			...
			if (this.nodeContentType === NodeContentType.Hotspot && !this._hasHostpotAncestor()) {
				if (this.isSelected(mask)) {
					domRef.setAttribute("filter", "");
				} else {
	...
	Element.prototype.getHotspotEffectDef = function() {
		if (this.getTintColor() == null) {
			return {
				name: "hotspot-effect"
			};
		}

		var color = cssColorToColor(this.getTintColor(), 1);
		return {
			name: "hotspot-effect-" + color.red + color.green + color.blue + color.alpha,
			color: color
		};
	};
	...
	Element.prototype.setSelected = function(mask, selected, highlightColor) {
		...
		if (this.opacity !== undefined) {
			var alpha = abgrToColor(highlightColor).alpha;
			if (selected) {
	...
	Element.prototype._setOpacity = function(opacity) {
		if (this.domRef !== null) {
			if (this.parent && ((this.domRef === this.parent.domRef) ^ isSimpleGroupElement(this))) {
				this.invalidate();
	...
	Element.prototype.setOpacity = function(opacity) {
		...
		allLeafNodes.forEach(function(l) {
			...
			l._setOpacity(setOpacity);
		});
	};

	Element.prototype.resetOpacity = function() {
		if (this._alwaysVisible) {
			this.setOpacity(1);
		} else if (this._isHovered) {
			var hoverAlpha = abgrToColor(this._hoverColor).alpha;
			this.setOpacity(hoverAlpha);
		} else if (this.getTintColor()) {
			this.setOpacity(cssColorToColor(this.getTintColor()).alpha);
		} else {
			this.setOpacity(0);
		}
	};
	...
	Element.prototype._setBaseAttributes = function(setAttributeFunc, mask) {
		...
		if (this.nodeContentType === NodeContentType.Hotspot && !this.isSelected(mask)) {
	...
	Element.prototype._getParametricShape = function(fillStyles, lineStyles, textStyles) {
		...
		if (this.fill !== undefined) {
			parametric.fill = fillStyles.length;
			fillStyles.push({ colour: colorToCSS(this.fill) });
		}

		if (this.stroke !== undefined && this.stroke[ 3 ] > 0 && this.strokeWidth) {
			parametric.stroke = lineStyles.length;
			var lineStyle = { colour: colorToCSS(this.stroke), width: this.strokeWidth };
			if (this.strokeDashArray.length > 0) {
	...
	Element.prototype._hasHostpotAncestor = function() {
		var ancestor = this.parent, hasHotspotAncestor = false;
		while (ancestor && !hasHotspotAncestor) {
			if (ancestor.nodeContentType === NodeContentType.Hotspot) {
				hasHotspotAncestor = true;
			}
			ancestor = ancestor.parent;
		}

		return hasHotspotAncestor;
	};
