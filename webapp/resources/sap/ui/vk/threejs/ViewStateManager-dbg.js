...
sap.ui.define([
	...
	/**
	 * Get visibility state of the node taking its ancestors' visibility into account.
	 * @param {any} nodeRef The node reference
	 * @returns {boolean} node's visibility state
	 *
	 * @private
	 */
	ViewStateManager.prototype._getEffectiveVisibility = function(nodeRef) {
		var result = this.getVisibilityState(nodeRef);

		var currentNode = nodeRef.parent;
		while (result && currentNode) {
			result = result && this.getVisibilityState(currentNode);
