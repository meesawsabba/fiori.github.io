...
sap.ui.define([
	...
	Billboard.prototype._billboardViewUpdate = function(renderer, camera) {
		...
		if (this._vkGetNodeContentType() === NodeContentType.Symbol) {
			// scale billboard view in 360 scene
			var fov = THREE.Math.degToRad(camera.fov);
			var dir = camera.getWorldDirection(new THREE.Vector3());
			var pos = this.getWorldPosition(new THREE.Vector3());
			var theta = Math.asin(pos.distanceTo(dir.multiplyScalar(500)) / 1000);
			this.baseScale = this.baseScale ? this.baseScale : this.scale.clone();
			this.scale.copy(this.baseScale).multiplyScalar(fov * Math.cos(2 * theta));
			var dist = camera.position.z;
			if (dist) {
				var ratio = (99 - dist) / 99;
				this.scale.multiplyScalar(ratio);
			}
		}
