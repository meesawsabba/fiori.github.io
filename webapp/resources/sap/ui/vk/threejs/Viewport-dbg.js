...
sap.ui.define([
	...
	Viewport.prototype.onSetViewStateManager = function(viewStateManager) {
		...
		viewStateManager.attachVisibilityChanged(this.setShouldRenderFrame, this);
	...
	Viewport.prototype.onUnsetViewStateManager = function(viewStateManager) {
		...
		viewStateManager.detachVisibilityChanged(this.setShouldRenderFrame, this);
	...
	Viewport.prototype.setCamera = function(camera) {
		...
		if (cam && this._renderer) {
			this._renderer.getSize(viewportSize);
			cam.update(viewportSize.x, viewportSize.y);
	...
	Viewport.prototype.hitTest = function(x, y) {
		...
		return this._hitTester.hitTest(x - element.clientLeft, y - element.clientTop, element.clientWidth, element.clientHeight,
			this._renderer, nativeScene, nativeCamera, this._clippingPlanes);
	};
	...
	Viewport.prototype._isPanoramicActivated = function() {
		return this._backgroundProjection === "spherical" || (this._viewStateManager && this._viewStateManager.getBackgroundImageType()) === "360Image";
	};
	...
	Viewport.prototype.tap = function(x, y, isDoubleClick) {
		...
		} else if (!this.getFreezeCamera() && !this._isRedlineActivated()) {
	...
	Viewport.prototype.setSelectionRect = function(rect) {
		...
		this._renderer.getSize(viewportSize);
		var x1 = (rect.x1 / viewportSize.x) * 2 - 1,
			y1 = (rect.y1 / viewportSize.y) * -2 + 1,
			x2 = (rect.x2 / viewportSize.x) * 2 - 1,
			y2 = (rect.y2 / viewportSize.y) * -2 + 1;
	...
	Viewport.prototype.getObjectImage = function(nodeRef, width, height, topColor, bottomColor, quaternion, margin) {
		...
		if (quaternion) {
			cameraRef.quaternion.copy(quaternion);
		...
		eyePointLight.updateMatrix();
		eyePointLight.updateMatrixWorld(true);
		nodeRef.add(lightNode);

		renderer.render(nodeRef, cameraRef);

		nodeRef.remove(lightNode);
	...
	Viewport.prototype._activateSingleView = function(view, playViewGroup, notAnimateCameraChange) {
		...
		this._backgroundProjection = view.userData && view.userData.viewInfo && view.userData.viewInfo.backgroundProjection;
	 ...
	 * More information about normalized coordinates can be found here: {@link https://learnopengl.com/Getting-started/Coordinate-Systems}
	 */
	Viewport.prototype.projectToScreen = function(x, y, z, camera) {
		...
		return {
			x: screenPoint.x * halfWidth + halfWidth,
			y: screenPoint.y * halfHeight + halfHeight,
			depth: screenPoint.z
		};
	};
	...
	Viewport.prototype.normalizeRectangle = function(x, y, width, height) {
		var saComputedStyle = window.getComputedStyle(this.getSafeArea().getDomRef());
	...
	Viewport.prototype.deNormalizeRectangle = function(x, y, width, height) {
		var computedStyle = window.getComputedStyle(this.getSafeArea().getDomRef());
