...
sap.ui.define([
			...
			var ThreeJsCameraController = function(vp) {
				...
				this.zoom = function(zoomFactorScaler) {
					...
					if (!camera.userData.isRedlineActivated) {
						...
						var bgImageType = viewport._viewStateManager && viewport._viewStateManager.getBackgroundImageType();
						var ignoreDynamicObjects = bgImageType ? false : true;  // do not ignore background image node
					...
					if (camera.isPerspectiveCamera) {
						if (viewport._isPanoramicActivated()) {
							var newFov = zoomFactorScaler > 1 ? camera.fov - 1 : camera.fov + 1;
							camera.fov = Math.min(Math.max(5, newFov), 90);
						} else if (camera.view && camera.view.enabled) { // Redline mode is activated
	...
	ViewportGestureHandler.prototype.zoomTo = function(boundingBox, quaternion, margin, timeInterval, nodeRef, isIn) {
		var camera = this._viewport.getCamera().getCameraRef();
		if (camera.userData.isRedlineActivated) {
			Log.error("zoomTo() method is disabled when Redlining is activated");
			return this;
		}
	...
	ViewportGestureHandler.prototype.zoomObject = function(nodeRef, isIn, timeInterval) {
		var camera = this._viewport.getCamera().getCameraRef();
		if (camera.userData.isRedlineActivated) {
			Log.error("zoomObject() method is disabled when Redlining is activated");
			return this;
		}
