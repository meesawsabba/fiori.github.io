...
sap.ui.define([
	...
	THREE.Object3D.prototype._vkUpdateMaterialColor = function() {
		...
		if (userData.originalMaterial) {
			if (userData.originalMaterial.color.r === undefined) {
			...
			} else if (userData.originalMaterial.userData && userData.originalMaterial.userData.needsUpdateTexture) {
				...
				delete userData.originalMaterial.userData.needsUpdateTexture;
			} else {
		...
		if (userData.highlightColor !== undefined || userData.tintColor !== undefined || userData.highlightingColor !== undefined) {
			if (!userData.originalMaterial) {
				...
				if (userData.originalMaterial.userData && userData.originalMaterial.userData.needsUpdateTexture) {
					// textures are already loaded, no need to update again
					delete userData.originalMaterial.userData.needsUpdateTexture;
				}
