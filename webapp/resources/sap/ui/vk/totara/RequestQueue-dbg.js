sap.ui.define([
	...
	RequestQueue.prototype.generateRequestCommand = function() {
		...
		if (!this.meshes.isEmpty()) {
			...
			var expand = ["nodes", "bounds", "highlight", "annotation"];
			if (this.context.includeAnimation) {
				expand.push("animation");
			}
			if (this.context.includeParametric) {
				expand.push("parametric");
			}
			if (this.context.includeUsageId) {
				expand = expand.concat(IncludeUsageIdType.to$expandQueryParameter(this.context.includeUsageId));
			}
			if (this.context.pushPMI) {
				expand.push("pmi");
			}
			var selectField = "name,transform,meshId,annotationId,materialId,contentType,visible,opacity,renderOrder,entityId,highlightStyleId,closed";
			...
			command = {
				...
				parameters: {
					...
					viewId: ids[0].viewId,
					query: {
						hidden: this.context.includeHidden,
						$select: selectField,
						$expand: expand.join(",")
					}
				}
