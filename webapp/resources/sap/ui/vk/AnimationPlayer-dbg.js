...
sap.ui.define([
	...
	AnimationPlayer.prototype._collectSequenceNodeChanges = function(time, nodeChanges, playback, infiniteTracksOnly) {
		var sequence = playback.getSequence();
		var animations = sequence.getNodeAnimation();
		if (!animations) {
			return;
	...
	AnimationPlayer.prototype._collectPlaybackNodeChanges = function(currentTime, nodeChanges, playback) {
		...
		if (timeInPlayback < playback.getPreDelay()) {
			// reset view to start
			if (timeInPlayback === 0) {
				this._collectSequenceNodeChanges(sequenceTime, nodeChanges, playback, false);
			}
		} else if (timeInPlayback > playback.getPreDelay() + playbackPlayTime + tolerance) {
			// apply only infinite tracks
			this._collectSequenceNodeChanges(sequenceTime, nodeChanges, playback, true);
		} else {
