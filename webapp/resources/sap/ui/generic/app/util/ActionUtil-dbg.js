...
sap.ui.define([
	...
	ActionUtil.prototype.call = function (sFunctionImportPath, sFunctionImportLabel, bIsDraftEnabled, oSkipProperties, bIsStrict, mAdditionalParmData) {
		...
		return new Promise(function (resolve, reject) {
			...
			var fnPrepareParamAndInvokeAction = function () {
				...
				var oGetDefaultValuesPromise = that.getApplicationController().getTransactionController().getDefaultValues(aContexts[0] , mActionParams[0].parameterData, undefined , sFunctionName);
				if (oGetDefaultValuesPromise instanceof Promise) {
					var fnGetPredefinedValues = function (oResponse) {
						mActionParams[0].parameterData = oResponse;
		...
		aContexts.forEach(function(oSingleContext) {
			...
			if (this._oFunctionImport.parameter) {
				...
				for (var i = 0; i < this._oFunctionImport.parameter.length; i++) {
					...
					if (!skip && !bIsKey && oParameter.mode.toUpperCase() == "IN") {
	 ...
	 * @private
	 */
	ActionUtil.prototype._setAdditionalParameters = function (aAdditionalParameters, sKey, aParameterData, ctx) {
		aAdditionalParameters.filter(function (oAdditionalParameter){
			if (sKey === oAdditionalParameter.name) {
				if (oAdditionalParameter.hasOwnProperty("com.sap.vocabularies.UI.v1.Hidden")) {
					// set context value to model for multiple selection if UI Hidden Annotation exists
	...
	ActionUtil.prototype._initiateCall = function (mActionParams, bIsDraftEnabled, bIsStrict) {
		...
		if (mActionParam != undefined && mActionParam.additionalParameters && mActionParam.additionalParameters.length == 0) {
			...
			Promise.all(contexts).then(function (oActionContexts) {
				var oActionContext = oActionContexts[0];
				// get all additional params keys
				var _additionalParametersKeys = mActionParam.additionalParameters.map(function (param) {
					return param.name;
				});

				// set action context value to oModel for single selection & multiple selection
				oActionContexts.forEach(function(ctx, i){
					var _parameterData = mActionParams[i].parameterData;
					for (var sKey in _parameterData) {
						// set default value(undefined) for multiple selection
						if (oActionContexts.length > 1 && _additionalParametersKeys.indexOf(sKey) > -1) {
							that._setAdditionalParameters(mActionParams[i].additionalParameters, sKey, _parameterData, ctx);
						} else {
							// set context value to model for single selection
							ctx.oModel.setProperty(sKey, _parameterData[sKey], ctx);
						}
					}
				});
				if (bIsStrict) {
							...
							press: function (oEvent) {
								if (mParameterForm.hasNoClientErrors()) {
									if (mParameterForm.getEmptyMandatoryFields().length == 0) {
										...
										bActionPromiseDone = that._triggerActionPromise(oFuncHandles, aEntityContext, mActionParam, oActionContexts, bIsStrict);
									} else {
					...
					if (aFormElements.length === 0 || bIsEmptyPopup){
						that._triggerActionPromise(oFuncHandles, aEntityContext, mActionParam, oActionContexts, bIsStrict);
						oParameterDialog.destroy();
					} else {
						oParameterDialog.open();
					}
				} else {
					that._triggerActionPromise(oFuncHandles, aEntityContext, mActionParam, oActionContexts);
				}
			}.bind(this));
