...
sap.ui.define(["sap/ui/base/EventProvider", "sap/ui/generic/app/util/ModelUtil", "sap/ui/generic/app/util/DraftUtil", "sap/ui/generic/app/util/Queue", "sap/base/Log"],
	function (EventProvider, ModelUtil, DraftUtil, Queue, Log) {
		...
		var BaseController = EventProvider.extend("sap.ui.generic.app.transaction.BaseController", {
			metadata: {
				publicMethods: [
					"hasClientMessages", "triggerSubmitChanges", "attachFatalError", "detachFatalError", "destroy", "updateMultipleEntities"
				]
			},
		...
		BaseController.prototype._updateEntity = function (sContextPath, oUpdateData) {
			var that = this;
			return  new Promise(function (resolve, reject) {
				var mCallBacks = that._getRequestCallbacks(resolve, reject);
				var mParameters = {
		 ...
		 *
		 * @param {array} context path and data to be updated
		 * @public
		 */
		BaseController.prototype.updateMultipleEntities = function (aContextToBeUpdated) {
			var aPromises = [];
			for (var i = 0; i < aContextToBeUpdated.length; i++) {
				var oPromise = this._updateEntity(aContextToBeUpdated[i].sContextPath, aContextToBeUpdated[i].oUpdateData);
