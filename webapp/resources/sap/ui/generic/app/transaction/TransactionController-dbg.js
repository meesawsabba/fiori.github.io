...
sap.ui.define(["sap/ui/thirdparty/jquery", "./BaseController", "./DraftController", "sap/ui/generic/app/util/ModelUtil"], function (jQuery, BaseController, DraftController, ModelUtil) {
	...
	var TransactionController = BaseController.extend("sap.ui.generic.app.transaction.TransactionController", {
		metadata: {
			publicMethods: [
				"destroy", "setBatchStrategy", "getDraftController", "invokeAction", "editEntity", "deleteEntity", "deleteEntities", "propertyChanged", "hasClientValidationErrors", "resetChanges", "getDefaultValues", "getDefaultValuesFunction"
			]
		},
	 ...
	 * The values in oPredefinedValue is written over the values of default values
	 * If DVF is not annotated then oPredefinedValue is returned unmodified.
	 * @param {object} oContext the given binding context
	 * @param {object} oPredefinedValue Existing predefined values
	 ...
	 * @private
	 */
	TransactionController.prototype.getDefaultValues = function (oContext, oPredefinedValue, sNavigationProperty, sFunctionImportName) {
		var oEntityObject;
		...
		if (sDefaultValueFunctionPath) {
			...
			var oPromise = this._callAction(sDefaultValueFunctionPath, oContext, mParameters).then(function (oResponse) {
				if (oResponse.httpResponse.statusCode === "200") {
					var aParts = sDefaultValueFunctionPath.split("/");
					var sFunctionImportName = aParts[1];
					//this logic is added because of the current response type and will be modified if the response type changes.
					var oDefaultValue = oResponse.responseData[sFunctionImportName] || oResponse.responseData;
					if (oPredefinedValue) {
						oPredefinedValue = that.prepareDefaultValues(oPredefinedValue, oDefaultValue);
					} else {
						oPredefinedValue = oDefaultValue;
					}
					return oPredefinedValue;
				}
			});
	...
	/**
	 * @param {object} oExistingValues Existing predefined values
	 * @param {object} oDefaultValues  default values received from default values function
	 * Loop through oExistingValues properties and if property is not null or empty assign that value to oDefaultValue's property.
	 * Hence oExistingValue's property is null or empty override that with oDefaultValue's property
	 * @private
	 */
	TransactionController.prototype.prepareDefaultValues = function (oExistingValues, oDefaultValues) {
		for (var sPropertyName in oExistingValues) {
			oDefaultValues[sPropertyName] = ( oExistingValues[sPropertyName] && oExistingValues[sPropertyName] !== null && oExistingValues[sPropertyName] !== "" ) ? oExistingValues[sPropertyName] : oDefaultValues[sPropertyName];
		}
		return oDefaultValues;
	};
