...
sap.ui.define([
			...
			if (aResults.find(function(bResult){
			    return bResult;
					...
					var oPromise = that.triggerSubmitChanges({
						...
						noBlockUI: true,
						urlParameters: {},
						draftSave: that._oModel.hasPendingChanges()
					});
	...
	ApplicationController.prototype._executeFieldGroup = function (oGroup, mRequests, oView) {
		...
		return this._hasClientErrors(oGroup.uuid, oView).then(function () {
			...
			if (!that._oGroupChanges[sSideEffectsQualifier] && !that._oModel.hasPendingChanges()) {
	...
	ApplicationController.prototype._executeSideEffects = function (oSideEffects, oContext, mParameters, mRequests) {
		var that = this, fFunction, sTriggerAction;

		// check if side effect has trigger action
		if (that.getTransactionController().getDraftController().getDraftContext().hasDraft(oContext)) {
			if (oSideEffects.TriggerAction && oSideEffects.TriggerAction.String) {
	...
	ApplicationController.prototype._hasClientErrors = function (sGroupId, oView) {
		...
		aControls.forEach(function(oControl){
			...
			aCheckValidityPromise.push(oParentControl && oParentControl.checkValuesValidity && oParentControl.checkValuesValidity());
		});
