...
sap.ui.define(["sap/ui/base/Object",
...
], function(BaseObject, merge, extend, isEmptyObject, UriParameters, ComponentContainer, HashChanger, History, coreLibrary, FeError, FeLogger, oDataModelHelper, ProcessObserver, Queue, testableHelper, CRUDHelper, routingHelper, startupParameterHelper,
			TemplateComponent, ControlPersonalizationAPI) {
	"use strict";
	...
	function getMethods(oTemplateContract, oNavigationControllerProxy) {
		...
		// bEnablePlaceholder holds the placeholder information from the URL
		var bEnablePlaceholder;
		...
		// Note that during navigation it is sometomes necessary to have access to both information (source and target identity of the navigation).
		...
		function fnComeBack(oLeaveHash, oSerializedBackOption){
		...
		function fnInitSession(bIsFirst){
			...
			if (bIsRestoring){
				...
				if (aSessions.length){ // the old session can be restored from our own memory
					...
					fnComeBack(oOldSession.currentHash, oOldSession.serializedBackOption);
			...
			oTemplateContract.myIntentPromise = sap.ushell && sap.ushell.renderers && sap.ushell.Container && new Promise(function(fnResolve){
			...
			oMyIntentPromise = oTemplateContract.myIntentPromise ? Promise.all([oTemplateContract.myIntentPromise, sap.ushell.Container.getServiceAsync("URLParsing")]).then(function(aResult){
			...
			fnInitializeHistory();
			if (bIsFirst){	// evaluation of startup parameters and initialization of routers is only to be done of first start		
				var oData = !bIsRestoring && !oNavigationControllerProxy.oHashChanger.getHash() && oTemplateContract.oAppComponent.getComponentData();
				...
				if (oStartupParameters){
					startupParameterHelper.parametersToNavigation(oTemplateContract, oStartupParameters).then(function(oMessageParameters){
						...
						oNavigationControllerProxy.initialize(!!oMessageParameters);
					});
				} else {
					oNavigationControllerProxy.initialize();	
				}
			}		
		}
		...
		// this function is called when the draft given by oContext is being cancelled. It returns either a faulty value (no special navigation needed)
		// or a Promise that resolves to a function performing the navigation that should be done when the draft was sucessfully cancelled.
		function getSpecialDraftCancelPromise(oContext){
			var oOptionPromise = getSpecialDraftCancelOptionPromise(oContext);
			return oOptionPromise && oOptionPromise.then(fnNavigateToRoute);
		}
		...
		// oSiblingeiContext is the target context. It is assumed that we are currently displaying its sibling (and in FCL possibly descendants of that).
		...
		function getSwitchToSiblingPromise(oSiblingContext, iDisplayMode, oTargetInfo){
			...
			var fnRet = oNavigationQueue.makeQueuable(fnNavigateToIdentity.bind(null, oTargetIdentity, true, iDisplayMode)); // The navigation function the Promise which is returned will resolve to
		...
		function fnNavigateToChild(oTreeNode, sChildSpec, sEmbedded, sKey, bReplace, iDisplayMode){
			var sRouteName;
			var bWithKey = true;
		...
		function fnNavigateToRoot(bReplace) {
			...
			var oRet = oAppStatePromise.then(fnNavigateToIdentity.bind(null, oTargetIdentity, bReplace));
			oTemplateContract.oBusyHelper.setBusy(oRet, undefined, undefined, true);
			return oRet;
		}
		...
		function getActiveComponents(){
			var aRet = [];
			/**
			 * check required as the values (oCurrentIdentity and oCurrentHash) are not yet initialised while the app is 
			 * navigating externally and the method is called for setting the refresh behaviour of the component (used 
			 * for keep alive refresh feature)
			 */
			if (oCurrentIdentity || oCurrentHash) {
		...
		function fnPreloadComponent(sRouteName) {
			...
			if (mRoutingTree && mRoutingTree[sRouteName]) {
		...
		function fnHandleRouteMatchedImpl() {
			...
			if (oRoutingOptions && oRoutingOptions.followUpNeeded && (oRoutingOptions.identity ? !isIdentityReached(oRoutingOptions.identity) : (typeof oRoutingOptions.followUpNeeded === "object"))){
				if (!oRoutingOptions.identity){
					oRoutingOptions = oRoutingOptions.followUpNeeded;	
				}
		...
		function fnHistoryRestored(oHistoryObject){
			...
			fnComeBack(oHistoricHash, oSerializedBackNavigationOption); // add an entry for the external app we have come back from
		...
		function getEditScopeHeaderNode(oTreeNode){
			return oTreeNode.isDraft ? oTemplateContract.oApplicationProxy.getAncestralNode(oCurrentIdentity.treeNode, 1) : oTreeNode;                         
		}
		...
		function isBackLeavingTheEditScope(){
			...
			var aHistoricalEntires = oHistoryState.getData().historicalEntries;
			if (aHistoricalEntires.length === 1 || aHistoricalEntires[1].sRouteName === "root"){
			...
			var oPreviousNode = oTemplateContract.mRoutingTree[aHistoricalEntires[1].sRouteName];
		...
		fnMakeNavigationFunction("navigateToChild", fnNavigateToChild);
		...
		oTemplateContract.oBusyHelper.setBusy((getParsedShellHashFromFLP() || Promise.resolve()).then(function(oResults){
			...
			if (oParsedUrl && oParsedUrl["sap-ui-xx-placeholder"]){
				var bPlaceholderValueFromURL = oParsedUrl["sap-ui-xx-placeholder"][0] === 'false' ? false : true ;
				oTemplateContract.bEnablePlaceholder = bPlaceholderValueFromURL;	
			} 
			bEnablePlaceholder = oTemplateContract.bEnablePlaceholder;
