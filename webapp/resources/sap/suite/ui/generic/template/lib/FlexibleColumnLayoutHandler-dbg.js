		...
		function getMethods(oFlexibleColumnLayout, oNavigationControllerProxy) {
			...
			function fnAdaptLayout(bChangedRoute){
				...
				if (oCurrentIdentity && sCurrentLayout !== oUiState.layout){
					...
					if ((oVisibilityChanged.begin || oVisibilityChanged.mid || oVisibilityChanged.end) && !bChangedRoute){
			...
			function onActionButtonPressed(oTreeNode, sActionName, bCloseColumn) {
				var fnProcessFunction = function () {
					oTemplateContract.oApplicationProxy.performAfterSideEffectExecution(function () {
						...
						oTemplateContract.oTemplatePrivateGlobalModel.setProperty("/generic/FCL/FCLActionButton", 0);
						// if Full Screen or ExitFullScreen is clicked set this property and unset for other actions
						if (sActionName === "fullScreen" || sActionName === "exitFullScreen") {
							oTemplateContract.oTemplatePrivateGlobalModel.setProperty("/generic/FCL/FCLActionButton", 1);
						} else {
							oTemplateContract.oTemplatePrivateGlobalModel.setProperty("/generic/FCL/FCLActionButton", 0);
						}
			...
			function fnHidePlaceholder(iFCLLevel,bForAllParents){
				var oRet = {};
				if (bForAllParents){
					switch (sCurrentLayout) {
					...
					for (var i = 0; i <= iFCLLevel; i++){
						oRet.aggregation = getPagesAggregation(i);
						oTemplateContract.oNavigationHost.hidePlaceholder(oRet);
					}
					return;
				}
				oRet.aggregation = getPagesAggregation(iFCLLevel);
				oTemplateContract.oNavigationHost.hidePlaceholder(oRet);
			}
