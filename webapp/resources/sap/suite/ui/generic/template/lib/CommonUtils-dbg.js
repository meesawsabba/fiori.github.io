sap.ui.define(["sap/ui/base/Object",
	...
	"sap/suite/ui/generic/template/genericUtilities/oDataModelHelper"
			...
			testableHelper, FeLogger, odata4analytics, extend, deepExtend, FeError, StableIdHelper, ControlStateWrapperFactory, oDataModelHelper) {
	"use strict";
	...
	function getMethods(oController, oServices, oComponentUtils) {
		...
		function getContextText(sKey, sSmartControlId, sFrameworkKey, aParameters) {
			...
			sText = getText(sContextKey, aParameters, true);
		...
		*/  
		function fnRefreshModel(sEntitySet) {
			...
			if (!oServices.oApplication.checkEtags() && sEntitySet) {
				...
				if (oController.getMetadata().getName() === 'sap.suite.ui.generic.template.AnalyticalListPage.view.AnalyticalListPage' && isParameterizedEntitySet(oModel, oEntitySet)) {
					...
					oModel.invalidate(fnCheckEntry.bind(null, sEntitySet));
				}
		 ...
		 */
		function fnCheckEntry(sEntitySet, sKey) {
			return sKey.split("(")[0] === sEntitySet;
		}
							...
							function(oBrowserEvent) {
								...
								if (oBrowserEvent.keyCode === 13 && oBrowserEvent.ctrlKey !== true && oUIModel.getProperty("/editable") && !isSearchField && !isTextArea && !isRowAction && !isColumnListItem) {
									...
									oServices.oApplication.addSideEffectPromise(new Promise(function(fnResolve, fnReject){
										 ...
										 */
										var oSourceForm;
										if (oBrowserEvent.target.type === "text") {
											var oElement = oBrowserEvent.target.parentElement;
											/* The smartField inside a table is also of type 'text'. Note that while iterating though its parentElement
										 	 * searching for form, atlast the oElement will be null and hence no validation will be run for such smartField.
										 	 */
											while (oElement) {
												oSourceForm = oView.byId(oElement.id);
												if (oSourceForm && oSourceForm.getMetadata().getElementName() === "sap.ui.comp.smartform.SmartForm") {
													break;
												}
