...
sap.ui.define([
	...
	"sap/suite/ui/generic/template/genericUtilities/FeLogger"
], function(GenericMessageUtil, controlHelper, FeLogger) {
		"use strict";
		...
		function fnNavigateToMessageTarget(oCommonUtils, oMessage) {
			...
			controlHelper.focusControl(sId);
		}
		 ...
		 * @param {object} fnPrepareForMessage - an optional function that prepares the view for the navigation to a message
		 * @private
		 * @ignore
		 */
		function fnNavigateFromMessageTitleEvent(oCommonUtils, oEvent, fnPrepareForMessageNavigation) {
			var oMessageItem = oEvent.getParameter("item");
			var oMessage = oMessageItem.getBindingContext("msg").getObject();
			if (fnPrepareForMessageNavigation){
				fnPrepareForMessageNavigation(oMessage);
			}
		...
		*/
		...
		return {
			...
			getTransientMessages: GenericMessageUtil.getTransientMessages,
