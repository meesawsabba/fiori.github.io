sap.ui.define(["sap/ui/base/Object",
	...
	], function(BaseObject, ManagedObjectModel, controlHelper, FeLogger, CRUDActionHandler, StatePreserver, extend, isEmptyObject, FeError) {
	"use strict";
	...
	function getMethods(oComponent, oComponentRegistryEntry) {
		...
		function fnNavigateRoute(sRouteName, sKey, sEmbeddedKey, bReplace, iDisplayMode){
			oComponentRegistryEntry.oTemplateContract.oNavigationControllerProxy.navigateToChild(oTreeNode, sRouteName, sEmbeddedKey, sKey, bReplace, iDisplayMode);
		}
		...
		function getControllerExtensions(){
			...
			return oExtensions && oExtensions[oComponent.getEntitySet()];
		}
		...
		function getViewExtensions(){
			...
			return oExtensions && oExtensions[oComponent.getTemplateName()];
		}
		...
		function fnHidePlaceholder(){
			if (oComponentRegistryEntry.oTemplateContract.oFlexibleColumnLayoutHandler){
				oComponentRegistryEntry.oTemplateContract.oFlexibleColumnLayoutHandler.hidePlaceholder(oTreeNode.fCLLevel);
			} else {
