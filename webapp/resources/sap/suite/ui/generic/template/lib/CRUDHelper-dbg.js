sap.ui.define(["sap/ui/base/Object", "sap/ui/model/Context", "sap/suite/ui/generic/template/lib/MessageUtils", "sap/ui/model/Filter", "sap/ui/model/FilterOperator"],
	function(BaseObject, Context, MessageUtils, Filter, FilterOperator) {
		...
		function create(oDraftController, sEntitySet, sBindingPath, oModel, oApplication, vPredefinedValues, oParameters) {
		...
		*/
		...
		function fnUnsavedChangesDialog(oTemplateContract, oDraftAdministrativeData, fnBeforeDialogCallback) {
			return new Promise(function(resolve, reject) {
				...
				oTemplateContract.oApplicationProxy.getDialogFragmentAsync(
		...
		function discardDraft(oDraftController, oTransactionController, oApplicationProxy, oContext){
			...
			var oSiblingPromise = bHasActiveEntity ? oApplicationProxy.getDraftSiblingPromise(oContext) : Promise.resolve();
			return oSiblingPromise.then(function(oActive){
				var fnTransformActiveContext = function(){
					return  { context: oActive };
				};
				var oDeletePromise = oTransactionController.deleteEntity(oContext);
				var oCancellationPromise = oDeletePromise.then(fnTransformActiveContext);
				oApplicationProxy.cancellationStarted(oContext, oCancellationPromise, "discardAction");
