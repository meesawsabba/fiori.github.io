...
], function(BaseObject, Device, Filter, testableHelper, extend, StableIdHelper) {
	"use strict";
	/*
	...
	function getMethods(oController, oTemplateUtils, oConfiguration) {
		...
		function fnCleanupIrrelevantFilters(oViewMeta, aSmartFilterBarFilters, mIgnoredFilters) {
			...
			var fnHandlerForNonMultiFilters = function(oFilter) {
				...
				var aEntityProperties = getEntityTypePropertiesOfPath(sPath, oMetaModel, oEntityType, aNavProperties, oViewMeta);
			...
			if (mIgnoredFilters) {
				...
				var aIgnoredLabels = aIgnoredFilters.map(function(sPath) {
					var oProperty = fnGetFilterProperty(sPath, oPresentationControlHandler);
					return oProperty["sap:label"] || (oProperty["com.sap.vocabularies.Common.v1.Label"] ? oProperty["com.sap.vocabularies.Common.v1.Label"].String : oProperty.name);
				});
		 ...
		 */
		function getEntityTypePropertiesOfPath(sPath, oMetaModel, oEntityType, aNavProperties, oViewMeta) {
		 ...
		 * @param  {object} oPresentationControlHandler - selected table or chart handler
		 * @return {string} oUnappliedProperty - Property that is not applied in filter
		 */
		function fnGetFilterProperty(sPropertyName, oPresentationControlHandler) {
			var oUnappliedProperty;
			var aEntityProperties = findOtherEntityProperties(oPresentationControlHandler);
		 ...
		 * @param  {object} oControlHandler - selected table or chart handler
		 * @return {array} aProperty - all properties of other entity sets
		 */
		function findOtherEntityProperties(oControlHandler) {
			var  aProperties = [];
			for (var sKey in mSwitchingKeyToViewMeta) {
				var oViewMeta = mSwitchingKeyToViewMeta[sKey];
				if (oViewMeta.presentationControlHandler !== oControlHandler) {
		...
		function fnSetActiveButtonState(oViewMeta) {
			var oPresentationControlHandler = oViewMeta.presentationControlHandler;
			/* getActiveButtonTableStateCustomData should not be a method of PresentationControlHandler. For more details, refer to the comment 
			   mentioned before the method's definition in the class */
			var oCustomData = oPresentationControlHandler.getActiveButtonTableStateCustomData();
			var bActiveButtonState = JSON.parse(oCustomData.getValue());
			/* setActiveButtonTableStateCustomData should not be a method of PresentationControlHandler. For more details, refer to the comment 
			   mentioned before the method's definition in the class */
			oPresentationControlHandler.setActiveButtonTableStateCustomData(!bActiveButtonState);
			oTemplateUtils.oComponentUtils.getTemplatePrivateModel().setProperty("/listReport/activeObjectEnabled", !bActiveButtonState);
		}

		function fnRestoreActiveButtonState(oViewMeta) {
			var bActiveButtonState = oTemplateUtils.oComponentUtils.getTemplatePrivateModel().getProperty("/listReport/activeObjectEnabled");
			/* setActiveButtonTableStateCustomData should not be a method of PresentationControlHandler. For more details, refer to the comment 
			   mentioned before the method's definition in the class */
			oViewMeta.presentationControlHandler.setActiveButtonTableStateCustomData(bActiveButtonState);
		}
