sap.ui.define(["sap/ui/base/Object",
...
], function(BaseObject, Event, MessageBox, Table, Sorter, controlHelper, expressionHelper, testableHelper, JSONModel, coreLibrary, AnnotationHelper, 
	SelectionVariant, Controller, FeLogger, MessageUtils, extend, isEmptyObject, genericAppLibrary, deepExtend) {
	...
	function getMethods(oController, oComponentUtils, oServices, oCommonUtils) {
		...
		function getPaginatorInfo(oTable, oRow, oState){
			var oControlBindingInfo = oServices.oPresentationControlHandlerFactory.getPresentationControlHandler(oCommonUtils.getOwnerPresentationControl(oTable)).getBindingInfo();
			var oListBinding = oControlBindingInfo && oControlBindingInfo.binding;			
			if (!oListBinding){
				return null;
			}
			
			var bIsMTable = controlHelper.isMTable(oTable);
			var bIsUiTable = controlHelper.isUiTable(oTable);			
			
			var aCurrContexts;

			var mSelectedItems = Object.create(null);
			//Getting context for Tree Table.
			var sSelectedBindingPath = oRow.getBindingContext().getPath();
			var iIdx = -1; // index of the selected item
			if (controlHelper.isTreeTable(oTable)) {
				...
				for (var index = 0; index < iContextsLength; index++) {
					...
					aCurrContexts.push(oContext);
					if (oContext && oContext.getPath() === sSelectedBindingPath){
						iIdx = index;	
					}
				}
				if (iIdx < 0){
					return null; // selected item cannot be found in the table
				}
			} else	if (bIsUiTable) {
				...
				// Do not create a virtual item (see below) with same context, as this would remove the selection from the original item.
			...
			var fnPaginate = function(oContext){
				...
				onListNavigate(oSourceItem, oState, oContext, true);
			};
        ...
        function storeObjectPageNavigationRelatedInformation(oRow, oState) {
			...
			oComponentUtils.setPaginatorInfo(bIsAnalyticalTbl ? null : getPaginatorInfo(oControl, oRow, oState), true);
		}
		...
		*/
			...
			if (oSmartTable) {
				...
				if (isFCLApp) {
					...
					if (!oTemplatePrivateGlobalModel.getProperty("/generic/FCL/isVisuallyFullScreen")) {
						var oAnnotationFormatter = expressionHelper.getAnnotationFormatter(oModel, oSmartTable.getEntitySet(), "/com.sap.vocabularies.UI.v1.HeaderInfo/Description/Value", oRb);
		...
		function fnOnSemanticObjectLinkNavigationTargetObtained(sEntitySet, mFieldSemanticObjects, oEventParameters, oState, sTitle, oMainNavigation) {
		...
		function onDataReceived(oEvent){
			if (controlHelper.isSmartTable(oEvent.getSource())) {
		...
		function onListNavigate(oEventOrSource, oState, oBindingContext, bIsProgrammatic) {
			...
			if (bIsProgrammatic){
				...
				if (oEventSource.data("CrossNavigation") || (!oEventSource.data("CrossNavigation") && !oComponentUtils.isDraftEnabled())){
					...
					onListNavigateImpl(oEventSource, oEventParameters, oState, oBindingContext, bReplace);
				}
...
//			}
		...
		function onDeterminingDataFieldForAction(oEvent, oPresentationControlHandler) {
			...
			var aContexts = oPresentationControlHandler ? oPresentationControlHandler.getSelectedContexts() : [oView.getBindingContext()];
			...
			if (isValidEntitySet){
				...
				for (var i = 0; i < aContexts.length; i++) {
					...
					if (bActionName && oEntity[bActionName] === false) {
						...
						aItems.push({
							sKey: oAnnotationFormatter.format(aContexts[i])
						});
					} else {
			...
			var fnCRUDManagerCall = function (oPredefined) {
				var oRet = oServices.oCRUDManager.addEntry(oTable, oPredefined).then(function (oNewContext) {
					if (bCreateWithDialogEnabled) {
						...
						oServices.oPresentationControlHandlerFactory.getPresentationControlHandler(oTable).refresh();
