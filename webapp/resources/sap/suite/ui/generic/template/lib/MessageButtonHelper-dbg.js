sap.ui.define(["sap/ui/base/Object", "sap/m/MessagePopover", "sap/m/MessagePopoverItem", "sap/ui/model/Filter",
	"sap/ui/model/FilterOperator", "sap/suite/ui/generic/template/lib/MessageUtils", "sap/suite/ui/generic/template/genericUtilities/testableHelper", 'sap/ui/core/Element', "sap/base/util/extend"
	...
	function getMethods(oTemplateUtils, oHost, bIsODataBased) {
		...
		function getGroupTitle(sMsgId, aControlIds) {
			return oHost.getGroupTitle ? oHost.getGroupTitle(sMsgId, aControlIds) : "";
		}
		var oItemBinding;
		...
		var oMessagePopover = oTemplateUtils.oCommonUtils.getDialogFragment("sap.suite.ui.generic.template.fragments.MessagePopover", {
			...
			titlePressed: function (oEvent) { // the user wants to navigate from the message to the corresponding control
				MessageUtils.navigateFromMessageTitleEvent(oTemplateUtils.oCommonUtils, oEvent, oHost.prepareForMessageNavigation);
			}
		});
		...
		oItemBinding.filter(oImpossibleFilter); // let the binding be empty until we have a context in order to avoid unwanted sorting and grouping
		...
		(function () {
			...
			oItemBinding.attachChange(function () {
				...
				//errorMessageCount is used to display count on message button.
				oTemplatePrivate.setProperty("/generic/errorMessageCount", iErrorCount > 0 ? iErrorCount : undefined);
		...
		function adaptToContext(sBindingPath) {
			...
			var bIsNonDraftCreate = bIsODataBased && !bIsDraftEnabled && oUiModel.getProperty("/createMode");
