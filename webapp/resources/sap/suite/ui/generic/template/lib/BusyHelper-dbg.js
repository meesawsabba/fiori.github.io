	...
	"sap/suite/ui/generic/template/genericUtilities/FeError"
		...
		function getMethods(oTemplateContract) {
				...
				if (bIsBusyNew || bImmediate) {
					...
					if (bIsBusyNew !== bIsBusy) {
						...
						if (!bIsBusy){ // end of a busy session
							...
							var aTransientMessages = MessageUtils.getTransientMessages();
							...
							oUnbusyPromise = MessageUtils.handleTransientMessages(oTemplateContract.oApplicationProxy.getDialogFragment, oBusySessionParams.actionLabel, oRefreshActivePagesConfig);
							oBusySessionParams = {};
			...
			function setBusyReason(sReason, bIsActive, bImmediate, oSessionParams, bStartsANavigationSession) {
				bStartsANavigationSession ? bIsOnlyLogicalBusyNeeded = oTemplateContract.bEnablePlaceholder : bIsOnlyLogicalBusyNeeded = false;
