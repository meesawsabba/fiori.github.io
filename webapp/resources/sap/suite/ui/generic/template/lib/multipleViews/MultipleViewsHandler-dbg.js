sap.ui.define([
	...
	function getMethods(oController, oTemplateUtils, oConfiguration) {
		...
		var oImplementingHelper = new (oConfiguration.presentationControlHandler ? SingleTableModeHelper : MultipleTablesModeHelper)(oController, oTemplateUtils, oConfiguration);
				...
				for (var i in oVariant.SelectOptions) {
					if (oVariant.SelectOptions[i].PropertyName) {
						...
						for (var j in oVariant.SelectOptions[i].Ranges) {
							var oSign = oVariant.SelectOptions[i].Ranges[j].Sign;
							var oOperator = oVariant.SelectOptions[i].Ranges[j].Option;
							if (oSign) {
								oOperator.EnumMember = fnDetermineOperator(oSign.EnumMember.replace("com.sap.vocabularies.UI.v1.SelectionRangeSignType/", ""), oOperator.EnumMember.replace("com.sap.vocabularies.UI.v1.SelectionRangeOptionType/", ""));
							} else {
