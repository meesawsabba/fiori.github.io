sap.ui.define(["sap/ui/base/Object", "sap/base/util/extend", "sap/suite/ui/generic/template/js/StableIdHelper"], function(BaseObject, extend, StableIdHelper) {
		...
		function fnPerformWorklistSearch(oEvent) {
			...
			// - from SearchFieldWrapper (in case of live change - this is also the only case, that really indicates a stateChange from worklist. tab switch is also a state change, but indicated from
			//		multiple views. In a worklist, all appStates should have suppressDataSelection = false, as always data should be shown. This should be ensured during startup when the first state is 
			//		created.
		}
