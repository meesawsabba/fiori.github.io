sap.ui.define(["sap/ui/core/mvc/OverrideExecution",
			  ...
			  ], function(OverrideExecution, TemplateAssembler, ControllerImplementation, controllerFrameworkExtensions, testableHelper, filterSettingsPreparationHelper, staticChecksHelper, preparationHelper, deepExtend, FeError, StableIdHelper, Context) {
	"use strict";
		...
		return {
			...
			getTemplateSpecificParameters: function(oMetaModel, oSettings, Device, sLeadingEntitySet, oDraftContext, oModel) {
				...
				function getMultiEditSettings(oVariant, oLrSettings, oEntitySet, oEntityType) {
					...
					if (oTableSettings.multiEdit.enabled !== false && oLrSettings.isResponsiveTable && !oComponentUtils.isDraftEnabled() && !oLrSettings.isWorklist && !oComponent.getAppComponent().getFlexibleColumnLayout()) {
						bMultiEdit = true;
				...
				if (oLrSettings.quickVariantSelectionX) {
					...
					if (oLrSettings.tableSettings.multiEdit) {
						oMultiEditSettings = getMultiEditSettings(undefined, oLrSettings, oEntitySet, oEntityType);
