sap.ui.define(["sap/suite/ui/generic/template/js/AnnotationHelper"],
	...
	function getTextForItemAnalyticalListPage(oInterface, oQuickVariantSelection, oItemDef) {
		...
		if (oQuickVariantSelection.showCounts || bShowCountsForDifferentEntitySets) {
			return "{path: '_templPriv>/alp/multipleViews/items/" + oItemDef.key + "', formatter: '._templateFormatters.formatItemTextForMultipleView'}";
		}
		return AnnotationHelper.getIconTabFilterText(oInterface.getInterface(0), oItemDef);
	}
	...
	function getVisibleForALPTableTabs(oTabItem) {
		return "{= ${_templPriv>/alp/multipleViews/mode} !== 'multi' || ${_templPriv>/alp/multipleViews/selectedKey} === '" + (oTabItem && oTabItem.key) + "' }";
	}
