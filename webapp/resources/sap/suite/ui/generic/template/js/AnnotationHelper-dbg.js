...
], function (AnnotationHelperModel, SmartField, FeLogger, formatMessage, deepExtend, RuntimeFormatters, StableIdHelper, testableHelper, Device, SapMLibrary, SapCoreLibrary, ObjectPath) {
	"use strict";
...
*/
		...
		getShareButtonVisibility: function(bIsFlexibleColumnLayout, sTreeNodeLevel) {
			...
			var bIsJamActive = fnGetUser && fnGetUser().isJamActive();
			if (bIsJamActive) {
				return true;
			}
			if (bIsFlexibleColumnLayout) {
				return '{= ${_templPrivGlobal>/generic/FCL/highestViewLevel} === ' + sTreeNodeLevel + ' ? !${ui>/createMode} : false }';
			} else {
				return "{= !${ui>/createMode}}";
			}
		},
		...
		buildBreadCrumbExpression: function (oContext, oTreeNode, oTitle, oTypeName) {
			var sBinding,
		...
		getLoadingStrategy: function (oFacet, oSections) {
			var oSettings = oSections && oSections[oAnnotationHelper.getStableIdPartFromFacet(oFacet)];
			...
			if (oLoadingBehavior.waitForViewportEnter) {
				return oLoadingBehavior.waitForHeaderData ? "lazyLoadingAfterHeader" : "lazyLoading";
			 }
			 return oLoadingBehavior.waitForHeaderData ? "activateAfterHeaderDataReceived" : "activateWithBindingChange";
		},
		...
		matchesBreadCrumb: function (oCandidate, oTreeNode) {
			return oTreeNode.entitySet === oCandidate.name;
		},
		...
		isExtensionPointBeforeSubSectionLazyLoadingEnabled: function (sEntitySet, sFacetId, oManifestExtend) {
			return fnExtensionLazyLoadEnabled(
				"BeforeSubSection|" + sEntitySet + "|" + sFacetId,
				oManifestExtend);
		},
		isExtensionPointAfterSubSectionLazyLoadingEnabled: function (sEntitySet, sFacetId, oManifestExtend) {
			return fnExtensionLazyLoadEnabled(
				"AfterSubSection|" + sEntitySet + "|" + sFacetId,
				oManifestExtend);
		},
		isExtensionPointReplaceSubSectionLazyLoadingEnabled: function (sEntitySet, sFacetId, oManifestExtend) {
			return fnExtensionLazyLoadEnabled(
				"ReplaceSubSection|" + sEntitySet + "|" + sFacetId,
				oManifestExtend);
		},
		/**
		...
		isRelatedEntityCreatable: function (oInterface, oSourceEntitySet, oRelatedEntitySet, aSubPages, oFacet, oSections, bIsDraftEnabled, oCreateWithParameterDialog) {
			...
			if (oAnnotationHelper.hasSubObjectPage(oRelatedEntitySet, aSubPages) || (oSections && oAnnotationHelper.isInlineCreate(oFacet, oSections)) || !!oCreateWithParameterDialog) {
				if (bIsDraftEnabled) {
					result = "{= ${ui>/editable}}";
				} else {
					result = "{= !${ui>/editable}}";
		...
		hasDeterminingActionsRespectingApplicablePath: function (oContext, aTerm, sEntitySet, oManifestExt) {
			...
			if (sEntitySet && oManifestExt && oManifestExt["sap.suite.ui.generic.template.ObjectPage.view.Details"]) {
				var oManifestExtension = oManifestExt["sap.suite.ui.generic.template.ObjectPage.view.Details"]["sap.ui.generic.app"];
				if (oManifestExtension && oManifestExtension[sEntitySet]) {
					var oManifestExtEntitySet = oManifestExtension[sEntitySet];
					if (oManifestExtEntitySet.Header && oManifestExtEntitySet.Header.Actions) {
						for (var action in oManifestExtEntitySet.Header.Actions) {
							if (oManifestExtEntitySet.Header.Actions[action].determining) {
								if (oManifestExtEntitySet.Header.Actions[action].applicablePath) {
									if (sApplicablePaths.length > 0) {
										sApplicablePaths += " || ";
									}
									sApplicablePaths += "${path: '" + oManifestExtEntitySet.Header.Actions[action].applicablePath + "'}";
								} else {
								return true;
								}
							}
        ...
        */
		...
		checkIfChartNavigationIsEnabled: function(oItabItem, aSubPages, sChartEntitySet, oQuickVariantSelectionX) {
			...
			// Only one subpage is allowed in the list report that is why no loop over the aSubPages
				if (!oAnnotationHelper.checkIfDiffEntitySetsMode(oQuickVariantSelectionX) && aSubPages && aSubPages[0] && aSubPages[0].entitySet === sChartEntitySet && (!aSubPages[0].navigation || aSubPages[0].navigation && !aSubPages[0].navigation['display'])) { // check that the internal navigation is not overwritten by the external one
					return true;
				}
			return false;
		},
