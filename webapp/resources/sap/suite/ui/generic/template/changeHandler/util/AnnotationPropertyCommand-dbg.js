...
sap.ui.define([
	...
	var	sClassName = "changeHandler.util.ChangeHandlerUtils";
	/**
	...
	AnnotationPropertyCommand.getSelector = function (vControl, oAppComponent, mAdditionalSelectorInformation) {
		...
		if (typeof sControlId !== "string") {
			...
			throw new FeError(sClassName, "App Component instance needed to get a selector from string ID");
		}

		if (mAdditionalSelectorInformation && (mAdditionalSelectorInformation.id || mAdditionalSelectorInformation.idIsLocal)) {
			throw new FeError(sClassName, "A selector of control with the ID '" + sControlId + "' was requested, " +
	...
	AnnotationPropertyCommand.createChange = function (oChangeSpecificData, oControl) {
		...
		if (!oControl) {
			throw new FeError(sClassName, "A flexibility change cannot be created without a targeted control.");
		}
		...
		if (!oAppComponent) {
			throw new FeError(sClassName, "No Application Component found - to offer flexibility the control with the id '" + sControlId + "' has to have a valid relation to its owning application component.");
		}
		...
		if (!sControlType) {
			throw new FeError(sClassName, "No control type found - the change handler can not be retrieved.");
		}
		...
		if (ChangeHandler) {
			...
			throw new FeError(sClassName, "Change handler could not be retrieved for change " + JSON.stringify(oChangeSpecificData) + ".");
		}
