sap.ui.define([
	...
	"sap/ui/Device"
], function (ControllerExtension, MessageBox, Filter, Sorter, SelectionVariant, controlHelper, testableHelper, detailUtils, SideContentHandler,
	ExtensionAPI, AnnotationHelper, ResponsiveTable, DynamicSideContent, ShareUtils, Selectors, FeLogger, MultipleViewsHandler,
	MessageUtils, KeyCodes, merge, FilterProcessor, StableIdHelper, ObjectPageSubSection, MessageType, extend, UriParameters, isEmptyObject,
	getFirstEditableInput, Focusable, FeError, CreateWithDialogHandler, FlexRuntimeInfoAPI, Device) {
	"use strict";
	...
	var oTableThresholdInfo = {}; // holds table id as key and initial growingThreshold as value 
		...
		getMethods: function (oViewProxy, oTemplateUtils, oController) {
			...
			function onActivateImpl() {
				...
				oActivationPromise.catch(function(){
					oTemplateUtils.oCommonUtils.executeForAllInformationObjects("smartTable",function(oInfoObject){
			...
			function onSaveImpl(bStayInEdit) {
				...
				oSaveEntityPromise.catch(function(){
					oTemplateUtils.oCommonUtils.executeForAllInformationObjects("smartTable",function(oInfoObject){
						oInfoObject.onSaveWithError();
					});
				});
					...
					shareEmailPressed: function () {
						...
						var oTriggerPromise = ShareUtils.getCurrentUrl().then(function (sCurrentUrl) {
							if (IsActiveEntity || IsActiveEntity === undefined) {
								sap.m.URLHelper.triggerEmail(null, sEmailSubject, sCurrentUrl);
							} else {
								...
								oBusyHelper.setBusy(oSiblingContextPromise.then(function (oSiblingContext) {
									...
									sap.m.URLHelper.triggerEmail(null, sEmailSubject, sPath);
								}));
							}
						});
			...
			*/
			...
			function getTableDeleteDialog(oSmartTable, aContexts, sUiElementId) {
				...
				fnTableDeleteConfirmationOnDelete = function (oEvent) {
					...
					oDeletePromise.then(function (aFailedPath) {
						oTemplateUtils.oServices.oPresentationControlHandlerFactory.getPresentationControlHandler(oSmartTable).refresh();
			...
			// BEGIN: functions used for sorting and grouping of messages
			...
			var oMessageSorter = new Sorter("msg>");
			...
			// This function is only needed if the object page uses iconTabBar mode.
			// In this case it prepares the object page such that it is able to naviagte to the control that belongs to a message
			// by switching to the section this control belongs to.
			function setSectionForMessage(oMessage) {
				var bUseIconTabBar = oObjectPage.getUseIconTabBar();
				if (!bUseIconTabBar) {
					return;
				}
				var oPlacementInfo = getMessagePlacementInfo(oMessage.id, oMessage.controlIds);
				var aGroupers = oPlacementInfo.groupers;
				var oSection = aGroupers.find(function (oControl) {
					return controlHelper.isObjectPageSection(oControl);
				});
				var sSectionId = oSection && oSection.getId();
				var sSelectedSectionId = sSectionId && oObjectPage.getSelectedSection();
				if (sSectionId !== sSelectedSectionId) {
					oObjectPage.setSelectedSection(sSectionId);
				}
			}

			// END: functions used for sorting and grouping of messages
			...
			function fnApplyTableThresholdValueAndCssClass(oSmartTable, oTable, sCurrentFacet) {
				var oTableInfoObject = {
					growingThreshold: oTableThresholdInfo[oTable.getId()],
			...
			function fnGetFocusableElementOnObjectPage(oObjectPage, sMode) {
				if (sMode === "Display") {
					...
					if (bFocusAtTop && !bIconTabBar) {
						oObjectPage.setSelectedSection(null);
						return getFirstEditableInput(oObjectPage.getDomRef());
					}

					var fnGetFocusableElementOnSubSection = function (oSection, oSubSection) {
						oObjectPage.setSelectedSection(oSection);
						oSection.setSelectedSubSection(oSubSection);
						fnSubSectionEntered(oSubSection);
						var oSubSectionDomRef = oSubSection.getDomRef();
						var oFirstFocusableElement = getFirstEditableInput(oSubSectionDomRef);
						if (oFirstFocusableElement && oFirstFocusableElement.type !== "checkbox") {
							return oFirstFocusableElement;
						}
						// In case if getFirstEditableInput doesn't return any editable control or returns a checkbox control(which is not focusable)
						// return the first focusable element in the given section's Dom
						return Focusable(oSubSectionDomRef).firstFocusableDomRef();
					};
					var aSections = oObjectPage.getSections();
					var oFocusableElement, bSelectedSectionFound;
					aSections.some(function (oSection) {
						var bIsSelectedSection = !!(oSection.getId() === sSectionSelected);
						bSelectedSectionFound = bSelectedSectionFound || bIsSelectedSection;
						if (!bSelectedSectionFound || !oSection.getVisible()) {
							return false;
						}
						// start iteratively checking for focusable element from selected subsection
						var iStartIndex = 0;
						var aSubSections = oSection.getSubSections();
						// if selected subsection, start checking for focusable element from the index of selected subsection or else start from index 0
						if (bIsSelectedSection) {
							var sSelectedSubSection = oSection.getSelectedSubSection();
							var oSelectedSubSection = sSelectedSubSection && sap.ui.getCore().byId(sSelectedSubSection);
							iStartIndex = (oSelectedSubSection && oSection.indexOfSubSection(oSelectedSubSection)) || 0;
						}
						for (var i = iStartIndex; !oFocusableElement && i < aSubSections.length; i++) {
							oFocusableElement = aSubSections[i].getVisible() && fnGetFocusableElementOnSubSection(oSection, aSubSections[i]);
						}
						return oFocusableElement;
			...
			function fnInitSmartTableInfoObject(oSmartTableInfoObject, aCategories, oSmartTable) {
				...
				aCategories.push("smartControl");
				oSmartTableInfoObject.oControlStateWrapper = oTemplateUtils.oCommonUtils.getControlStateWrapper(oSmartTable);
				oSmartTableInfoObject.oControlStateWrapper.attachStateChanged(oTemplateUtils.oComponentUtils.stateChanged);
				...
				// prepare handling of errorenus rows
				var oDataStateIndicator = oSmartTable.getDataStateIndicator();
				oSmartTableInfoObject.onSaveWithError = function (){
			...
			function fnInitSmartChartInfoObject(oSmartChartInfoObject, aCategories, oSmartChart) {
				...
				aCategories.push("smartControl");
				oSmartChartInfoObject.oControlStateWrapper = oTemplateUtils.oCommonUtils.getControlStateWrapper(oSmartChart);
				oSmartChartInfoObject.oControlStateWrapper.attachStateChanged(oTemplateUtils.oComponentUtils.stateChanged);
				//Not ready yet : Do we still need this block of code
			...
			function fnInitSearchFieldInfoObject(oSearchFieldInfoObject, aCategories, oSearchField) {
				...
				var oParentTable = oSearchField.getParent().getParent(); // if inner table is an MTable this is the inner table. Otherwise it will be the smart table
				var oSmartTable = controlHelper.isMTable(oParentTable) ? oParentTable.getParent() : oParentTable; // the smart table hosting the search field
				oSearchFieldInfoObject.table = oSmartTable;
				var oSmartTableInfoObject = getSmartTableInfoObject(oSmartTable); // ensure initialization of the info object of the smart table
			...
			function onSearchObjectPage(oEvent) {
				...
				oTemplateUtils.oServices.oPresentationControlHandlerFactory.getPresentationControlHandler(oSearchFieldInfoObject.table).refresh();
			}
			...
			function addEntryImpl(oPredefinedValues, oButton) {
				...
				if (oGetDefaultValuesPromise instanceof Promise) {
					var fnGetPredefinedValues = function (oResponse) {
						callAddEntryImpl(oResponse, oButton);
					};
			...
			function callAddEntryImpl(oPredefinedValues, oButton) {
				...
				if (!oButton.data("CrossNavigation") && bSuppressNavigation) {
					...
					return;
				} else if (!oButton.data("CrossNavigation") && oTemplateUtils.oComponentUtils.isDraftEnabled()) {
			...
			function fnDataStateFilter(oMessage, oTable) {
				...
				if (!mTablesWithMessagesToDataStateIndicators[sTableId]) { // If table is already registered ignore registration step
					mTablesWithMessagesToDataStateIndicators[sTableId] = oTable.getDataStateIndicator ? oTable.getDataStateIndicator() : oTable.getParent().getDataStateIndicator(); // register
				}
				var oUIModel = oObjectPage.getModel("ui");
			...
			function setNoDataTextIfRequired(oSmartTable) {
				...
				var aSearchControl = oSmartTable.getToolbar().getContent().filter(function (oControl) {
					return oControl.sId === oSmartTable.getToolbar().getId() + '::SearchField';
				});
				...
				for (var section in oSettings.sections) {
					var oManifestSettings = oSettings.sections[section];
					if (oManifestSettings.entitySet === oSmartTable.getEntitySet() && oManifestSettings.quickVariantSelection) {
						bIsSegmentedButton = true;
						break;
					}
				}
				if (bIsSegmentedButton) {
					sNoDataText = oTemplateUtils.oCommonUtils.getContextText("NOITEMS_SMARTTABLE_WITH_FILTER_FOR_SEGMENTEDBUTTON", sSmartTableId);
			...
			oViewProxy.getCurrentState = function () {
				...
				var mSmartControls = {};
				oTemplateUtils.oCommonUtils.executeForAllInformationObjects("smartControl", function (oSmartControlInfoObject, oSmartControl) {
					mSmartControls[oSmartControl.getId()] = oSmartControlInfoObject.oControlStateWrapper.getState();
				});
			...
			oViewProxy.applyState = function (oState, bIsSameAsLast) {
				...
				oTemplateUtils.oCommonUtils.executeForAllInformationObjects("smartControl", function (oSmartControlInfoObject, oSmartControl) {
					oSmartControlInfoObject.oControlStateWrapper.setState(oState.smartControls && oState.smartControls[oSmartControl.getId()]);
				});
				//ends
			...
			oViewProxy.beforeRebind = function (oWaitForPromise) {
				mMessageToPlacementInfo = Object.create(null); // refresh this cash for every new object instance to avoid accumulation of garbage
				resetWaitForState();
			...
			testableHelper.testable(function () {
				return oStateAppliedAfterBindPromise;
			...
			var oControllerImplementation = {
				onInit: function () {
					...
					oBase.onInit(null, fnDisableLazyLoading, oMessageSorter, getGroupTitle, setSectionForMessage);
					...
					function fnInitializeInfoObjects(oBlock) {
						if (oBlock.tableSettings) {
							...
							var oSmartChart = oController.byId(oBlock.additionalData.facetId + "::Chart");
							if (oSmartChart) {
					...
					onTableInit: function (oEvent, sCurrentFacet) {
						...
						var fnOnSubSectionVisibilityChanged = function () {
							if (oTable.isA("sap.m.Table") || oTable.isA("sap.ui.table.Table")) {
								if (oTable.isA("sap.m.Table") && !oTableThresholdInfo[oTable.getId()]) {
									oTableThresholdInfo[oTable.getId()] = oTable.getGrowingThreshold && oTable.getGrowingThreshold(); // store the default growingthreshold value of the table
								}
						...
						// Subsequently, this method is called everytime the subsection visibility is changed.
