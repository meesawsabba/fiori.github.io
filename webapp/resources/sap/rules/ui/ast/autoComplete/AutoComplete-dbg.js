sap.ui.define(["sap/rules/ui/ast/autoComplete/dataStructure/Stack",
	...
	function (Stack, TermsProvider, OperatorProvider, FunctionProvider, astBundleLibrary, Constants, TermNode, ComparisionOperatorStack,
		...
		AutoComplete.prototype.getSuggestions = function (aTokens, aExpectedTokensCandidates, suggestionContext, oData) {
			...
			var bReferenceContext = suggestionContext.ReferenceContext;
			...
			if (aExpectedTokensCandidates && aExpectedTokensCandidates.length > 0) {
				if (aTokens && aTokens.length > 0) {
					...
					if (bIsLastTokenEndingWithDot || bAssociationContext || bReferenceContext) {
						sText = sText.replace(".", "");
					...
					var lIterator
					if ((bIsLastTokenEndingWithDot || bAssociationContext || bReferenceContext) && oResult && oResult.errorCode == 1) {
						expectedBusinessDataTypeList = oResult.expectedBusinessDataTypeList;
						if (oTopStack && "getTermPrefixId" in oTopStack && oTopStack.getTermPrefixId() && sText) {
							var aIds = sText.split(Constants.DOT);
							if(!TermsProvider.getInstance().isTermParentEntity(aIds[0])){
                        ...
                        terms = terms.concat(TermsProvider.getInstance().getReferencesGivenPrefixId(sText));
                        oAutoCompleteJson["autoComplete"]["categories"]["terms"] = this._transformTermsToUiModel(terms);
						return oAutoCompleteJson;
					}
					if (oResult) {
						return oAutoCompleteJson;
					} else if (bIsLastTokenEndingWithDot || bAssociationContext || bReferenceContext) {
						if (oTopStack && "getTermPrefixId" in oTopStack && oTopStack.getTermPrefixId() && sText) {
							var aIds = sText.split(Constants.DOT);
							if(!TermsProvider.getInstance().isTermParentEntity(aIds[0])){
								sText = oTopStack.getTermPrefixId() + Constants.DOT + sText;
						...
						terms = terms.concat(TermsProvider.getInstance().getReferencesGivenPrefixId(dataObjectId));
						oAutoCompleteJson["autoComplete"]["categories"]["terms"] = this._transformTermsToUiModel(terms);
						return oAutoCompleteJson;
					} else {
		...
		AutoComplete.prototype._updateAutoCompleteJsonBasedOnTokenCandidate = function (candidate, oAutoCompleteJson, topNode, oTopStack,
			...
			case astLibInstance.IDPLexer.ID:
				if (bIsOperationsContext && lastFunctionalStack && lastFunctionalStack.getFunction().getContext() == "result" &&
					(lastFunctionalStack.getSize() < 1)) {
					break;
				}

				if (bIsOperationsContext && lastFunctionalStack && lastFunctionalStack.getFunction().getContext() == "result" && 
					lastFunctionalStack.getFunction().getName().toUpperCase() === "FOREACH" && lastFunctionalStack.getSize() > 1)
				{
					break;
				}
				...
				if (topNode && "getTermPrefixId" in topNode && topNode.getTermPrefixId() && topNode.getTermPrefixId() != "") {
					var attributesAssocsRefs = TermsProvider.getInstance()._getAllAttrsRefsAssocsForDataObject(topNode.getTermPrefixId());
					var dataObjects = TermsProvider.getInstance()._getTermsNotStartingWithPrefix(topNode.getTermPrefixId());
					var vocabRules = TermsProvider.getInstance()._getAllVocabularyRules();
					categories["terms"] = this._transformTermsToUiModel(attributesAssocsRefs.concat(dataObjects).concat(vocabRules));
				} else if (oTopStack && "getTermPrefixId" in oTopStack && oTopStack.getTermPrefixId() && oTopStack.getTermPrefixId() != "") {
					categories["terms"] = this._transformTermsToUiModel(TermsProvider.getInstance()._getAllAttrsRefsAssocsForDataObject(oTopStack.getTermPrefixId()));
				} else if (bIsOperationsContext && lastFunctionalStack && lastFunctionalStack.getFunction().getContext() == "result" &&
					lastFunctionalStack.getSize() >= 1 && lastFunctionalStack.getFunction().getName() === Constants.APPEND) {
			...
			case astLibInstance.IDPLexer.OPERATION_FUNCTION_NAME:
				if (bIsOperationsContext) {
					this._constructResultContextAutocomplete(oAutoCompleteJson, bIsOperationsContext);
				}
				break;
			case astLibInstance.IDPLexer.LOOP_FUNCTION_NAME:
				if (bIsOperationsContext) {
					this._constructResultContextAutocomplete(oAutoCompleteJson, bIsOperationsContext);
				}
				break;
		...
		AutoComplete.prototype._constructFunctionsAutocomplete = function (oAutoCompleteJson, oTopNode, oTopStack, bIsResultContext,
			...
			var aAllFunctions = FunctionProvider.getInstance().getAllFunctionsExceptOperationsCategory();
			...
			if (lastFunctionalStack) {
				...
				if (oTopStack.getSize() != 0 || oTopStack instanceof ArrayAndRangeStack) {
		...
		AutoComplete.prototype._constructResultContextAutocomplete = function (oAutoCompleteJson, bIsResultContext) {	
				var oCategories = oAutoCompleteJson["autoComplete"]["categories"];
				if (oCategories["functions"] == undefined) {
					oCategories["functions"] = {};
				}
				var oFunction;
				var aAllFunctions = FunctionProvider.getInstance().getResultContextFunctions();
		...
		AutoComplete.prototype._categorizeFunctions = function (oFunction, oAutoCompleteFunctionJson, bIsResultContext) {
			...
			if(bIsResultContext){
				// Disable all other functions except operations if its result context and operations are not added
				if(!(lastFunctionalStack) && !(oFunction.getContext().toUpperCase() == "RESULT")){
					return;
				}
				if(lastFunctionalStack && lastFunctionalStack.getFunction().getContext().toUpperCase() == "RESULT"){
					
					if(lastFunctionalStack.getFunction().getName().toUpperCase() == "FOREACH" ){
						if(lastFunctionalStack.getSize() <= 1 && oFunction.getName().toUpperCase() != "FILTER"){
							return;
						} else if(lastFunctionalStack.getSize() > 1 && 
									(oFunction.getName() == "FOREACH" || oFunction.getContext().toUpperCase() !== "RESULT") ) {
						    return;
						}
						// Special handling for each functions
					}else{
						if(lastFunctionalStack.getSize() <= 1){
							return;
						}else if(oFunction.getContext().toUpperCase() == "RESULT"){
							return;
						}
					}
				}
			...
			// Special handling to restrict First function
			if(oFunction.getName() == Constants.FIRST && !bIsResultContext){
				return;
			}
		...
		AutoComplete.prototype._transformTermsToUiModel = function (terms) {
			...
			for (var lIndex = 0; lIndex < terms.length; lIndex++) {
				...
				if(term._bussinessDataType){
					uiTermObj.bussinessDataType = term._bussinessDataType;
				}
				...
				if(term.getCardinality()) {
					uiTermObj.cardinality = term.getCardinality();
				}
				uiTermObj.id = id;
				uiTermObj.label = term.getLabel();
				uiTermObj.type = term.getDataObjectType();
				if(term.getIsDataObjectTable() !== undefined) {
					uiTermObj.isDataObjectTable = term.getIsDataObjectTable();
				}
					...
					for (var lIndex = 0; lIndex < aAllFunctions.length; lIndex++) {
						...
						if (this._checkPreviousOperatorHasSupportedFunctions(oTopStack)) {
							...
							this._categorizeFunctionGivenDataObjectList(oFunction, aDataObjectTypeList, oCategories,
								...
								lastFunctionalStack && lastFunctionalStack.getFunction().getContext() ==
								"result" && lastFunctionalStack.getSize() != 2)) {
							this._categorizeFunctions(oFunction, oCategories["functions"], bIsResultContext);
						} else {
							this._categorizeFunctionGivenBusinessDataTypeAndDataObjectList(oFunction, aBusienssDataTypeList, aDataObjectTypeList, oCategories,
								bIsResultContext);
