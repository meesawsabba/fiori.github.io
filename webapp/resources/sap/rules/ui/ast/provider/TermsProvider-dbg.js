sap.ui.define(["sap/rules/ui/ast/model/Term", "sap/rules/ui/ast/constants/Constants"], function (termObj, constants) {
	...
	TermsProvider.prototype._getAllAttrsRefsAssocsForDataObject = function (id) {
		var list = [],
			term;
		...
		for (var termsKey in this._termsIdMap) {
			...
			if (termsKey.startsWith(id + constants.DOT) && (term.getBusinessDataType() ||
				term.getDataObjectType() === 'AO' || term.getDataObjectType() === 'R') && (termsKey.split(constants.DOT).length) <= doDepthLevel + 1) {
				list.push(term);
			}
		}
		return list;
	...
	TermsProvider.prototype._getTermsNotStartingWithPrefix = function (id) {
		var list = [],
			term;
		for (var termsKey in this._termsIdMap) {
			term = this._termsIdMap[termsKey];
			if (!termsKey.startsWith(id) && (term._dataObjectType === 'S' || term._isDataObjectElement)) {
				list.push(term);
			}
		}
		return list;
	...
	TermsProvider.prototype._getAllAttrsRefersAndAssocsForDataObjectByName  = function (name) {
		var list = [],
			term, termName;
		...
		for (var termsKey in this._termsNameMap) {
			...
			if (termName.startsWith(name + constants.DOT) && (term.getBusinessDataType() ||
				term.getDataObjectType() === 'AO' || term.getDataObjectType() === 'R') && (termName.split(constants.DOT).length) <= doDepthLevel + 1) {
				list.push(term);
			}
		}
		return list;
	...
	TermsProvider.prototype._getAttributesGivenPrefixId = function (prefix, businessDatatype) {
        ...
        for (var termsKey in this._termsIdMap) {
            ...
            if (termId && termId && termId.startsWith(prefix + constants.DOT) && term.getDataObjectType() != 'AO'
                && term.getDataObjectType() != 'R' && (termId.split(constants.DOT).length) <= (doDepthLevel + 1)) {
                terms.push(term);
            }
        }
        return terms;
	...
	TermsProvider.prototype._getAllDataObjects = function () {
		...
		for (var termsKey in this._termsIdMap) {
			...
			if (type && (('E'.localeCompare(type) === 0 && term._bussinessDataType === null) || ('E'.localeCompare(type) != 0 && 'R'.localeCompare(
					type) && 'AO'.localeCompare(
	...
	TermsProvider.prototype.getTermsById = function (id) {
		...
		if (term) {
			...
			if (dataObjectType && ('E'.localeCompare(dataObjectType) != 0) && (constants.ELEMENT.localeCompare(dataObjectType) != 0)) {
				return this._getAllAttrsRefsAssocsForDataObject(id);
			}
		}
		return [];
	};
	...
	*/
	TermsProvider.prototype.getTermsByName = function (name) {
		...
		if (term) {
			...
			if (dataObjectType && (constants.ELEMENT.localeCompare(dataObjectType) != 0)) {
				return this._getAllAttrsRefersAndAssocsForDataObjectByName(name);
			}
		}
		return [];
	};
	...
	TermsProvider.prototype.getReferencesGivenPrefixName = function (prefixName) {
		var term;
		var finalTerms = [];
		var terms = this.getTermsByName(prefixName); //get all terms for this prefix
		if (terms && terms.length > 0) {
	...
	TermsProvider.prototype.getReferencesGivenPrefixId = function (prefixName) {
		var term;
		var finalTerms = [];
		var terms = this.getTermsById(prefixName); //get all terms for this prefix
		if (terms && terms.length > 0) {
	...
	TermsProvider.prototype.isTermParentEntity = function (id) {
		if(id){
			var oTerm = this.getTermByTermId(id);
			if(oTerm && (oTerm._dataObjectType === "S" || oTerm._dataObjectType === "T" || 
			    oTerm._isDataObjectElement || (oTerm.Type && oTerm.Type === "Rule"))){
			        return true;
		    }		
		}
        return false;
