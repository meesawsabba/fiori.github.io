sap.ui.define([
...
], function (Control, List, JSONModel, Constants, infraUtils) {
    "use strict";
    ...
    SelectFunctionDialog.prototype._createFunctionSelectionDropDown = function (selectFunction, sAriaValue) {
        var that = this;
    ...
    SelectFunctionDialog.prototype._getAttributeAstExpressionBasic = function (sAriaValue) {
        var that = this;
    ...
    SelectFunctionDialog.prototype._tableColumnsFactory = function (sId, oContext) {
        ...
        var itemTemplate = new sap.m.ColumnListItem({
            cells: [that._getAttributeAstExpressionBasic("selectAttributeLabelId" + that.selectLabelUUID),
                new sap.m.Select({
                    ...
                    width: '200px',
                    tooltip: that.oBundle.getText("sort_attributes_tooltip"),
    ...
    SelectFunctionDialog.prototype._createWhereAstExpressionBasic = function (sAriaValue) {
        var that = this;
    ...
    SelectFunctionDialog.prototype._setDataObjectInfoForWhereConditionAutoSuggestion = function () {
        if (this.dataObjectId) {
            var dataObjectId = this._returnDataObjectBasedOnCommonAssociationReferenceAndType(this.dataObjectId, this._getSelectedAttributeJsons())
    ...
    SelectFunctionDialog.prototype._createSelectFunctionDialog = function (data, callBack, AstExpressionLanguage, dialogOpenedCallbackReference, jsonData) {
        ...
        var sequence = 0;
        this._clearData();
        //create a uuid and append to the existing id to enable Select within Select as the Id becomes duplicate.
        this.selectLabelUUID = this._createUUId();
        this.oBundle = sap.ui.getCore().getLibraryResourceBundle("sap.rules.ui.i18n");
        ...
        if (jsonData) {
            ...
            if (this.dataObjectId) {
                var dataObjectWithCommonInfo = this._returnDataObjectBasedOnCommonAssociationoReferenceSelection(jsonData.doVocabId, jsonData.attributes);
                this.attributeJson = this._addTableIdFromAttribute(jsonData.attributes, dataObjectWithCommonInfo, this.dataObjectId);
        ...
        var functionSelect = that._createFunctionSelectionDropDown(data,"selectFunctionSelectionLabelId", + that.selectLabelUUID);
        ...
        var oWhereControl = that._createWhereAstExpressionBasic("selectWhereLabelId" + that.selectLabelUUID);
        ...
        var verticalLayoutForSelectFunction = new sap.ui.layout.form.SimpleForm({
            ...
            editable: true,
            content: [
                new sap.m.Label("selectFunctionSelectionLabelId" + that.selectLabelUUID ,{
                ...
                new sap.m.Label("selectAttributeLabelId" + that.selectLabelUUID ,{
                ...
                attributeTable,
                new sap.m.Label("selectWhereLabelId" + that.selectLabelUUID,{
    ...
    SelectFunctionDialog.prototype._returnDataObjectBasedOnCommonAssociationoReferenceSelection = function (sDataObject, attributes) {
        var shortestKey = "";
        var str = "";
        if (sDataObject && Object.keys(attributes).length !== 0 && attributes.constructor === Object) {
            ...
            if (this._hasAssociationId(shortestKey) || this._hasReferenceId(shortestKey)) {
                return shortestKey;
            }
        }
    ...
    SelectFunctionDialog.prototype._returnDataObjectBasedOnCommonAssociationReferenceAndType = function (sDataObject, attributes) {
        var shortestKey = "";
        var str = "";
        if (sDataObject && Object.keys(attributes).length !== 0 && attributes.constructor === Object) {
            var firstKey = Object.keys(attributes)[0];
            ...
            if (this._hasAssociationId(shortestKey) || this._hasReferenceId(shortestKey)) {
                return this._returnDataObjectBasedOnType(shortestKey);
            }
        }
        return sDataObject;
    };
 
    SelectFunctionDialog.prototype._returnDataObjectBasedOnType  = function (vocabId) {
    ...
    SelectFunctionDialog.prototype._hasReferenceId = function (vocabId) {
        var vocabularyId = vocabId;
        if (vocabularyId) {
            var splitArray = vocabularyId.split("/");
            var vocabulary = splitArray[1];
            ...
            if (this._oAstExpressionLanguage._astBunldeInstance.TermsProvider.TermsProvider.getTermByTermId(vocabulary) && this._oAstExpressionLanguage
                    ._astBunldeInstance.TermsProvider.TermsProvider.getTermByTermId(vocabulary)._dataObjectType === "R") {
            	return true;
            }
        }
        return false;
    ...
    SelectFunctionDialog.prototype._returnTableIdForAttributesBasedOnType = function (associationOrReferenceInfo) {
        if (associationOrReferenceInfo) {
            var aSplitArray = associationOrReferenceInfo.split("/");
            var str = aSplitArray[1];
            var index = 0;
            var term;
            var associationOrReference = "";
            if (aSplitArray.length > 2) {
                for (var iterator = 2; iterator < aSplitArray.length; iterator++) {
                    str += "." + aSplitArray[iterator];
                    term = this._oAstExpressionLanguage._astBunldeInstance.TermsProvider.TermsProvider.getTermByTermId(str);
                ...
                if (index > 0) {
                    for (var iterator = 2; iterator <= index; iterator++) {
                    	associationOrReference += "/" + aSplitArray[iterator];
                    }
                }
            }
            return associationOrReference;
        }
    };
    ...
    SelectFunctionDialog.prototype._removetableIdFromAttribute = function (attributeJson, dataObjectWithCommonInfo, sDataObjectInfo) {
        var expressionID = "";
        var aId;
        var newattributeJson = {};
        var tableId = this._returnTableIdForAttributesBasedOnType(dataObjectWithCommonInfo);
        if (sDataObjectInfo && Object.keys(attributeJson).length !== 0 && attributeJson.constructor === Object && tableId) {
            var keys = Object.keys(attributeJson);
            for (var j = 0; j < keys.length; j++) {
                var key = keys[j];
                var value = attributeJson[key];
                key = key.replace(tableId, "");
    ...
    SelectFunctionDialog.prototype._addTableIdFromAttribute = function (attributeJson, dataObjectWithCommonInfo, sDataObjectInfo) {
        var expressionID = "";
        var aId;
        var newattributeJson = {};
        var tableId = this._returnTableIdForAttributesBasedOnType(dataObjectWithCommonInfo);
    ...
    SelectFunctionDialog.prototype._createJson = function () {
        ...
        var dataObjectWithCommonInfo  = this._returnDataObjectBasedOnCommonAssociationoReferenceSelection(this.dataObjectId, attributeJson);
        var doVocaId = this._returnDataObjectBasedOnCommonAssociationReferenceAndType(this.dataObjectId, attributeJson);
        attributeJson = this._removetableIdFromAttribute(attributeJson, dataObjectWithCommonInfo, this.dataObjectId);
        var json = {
	...
	SelectFunctionDialog.prototype._createUUId = function () {
