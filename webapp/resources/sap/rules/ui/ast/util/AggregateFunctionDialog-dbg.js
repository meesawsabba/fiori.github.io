sap.ui.define([
    ...
    AggregateFunctionDialog.prototype._createFunctionSelectionDropDown = function (aggregateFunction, sAriaValue) {
        var that = this;
        ...
        this.functionSelect = new sap.m.Select({
            ...
            change: function (oEvent) {
				...
				this.astExpressionBasicForAttributes.setEditable(false);
                this.groupBySelect.setEnabled(false);
				this.astExpressionBasicForAttributes.setValue("");
                this._attributeId = "";
                this._groupBy = [];
    ...
    AggregateFunctionDialog.prototype._enableFieldAttribute = function () {
        var bIsAttributeFieldEnabled = false 
        if (this._vocabularyExpressionId && this._aggregateFunctionSelected && this.jsonDataArray.length === 0) {
            bIsAttributeFieldEnabled = true;
        } 
        return bIsAttributeFieldEnabled;
    };
                ...
                if (that._vocabularyExpressionId && this._aggregateFunctionSelected) {
                    if (oEvent.getSource().getProperty("jsonData") && oEvent.getSource().getProperty("jsonData").length > 0) {
                        ...
                        if (this.sDataObjectSelected) {
                            ...
                            this._whereExpressionId = "";
                            this._attributeId = "";
                            this.applyButton.setEnabled((this._aggregateFunctionSelected === Constants.COUNT) ? true : false);
                            this.jsonDataArray = [];
                            var str = this._setDataObjectInfoForWhereConditionAutoSuggestion();
                            this.astExpressionBasicForWhere.setDataObjectInfo(str);
							this.astExpressionBasicForAttributes.setDataObjectInfo(this._getDataObjectInfoForAttribute());
                            this._bindGroupByDropDown();
                            this._groupBy = [];
                            this.groupBySelected = [];
                            this.astExpressionBasicForWhere.setEditable(true);
							this.astExpressionBasicForAttributes.setEditable((this._aggregateFunctionSelected === Constants.COUNT) ? false : true);
    ...
    AggregateFunctionDialog.prototype._setDataObjectInfoForWhereConditionAutoSuggestion = function () {
        ...
        if (this.jsonDataArray && this.jsonDataArray.length > 0) {
            ...
            //We get the table info based on the Association Cardinality and reference type to set the context for the where Clause.
            sVocabulary = this._returnDataObjectBasedOnType(this._vocabularyExpressionId);
            if (sVocabulary) {
                return this._aggregateFunctionSelected + "(FILTER(" + sVocabulary + ",";
            }
            //In case of no association or reference the table will be the dataObject.
            return this._aggregateFunctionSelected + "(FILTER(/" + dataObjectId + ",";
 
        }
    };
	
    AggregateFunctionDialog.prototype._getDataObjectInfoForAttribute= function() {
        var sVocabulary = "";
        if (this.jsonDataArray && this.jsonDataArray.length === 0 && this._vocabularyExpressionId) {
            sVocabulary = this._vocabularyExpressionId;
        }
        return sVocabulary;
    };
	...
	AggregateFunctionDialog.prototype._createAttributeAstExpressionBasic = function (sAriaValue) {
        var that = this;
        this.astExpressionBasicForAttributes = new sap.rules.ui.AstExpressionBasic({
            value: this._attributeId,
            astExpressionLanguage: this._oAstExpressionLanguage,
            dataObjectInfo: this._getDataObjectInfoForAttribute(),
            ariaLabelledBy: sAriaValue,
            enableAggregateFunctionAttribute: true,
            isAttributeContext: true,
            editable: that._enableFieldAttribute(),
            change: function (oEvent) {
            	that._attributeId = oEvent.getParameter("newValue");
            	if(that._attributeId) {
        ...
        this.astExpressionBasicForAttributes.addStyleClass("sapAstExpressionDialog");
        return this.astExpressionBasicForAttributes;
    };

    AggregateFunctionDialog.prototype._createWhereAstExpressionBasic = function (sAriaValue) {
        var that = this;
    ...
    AggregateFunctionDialog.prototype._createGroupByDropDown = function (sAriaValue) {
        var that = this;
    ...
    AggregateFunctionDialog.prototype._bindGroupByDropDown = function () {
        ...
        if (this.dataObjectId) {
            //We get the table info based on the Association Cardinality and reference type to get the Attributes for the same.
            sVocabulary = this._returnDataObjectBasedOnType(this._vocabularyExpressionId);
            if (sVocabulary) {
    ...
    AggregateFunctionDialog.prototype._createAggregationFunctionDialog = function (data, callBack, AstExpressionLanguage, dialogOpenedCallbackReference, jsonData) {
        ...
        if (jsonData) {
			...
			this._attributeId = jsonData.attribute;
        ...
        var oFunctionSelect = that._createFunctionSelectionDropDown(data,"aggregateFunctionSelectLabelId");
        ...
        var oWhereControl = that._createWhereAstExpressionBasic("aggregateWhereLabelId");
        var oGroupByControl = that._createGroupByDropDown("aggregateGroupByLabelId");
        var oAttributeControl = that._createAttributeAstExpressionBasic("aggregateAttributeLabelId");
        var oFunctionLabelControl = that._createFunctionLabel("aggregateFunctionLabelId");

        var verticalLayoutForAggregateFunction = new sap.ui.layout.form.SimpleForm({
            layout: sap.ui.layout.form.SimpleFormLayout.ResponsiveGridLayout,
            editable: true,
            content: [
                new sap.m.Label("aggregateFunctionSelectLabelId",{
				...
				new sap.m.Label("aggregateAttributeLabelId",{
                    text: that.oBundle.getText("attribute"),
                    tooltip: that.oBundle.getText("attribute"),
                    labelFor: oWhereControl.getId()
                }),
                oAttributeControl,

                new sap.m.Label("aggregateWhereLabelId",{
                    text: that.oBundle.getText("where"),
                    tooltip: that.oBundle.getText("where"),
                    labelFor: oWhereControl.getId()
                }),
                oWhereControl,
                new sap.m.Label("aggregateGroupByLabelId",{
                    text: that.oBundle.getText("group_by"),
                    tooltip: that.oBundle.getText("group_by"),
                    labelFor: oGroupByControl.getId()
                }),
                oGroupByControl,
                new sap.m.Label("aggregateFunctionLabelId",{
    ...
    AggregateFunctionDialog.prototype._returnDataObjectBasedOnType = function (vocabId) {
        var vocabularyId = "";
        if (this.jsonDataArray && this.jsonDataArray.length > 0) {
    ...
    AggregateFunctionDialog.prototype._isDataObjectStructure = function () {
        if (this.jsonDataArray && this.jsonDataArray.length === 0) {
            ...
            if (term && term._dataObjectType && term._dataObjectType === "S") {
                ...
                return true;
            } else if(term && term._dataObjectType && term._dataObjectType === "R" && !term._isDataObjectTable) {
                return true;
            }
        } else {
            this.tableId = this._returnDataObjectBasedOnType(this._vocabularyExpressionId);
    ...
    AggregateFunctionDialog.prototype._createJson = function (func, vocabulary, whereCondition, groupBy, functionLabel, dataObjectId, attributeId) {
        return {
            "function": func,
