sap.ui.define([
...
], function (Log, jQuery, library, Control, ResponsivePopover, List, JSONModel, ListMode, PlacementType, StandardListItem, CustomData, Sorter,
	...
	SelectFunctionDialog, TermsProvider, astBundleLibrary, LocaleData, LoopFunctionDialog) {
	"use strict";
	...
	var AstExpressionBasic = ExpressionBase.extend("sap.rules.ui.AstExpressionBasic", {
		...
		_setUpEventHandlers: function () {
			...
			this._input.on('keyup', function (event) {
				...
				if (event.key === "Control" || event.keyCode === 35 || event.keyCode === 36 || event.keyCode === 37 || event.keyCode === 39 || event.keyCode === 20 || event.keyCode === 144) {
					return;
				}
		...
		_updateUIModelForFreeFlow: function (sChar) {
            ...
            } else if (sChar === Constants.SINGLE_QUOTE && this.bEditInBetween && span && span.className != 'sapAstLiteralClass') {
		...
		_showAutoSuggestion: function (suggestionContent, bExpand) {
			...
			setTimeout(function(){
				...
				if (that._oAutoSuggestionPopUp && !that._oAutoSuggestionPopUp.isOpen()) {
					that._oAutoSuggestionPopUp.destroyContent();
					that._oAutoSuggestionPopUp.addContent(new sap.rules.ui.AutoCompleteSuggestionContent({
				...
				if (!that._oAutoSuggestionPopUp.isOpen()) {
					that._oAutoSuggestionPopUp.setOffsetX(xPos);
					that._oAutoSuggestionPopUp.setOffsetY(yPos);
					that._oAutoSuggestionPopUp.openBy(that.getDomRef("input"));
				}
